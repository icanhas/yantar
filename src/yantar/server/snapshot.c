...
SV_EmitPacketEntities(clientSnapshot_t *from, clientSnapshot_t *to, Bitmsg *msg)
{
	...
	while(newindex < to->num_entities || oldindex < from_num_entities){
		if(newindex >= to->num_entities)
			...
			newent =
				&svs.snapshotEntities[(to->first_entity+
						       newindex) %
		...
		if(oldindex >= from_num_entities)
			...
			oldent =
				&svs.snapshotEntities[(from->first_entity+
						       oldindex) %
						      svs.numSnapshotEntities];
...
SV_WriteSnapshotToClient(Client *client, Bitmsg *msg)
{
	...
	if(client->deltaMessage <= 0 || client->state != CS_ACTIVE){
		...
		comdprintf ("%s: Delta request from out of date packet.\n",
			client->name);
		oldframe = NULL;
		lastframe = 0;
	}else{
		...
		oldframe =
			&client->frames[ client->deltaMessage & PACKET_MASK ];
		lastframe = client->netchan.outgoingSequence -
			    client->deltaMessage;
...
SV_QsortEntityNumbers(const void *a, const void *b)
{
	...
	if(*ea == *eb)
		comerrorf(ERR_DROP, "SV_QsortEntityStates: duplicated entity");
			       ...
			       snapshotEntityNumbers_t *eNums, qbool portal)
{
	...
	for(e = 0; e < sv.num_entities; e++){
		...
		if(ent->s.number != e){
			comdprintf ("FIXING ENT->S.NUMBER!!!\n");
			ent->s.number = e;
		}
...
SV_BuildClientSnapshot(Client *client)
{
	...
	Q_Memset(frame->areabits, 0, sizeof(frame->areabits));
	...
	for(i = 0; i < entityNumbers.numSnapshotEntities; i++){
		...
		state =
			&svs.snapshotEntities[svs.nextSnapshotEntities %
					      svs.numSnapshotEntities];
		*state = ent->s;
...
SV_SendMessageToClient(Bitmsg *msg, Client *client)
{
	...
	client->frames[client->netchan.outgoingSequence &
		       PACKET_MASK].messageSize = msg->cursize;
	client->frames[client->netchan.outgoingSequence &
		       PACKET_MASK].messageSent = svs.time;
	client->frames[client->netchan.outgoingSequence &
		       PACKET_MASK].messageAcked = -1;
	...
	for(i=0; i < sv_maxclients->integer; i++){
		...
		if(!(c->netchan.remoteAddress.type == NA_LOOPBACK ||
		     (sv_lanForceRate->integer &&
		      sysisLANaddr(c->netchan.remoteAddress)))){
