...
SV_GetChallenge(Netaddr from)
{
	   ...
	   !sysisLANaddr(from)){
		...
		if(svs.authorizeAddress.type == NA_BAD){
			...
			if(strtoaddr(AUTHORIZE_SERVER_NAME,
				...
				svs.authorizeAddress.port = BigShort(
					PORT_AUTHORIZE);
		...
		if(svs.authorizeAddress.type == NA_BAD)
			comprintf("Couldn't resolve auth server address\n");

		/* if they have been challenging for a long time and we
		 * haven't heard anything from the authorize server, go ahead and
		 * let them in, assuming the id server is down */
		else if(svs.time - oldestClientTime > AUTHORIZE_TIMEOUT)
			comdprintf("authorize server timed out\n");
		else{
			/* otherwise send their ip to the authorize server */
			Cvar	*fs;
			char	game[1024];

			comdprintf("sending getIpAuthorize for %s\n",
				addrtostr(
					from));
			...
			netprintOOB(
				NS_SERVER, svs.authorizeAddress,
				"getIpAuthorize %i %i.%i.%i.%i %s 0 %s",
				challenge->challenge,
				from.ip[0], from.ip[1], from.ip[2], from.ip[3],
				game, sv_strictAuth->string);
	...
	netprintOOB(NS_SERVER, challenge->adr,
...
SV_AuthorizeIpPacket(Netaddr from)
{
	...
	if(!equalbaseaddr(from, svs.authorizeAddress)){
		comprintf("SV_AuthorizeIpPacket: not from authorize server\n");
		return;
	}
	...
	for(i = 0; i < MAX_CHALLENGES; i++)
		if(svs.challenges[i].challenge == challenge)
			break;
	if(i == MAX_CHALLENGES){
		comprintf("SV_AuthorizeIpPacket: challenge not found\n");
		return;
	}
	...
	if(!Q_stricmp(s, "demo")){
		...
		netprintOOB(NS_SERVER, challengeptr->adr,
			"print\nServer is not a demo server\n");
		/* clear the challenge record so it won't timeout and let them through */
		Q_Memset(challengeptr, 0, sizeof(*challengeptr));
		return;
	}
	...
	if(!r)
		...
		netprintOOB(NS_SERVER, challengeptr->adr, "print\n%s\n",
			r);
...
SV_IsBanned(Netaddr *from, qbool isexception)
{
	...
	for(index = 0; index < serverBansCount; index++){
		...
		if(curban->isexception == isexception)
			if(equalbaseaddrmask(curban->ip, *from,
				   curban->subnet))
				return qtrue;
	}
...
SV_DirectConnect(Netaddr from)
{
...

	{
		if(version != com_protocol->integer){
			...
			comdprintf("    rejected connect from version %i\n",
				version);
			return;
		}
	}
	...
	if(!islocaladdr(from)){
		...
		for(i=0; i<MAX_CHALLENGES; i++)
			if(equaladdr(from, svs.challenges[i].adr))
		...
		if(!sysisLANaddr(from)){
			...
			if(sv_maxPing->value && ping > sv_maxPing->value){
				...
				comdprintf (
					"Client %i rejected on a too high ping\n",
					i);
				challengeptr->wasrefused = qtrue;
				return;
			}
		...
		comprintf("Client %i connecting with %i challenge ping\n", i,
			ping);
 ...
 *              vmcall( gvm, GAME_CLIENT_DISCONNECT, newcl - svs.clients ); */
	...
	if(!newcl){
		if(islocaladdr(from)){
			...
			netprintOOB(NS_SERVER, from,
				"print\nServer is full.\n");
			comdprintf ("Rejected a connection.\n");
			return;
		}
	}
...
gotnewcl:
	...
	*newcl = temp;
	...
	if(denied){
		...
		netprintOOB(NS_SERVER, from, "print\n%s\n", str);
		comdprintf ("Game rejected a connection: %s.\n", str);
		return;
	}
	...
	netprintOOB(NS_SERVER, from, "connectResponse %d", challenge);

	comdprintf("Going from CS_FREE to CS_CONNECTED for %s\n", newcl->name);
...
SV_DropClient(Client *drop, const char *reason)
{
	...
	if(!isBot){
		...
		for(i = 0; i < MAX_CHALLENGES; i++, challenge++)
			if(equaladdr(drop->netchan.remoteAddress,
				   challenge->adr)){
				Q_Memset(challenge, 0, sizeof(*challenge));
				break;
			}
	}
	...
	if(isBot)
		...
		comdprintf("Going to CS_ZOMBIE for %s\n", drop->name);
...
SV_SendClientGameState(Client *client)
{
	...
	comdprintf ("SV_SendClientGameState() for %s\n", client->name);
	comdprintf("Going from CS_CONNECTED to CS_PRIMED for %s\n",
		client->name);
...
SV_ClientEnterWorld(Client *client, Usrcmd *cmd)
{
	...
	comdprintf("Going from CS_PRIMED to CS_ACTIVE for %s\n", client->name);
...
{
	if(*cl->downloadName)
		comdprintf("clientDownload: %d : file \"%s\" aborted\n",
...
{
	comdprintf("clientDownload: %s Done\n", cl->name);
...
SV_WriteDownloadToClient(Client *cl, Bitmsg *msg)
{
	...
	if(!cl->download){
		...
		comprintf("clientDownload: %d : beginning \"%s\"\n",
			(int)(cl - svs.clients), cl->downloadName);
	...
	while(cl->downloadCurrentBlock - cl->downloadClientBlock <
		...
		if(!cl->downloadBlocks[curindex])
			cl->downloadBlocks[curindex] = zalloc(
	...
	if(cl->downloadCount == cl->downloadSize &&
	   ...
	   MAX_DOWNLOAD_WINDOW){
		...
		cl->downloadBlockSize[cl->downloadCurrentBlock %
				      MAX_DOWNLOAD_WINDOW] = 0;
		cl->downloadCurrentBlock++;
...
SV_VerifyPaks_f(Client *cl)
{
	...
	if(sv_pure->integer != 0){
		...
		if(atoi(pArg) < sv.checksumFeedServerId){
			comdprintf(
				"ignoring outdated cp command from client %s\n",
				cl->name);
			return;
		}
...
SV_UserinfoChanged(Client *cl)
{
	...
	/* name for C code */
	Q_strncpyz(cl->name,
		Info_ValueForKey (cl->userinfo, "name"), sizeof(cl->name));
	...
	if(islocaladdr(cl->netchan.remoteAddress))
		ip = "localhost";
	else
		ip = (char*)addrtostr(cl->netchan.remoteAddress);
...
SV_ExecuteClientCommand(Client *cl, const char *s, qbool clientOK)
{
	...
	if(clientOK){
		...
		comdprintf("client text ignored for %s: %s\n", cl->name,
			cmdargv(
				0));
}
...
SV_ClientCommand(Client *cl, Bitmsg *msg)
{
	...
	comdprintf("clientCommand: %s : %i : %s\n", cl->name, seq, s);

	/* drop the connection if we have somehow lost commands */
	if(seq > cl->lastClientCommand + 1){
		comprintf("Client %s lost %i clientCommands\n", cl->name,
	...
	Q_sprintf(cl->lastClientCommandString,
...
SV_UserMove(Client *cl, Bitmsg *msg, qbool delta)
{
	...
	if(cmdCount > MAX_PACKET_USERCMDS){
		comprintf("cmdCount > MAX_PACKET_USERCMDS\n");
		return;
	}
	...
	cl->frames[ cl->messageAcknowledge &
		    PACKET_MASK ].messageAcked = svs.time;
	...
	if(sv_pure->integer != 0 && cl->pureAuthentic == 0 && !cl->gotCP){
		if(cl->state == CS_ACTIVE){
			...
			comdprintf(
				"%s: didn't get cp command, resending gamestate\n",
				cl->name);
			SV_SendClientGameState(cl);
		}
		return;
...
SV_UserVoip(Client *cl, Bitmsg *msg)
{
	...
	for(i = 0, client = svs.clients; i < sv_maxclients->integer;
		...
		if(comisvoiptarget(recips, sizeof(recips), i))
			flags |= VOIP_DIRECT;
		else
			flags &= ~VOIP_DIRECT;
		...
		if(client->queuedVoipPackets >= ARRAY_LEN(client->voipPacket)){
			comprintf(
				"Too many VoIP packets queued for client #%d\n",
				i);
			continue;	/* no room for another packet right now. */
		}

		packet = zalloc(sizeof(*packet));
...
SV_ExecuteClientMessage(Client *cl, Bitmsg *msg)
{
	...
	if(serverId != sv.serverId && !*cl->downloadName &&
	   !strstr(cl->lastClientCommandString, "nextdl")){
		if(serverId >= sv.restartedServerId && serverId <
		...
		if(cl->messageAcknowledge > cl->gamestateMessageNum){
			comdprintf("%s : dropped gamestate, resending\n",
				cl->name);
			SV_SendClientGameState(cl);
		}
		return;
	...
	if(cl->oldServerTime && serverId == sv.serverId){
		comdprintf("%s acknowledged gamestate\n", cl->name);
		cl->oldServerTime = 0;
	}
	...
	if(c == clc_move)
		...
		comprintf("WARNING: bad command byte for client %i\n",
			(int)(cl - svs.clients));
	/*	if ( msg->readcount != msg->cursize ) { */
/*      comprintf( "WARNING: Junk at end of packet for client %i\n", cl - svs.clients );
 * } */
}
