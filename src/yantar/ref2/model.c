...
R_RegisterMD3(const char *name, model_t *mod)
{
	...
	for(lod = MD3_MAX_LODS - 1; lod >= 0; lod--){
		...
		size = ri.fsreadfile(namebuf, &buf.v);
		if(!buf.u)
			continue;
...
R_LoadMD3(model_t * mod, int lod, void *buffer, int bufferSize, const char *modName)
{
	...
	mod->dataSize += size;
	mdvModel = mod->mdv[lod] = ri.hunkalloc(sizeof(mdvModel_t), Hlow);
...
/*  Q_Memcpy(mod->md3[lod], buffer, LittleLong(md3Model->ofsEnd)); */
	...
	mdvModel->frames = frame = ri.hunkalloc(sizeof(*frame) * md3Model->numFrames, Hlow);
	...
	mdvModel->tags = tag = ri.hunkalloc(sizeof(*tag) * (md3Model->numTags * md3Model->numFrames), Hlow);
	...
	mdvModel->tagNames = tagName = ri.hunkalloc(sizeof(*tagName) * (md3Model->numTags), Hlow);
	...
	mdvModel->surfaces = surf = ri.hunkalloc(sizeof(*surf) * md3Model->numSurfaces, Hlow);
	...
	for(i = 0; i < md3Model->numSurfaces; i++){
		...
		surf->triangles = tri = ri.hunkalloc(sizeof(*tri) * md3Surf->numTriangles, Hlow);
		...
		surf->verts = v = ri.hunkalloc(sizeof(*v) * (md3Surf->numVerts * md3Surf->numFrames), Hlow);
		...
		/* swap all the ST */
		surf->st = st = ri.hunkalloc(sizeof(*st) * md3Surf->numVerts, Hlow);
