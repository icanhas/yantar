...
R_PerformanceCounters(void)
{
	...
	if(r_speeds->integer == 1){
		ri.Printf (PRINT_ALL,
			...
			R_SumOfUsedImages()/(1000000.0f), backEnd.pc.c_overDraw /
		...
		ri.Printf(PRINT_ALL, "VBO draws: static %i dynamic %i\nMultidraws: %i merged %i\n",
			backEnd.pc.c_staticVboDraws, backEnd.pc.c_dynamicVboDraws, backEnd.pc.c_multidraws,
 ...
 */
void
R_AddCapShadowmapCmd(int map, int cubeSide)
{
...
RE_BeginFrame(Stereoframe stereoFrame)
{
	...
	if(r_measureOverdraw->integer){
		if(glConfig.stencilBits < 4){
			...
			ri.cvarsetstr("r_measureOverdraw", "0");
