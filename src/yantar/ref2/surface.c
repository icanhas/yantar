...
void
RB_CheckVBOandIBO(VBO_t *vbo, IBO_t *ibo)
{
	if(!(vbo == glState.currentVBO && ibo ==
	     glState.currentIBO) || tess.multiDrawPrimitives >= MAX_MULTIDRAW_PRIMITIVES){
		RB_EndSurface();
		RB_BeginSurface(tess.shader, tess.fogNum);
 ...
 */
void
RB_AddQuadStampExt(Vec3 origin, Vec3 left, Vec3 up, float color[4], float s1, float t1, float s2,
		   float t2)
{
	Vec3	normal;
	int	ndx;
	...
	subv3(vec3_origin, backEnd.viewParms.or.axis[0], normal);

	tess.normal[ndx][0] =
		tess.normal[ndx+1][0]	= tess.normal[ndx+2][0] = tess.normal[ndx+3][0] = normal[0];
	tess.normal[ndx][1]		=
		tess.normal[ndx+1][1]	= tess.normal[ndx+2][1] = tess.normal[ndx+3][1] = normal[1];
	tess.normal[ndx][2]		=
		tess.normal[ndx+1][2]	= tess.normal[ndx+2][2] = tess.normal[ndx+3][2] = normal[2];
 ...
 */
void
RB_AddQuadStamp(Vec3 origin, Vec3 left, Vec3 up, float color[4])
{
 ...
 */
void
RB_InstantQuad2(Vec4 quadVerts[4], Vec2 texCoords[4], Vec4 color, shaderProgram_t *sp, Vec2 invTexRes)
{
	...
	copyv34(quadVerts[0], tess.xyz[tess.numVertexes]);
	copyv32(texCoords[0], tess.texCoords[tess.numVertexes][0]);
	tess.numVertexes++;

	copyv34(quadVerts[1], tess.xyz[tess.numVertexes]);
	copyv32(texCoords[1], tess.texCoords[tess.numVertexes][0]);
	tess.numVertexes++;

	copyv34(quadVerts[2], tess.xyz[tess.numVertexes]);
	copyv32(texCoords[2], tess.texCoords[tess.numVertexes][0]);
	tess.numVertexes++;

	copyv34(quadVerts[3], tess.xyz[tess.numVertexes]);
	copyv32(texCoords[3], tess.texCoords[tess.numVertexes][0]);
	tess.numVertexes++;
...
void
RB_InstantQuad(Vec4 quadVerts[4])
{
	Vec4	color;
	Vec2	texCoords[4];
	Vec2	invTexRes;
...
RB_SurfaceSprite(void)
{
	...
	radius = ent->e.radius;
	if(ent->e.rotation == 0){
		scalev3(backEnd.viewParms.or.axis[1], radius, left);
		scalev3(backEnd.viewParms.or.axis[2], radius, up);
	}else{
		float	s, c;
		float	ang;
	...
	if(ent->e.renderfx & RF_SUNFLARE){
		...
		if(R_CullPointAndRadiusEx(ent->e.origin, ent->e.radius, backEnd.viewParms.frustum,
			   ARRAY_LEN(backEnd.viewParms.frustum)) == CULL_OUT)
			return;
		colors[0] = colors[1] = colors[2] = colors[3] =
							    ent->e.shaderRGBA[glRefConfig.framebufferObject]
							    / 255.0f;
		if(colors[0] == 0)
			return;
...
#endif
	{
		scalev34(ent->e.shaderRGBA, 1.0f / 255.0f, colors);
	}
...
RB_SurfacePolychain(srfPoly_t *p)
{
	...
	for(i = 0; i < p->numVerts; i++){
		copyv3(p->verts[i].xyz, tess.xyz[numv]);
...
static void
RB_SurfaceHelper(int numVerts, srfVert_t *verts, int numTriangles, srfTriangle_t *triangles, int dlightBits,
		 int pshadowBits)
{
	int i;
	srfTriangle_t *tri;
	...
	if(tess.shader->vertexAttribs & ATTR_LIGHTCOORD){
		...
		for(i = 0; i < numVerts; i++, dv++, lightCoords+=4)
			copyv32(dv->lightmap, lightCoords);
	}
		    ...
		    qbool shaderCheck)
{
	...
	if(shaderCheck &&
	   !(!ShaderRequiresCPUDeforms(tess.shader) && !tess.shader->isSky && !tess.shader->isPortal)){
		return qfalse;
	}
	...
	if(mergeBack != -1 && mergeForward == -1){
		...
		tess.multiDrawLastIndex[mergeBack]	= (byte*)tess.multiDrawFirstIndex[mergeBack] +
							  tess.multiDrawNumIndexes[mergeBack] *
		...
		tess.multiDrawLastIndex[mergeForward]	= (byte*)tess.multiDrawFirstIndex[mergeForward] +
							  tess.multiDrawNumIndexes[mergeForward] *
							  sizeof(GL_INDEX_TYPE);
		backEnd.pc.c_multidrawsMerged++;
	}else if(mergeBack != -1 && mergeForward != -1){
		tess.multiDrawNumIndexes[mergeBack] += numIndexes +
		...
		if(mergeForward != tess.multiDrawPrimitives){
			tess.multiDrawNumIndexes[mergeForward] =
				tess.multiDrawNumIndexes[tess.multiDrawPrimitives];
			tess.multiDrawFirstIndex[mergeForward] =
				tess.multiDrawFirstIndex[tess.multiDrawPrimitives];
		}
...
{
	if(RB_SurfaceHelperVBO (srf->vbo, srf->ibo, srf->numVerts, srf->numTriangles * 3, srf->firstIndex,
		   srf->dlightBits[backEnd.smpFrame], srf->pshadowBits[backEnd.smpFrame], qtrue)){
		return;
	}

	RB_SurfaceHelper(srf->numVerts, srf->verts, srf->numTriangles, srf->triangles,
		srf->dlightBits[backEnd.smpFrame], srf->pshadowBits[backEnd.smpFrame]);
}
...
#define NUM_BEAM_SEGS 6
	...
	if(normv3(normalized_direction) == 0)
		return;

	perpv3(perpvec, normalized_direction);

	scalev3(perpvec, 4, perpvec);

	for(i = 0; i < NUM_BEAM_SEGS; i++){
		RotatePointAroundVector(start_points[i], normalized_direction, perpvec,
			(360.0/NUM_BEAM_SEGS)*i);
/*		addv3( start_points[i], origin, start_points[i] ); */
		addv3(start_points[i], direction, end_points[i]);
	}
	...
	for(i = 0; i <= NUM_BEAM_SEGS; i++){
		copyv3(start_points[ i % NUM_BEAM_SEGS ], tess.xyz[tess.numVertexes++]);
		copyv3(end_points  [ i % NUM_BEAM_SEGS ], tess.xyz[tess.numVertexes++]);
	}
...
static void
DoRailCore(const Vec3 start, const Vec3 end, const Vec3 up, float len, float spanWidth)
{
	float	spanWidth2;
	int	vbase;
	float	t = len / 256.0f;
...
static void
DoRailDiscs(int numSegs, const Vec3 start, const Vec3 dir, const Vec3 right, const Vec3 up)
{
	int i;
	Vec3	pos[4];
	Vec3	v;
	...
	for(i = 0; i < numSegs; i++){
		...
		for(j = 0; j < 4; j++){
			copyv3(pos[j], tess.xyz[tess.numVertexes]);
...
RB_SurfaceRailRings(void)
{
	...
	scalev3(vec, r_railSegmentLength->value, vec);
...
RB_SurfaceRailCore(void)
{
	...
	/* compute side vector */
	subv3(start, backEnd.viewParms.or.origin, v1);
	normv3(v1);
	subv3(end, backEnd.viewParms.or.origin, v2);
	normv3(v2);
	crossv3(v1, v2, right);
	normv3(right);
...
RB_SurfaceLightningBolt(void)
{
	...
	copyv3(e->oldorigin, end);
	copyv3(e->origin, start);
...
*/
static void
VectorArrayNormalize(Vec4 *normals, unsigned int count)
{
/*    assert(count); */
	...
	/* given the input, it's safe to call fastnormv3 */
	while(count--){
		fastnormv3(normals[0]);
		normals++;
	}
#endif
...
LerpMeshVertexes_altivec(md3Surface_t *surf, float backlerp)
{
	...
	if(backlerp == 0){
		...
		for(vertNum=0; vertNum < numVerts; vertNum++,
			...
			newNormalsVec0		= vec_perm(newNormalsVec0,newNormalsVec1,
				newNormalsLoadPermute);
		...
		for(vertNum=0; vertNum < numVerts; vertNum++,
		    ...
		    outXyz += 4, outNormal += 4){
			Vec3 uncompressedOldNormal, uncompressedNewNormal;
...
/*			normv3 (outNormal); */
		...
		VectorArrayNormalize((Vec4*)tess.normal[tess.numVertexes], numVerts);
	}
}
#endif

static void
LerpMeshVertexes_scalar(mdvSurface_t *surf, float backlerp)
{
#if 0
	short	*oldXyz, *newXyz, *oldNormals, *newNormals;
	float	*outXyz, *outNormal;
...
/*			normv3 (outNormal); */
	...
	if(backlerp == 0){
		...
		for(vertNum=0; vertNum < surf->numVerts; vertNum++){
			copyv3(newVerts->xyz,    outXyz);
			copyv3(newVerts->normal, outNormal);
			newVerts++;
			outXyz += 4;
			outNormal += 4;
		}
		...
		for(vertNum=0; vertNum < surf->numVerts; vertNum++){
			lerpv3(newVerts->xyz,    oldVerts->xyz,    backlerp, outXyz);
			lerpv3(newVerts->normal, oldVerts->normal, backlerp, outNormal);
			/* normv3(outNormal); */
			newVerts++;
			oldVerts++;
			outXyz += 4;
		...
		VectorArrayNormalize((Vec4*)tess.normal[tess.numVertexes], surf->numVerts);
	}

}

static void
LerpMeshVertexes(mdvSurface_t *surf, float backlerp)
{
#if 0
#if idppc_altivec
...
static float
LodErrorForVolume(Vec3 local, float radius)
{
	Vec3	world;
	float	d;
	...
	subv3(world, backEnd.viewParms.or.origin, world);
	d = dotv3(world, backEnd.viewParms.or.axis[0]);
...
static void
RB_SurfaceFlare(srfFlare_t *surf)
{
	if(r_flares->integer)
...
static void
RB_SurfaceVBOMesh(srfVBOMesh_t * srf)
{
...
void
RB_SurfaceVBOMDVMesh(srfVBOMDVMesh_t * surface)
{
	mdvModel_t *mdvModel;
...
static void
RB_SurfaceDisplayList(srfDisplayList_t *surf)
{
