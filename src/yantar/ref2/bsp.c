...
ColorToRGBE(const Vec3 color, unsigned char rgbe[4])
{
	...
	if(maxComponent < 1e-32){
		...
		scalev3(sample, 1.0 / pow(2.0f, e) /*exp2(e)*/, sample);
...
R_LoadLightmaps(Lump *l, Lump *surfs)
{
	...
	if(r_mergeLightmaps->integer){
		...
		while(tr.fatLightmapStep * tr.fatLightmapStep * MAX_LIGHTMAP_PAGES < numLightmaps &&
		      tr.fatLightmapSize != glConfig.maxTextureSize){
	...
	if(tr.worldDeluxeMapping || r_deluxeMapping->integer == 2){
		tr.deluxemaps = ri.hunkalloc(tr.numLightmaps * sizeof(Img *), Hlow);
	}
	...
	if(r_mergeLightmaps->integer){
		for(i = 0; i < tr.numLightmaps; i++){
			tr.lightmaps[i] = R_CreateImage2(va("_fatlightmap%d",
					i), NULL, tr.fatLightmapSize, tr.fatLightmapSize,
				IMGFLAG_NOLIGHTSCALE |
				IMGFLAG_NO_COMPRESSION | IMGFLAG_CLAMPTOEDGE, textureInternalFormat);

			if(tr.worldDeluxeMapping){
				tr.deluxemaps[i] = R_CreateImage2(va("_fatdeluxemap%d",
						i), NULL, tr.fatLightmapSize, tr.fatLightmapSize,
					IMGFLAG_NORMALIZED |
	...
	for(i = 0; i < numLightmaps; i++){
		...
		/* if (tr.worldLightmapping) */
		{
			...
			for(j = 0; j < tr.lightmapSize * tr.lightmapSize; j++){
				if(r_hdr->integer){
					...
					scalev3(color, lightScale, color);
			...
			if(r_mergeLightmaps->integer)
				...
				tr.lightmaps[i] = R_CreateImage2(va("*lightmap%d",
						i), image, tr.lightmapSize, tr.lightmapSize,
					IMGFLAG_NOLIGHTSCALE |
					IMGFLAG_NO_COMPRESSION | IMGFLAG_CLAMPTOEDGE, textureInternalFormat);
		...
		if(tr.worldDeluxeMapping){
			...
			if(r_mergeLightmaps->integer){
				...
				tr.deluxemaps[i] = R_CreateImage2(va("*deluxemap%d",
						i), image, tr.lightmapSize, tr.lightmapSize,
					IMGFLAG_NORMALIZED |
					IMGFLAG_NOLIGHTSCALE | IMGFLAG_NO_COMPRESSION | IMGFLAG_CLAMPTOEDGE,
					0);
...
static float
FatPackU(float input, int lightmapnum)
{
	if(lightmapnum < 0)
		return input;
	...
	if(tr.fatLightmapSize > 0){
		...
		return (input /
			((float)tr.fatLightmapStep)) + ((1.0 / ((float)tr.fatLightmapStep)) * (float)x);
	}
...
static float
FatPackV(float input, int lightmapnum)
{
	if(lightmapnum < 0)
		return input;
	...
	if(tr.fatLightmapSize > 0){
		...
		return (input /
			((float)tr.fatLightmapStep)) + ((1.0 / ((float)tr.fatLightmapStep)) * (float)y);
	}
...
ParseFace(Dsurf *ds, Drawvert *verts, float *hdrVertColors, msurface_t *surf, int *indexes)
{
	...
	/* cv = ri.hunkalloc(sizeof(*cv), Hlow); */
	cv = (void*)surf->data;
	cv->surfaceType = SF_FACE;

	cv->numTriangles = numTriangles;
	cv->triangles = ri.hunkalloc(numTriangles * sizeof(cv->triangles[0]), Hlow);

	cv->numVerts = numVerts;
	cv->verts = ri.hunkalloc(numVerts * sizeof(cv->verts[0]), Hlow);
	...
	cv->plane.dist = dotv3(cv->verts[0].xyz, cv->plane.normal);
...
ParseTriSurf(Dsurf *ds, Drawvert *verts, float *hdrVertColors, msurface_t *surf, int *indexes)
{
	...
	for(i = 0, tri = cv->triangles; i < numTriangles; i++, tri++){
		...
		if((tri->indexes[0] == tri->indexes[1]) || (tri->indexes[1] == tri->indexes[2]) ||
		   (tri->indexes[0] == tri->indexes[2])){
			tri--;
			badTriangles++;
		}
	}
	...
	if(badTriangles){
		ri.Printf(PRINT_WARNING,
			"Surface has bad triangles, originally shader %s %d tris %d verts, now %d tris\n",
			surf->shader->name, numTriangles, numVerts,
			numTriangles - badTriangles);
...
R_MergedHeightPoints(srfGridMesh_t *grid, int offset)
{
	...
	for(i = 1; i < grid->height-1; i++)
		for(j = i + 1; j < grid->height-1; j++){
			if(fabs(grid->verts[grid->width * i +
					    offset].xyz[0] - grid->verts[grid->width * j + offset].xyz[0]) >
			   .1) continue;
			if(fabs(grid->verts[grid->width * i +
					    offset].xyz[1] - grid->verts[grid->width * j + offset].xyz[1]) >
			   .1) continue;
			if(fabs(grid->verts[grid->width * i +
					    offset].xyz[2] - grid->verts[grid->width * j + offset].xyz[2]) >
			   .1) continue;
			return qtrue;
		}
	return qfalse;
			...
			for(k = 1; k < grid1->width-1; k++){
				for(m = 0; m < 2; m++){
					...
					for(l = 1; l < grid2->width-1; l++){
						...
						if(fabs(grid1->verts[k +
								     offset1].xyz[1] -
							   grid2->verts[l + offset2].xyz[1]) >
						   .1) continue;
						if(fabs(grid1->verts[k +
								     offset1].xyz[2] -
							   grid2->verts[l + offset2].xyz[2]) >
						   .1) continue;
				...
				for(m = 0; m < 2; m++){
					...
					for(l = 1; l < grid2->height-1; l++){
						...
						if(fabs(grid1->verts[k +
								     offset1].xyz[1] -
							   grid2->verts[grid2->width * l + offset2].xyz[1]) >
						   .1) continue;
						if(fabs(grid1->verts[k +
								     offset1].xyz[2] -
							   grid2->verts[grid2->width * l + offset2].xyz[2]) >
						   .1) continue;
						/* ok the points are equal and should have the same lod error */
...
R_MovePatchSurfacesToHunk(void)
{
	...
	for(i = 0; i < s_worldData.numsurfaces; i++){
		...
		hunkgrid->triangles = ri.hunkalloc(grid->numTriangles * sizeof(srfTriangle_t), Hlow);
		Q_Memcpy(hunkgrid->triangles, grid->triangles, grid->numTriangles * sizeof(srfTriangle_t));

		hunkgrid->numVerts = grid->numVerts;
		hunkgrid->verts = ri.hunkalloc(grid->numVerts * sizeof(srfVert_t), Hlow);
...
static void
CopyVert(const srfVert_t * in, srfVert_t * out)
{
	int j;
...
R_CreateWorldVBO(void)
{
				...
				for(i = 0, tri = srf->triangles; i < srf->numTriangles; i++, tri++)
					for(j = 0; j < 3; j++)
						triangles[numTriangles +
							  i].indexes[j] = numVerts + tri->indexes[j];
	...
	s_worldData.ibo = R_CreateIBO2(va("staticBspModel0_IBO %i",
			0), numTriangles, triangles, VBO_USAGE_STATIC);
...
R_LoadSurfaces(Lump *surfs, Lump *verts, Lump *indexLump)
{
		...
		if(hdrVertColors){
			...
			if(size != sizeof(float) * 3 * (verts->filelen / sizeof(*dv)))
				ri.Error(ERR_DROP, "Bad size for %s (%i, expected %i)!\n", filename, size,
					sizeof(float) * 3 * (verts->filelen / sizeof(*dv)));
		}
	}
	...
	for(i = 0; i < count; i++, in++, out++){
		...
		case MST_TRIANGLE_SOUP:
			out->data = ri.hunkalloc(sizeof(srfTriangles_t), Hlow);
			break;
		case MST_PLANAR:
			out->data = ri.hunkalloc(sizeof(srfSurfaceFace_t), Hlow);
			break;
		case MST_FLARE:
			out->data = ri.hunkalloc(sizeof(srfFlare_t), Hlow);
			break;
		default:
			break;
		}
	...
	for(i = 0; i < count; i++, in++, out++){
		...
		case MST_PATCH:
			ParseMesh (in, dv, hdrVertColors, out);
			{
				...
				copyv3(surface->meshBounds[0], out->cullinfo.bounds[0]);
				copyv3(surface->meshBounds[1], out->cullinfo.bounds[1]);
				copyv3(surface->localOrigin, out->cullinfo.localOrigin);
		...
		case MST_TRIANGLE_SOUP:
			ParseTriSurf(in, dv, hdrVertColors, out, indexes);
			{
				...
				copyv3(surface->bounds[0], out->cullinfo.bounds[0]);
				copyv3(surface->bounds[1], out->cullinfo.bounds[1]);
			}
			numTriSurfs++;
			break;
		case MST_PLANAR:
...
R_LoadLightGrid(Lump *l)
{
	...
	if(r_hdr->integer){
		...
		if(hdrLightGrid){
			...
			w->hdrLightGrid = ri.hunkalloc(size, Hlow);
...
R_MergeLeafSurfaces(void)
{
	...
	/* create ibo */
	ibo = tr.ibos[tr.numIBOs++] = ri.hunkalloc(sizeof(*ibo), Hlow);
	...
	s_worldData.mergedSurfacesViewCount	= ri.hunkalloc(
		sizeof(*s_worldData.mergedSurfacesViewCount) * numMergedSurfaces, Hlow);
	s_worldData.mergedSurfacesDlightBits	= ri.hunkalloc(
		sizeof(*s_worldData.mergedSurfacesDlightBits) * numMergedSurfaces, Hlow);
	...
	s_worldData.viewSurfaces = ri.hunkalloc(
		sizeof(*s_worldData.viewSurfaces) * s_worldData.nummarksurfaces, Hlow);
	...
	for(i = 0; i < numWorldSurfaces; i++){
		...
		for(j = i; j < numWorldSurfaces; j++){
			...
			case SF_FACE:
			{
				...
				for(k = 0; k < face->numTriangles; k++){
					*outIboIndexes++ = face->triangles[k].indexes[0] +
							   face->firstVert;
					*outIboIndexes++ = face->triangles[k].indexes[1] +
							   face->firstVert;
					*outIboIndexes++ = face->triangles[k].indexes[2] +
							   face->firstVert;
					numIboIndexes += 3;
				}
			}
			...
			case SF_GRID:
			{
				...
				for(k = 0; k < grid->numTriangles; k++){
					*outIboIndexes++ = grid->triangles[k].indexes[0] +
							   grid->firstVert;
					*outIboIndexes++ = grid->triangles[k].indexes[1] +
							   grid->firstVert;
					*outIboIndexes++ = grid->triangles[k].indexes[2] +
							   grid->firstVert;
					numIboIndexes += 3;
				}
			}
			...
			case SF_TRIANGLES:
			{
				...
				for(k = 0; k < tris->numTriangles; k++){
					*outIboIndexes++ = tris->triangles[k].indexes[0] +
							   tris->firstVert;
					*outIboIndexes++ = tris->triangles[k].indexes[1] +
							   tris->firstVert;
					*outIboIndexes++ = tris->triangles[k].indexes[2] +
							   tris->firstVert;
					numIboIndexes += 3;
				}
			}
		...
		vboSurf = ri.hunkalloc(sizeof(*vboSurf), Hlow);
		...
		copyv3(bounds[0], vboSurf->bounds[0]);
		copyv3(bounds[1], vboSurf->bounds[1]);

		copyv3(bounds[0], mergedSurf->cullinfo.bounds[0]);
		copyv3(bounds[1], mergedSurf->cullinfo.bounds[1]);
	...
	qglBufferDataARB(GL_ELEMENT_ARRAY_BUFFER_ARB, numIboIndexes * sizeof(*iboIndexes), iboIndexes,
		GL_STATIC_DRAW_ARB);
...
R_CalcVertexLightDirs(void)
{
	...
	for(k = 0, surface = &s_worldData.surfaces[0]; k < s_worldData.numsurfaces /* s_worldData.numWorldSurfaces */;
		...
		if(*surface->data == SF_FACE){
			...
			if(srf->numVerts){
				for(i = 0; i < srf->numVerts; i++)
					R_LightDirForPoint(srf->verts[i].xyz, srf->verts[i].lightdir,
						srf->verts[i].normal,
						&s_worldData);
			}
		}else if(*surface->data == SF_GRID){
