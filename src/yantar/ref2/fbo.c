...
R_CheckFBO(const FBO_t * fbo)
{
	...
	case GL_FRAMEBUFFER_INCOMPLETE_DIMENSIONS_EXT:
		ri.Printf(
			PRINT_WARNING,
			"R_CheckFBO: (%s) Framebuffer incomplete, attached images must have same dimensions\n",
			fbo->name);
		break;

	case GL_FRAMEBUFFER_INCOMPLETE_FORMATS_EXT:
		ri.Printf(PRINT_WARNING,
			"R_CheckFBO: (%s) Framebuffer incomplete, attached images must have same format\n",
			fbo->name);
		break;
...
FBO_Create(const char *name, int width, int height)
{
	...
	fbo = tr.fbos[tr.numFBOs] = ri.hunkalloc(sizeof(*fbo), h_low);
...
void
FBO_CreateBuffer(FBO_t *fbo, int format, int index)
{
	...
	if(absent){
		if(attachment == 0){
			...
			qglFramebufferRenderbufferEXT(GL_FRAMEBUFFER_EXT, attachment, GL_RENDERBUFFER_EXT,
				*pRenderBuffer);
	}
}
...
R_AttachFBOTexture1D(int texId, int index)
{
	...
	qglFramebufferTexture1DEXT(GL_FRAMEBUFFER_EXT, GL_COLOR_ATTACHMENT0_EXT + index, GL_TEXTURE_1D, texId,
		0);
}
...
R_AttachFBOTexture3D(int texId, int index, int zOffset)
{
	...
	qglFramebufferTexture3DEXT(GL_FRAMEBUFFER_EXT, GL_COLOR_ATTACHMENT0_EXT + index, GL_TEXTURE_3D_EXT,
		texId, 0,
		zOffset);
}
...
FBO_Init(void)
{
...
#ifdef REACTION
	{
		tr.godRaysFbo = FBO_Create("_godRays", tr.renderDepthImage->width,
	...
	for(i = 0; i < MAX_DRAWN_PSHADOWS; i++){
		tr.pshadowFbos[i] = FBO_Create(va("_shadowmap%d",
				i), tr.pshadowMaps[i]->width, tr.pshadowMaps[i]->height);
...

	{
		...
		tr.screenScratchFbo = FBO_Create("_screenscratch", tr.screenScratchImage->width,
	...
	for(i = 0; i < 2; i++){
		tr.quarterFbo[i] = FBO_Create(va("_quarter%d",
				i), tr.quarterImage[i]->width, tr.quarterImage[i]->height);
		    ...
		    int blend)
{
	...
	texCoords[1][0] =
		(srcBox[0] +
		 srcBox[2]) / (float)src->width; texCoords[1][1] = 1.0f - (srcBox[1]) / (float)src->height;
	texCoords[2][0] =
		(srcBox[0] +
		 srcBox[2]) / (float)src->width; texCoords[2][1] = 1.0f -
								   (srcBox[1] +
								    srcBox[3]) / (float)src->height;
	texCoords[3][0] = (srcBox[0]) / (float)src->width; texCoords[3][1] = 1.0f -
									     (srcBox[1] +
									      srcBox[3]) / (float)src->height;
	 ...
	 int blend)
{
	if(1){	/* (glRefConfig.extFramebufferBlit && (sp == NULL || sp == &tr.textureColorShader)) */
		FBO_BlitFromTexture(src->colorImage[0], srcBox, srcTexScale, dst, dstBox, shaderProgram,
			color,
			blend);
	}
}
