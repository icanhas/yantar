...
Cvar *r_ext_multi_draw_arrays;
Cvar *r_ext_framebuffer_object;
Cvar *r_ext_texture_float;
Cvar *r_arb_half_float_pixel;

Cvar *r_mergeMultidraws;
Cvar *r_mergeLeafSurfaces;

Cvar *r_cameraExposure;
Cvar *r_hdr;
Cvar *r_postProcess;
Cvar *r_toneMap;
Cvar *r_autoExposure;
...
Cvar *r_deluxeMapping;
Cvar *r_parallaxMapping;
Cvar *r_normalAmbient;
Cvar *r_recalcMD3Normals;
Cvar *r_mergeLightmaps;
Cvar *r_dlightMode;
Cvar *r_pshadowDist;
Cvar *r_imageUpsample;
Cvar *r_imageUpsampleMaxSize;
Cvar *r_imageUpsampleType;
 ...
 */
void
GL_CheckErrs(char *file, int line)
{
	int	err;
	char	s[64];
...
R_Register(void)
{
	...
	r_ext_multi_draw_arrays		= ri.cvarget("r_ext_multi_draw_arrays", "1",
		CVAR_ARCHIVE | CVAR_LATCH);
	r_ext_framebuffer_object	= ri.cvarget("r_ext_framebuffer_object", "1",
		CVAR_ARCHIVE | CVAR_LATCH);
	r_ext_texture_float = ri.cvarget("r_ext_texture_float", "1", CVAR_ARCHIVE | CVAR_LATCH);
	r_arb_half_float_pixel = ri.cvarget("r_arb_half_float_pixel", "1", CVAR_ARCHIVE | CVAR_LATCH);
	...
	r_hdr = ri.cvarget("r_hdr", "1", CVAR_ARCHIVE | CVAR_LATCH);
	r_postProcess = ri.cvarget("r_postProcess", "1", CVAR_ARCHIVE);
	r_toneMap = ri.cvarget("r_toneMap", "1", CVAR_ARCHIVE);
	r_autoExposure		= ri.cvarget("r_autoExposure", "1", CVAR_ARCHIVE);
	r_cameraExposure	= ri.cvarget("r_cameraExposure", "0", CVAR_CHEAT);

	r_normalMapping		= ri.cvarget("r_normalMapping", "1", CVAR_ARCHIVE | CVAR_LATCH);
	r_specularMapping	= ri.cvarget("r_specularMapping", "1", CVAR_ARCHIVE | CVAR_LATCH);
	r_deluxeMapping		= ri.cvarget("r_deluxeMapping", "1", CVAR_ARCHIVE | CVAR_LATCH);
	r_parallaxMapping	= ri.cvarget("r_parallaxMapping", "0", CVAR_ARCHIVE | CVAR_LATCH);
	r_normalAmbient		= ri.cvarget("r_normalAmbient", "0", CVAR_ARCHIVE | CVAR_LATCH);
	r_dlightMode		= ri.cvarget("r_dlightMode", "0", CVAR_ARCHIVE | CVAR_LATCH);
	r_pshadowDist		= ri.cvarget("r_pshadowDist", "128", CVAR_ARCHIVE);
	r_recalcMD3Normals	= ri.cvarget("r_recalcMD3Normals", "0", CVAR_ARCHIVE | CVAR_LATCH);
	r_mergeLightmaps = ri.cvarget("r_mergeLightmaps", "1", CVAR_ARCHIVE | CVAR_LATCH);
	r_imageUpsample = ri.cvarget("r_imageUpsample", "0", CVAR_ARCHIVE | CVAR_LATCH);
	r_imageUpsampleMaxSize = ri.cvarget("r_imageUpsampleMaxSize", "1024", CVAR_ARCHIVE | CVAR_LATCH);
	r_imageUpsampleType = ri.cvarget("r_imageUpsampleType", "1", CVAR_ARCHIVE | CVAR_LATCH);
	...
	r_mergeMultidraws	= ri.cvarget("r_mergeMultidraws", "1", CVAR_ARCHIVE);
	r_mergeLeafSurfaces	= ri.cvarget("r_mergeLeafSurfaces", "1", CVAR_ARCHIVE);
	...
	ri.cmdadd("gfxmeminfo", GfxMemInfo_f);
}
