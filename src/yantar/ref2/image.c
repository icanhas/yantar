...
*/
void
R_GammaCorrect(byte *buffer, int bufSize)
{
	int i;
...
R_ImageList_f(void)
{
	...
	for(i = 0; i < tr.numImages; i++){
		...
		ri.Printf (PRINT_ALL,  "%4i: %4i %4i  %s   %d   ",
			i, image->uploadWidth, image->uploadHeight,
			yesno[(image->flags & IMGFLAG_MIPMAP) ? 1 : 0], image->TMU);
...
static void
DoFCBI(byte *in, byte *out, int width, int height)
{
	int x, y, i;
	byte *outbyte, *inbyte;
	...
	for(y = 3; y < height - 4; y += 2){
		...
		COPYSAMPLE(sc, line2); line2 += 8; COPYSAMPLE(sd, line2); line2 += 8; COPYSAMPLE(se, line2);
		line2 += 8;
		COPYSAMPLE(sg, line3); line3 += 8; COPYSAMPLE(sh, line3); line3 += 8; COPYSAMPLE(si, line3);
		line3 += 8;
...
static void
DoFCBIQuick(byte *in, byte *out, int width, int height)
{
	int x, y, i;
	byte *outbyte, *inbyte;
...
static void
ExpandHalfTextureToGrid(byte *data, int width, int height)
{
	int x, y;
		...
		for(x = 0; x < width; x += WORKBLOCK_SIZE){
			...
			for(y2 = 0; y2 < fullworkheight; y2 += 2)
				for(x2 = 0; x2 < fullworkwidth; x2 += 2){
					...
					for(i = 0; i < 4; i++)
						workdata[(dsty * fullworkwidth +
							  dstx) * 4 +
							 i] = data[(srcy * width + srcx) * 4 + i];
				}
			...
			for(y2 = 0; y2 < workheight; y2++){
				inbyte =
					&outdata[((y2 +
						   WORKBLOCK_BORDER) * fullworkwidth + WORKBLOCK_BORDER) * 4];
		...
		for(j = 0; j < outWidth; j++){
			...
			for(k = 0; k < 4; k++){
				total =
					...
					(&in[ 4*
					      (((i*2-
						 1)&inHeightMask)*inWidth + ((j*2-1)&inWidthMask)) ])[k] +
					2 *
					(&in[ 4*
					      (((i*2-1)&inHeightMask)*inWidth + ((j*2)&inWidthMask)) ])[k] +
					2 *
					(&in[ 4*
					      (((i*2-
						 1)&inHeightMask)*inWidth + ((j*2+1)&inWidthMask)) ])[k] +
					...
					(&in[ 4*
					      (((i*2-
						 1)&inHeightMask)*inWidth + ((j*2+2)&inWidthMask)) ])[k] +

					2 *
					(&in[ 4*
					      (((i*2)&inHeightMask)*inWidth + ((j*2-1)&inWidthMask)) ])[k] +
					4 *
					(&in[ 4*(((i*2)&inHeightMask)*inWidth + ((j*2)&inWidthMask)) ])[k] +
					4 *
					(&in[ 4*
					      (((i*2)&inHeightMask)*inWidth + ((j*2+1)&inWidthMask)) ])[k] +
					2 *
					(&in[ 4*
					      (((i*2)&inHeightMask)*inWidth + ((j*2+2)&inWidthMask)) ])[k] +
					...
					(&in[ 4*
					      (((i*2+
						 2)&inHeightMask)*inWidth + ((j*2-1)&inWidthMask)) ])[k] +
					2 *
					(&in[ 4*
					      (((i*2+2)&inHeightMask)*inWidth + ((j*2)&inWidthMask)) ])[k] +
					2 *
					(&in[ 4*
					      (((i*2+
						 2)&inHeightMask)*inWidth + ((j*2+1)&inWidthMask)) ])[k] +
					1 *
					(&in[ 4*
					      (((i*2+2)&inHeightMask)*inWidth + ((j*2+2)&inWidthMask)) ])[k];
				outpix[k] = total / 36;
			}
		}
...
static void
RawImage_SwizzleRA(byte *data, int width, int height)
{
	int i;
	byte *ptr = data, swap;
...
RawImage_ScaleToPower2(byte **data, int *inout_width, int *inout_height, int *inout_scaled_width,
		       int *inout_scaled_height, imgFlags_t flags,
		       byte **resampledBuffer)
{
	...
	if(picmip && data && resampledBuffer && r_imageUpsample->integer &&
	   scaled_width < r_imageUpsampleMaxSize->integer && scaled_height <
	   r_imageUpsampleMaxSize->integer){
	...
	if(flags & IMGFLAG_MIPMAP){
		...
		while(width > 1 || height > 1){
			...
			if(subtexture){
				x >>= 1;
				y >>= 1;
				qglTexSubImage2D(GL_TEXTURE_2D, miplevel, x, y, width, height, dataFormat,
					dataType,
					data);
			}else{
				qglTexImage2D (GL_TEXTURE_2D, miplevel, internalFormat, width, height, 0,
					dataFormat, dataType,
					data);
			}
		}
	 ...
	 qbool lightMap, int *format, int *pUploadWidth, int *pUploadHeight)
{
	...
	if((scaled_width == width) &&
	   (scaled_height == height)){
		if(!(flags & IMGFLAG_MIPMAP)){
			RawImage_UploadTexture(data, 0, 0, scaled_width, scaled_height, internalFormat, flags,
				qfalse);
...
R_CreateImage2(const char *name, byte *pic, int width, int height, imgFlags_t flags, int internalFormat)
{
	...
	if(image->flags & IMGFLAG_CUBEMAP){
		...
		qglTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X, 0, GL_RGBA8, width, height, 0, GL_BGRA,
			GL_UNSIGNED_BYTE,
			pic);
		qglTexImage2D(GL_TEXTURE_CUBE_MAP_NEGATIVE_X, 0, GL_RGBA8, width, height, 0, GL_BGRA,
			GL_UNSIGNED_BYTE,
			pic);
		qglTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_Y, 0, GL_RGBA8, width, height, 0, GL_BGRA,
			GL_UNSIGNED_BYTE,
			pic);
		qglTexImage2D(GL_TEXTURE_CUBE_MAP_NEGATIVE_Y, 0, GL_RGBA8, width, height, 0, GL_BGRA,
			GL_UNSIGNED_BYTE,
			pic);
		qglTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_Z, 0, GL_RGBA8, width, height, 0, GL_BGRA,
			GL_UNSIGNED_BYTE,
			pic);
		qglTexImage2D(GL_TEXTURE_CUBE_MAP_NEGATIVE_Z, 0, GL_RGBA8, width, height, 0, GL_BGRA,
			GL_UNSIGNED_BYTE,
			pic);
...
void
R_UpdateSubImage(image_t *image, byte *pic, int x, int y, int width, int height)
{
	byte *scaledBuffer = NULL;
	byte *resampledBuffer = NULL;
	...
	if((scaled_width == width) &&
	   (scaled_height == height)){
		if(!(image->flags & IMGFLAG_MIPMAP)){
			...
			RawImage_UploadTexture(data, scaled_x, scaled_y, scaled_width, scaled_height,
				image->internalFormat, image->flags,
				qtrue);
	...
	scaled_x = x * scaled_width / width;
	scaled_y = y * scaled_height / height;
	RawImage_UploadTexture((byte*)data, scaled_x, scaled_y, scaled_width, scaled_height,
		image->internalFormat, image->flags,
		qtrue);
...
R_CreateDlightImage(void)
{
	...
	tr.dlightImage = R_CreateImage("*dlight", (byte*)data, DLIGHT_SIZE, DLIGHT_SIZE, qfalse, qfalse,
		GL_CLAMP_TO_EDGE);
}
...
R_CreateBuiltinImages(void)
{
...

	{
		...
		tr.renderImage =
			R_CreateImage2("_render", NULL, width, height, IMGFLAG_NO_COMPRESSION |
				IMGFLAG_CLAMPTOEDGE,
				hdrFormat);
#ifdef REACTION
		tr.godRaysImage =
			R_CreateImage2("*godRays", NULL, width, height, IMGFLAG_NO_COMPRESSION |
				IMGFLAG_CLAMPTOEDGE,
				GL_RGBA8);
#endif

		tr.screenScratchImage =
			R_CreateImage2("*screenScratch", NULL, width, height, IMGFLAG_NO_COMPRESSION |
				IMGFLAG_CLAMPTOEDGE,
				GL_RGBA8);

		if(glRefConfig.framebufferObject){
			tr.renderDepthImage =
				R_CreateImage2("*renderdepth",  NULL, width, height, IMGFLAG_NO_COMPRESSION |
					IMGFLAG_CLAMPTOEDGE,
					GL_DEPTH_COMPONENT24_ARB);
			tr.textureDepthImage =
				R_CreateImage2("*texturedepth", NULL, 256, 256, IMGFLAG_NO_COMPRESSION |
					IMGFLAG_CLAMPTOEDGE,
					GL_DEPTH_COMPONENT24_ARB);
		}

		{
			...
			tr.calcLevelsImage =  R_CreateImage2(
				"*calcLevels",  p, 1, 1,IMGFLAG_NO_COMPRESSION | IMGFLAG_CLAMPTOEDGE,
				hdrFormat);
			tr.fixedLevelsImage =
				R_CreateImage2("*fixedLevels", p, 1, 1, IMGFLAG_NO_COMPRESSION |
					IMGFLAG_CLAMPTOEDGE,
					hdrFormat);
		}

		for(x = 0; x < 2; x++)
			tr.textureScratchImage[x] = R_CreateImage2(va("*textureScratch%d",
					x), NULL, 256, 256, IMGFLAG_NO_COMPRESSION | IMGFLAG_CLAMPTOEDGE,
				GL_RGBA8);
		for(x = 0; x < 2; x++)
			tr.quarterImage[x] = R_CreateImage2(va("*quarter%d",
					x), NULL, 512, 512, IMGFLAG_NO_COMPRESSION | IMGFLAG_CLAMPTOEDGE,
				GL_RGBA8);
	}
