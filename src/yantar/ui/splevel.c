...
#define ART_LEVELFRAME_FOCUS	Pmenuart "/maps_select"
#define ART_LEVELFRAME_SELECTED Pmenuart "/maps_selected"
#define ART_ARROW		Pmenuart "/narrow_0"
#define ART_ARROW_FOCUS		Pmenuart "/narrow_1"
#define ART_MAP_UNKNOWN		Pmenuart "/unknownmap"
#define ART_MAP_COMPLETE1	Pmenuart "/level_complete1"
#define ART_MAP_COMPLETE2	Pmenuart "/level_complete2"
#define ART_MAP_COMPLETE3	Pmenuart "/level_complete3"
#define ART_MAP_COMPLETE4	Pmenuart "/level_complete4"
#define ART_MAP_COMPLETE5	Pmenuart "/level_complete5"
#define ART_BACK0		Pmenuart "/back_0"
#define ART_BACK1		Pmenuart "/back_1"
#define ART_FIGHT0		Pmenuart "/fight_0"
#define ART_FIGHT1		Pmenuart "/fight_1"
#define ART_RESET0		Pmenuart "/reset_0"
#define ART_RESET1		Pmenuart "/reset_1"
#define ART_CUSTOM0		Pmenuart "/skirmish_0"
#define ART_CUSTOM1		Pmenuart "/skirmish_1"
...
UI_SPLevelMenu_SetBots(void)
{
	...
	Q_strncpyz(bots,
		Info_ValueForKey(levelMenuInfo.selectedArenaInfo,
			"bots"), sizeof(bots));
	...
	while(*p && levelMenuInfo.numBots < 7){
		...
		if(botInfo){
			levelMenuInfo.botPics[levelMenuInfo.numBots] =
				PlayerIconHandle(Info_ValueForKey(botInfo,
						"model"));
			Q_strncpyz(levelMenuInfo.botNames[levelMenuInfo.numBots],
				Info_ValueForKey(botInfo,
					"name"), 10);
		}else{
		...
		if(selectedArena != -1)
			levelMenuInfo.selectedArenaInfo =
				UI_GetArenaInfoByNumber(
					selectedArenaSet * ARENAS_PER_TIER +
					selectedArena);
	}
...
UI_SPLevelMenu_ResetDraw(void)
{
	UI_DrawProportionalString(SCREEN_WIDTH/2, 356 + PROP_HEIGHT * 0,
		"WARNING: This resets all of the", UI_CENTER|UI_SMALLFONT,
		color_yellow);
	UI_DrawProportionalString(SCREEN_WIDTH/2, 356 + PROP_HEIGHT * 1,
		"single player game variables.", UI_CENTER|UI_SMALLFONT,
		color_yellow);
	UI_DrawProportionalString(SCREEN_WIDTH/2, 356 + PROP_HEIGHT * 2,
		"Do this only if you want to", UI_CENTER|UI_SMALLFONT,
		color_yellow);
	UI_DrawProportionalString(SCREEN_WIDTH/2, 356 + PROP_HEIGHT * 3,
		"start over from the beginning.", UI_CENTER|UI_SMALLFONT,
		color_yellow);
}
...
/*	int				fraglimit; */
	...
	if(Q_stricmp(buf, levelMenuInfo.playerModel) != 0){
		...
		PlayerIcon(levelMenuInfo.playerModel,
	...
	UI_DrawProportionalString(18, 38, va("Tier %i",
			selectedArenaSet + 1), UI_LEFT|UI_SMALLFONT,
		color_orange);
	...
	if(selectedArenaSet > currentSet){
		UI_DrawProportionalString(320, 216, "ACCESS DENIED", UI_CENTER|
			UI_BIGFONT,
			color_red);
		return;
	}
	...
	for(n = 0; n < levelMenuInfo.numMaps; n++){
		...
		if(levelMenuInfo.levelScores[n] == 1)
			UI_DrawHandlePic(
				x, y, 128, 96,
				levelMenuInfo.levelCompletePic[levelMenuInfo.
							       levelScoresSkill[
								       n] - 1]);
 ...
 * UI_DrawString( 18, 212, va("Frags %i", fraglimit) , UI_LEFT|UI_SMALLFONT, color_orange ); */
	...
	for(n = 0; n < levelMenuInfo.numBots; n++){
		...
		if(levelMenuInfo.botPics[n])
			...
			UI_DrawProportionalString(x+22, y+18, "?", UI_BIGFONT,
				color_orange);
		}
		UI_DrawString(x, y + 64, levelMenuInfo.botNames[n],
			UI_SMALLFONT|UI_LEFT,
			color_orange);
	}
}
...
UI_SPLevelMenu_Cache(void)
{
	...
	levelMenuInfo.levelSelectedPic = trap_R_RegisterShaderNoMip(
		ART_LEVELFRAME_SELECTED);
	levelMenuInfo.levelFocusPic = trap_R_RegisterShaderNoMip(
		ART_LEVELFRAME_FOCUS);
	levelMenuInfo.levelCompletePic[0] = trap_R_RegisterShaderNoMip(
		ART_MAP_COMPLETE1);
	levelMenuInfo.levelCompletePic[1] = trap_R_RegisterShaderNoMip(
		ART_MAP_COMPLETE2);
	levelMenuInfo.levelCompletePic[2] = trap_R_RegisterShaderNoMip(
		ART_MAP_COMPLETE3);
	levelMenuInfo.levelCompletePic[3] = trap_R_RegisterShaderNoMip(
		ART_MAP_COMPLETE4);
	levelMenuInfo.levelCompletePic[4] = trap_R_RegisterShaderNoMip(
		ART_MAP_COMPLETE5);
}
...
UI_SPLevelMenu_Init(void)
{
	...
	levelMenuInfo.item_leftarrow.generic.flags	= QMF_LEFT_JUSTIFY|
							  QMF_PULSEIFFOCUS;
	levelMenuInfo.item_leftarrow.generic.x	= 18;
	levelMenuInfo.item_leftarrow.generic.y	= 64;
	levelMenuInfo.item_leftarrow.generic.callback =
		UI_SPLevelMenu_LeftArrowEvent;
	...
	levelMenuInfo.item_maps[0].generic.name =
		levelMenuInfo.levelPicNames[0];
	...
	levelMenuInfo.item_maps[1].generic.name =
		levelMenuInfo.levelPicNames[1];
	...
	levelMenuInfo.item_maps[2].generic.name =
		levelMenuInfo.levelPicNames[2];
	...
	levelMenuInfo.item_maps[3].generic.name =
		levelMenuInfo.levelPicNames[3];
	...
	levelMenuInfo.item_rightarrow.generic.flags	= QMF_LEFT_JUSTIFY|
							  QMF_PULSEIFFOCUS;
	...
	levelMenuInfo.awardLevels[AWARD_FRAGS] = 100 *
						 (levelMenuInfo.awardLevels[
							  AWARD_FRAGS] / 100);
	...
	for(n = 0; n < 6; n++)
		if(levelMenuInfo.awardLevels[n]){
			...
			levelMenuInfo.item_awards[count].generic.type =
				MTYPE_BITMAP;
			levelMenuInfo.item_awards[count].generic.name =
				ui_medalPicNames[n];
			...
			levelMenuInfo.item_awards[count].generic.id	=
				ID_AWARD1 + n;
	...
	levelMenuInfo.item_back.generic.flags	= QMF_LEFT_JUSTIFY|
						  QMF_PULSEIFFOCUS;
	...
	levelMenuInfo.item_reset.generic.flags	= QMF_LEFT_JUSTIFY|
						  QMF_PULSEIFFOCUS;
	...
	levelMenuInfo.item_custom.generic.flags = QMF_LEFT_JUSTIFY|
						  QMF_PULSEIFFOCUS;
	...
	levelMenuInfo.item_next.generic.flags	= QMF_RIGHT_JUSTIFY|
						  QMF_PULSEIFFOCUS;
	...
	levelMenuInfo.item_null.generic.flags	= QMF_LEFT_JUSTIFY|
						  QMF_MOUSEONLY|
						  QMF_SILENT;
