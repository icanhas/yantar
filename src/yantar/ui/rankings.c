...
#define RANKINGS_FRAME	Pmenuart "/cut_frame"
...
Rankings_MenuInit(void)
{
	...
	s_rankings.spectate.generic.flags	= QMF_CENTER_JUSTIFY|
						  QMF_PULSEIFFOCUS;
	...
	if((status != QGR_STATUS_NEW) && (status != QGR_STATUS_SPECTATOR)){
		s_rankings.login.generic.flags |= QMF_HIDDEN |
						  QMF_INACTIVE;
		s_rankings.create.generic.flags |= QMF_HIDDEN |
						   QMF_INACTIVE;
		s_rankings.spectate.generic.flags |= QMF_HIDDEN |
						     QMF_INACTIVE;
