...
StartServer_Update(void)
{
	...
	for(i=0; i<MAX_MAPSPERPAGE; i++){
		...
		Q_strncpyz(mapname, Info_ValueForKey(info,
				"map"), MAX_NAMELENGTH);
		Q_strupr(mapname);
	...
	if(!s_startserver.nummaps){
		...
		info =
			UI_GetArenaInfoByNumber(
				s_startserver.maplist[
					s_startserver.
					currentmap
				]);
		Q_strncpyz(s_startserver.mapname.string,
			Info_ValueForKey(info, "map"), MAX_NAMELENGTH);
	}
...
StartServer_MapEvent(void* ptr, int event)
{
	...
	s_startserver.currentmap =
		(s_startserver.page*
		 MAX_MAPSPERPAGE) + (((menucommon_s*)ptr)->id - ID_PICTURES);
	StartServer_Update();
}
...
StartServer_LevelshotDraw(void *self)
{
	...
	x	+= b->width / 2;
	y	+= 4;
	n	= s_startserver.page * MAX_MAPSPERPAGE + b->generic.id -
		  ID_PICTURES;

	info = UI_GetArenaInfoByNumber(s_startserver.maplist[ n ]);
	UI_DrawString(x, y, Info_ValueForKey(info,
			"map"), UI_CENTER|UI_SMALLFONT, color_orange);
...
StartServer_MenuInit(void)
{
	...
	for(i=0; i<MAX_MAPSPERPAGE; i++){
		...
		s_startserver.mappics[i].generic.ownerdraw =
			StartServer_LevelshotDraw;
		...
		s_startserver.mapbuttons[i].generic.callback =
			StartServer_MapEvent;
		...
		s_startserver.mapbuttons[i].focuspic =
			GAMESERVER_SELECT;
	}
	...
	s_startserver.prevpage.generic.flags = QMF_LEFT_JUSTIFY|
					       QMF_PULSEIFFOCUS;
	...
	s_startserver.nextpage.generic.flags = QMF_LEFT_JUSTIFY|
					       QMF_PULSEIFFOCUS;
	...
	s_startserver.mapname.generic.flags	= QMF_CENTER_JUSTIFY|
						  QMF_INACTIVE;
	...
	s_startserver.back.generic.flags = QMF_LEFT_JUSTIFY|
					   QMF_PULSEIFFOCUS;
	...
	s_startserver.next.generic.flags = QMF_RIGHT_JUSTIFY|
					   QMF_PULSEIFFOCUS;
	...
	s_startserver.item_null.generic.flags = QMF_LEFT_JUSTIFY|
						QMF_MOUSEONLY|
						QMF_SILENT;
	...
	for(n = 1; n < PLAYER_SLOTS; n++){
		...
		if((s_serveroptions.gametype >= GT_TEAM) &&
		   (s_serveroptions.playerTeam[n].curvalue !=
		    s_serveroptions.playerTeam[s_serveroptions.newBotIndex].
		    curvalue))
			continue;
		if(Q_stricmp(checkName,
			   s_serveroptions.playerNameBuffers[n]) == 0)
			return qtrue;
	}
...
ServerOptions_Start(void)
{
	...
	info =
		UI_GetArenaInfoByNumber(s_startserver.maplist[ s_startserver.
							       currentmap ]);
	...
	if(dedicated == 0 && s_serveroptions.gametype >= GT_TEAM)
		trap_Cmd_ExecuteText(EXEC_APPEND,
			va("wait 5; team %s\n",
				playerTeam_list[s_serveroptions.playerTeam[0].
						curvalue]));
}
...
ServerOptions_LevelshotDraw(void *self)
{
	...
	if(s_serveroptions.newBot){
		Q_strncpyz(
			s_serveroptions.playerNameBuffers[s_serveroptions.
							  newBotIndex],
	...
	x	+= b->width / 2;
	y	+= 4;
	UI_DrawString(x, y, s_serveroptions.mapnamebuffer, UI_CENTER|
		UI_SMALLFONT,
		color_orange);
...
ServerOptions_InitBotNames(void)
{
	...
	if(s_serveroptions.gametype >= GT_TEAM){
		...
		if(s_serveroptions.gametype == GT_TEAM)
			Q_strncpyz(s_serveroptions.playerNameBuffers[3], "visor",
				16);
		else
		...
		if(s_serveroptions.gametype == GT_TEAM)
			Q_strncpyz(s_serveroptions.playerNameBuffers[9], "visor",
				16);
		else
...
PlayerName_Draw(void *item)
{
	...
	if(focus){
		/* draw cursor */
		UI_FillRect(s->generic.left, s->generic.top, s->generic.right-
			s->generic.left+1, s->generic.bottom-s->generic.top+1,
			listbar_color);
...
ServerOptions_MenuInit(qbool multiplayer)
{
	...
	if(s_serveroptions.gametype != GT_CTF){
		...
		s_serveroptions.fraglimit.generic.flags = QMF_NUMBERSONLY|
							  QMF_PULSEIFFOCUS|
							  QMF_SMALLFONT;
		...
		s_serveroptions.flaglimit.generic.flags = QMF_NUMBERSONLY|
							  QMF_PULSEIFFOCUS|
							  QMF_SMALLFONT;
	...
	s_serveroptions.timelimit.generic.flags = QMF_NUMBERSONLY|
	...
	if(s_serveroptions.gametype >= GT_TEAM){
		y += BIGCHAR_HEIGHT+2;
		s_serveroptions.friendlyfire.generic.type =
			MTYPE_RADIOBUTTON;
	...
	if(s_serveroptions.multiplayer){
		...
		s_serveroptions.dedicated.generic.flags = QMF_PULSEIFFOCUS|
							  QMF_SMALLFONT;
	...
	s_serveroptions.botSkill.generic.x = 32 +
					     (strlen(s_serveroptions.botSkill.
						      generic.name) +
					      2) * SMALLCHAR_WIDTH;
	...
	for(n = 0; n < PLAYER_SLOTS; n++){
		...
		s_serveroptions.playerName[n].generic.left =
			s_serveroptions.playerName[n].generic.x -
			SMALLCHAR_HEIGHT/ 2;
		s_serveroptions.playerName[n].generic.right =
			s_serveroptions.playerName[n].generic.x + 16 *
			SMALLCHAR_WIDTH;

		s_serveroptions.playerTeam[n].generic.type =
			MTYPE_SPINCONTROL;
	...
	s_serveroptions.back.generic.flags = QMF_LEFT_JUSTIFY|
					     QMF_PULSEIFFOCUS;
	...
	s_serveroptions.next.generic.flags = QMF_RIGHT_JUSTIFY|
					     QMF_PULSEIFFOCUS|
					     QMF_INACTIVE|QMF_GRAYED|
					     QMF_HIDDEN;
	...
	s_serveroptions.go.generic.flags = QMF_RIGHT_JUSTIFY|
					   QMF_PULSEIFFOCUS;
	...
	for(n = 0; n < PLAYER_SLOTS; n++){
		if(n != 0)
			Menu_AddItem(&s_serveroptions.menu,
				&s_serveroptions.playerType[n]);
		Menu_AddItem(&s_serveroptions.menu,
			&s_serveroptions.playerName[n]);
		if(s_serveroptions.gametype >= GT_TEAM)
			Menu_AddItem(&s_serveroptions.menu,
				&s_serveroptions.playerTeam[n]);
	}
...
UI_BotSelectMenu_BuildList(void)
{
	...
	qsort(botSelectInfo.sortedBotNums, botSelectInfo.numBots,
		sizeof(botSelectInfo.sortedBotNums[0]),
		UI_BotSelectMenu_SortCompare);
}
	...
	for(i = 0; i < (PLAYERGRID_ROWS * PLAYERGRID_COLS); i++, j++){
		if(j < botSelectInfo.numBots){
			info = UI_GetBotInfoByNumber(
				botSelectInfo.sortedBotNums[j]);
			ServerPlayerIcon(Info_ValueForKey(info,
					"model"), botSelectInfo.boticons[i],
				MAX_QPATH);
...
UI_BotSelectMenu_LeftEvent(void* ptr, int event)
{
	...
	if(botSelectInfo.modelpage > 0){
		...
		botSelectInfo.selectedmodel = botSelectInfo.modelpage *
					      MAX_MODELSPERPAGE;
		UI_BotSelectMenu_UpdateGrid();
	}
}
...
UI_BotSelectMenu_SelectEvent(void* ptr, int event)
{
	...
	Q_strncpyz(
		...
		botSelectInfo.botnames[botSelectInfo.selectedmodel %
				       MAX_MODELSPERPAGE],
		16);
}
...
UI_BotSelectMenu_Init(char *bot)
{
	...
	for(i = 0, k = 0; i < PLAYERGRID_ROWS; i++){
		...
		for(j = 0; j < PLAYERGRID_COLS; j++, k++){
			...
			botSelectInfo.pics[k].generic.flags =
				QMF_LEFT_JUSTIFY|
				QMF_INACTIVE;
			botSelectInfo.pics[k].generic.x = x;
			botSelectInfo.pics[k].generic.y = y;
			botSelectInfo.pics[k].generic.name =
				botSelectInfo.boticons[k];
			botSelectInfo.pics[k].width = 64;
			botSelectInfo.pics[k].height = 64;
			botSelectInfo.pics[k].focuspic =
				BOTSELECT_SELECTED;
			botSelectInfo.pics[k].focuscolor = colorRed;

			botSelectInfo.picbuttons[k].generic.type =
				MTYPE_BITMAP;
			botSelectInfo.picbuttons[k].generic.flags =
				QMF_LEFT_JUSTIFY|QMF_NODEFAULTINIT|
				QMF_PULSEIFFOCUS;
			botSelectInfo.picbuttons[k].generic.callback =
				UI_BotSelectMenu_BotEvent;
			...
			botSelectInfo.picbuttons[k].focuspic =
				BOTSELECT_SELECT;
			...
			botSelectInfo.picnames[k].string =
				botSelectInfo.botnames[k];
	...
	botSelectInfo.left.generic.flags = QMF_LEFT_JUSTIFY|
					   QMF_PULSEIFFOCUS;
	...
	botSelectInfo.right.generic.flags = QMF_LEFT_JUSTIFY|
					    QMF_PULSEIFFOCUS;
	...
	botSelectInfo.back.generic.flags = QMF_LEFT_JUSTIFY|
					   QMF_PULSEIFFOCUS;
	...
	for(i = 0; i < MAX_MODELSPERPAGE; i++){
		...
		Menu_AddItem(&botSelectInfo.menu,
			&botSelectInfo.picbuttons[i]);
		Menu_AddItem(&botSelectInfo.menu,
			&botSelectInfo.picnames[i]);
	}
	...
	botSelectInfo.modelpage = botSelectInfo.selectedmodel /
				  MAX_MODELSPERPAGE;
	UI_BotSelectMenu_UpdateGrid();
}
