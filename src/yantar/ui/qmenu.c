...
Handle	menu_in_sound;
Handle	menu_move_sound;
Handle	menu_out_sound;
Handle	menu_buzz_sound;
Handle	menu_null_sound;
Handle	weaponChangeSound;
...
static Handle RadioButton_Key(menuradiobutton_s *rb, int key);

/* slider widget */
static void Slider_Init(menuslider_s *s);
static Handle Slider_Key(menuslider_s *s, int key);
static void     Slider_Draw(menuslider_s *s);

/* spin control widget */
static void     SpinControl_Init(menulist_s *s);
static void     SpinControl_Draw(menulist_s *s);
static Handle SpinControl_Key(menulist_s *l, int key);
...
Handle ScrollList_Key(menulist_s *l, int key);
...
Slider_Init(menuslider_s *s)
{
	...
	s->generic.right	= s->generic.x +
				  (SLIDER_RANGE+2+1)*SMALLCHAR_WIDTH;
	...
	case K_MOUSE1:
		...
		s->curvalue =
			(x/
			 (float)(SLIDER_RANGE*
				 SMALLCHAR_WIDTH)) *
			(s->maxvalue-s->minvalue) + s->minvalue;
...
Slider_Draw(menuslider_s *s)
{
	...
	if(s->maxvalue > s->minvalue){
		s->range =
			(s->curvalue -
			 s->minvalue) / ( float )(s->maxvalue - s->minvalue);
		if(s->range < 0)
			s->range = 0;
		else if(s->range > 1)
			s->range = 1;
	...
	UI_DrawHandlePic(
		(int)(x + 2*SMALLCHAR_WIDTH +
		      (SLIDER_RANGE-1)*SMALLCHAR_WIDTH* s->range) - 2,
		y - 2, 12, 20, button);
}
#else
/*
...
Slider_Draw(menuslider_s *s)
{
	...
	for(i = 0; i < SLIDER_RANGE; i++)
		UI_DrawChar(x + (i+2)*SMALLCHAR_WIDTH, y, 129, UI_LEFT|style,
			color);
	...
	UI_DrawChar(
		(int)(x + 2*SMALLCHAR_WIDTH +
		      (SLIDER_RANGE-1)*SMALLCHAR_WIDTH* s->range), y, 131,
		UI_LEFT|style, color);
}
#endif
...
SpinControl_Draw(menulist_s *s)
{
	...
	UI_DrawString(x - SMALLCHAR_WIDTH, y, s->generic.name, style|UI_RIGHT,
		color);
	UI_DrawString(x + SMALLCHAR_WIDTH, y, s->itemnames[s->curvalue], style|
		UI_LEFT,
		color);
}
...
ScrollList_Init(menulist_s *l)
{
	...
	w =
		((l->width +
		  l->seperation) * l->columns - l->seperation) * SMALLCHAR_WIDTH;
...
ScrollList_Key(menulist_s *l, int key)
{
	...
	case K_MOUSE1:
		if(l->generic.flags & QMF_HASMOUSEFOCUS){
			...
			w	=
				((l->width +
				  l->seperation) * l->columns -
				 l->seperation) * SMALLCHAR_WIDTH;
			if(l->generic.flags & QMF_CENTER_JUSTIFY)
				x -= w / 2;
			if(UI_CursorInRect(x, y, w, l->height*
				   SMALLCHAR_HEIGHT)){
	...
	for(column = 0; column < l->columns; column++){
		...
		for(i = base; i < base + l->height; i++){
			...
			if(i == l->curvalue){
				...
				UI_FillRect(u,y,l->width*SMALLCHAR_WIDTH,
					SMALLCHAR_HEIGHT+2,
					listbar_color);
				color = text_color_highlight;
...
Menu_AddItem(menuframework_s *menu, void *item)
{
	...
	((menucommon_s*)menu->items[menu->nitems])->menuPosition =
		menu->nitems;
	((menucommon_s*)menu->items[menu->nitems])->flags &=
		~QMF_HASMOUSEFOCUS;
	...
	if(!(itemptr->flags & QMF_NODEFAULTINIT)){
		...
		default:
			trap_Error(va("Menu_Init: unknown type %d",
					itemptr->type));
		}
	}
...
Menu_CursorMoved(menuframework_s *m)
{
	...
	if(m->cursor_prev >= 0 && m->cursor_prev < m->nitems){
		callback =
			((menucommon_s*)(m->items[m->cursor_prev]))->callback;
		if(callback)
...
Menu_Draw(menuframework_s *menu)
{
	...
	for(i=0; i<menu->nitems; i++){
		...
		if(itemptr->ownerdraw)
			...
			default:
				trap_Error(va("Menu_Draw: unknown type %d",
						itemptr->type));
			}
		}
#ifndef NDEBUG
...
Menu_DefaultKey(menuframework_s *m, int key)
{
	...
	case K_ENTER:
		if(item)
			if(!(item->flags &
			     (QMF_MOUSEONLY|QMF_GRAYED|QMF_INACTIVE)))
				return (Menu_ActivateItem(m, item));
		break;
	}
