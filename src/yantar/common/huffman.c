...
Huff_Receive(node_t *node, int *ch, byte *fin)
{
	...
	if(!node)
		return 0;
/*		Com_Errorf(ERR_DROP, "Illegal tree!"); */
...
Huff_Init(Huffman *huff)
{
	...
	Q_Memset(&huff->compressor, 0, sizeof(Huff));
	Q_Memset(&huff->decompressor, 0, sizeof(Huff));
