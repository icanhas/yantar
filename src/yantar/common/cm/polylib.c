...
FreeWinding(Winding *w)
{
	if(*(unsigned*)w == 0xdeaddead)
		comerrorf (ERR_FATAL, "FreeWinding: freed a freed winding");
...
void
RemoveColinearPoints(Winding *w)
{
	int	i, j, k;
	Vec3 v1, v2;
	int	nump;
 ...
 */
void
WindingPlane(Winding *w, Vec3 normal, Scalar *dist)
{
	Vec3 v1, v2;
 ...
 */
void
WindingBounds(Winding *w, Vec3 mins, Vec3 maxs)
{
	Scalar	v;
	int	i,j;
 ...
 */
void
WindingCenter(Winding *w, Vec3 center)
{
	int i;
	float scale;
...
/* find the major axis */
	...
	if(x==-1)
		comerrorf (ERR_DROP, "BaseWindingForPlane: no axis found");
...
CopyWinding(Winding *w)
{
	...
	size = (intptr_t)((Winding*)0)->p[w->numpoints];
	Q_Memcpy (c, w, size);
	return c;
}
 ...
 */
void
ClipWindingEpsilon(Winding *in, Vec3 normal, Scalar dist,
...
/* determine sides for each point */
	...
	*back	= b = AllocWinding (maxpts);
	...
	if(f->numpoints > maxpts || b->numpoints > maxpts)
		comerrorf (ERR_DROP, "ClipWinding: points exceeded estimate");
	if(f->numpoints > MAX_POINTS_ON_WINDING || b->numpoints >
	   MAX_POINTS_ON_WINDING)
		comerrorf (ERR_DROP, "ClipWinding: MAX_POINTS_ON_WINDING");
}
 ...
 */
void
ChopWindingInPlace(Winding **inout, Vec3 normal, Scalar dist, Scalar epsilon)
{
	Winding *in;
	Scalar	dists[MAX_POINTS_ON_WINDING+4];
	int	sides[MAX_POINTS_ON_WINDING+4];
 ...
 */
Winding       *
ChopWinding(Winding *in, Vec3 normal, Scalar dist)
{
	Winding *f, *b;
...
CheckWinding(Winding *w)
{
	...
	if(w->numpoints < 3)
		comerrorf (ERR_DROP, "CheckWinding: %i points",w->numpoints);

	area = WindingArea(w);
	if(area < 1)
		comerrorf (ERR_DROP, "CheckWinding: %f area", area);
	...
	for(i=0; i<w->numpoints; i++){
		...
		if(d < -ON_EPSILON || d > ON_EPSILON)
			comerrorf (ERR_DROP, "CheckWinding: point off plane");
		...
		if(lenv3 (dir) < ON_EPSILON)
			comerrorf (ERR_DROP, "CheckWinding: degenerate edge");
		...
		for(j=0; j<w->numpoints; j++){
			...
			if(d > edgedist)
				comerrorf (ERR_DROP, "CheckWinding: non-convex");
		}
	}
}
 ...
 */
int
WindingOnPlaneSide(Winding *w, Vec3 normal, Scalar dist)
{
	qbool		front, back;
	int i;
	Scalar		d;
...
void
AddWindingToConvexHull(Winding *w, Winding **hull, Vec3 normal)
{
	int i, j, k;
	float	*p, *copy;
	Vec3	dir;
