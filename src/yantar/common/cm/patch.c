 ...
 */
static int
CM_SignbitsForNormal(Vec3 normal)
{
	int bits, j;
 ...
 */
static qbool
CM_PlaneFromPoints(Vec4 plane, Vec3 a, Vec3 b, Vec3 c)
{
	Vec3 d1, d2;
 ...
 */
static qbool
CM_NeedsSubdivision(Vec3 a, Vec3 b, Vec3 c)
{
	Vec3	cmid;
	Vec3	lmid;
	Vec3	delta;
 ...
 */
static void
CM_Subdivide(Vec3 a, Vec3 b, Vec3 c, Vec3 out1, Vec3 out2, Vec3 out3)
{
	int i;
...
CM_SubdivideGridColumns(cGrid_t *grid)
{
	...
	for(i = 0; i < grid->width - 2; ){
		...
		for(j = 0; j < grid->height; j++)
			if(CM_NeedsSubdivision(grid->points[i][j],
				   grid->points[i+1][j], grid->points[i+2][j]))
				break;
		if(j == grid->height){
...
CM_RemoveDegenerateColumns(cGrid_t *grid)
{
	...
	for(i = 0; i < grid->width - 1; i++){
		for(j = 0; j < grid->height; j++)
			if(!CM_ComparePoints(grid->points[i][j],
				   grid->points[i+1][j]))
				break;
...
CM_FindPlane2(float plane[4], int *flipped)
{
	...
	if(numPlanes == MAX_PATCH_PLANES)
		comerrorf(ERR_DROP, "MAX_PATCH_PLANES");
	     ...
	     int tri)
{
	...
	/* should never happen */
	comprintf("WARNING: CM_GridPlane unresolvable\n");
	return -1;
}
		...
		int k)
{
	...
	comerrorf(ERR_DROP, "CM_EdgePlaneNum: bad k");
	return -1;
}
		   ...
		   int which)
{
	...
	case 1:
		...
		break;
	default:
		comerrorf(ERR_FATAL, "CM_SetBorderInward: bad parameter");
		numPoints = 0;
		break;
	}
	...
	for(k = 0; k < facet->numBorders; k++){
		...
		for(l = 0; l < numPoints; l++){
			...
			side = CM_PointOnPlaneSide(points[l],
	...
	for(axis = 0; axis < 3; axis++)
		for(dir = -1; dir <= 1; dir += 2, order++){
			...
			for(i = 0; i < facet->numBorders; i++)
				if(CM_PlaneEqual(&planes[facet->borderPlanes[i]],
					   plane, &flipped))
					break;

			if(i == facet->numBorders){
				if(facet->numBorders > 4 + 6 + 16) comprintf(
	...
	for(j = 0; j < w->numpoints; j++){
		...
		for(axis = 0; axis < 3; axis++)
			for(dir = -1; dir <= 1; dir += 2){
				...
				/* if it's the surface plane */
				if(CM_PlaneEqual(&planes[facet->surfacePlane],
					   plane, &flipped))
					continue;
				/* see if the plane is allready present */
				for(i = 0; i < facet->numBorders; i++)
					if(CM_PlaneEqual(&planes[facet->
								 borderPlanes[i]
						   ], plane, &flipped))
						break;
...
CM_PatchCollideFromGrid(cGrid_t *grid, patchCollide_t *pf)
{
	...
	for(i = 0; i < grid->width - 1; i++)
		for(j = 0; j < grid->height - 1; j++){
			...
			noAdjust[EN_TOP] =
				(borders[EN_TOP] == gridPlanes[i][j][0]);
			if(borders[EN_TOP] == -1 || noAdjust[EN_TOP])
				borders[EN_TOP] =
					CM_EdgePlaneNum(grid, gridPlanes, i, j,
						0);
			...
			noAdjust[EN_BOTTOM] =
				(borders[EN_BOTTOM] == gridPlanes[i][j][1]);
			...
			noAdjust[EN_LEFT] =
				(borders[EN_LEFT] == gridPlanes[i][j][1]);
			...
			noAdjust[EN_RIGHT] =
				(borders[EN_RIGHT] == gridPlanes[i][j][0]);
			...
			if(gridPlanes[i][j][0] == gridPlanes[i][j][1]){
				...
				facet->numBorders	= 4;
				facet->borderPlanes[0] =
					borders[EN_TOP];
				facet->borderNoAdjust[0] =
					noAdjust[EN_TOP];
				facet->borderPlanes[1] =
					borders[EN_RIGHT];
				...
				facet->borderNoAdjust[2] =
					noAdjust[EN_BOTTOM];
				facet->borderPlanes[3] =
					borders[EN_LEFT];
				facet->borderNoAdjust[3] =
					noAdjust[EN_LEFT];
				CM_SetBorderInward(facet, grid, gridPlanes, i, j,
					-1);
	...
	pf->facets = hunkalloc(numFacets * sizeof(*pf->facets), Hhigh);
	Q_Memcpy(pf->facets, facets, numFacets * sizeof(*pf->facets));
	pf->planes = hunkalloc(numPlanes * sizeof(*pf->planes), Hhigh);
...
CM_GeneratePatchCollide(int width, int height, Vec3 *points)
{
	patchCollide_t *pf;
	cGrid_t grid;
	int i, j;
	 ...
	 * collided against */
	pf = hunkalloc(sizeof(*pf), Hhigh);
	ClearBounds(pf->bounds[0], pf->bounds[1]);
	for(i = 0; i < grid.width; i++)
		for(j = 0; j < grid.height; j++)
			AddPointToBounds(grid.points[i][j], pf->bounds[0],
				pf->bounds[1]);
 ...
 */
int
CM_CheckFacetPlane(float *plane, Vec3 start, Vec3 end, float *enterFrac,
		   float *leaveFrac,
		   int *hit)
{
	float d1, d2, f;
	...
	for(i = 0, facet = pc->facets; i < pc->numFacets; i++, facet++)
		...
		for(k = 0; k < facet->numBorders + 1; k++){
			...
			if(w){
				...
				/* comprintf("blue facet has %d border planes\n", facet->numBorders); */
				else
