...
#define CRAP_INVALID_ARGS crap("invalid arguments %s, %s", argtype2str(	\
		arg1.type),argtype2str(arg2.type));
...
_crap(const char* func, const char* fmt, ...)
{
	va_list ap;
...
emit_op_rm(const char* mnemonic, arg_t arg1, arg_t arg2, void* data)
{
	...
	if((arg1.type != T_REGISTER && arg1.type !=
	    T_MEMORY) || arg2.type != T_NONE)
		CRAP_INVALID_ARGS;
...
{
	if((arg1.type != T_LABEL && arg1.type != T_REGISTER && arg1.type !=
	    T_MEMORY) || arg2.type != T_NONE)
		CRAP_INVALID_ARGS;
...
assemble_line(const char* input, size_t len)
{
	...
	if(line[len-1] == ':'){
		...
		if(assembler_pass)
			debug("   - %s%s", cur_line, cur_line[strlen(
								      cur_line)-
							      1]==
				'\n' ? "" : "\n");
	}
