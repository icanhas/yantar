...
Hex(int c)
{
	...
	VMFREE_BUFFERS();
	comerrorf(ERR_DROP, "Hex: bad char '%c'", c);
	...
	if(!jlabel){
		if(LastCommand == LAST_COMMAND_MOV_STACK_EAX){	/* mov [edi + ebx * 4], eax */
			...
			vm->instructionPointers[instruction - 1] = compiledOfs;
			 ...
			 buf[compiledOfs-6] == 0x04 && buf[compiledOfs - 5] ==
			...
			return;
		}else if(pop1 != OP_DIVI && pop1 != OP_DIVU && pop1 !=
			 OP_MULI && pop1 != OP_MULU &&
			 pop1 != OP_STORE4 && pop1 != OP_STORE2 && pop1 !=
			 OP_STORE1)
...
EmitMovEDXStack(Vm *vm, int andit)
{
	if(!jlabel){
		if(LastCommand == LAST_COMMAND_MOV_STACK_EAX){	/* mov dword ptr [edi + ebx * 4], eax */
			...
			buf[compiledOfs-6] == 0x07 && buf[compiledOfs - 5] ==
...
void
DoSyscall(int syscallNum, int programStack, int *opStackBase, uint8_t opStackOfs,
	  intptr_t arg)
{
#else
static void
DoSyscall(void)
{
	...
	__asm__ volatile (
		""
		: "=a" (syscallNum), "=S" (programStack), "=D" (opStackBase),
		"=b" (opStackOfs),
		"=c" (arg)
		);
#endif
	...
	if(syscallNum < 0){
...
#else
		data[0] = ~syscallNum;
		opStackBase[opStackOfs + 1] = savedVM->systemCall(
			(intptr_t*)data);
#endif
	}else{
		switch(syscallNum){
		...
		case VM_BLOCK_COPY:
			...
			break;
		default:
			comerrorf(ERR_DROP, "Unknown VM operation %d",
				syscallNum);
			break;
		}
	}
...
VM_Compile(Vm *vm, Vmheader *header)
{
	...
	Q_Memset(code, 0, header->codeLength+32);
	Q_Memcpy(code, (byte*)header + header->codeOffset,
		header->codeLength);
	...
	for(pass=0; pass < 3; pass++){
		...
		while(instruction < header->instructionCount){
			...
			case OP_LEAVE:
				...
				break;
			case OP_LOAD4:
				if(code[pc] == OP_CONST && code[pc+5] ==
				   OP_ADD && code[pc+6] == OP_STORE4){
					   ...
					   OP_LOCAL && pop1 == OP_LOCAL){
						...
						if(oc0 == oc1 && pop0 ==
						   ...
						   OP_LOCAL){
							...
							EmitCommand(
								LAST_COMMAND_SUB_BL_1);	/* sub bl, 1 */
							EmitString("8B 14 9F");		/* mov edx, dword ptr [edi + ebx * 4] */
							MASK_REG("E2",
								vm->dataMask);	/* and edx, 0x12345678 */
#if idx64
							EmitRexString(0x41,
								"89 04 11");	/* mov dword ptr [r9 + edx], eax */
#else
				...
				if(buf[compiledOfs - 3] == 0x89 &&
				   ...
				   buf[compiledOfs - 1] == 0x9F){
					compiledOfs -= 3;
					vm->instructionPointers[instruction -
								1] = compiledOfs;
	...
	if(vm->codeBase == MAP_FAILED)
		comerrorf(ERR_FATAL, "VM_CompileX86: can't mmap memory");
#elif _WIN32
	/* allocate memory with EXECUTE permissions under windows. */
	vm->codeBase = VirtualAlloc(NULL, compiledOfs, MEM_COMMIT,
		PAGE_EXECUTE_READWRITE);
	if(!vm->codeBase)
		comerrorf(ERR_FATAL, "VM_CompileX86: VirtualAlloc failed");
#else
	vm->codeBase = malloc(compiledOfs);
	if(!vm->codeBase)
		comerrorf(ERR_FATAL, "VM_CompileX86: malloc failed");
#endif
	...
	if(mprotect(vm->codeBase, compiledOfs, PROT_READ|PROT_EXEC))
		comerrorf(ERR_FATAL, "VM_CompileX86: mprotect failed");
#elif _WIN32
	{
		DWORD oldProtect = 0;
	...
	zfree(buf);
	zfree(jused);
	comprintf("VM file %s compiled to %i bytes of code\n", vm->name,
		compiledOfs);
...
vmcallCompiled(Vm *vm, int *args)
{
	...
	*opStack = 0xDEADBEEF;
	...
	__asm__ volatile (
		...
		: "g" (vm->instructionPointers), "g" (vm->dataBase),
		"g" (entryPoint)
	...
	if(opStackOfs != 1 || *opStack != 0xDEADBEEF)
		comerrorf(ERR_DROP, "opStack corrupted in compiled code");
