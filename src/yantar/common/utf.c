...
Q_tolowerrune(Rune ch)
{
	...
	if(p && ch >= p[0] && ch <= p[1])
		return ch + p[2] - 500;
	p = rbsearch(ch, _tolower1, ARRAY_LEN(_tolower1)/2, 2);
	if(p && ch == p[0])
		return ch + p[1] - 500;
	return ch;
}
...
Q_toupperrune(Rune ch)
{
	...
	if(p && ch >= p[0] && ch <= p[1])
		return ch + p[2] - 500;
	p = rbsearch(ch, _toupper1, ARRAY_LEN(_toupper1)/2, 2);
	if(p && ch == p[0])
		return ch + p[1] - 500;
	return ch;
}
...
Q_isalpharune(Rune ch)
{
	...
	if(Q_isupperrune(ch) || Q_islowerrune(ch))
		return 1;
	p = rbsearch(ch, _alpha2, ARRAY_LEN(_alpha2)/2, 2);
	if(p && ch >= p[0] && ch <= p[1])
		return 1;
	p = rbsearch(ch, _alpha1, ARRAY_LEN(_alpha1), 1);
	if(p && ch == p[0])
		return 1;
	return 0;
}
