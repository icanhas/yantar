...
#ifdef BUILD_FREETYPE
static void
R_GetGlyphInfo(FT_GlyphSlot glyph, int *left, int *right, int *width, int *top, int *bottom, int *height,
	       int *pitch)
{
...
WriteTGA(char *filename, byte *data, int width, int height)
{
	...
	ri.fswritefile(filename, buffer, c);
...
RE_RegisterFont(const char *fontName, int pointSize, Fontinfo *font)
{
	...
	Q_sprintf(name, sizeof(name), "fonts/fontImage_%i.dat",pointSize);
	...
	len = ri.fsreadfile(name, NULL);
	if(len == sizeof(Fontinfo)){
		ri.fsreadfile(name, &faceData);
		fdOffset = 0;
		fdFile = faceData;
		for(i=0; i<GLYPHS_PER_FONT; i++){
			...
			Q_Memcpy(font->glyphs[i].shaderName, &fdFile[fdOffset], 32);
			fdOffset += 32;
		}
...
/*		Q_Memcpy(font, faceData, sizeof(Fontinfo)); */
	...
	len = ri.fsreadfile(fontName, &faceData);
	if(len <= 0){
	...
	while(i <= GLYPH_END){
		...
		if(xOut == -1 || yOut == -1 || i == GLYPH_END){
			...
			Q_sprintf (name, sizeof(name), "fonts/fontImage_%i_%i.tga", imageNumber++,
				pointSize);
			if(r_saveFontData->integer){
				WriteTGA(name, imageBuff, 256, 256);
			}

			/* Q_sprintf (name, sizeof(name), "fonts/fontImage_%i_%i", imageNumber++, pointSize); */
	...
	if(r_saveFontData->integer){
		ri.fswritefile(va("fonts/fontImage_%i.dat", pointSize), font, sizeof(Fontinfo));
	}

	ri.Free(out);

	ri.fsfreefile(faceData);
#endif
}
