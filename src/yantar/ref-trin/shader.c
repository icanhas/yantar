...
static material_t shader;
static texModInfo_t	texMods[MAX_SHADER_STAGES][TR_MAX_TEXMODS];

#define FILE_HASH_SIZE 1024
static material_t * hashTable[FILE_HASH_SIZE];
...
#endif
static long
generateHashValue(const char *fname, const int size)
{
	int	i;
	long	hash;
	char	letter;
...
{
	char	*token;
	int	depthMaskBits = GLS_DEPTHMASK_TRUE, blendSrcBits = 0, blendDstBits = 0, atestBits = 0,
		depthFuncBits = 0;
...
GeneratePermanentShader(void)
{
	...
	newShader = ri.hunkalloc(sizeof(material_t), Hlow);
	...
	*newShader = shader;
	...
	for(i = 0; i < newShader->numUnfoggedPasses; i++){
		...
		newShader->stages[i]	= ri.hunkalloc(sizeof(stages[i]), Hlow);
		*newShader->stages[i]	= stages[i];

		for(b = 0; b < NUM_TEXTURE_BUNDLES; b++){
			size = newShader->stages[i]->bundle[b].numTexMods * sizeof(texModInfo_t);
			newShader->stages[i]->bundle[b].texMods = ri.hunkalloc(size, Hlow);
...
ScanAndLoadShaderFiles(void)
{
	...
	s_shaderText = ri.hunkalloc(sum + numShaderFiles*2, Hlow);
	s_shaderText[ 0 ] = '\0';
	...
	hashMem = ri.hunkalloc(size * sizeof(char *), Hlow);
