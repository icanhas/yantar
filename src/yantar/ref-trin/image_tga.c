...
typedef struct _TargaHeader {
	unsigned char	id_length, colormap_type, imgtype;
...
R_LoadTGA(const char *name, byte **pic, int *width, int *height)
{
	...
	*pic = NULL;
	...
	if(targa_header.imgtype!=2
	   && targa_header.imgtype!=10
	   && targa_header.imgtype != 3){
	...
	if((targa_header.pixel_size != 32 && targa_header.pixel_size != 24) && targa_header.imgtype != 3){
	...
	if(targa_header.imgtype==2 || targa_header.imgtype == 3){
	...
	}else if(targa_header.imgtype==10){	/* Runlength encoded RGB images */
