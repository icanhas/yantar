...
void
R_LoadBMP(const char *name, byte **pic, int *width, int *height)
{
	int columns, rows;
	unsigned numPixels;
	byte	*pixbuf;
	...
	*pic = NULL;
	...
	if(bmpHeader.bitsPerPixel == 8){
		...
		Q_Memcpy(bmpHeader.palette, buf_p, sizeof(bmpHeader.palette));
	...
	case 32:
		break;
	default:
		ri.Error(ERR_DROP, "LoadBMP: illegal pixel_size '%hu' in file '%s'", bmpHeader.bitsPerPixel,
			name);
		break;
	}
