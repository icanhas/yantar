...
CG_Item(centity_t *cent)
{
	...
	if(item->giType == IT_WEAPON && item->giTag == W1_RAILGUN){
...
/*	int	col; */
	...
	if(cent->currentState.weapon == W1_PLASMAGUN){
		ent.reType = RT_SPRITE;
		ent.radius = 16;
		ent.rotation = 0;
	...
	if(cent->currentState.weapon == W1_PROX_LAUNCHER)
