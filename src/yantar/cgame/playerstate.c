...
CG_CheckAmmo(void)
{
	...
	total	= 0;
	for(i = Wmachinegun; i < Wnumweaps; i++){
		if(!(weapons & (1 << i)))
			continue;
		switch(i){
	...
	if(cg.lowAmmoWarning != previous)
		trap_sndstartlocalsound(cgs.media.noAmmoSound, CHAN_LOCAL_SOUND);
}
...
CG_DamageFeedback(int yawByte, int pitchByte, int damage)
{
	...
	if(yawByte == 255 && pitchByte == 255){
		...
		subv3(vec3_origin, dir, dir);

		front	= dotv3 (dir, cg.refdef.viewaxis[0]);
		left	= dotv3 (dir, cg.refdef.viewaxis[1]);
		up = dotv3 (dir, cg.refdef.viewaxis[2]);
...
CG_CheckLocalSounds(Playerstate *ps, Playerstate *ops)
{
	...
	if(ps->persistant[PERS_HITS] > ops->persistant[PERS_HITS]){
		...
		if(armor > 50)
			trap_sndstartlocalsound(cgs.media.hitSoundHighArmor,
				CHAN_LOCAL_SOUND);
		else if(armor || health > 100)
			trap_sndstartlocalsound(cgs.media.hitSoundLowArmor,
				CHAN_LOCAL_SOUND);
		else
			trap_sndstartlocalsound(cgs.media.hitSound,
				CHAN_LOCAL_SOUND);

#else
		trap_sndstartlocalsound(cgs.media.hitSound, CHAN_LOCAL_SOUND);
#endif
	}else if(ps->persistant[PERS_HITS] < ops->persistant[PERS_HITS])
		trap_sndstartlocalsound(cgs.media.hitTeamSound, CHAN_LOCAL_SOUND);
	...
	if(ps->persistant[PERS_PLAYEREVENTS] !=
	   ops->persistant[PERS_PLAYEREVENTS]){
		if((ps->persistant[PERS_PLAYEREVENTS] &
		   ...
		   (ops->persistant[PERS_PLAYEREVENTS] &
		    PLAYEREVENT_DENIEDREWARD))
			trap_sndstartlocalsound(cgs.media.deniedSound,
				CHAN_ANNOUNCER);
		else if((ps->persistant[PERS_PLAYEREVENTS] &
			 PLAYEREVENT_GAUNTLETREWARD) !=
			(ops->persistant[PERS_PLAYEREVENTS] &
			 PLAYEREVENT_GAUNTLETREWARD))
			trap_sndstartlocalsound(cgs.media.humiliationSound,
				CHAN_ANNOUNCER);
		else if((ps->persistant[PERS_PLAYEREVENTS] &
			 PLAYEREVENT_HOLYSHIT) !=
			(ops->persistant[PERS_PLAYEREVENTS] &
			 PLAYEREVENT_HOLYSHIT))
			trap_sndstartlocalsound(cgs.media.holyShitSound,
				CHAN_ANNOUNCER);
		reward = qtrue;
	}
	...
	if(cgs.gametype > GT_TEAM)
		if((ps->powerups[PW_REDFLAG] != ops->powerups[PW_REDFLAG] &&
		   ...
		   (ps->powerups[PW_NEUTRALFLAG] !=
		    ops->powerups[PW_NEUTRALFLAG] &&
		    ps->powerups[PW_NEUTRALFLAG]))
			trap_sndstartlocalsound(cgs.media.youHaveFlagSound,
				CHAN_ANNOUNCER);
	...
	if(!reward){
		if(!cg.warmup){
			...
			if(ps->persistant[PERS_RANK] !=
			   ops->persistant[PERS_RANK])
				if(cgs.gametype < GT_TEAM){
					if(ps->persistant[PERS_RANK] == 0)
						CG_AddBufferedSound(
							cgs.media.takenLeadSound);
					else if(ps->persistant[PERS_RANK] ==
						RANK_TIED_FLAG)
						CG_AddBufferedSound(
							cgs.media.tiedLeadSound);
					else if((ops->persistant[PERS_RANK] &
						 ~RANK_TIED_FLAG) == 0)
						CG_AddBufferedSound(
	...
	if(cgs.timelimit > 0){
		...
		if(!(cg.timelimitWarnings & 4) && msec >
		   (cgs.timelimit * 60 + 2) * 1000){
			cg.timelimitWarnings |= 1 | 2 | 4;
			trap_sndstartlocalsound(cgs.media.suddenDeathSound,
				CHAN_ANNOUNCER);
		}else if(!(cg.timelimitWarnings & 2) && msec >
			 (cgs.timelimit - 1) * 60 * 1000){
			cg.timelimitWarnings |= 1 | 2;
			trap_sndstartlocalsound(cgs.media.oneMinuteSound,
				CHAN_ANNOUNCER);
		}else if(cgs.timelimit > 5 && !(cg.timelimitWarnings & 1) &&
			 msec > (cgs.timelimit - 5) * 60 * 1000){
			cg.timelimitWarnings |= 1;
			trap_sndstartlocalsound(cgs.media.fiveMinuteSound,
				CHAN_ANNOUNCER);
		}
	}
	...
	if(cgs.fraglimit > 0 && cgs.gametype < GT_CTF){
		...
		if(!(cg.fraglimitWarnings & 4) && highScore ==
		   (cgs.fraglimit - 1)){
