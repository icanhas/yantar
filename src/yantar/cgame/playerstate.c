...
CG_CheckLocalSounds(Playerstate *ps, Playerstate *ops)
{
	...
	if(ps->persistant[PERS_HITS] > ops->persistant[PERS_HITS]){
		...
		if(armor > 50)
			...
			trap_S_StartLocalSound(cgs.media.hitSoundLowArmor,
				CHAN_LOCAL_SOUND);
		else
			trap_S_StartLocalSound(cgs.media.hitSound,
				CHAN_LOCAL_SOUND);
	...
	if(ps->persistant[PERS_PLAYEREVENTS] !=
	   ops->persistant[PERS_PLAYEREVENTS]){
		if((ps->persistant[PERS_PLAYEREVENTS] &
		   ...
		   (ops->persistant[PERS_PLAYEREVENTS] &
		    PLAYEREVENT_DENIEDREWARD))
			trap_S_StartLocalSound(cgs.media.deniedSound,
				CHAN_ANNOUNCER);
		else if((ps->persistant[PERS_PLAYEREVENTS] &
			 PLAYEREVENT_GAUNTLETREWARD) !=
			(ops->persistant[PERS_PLAYEREVENTS] &
			 PLAYEREVENT_GAUNTLETREWARD))
			trap_S_StartLocalSound(cgs.media.humiliationSound,
				CHAN_ANNOUNCER);
		else if((ps->persistant[PERS_PLAYEREVENTS] &
			 PLAYEREVENT_HOLYSHIT) !=
			(ops->persistant[PERS_PLAYEREVENTS] &
			 PLAYEREVENT_HOLYSHIT))
			trap_S_StartLocalSound(cgs.media.holyShitSound,
				CHAN_ANNOUNCER);
		reward = qtrue;
	}
	...
	if(cgs.gametype > GT_TEAM)
		if((ps->powerups[PW_REDFLAG] != ops->powerups[PW_REDFLAG] &&
		   ...
		   (ps->powerups[PW_NEUTRALFLAG] !=
		    ...
		    ps->powerups[PW_NEUTRALFLAG]))
			trap_S_StartLocalSound(cgs.media.youHaveFlagSound,
				CHAN_ANNOUNCER);
	...
	if(cgs.timelimit > 0){
		...
		if(!(cg.timelimitWarnings & 4) && msec >
		   (cgs.timelimit * 60 + 2) * 1000){
			...
			trap_S_StartLocalSound(cgs.media.suddenDeathSound,
				CHAN_ANNOUNCER);
		}else if(!(cg.timelimitWarnings & 2) && msec >
			 (cgs.timelimit - 1) * 60 * 1000){
			cg.timelimitWarnings |= 1 | 2;
			trap_S_StartLocalSound(cgs.media.oneMinuteSound,
				CHAN_ANNOUNCER);
		}else if(cgs.timelimit > 5 && !(cg.timelimitWarnings & 1) &&
			 msec > (cgs.timelimit - 5) * 60 * 1000){
			cg.timelimitWarnings |= 1;
			trap_S_StartLocalSound(cgs.media.fiveMinuteSound,
				CHAN_ANNOUNCER);
		}
	}
