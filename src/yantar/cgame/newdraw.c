...
CG_CheckOrderPending(void)
{
	if(cgs.gametype < GT_CTF)
		return;
	if(cgs.orderPending){
		/* Clientinfo *ci = cgs.clientinfo + sortedTeamPlayers[cg_currentSelectedPlayer.integer]; */
		const char *p1, *p2, *b;
		p1 = p2 = b = NULL;
...
CG_GetSelectedPlayer(void)
{
	...
	n = cg_currentSelectedPlayer.integer;
	if(n < 0 || n >= numSortedTeamPlayers)
...
static void
CG_DrawPlayerArmorValue(rectDef_t *rect, float scale, Vec4 color,
			Handle shader, int textStyle)
{
	char	num[16];
	int	value;
	Playerstate *ps;
		...
		if(cent->currentState.weapon &&
		   cg_weapons[ cent->currentState.weapon ].ammoModel){
			...
			CG_Draw3DModel(
				...
				cg_weapons[ cent->currentState.weapon ].
				ammoModel, 0,
				origin, angles);
		}
}
...
static void
CG_DrawPlayerAmmoValue(rectDef_t *rect, float scale, Vec4 color,
		       Handle shader,
		       int textStyle)
{
	char	num[16];
...
static void
CG_DrawSelectedPlayerHealth(rectDef_t *rect, float scale, Vec4 color,
			    Handle shader, int textStyle)
{
	Clientinfo *ci;
	int	value;
...
static void
CG_DrawSelectedPlayerArmor(rectDef_t *rect, float scale, Vec4 color,
			   Handle shader, int textStyle)
{
	Clientinfo *ci;
	int	value;
...
CG_DrawPlayerStatus(rectDef_t *rect)
{
	...
	if(ci){
		Handle h = CG_StatusHandle(ci->teamTask);
		CG_DrawPic(rect->x, rect->y, rect->w, rect->h, h);
	}
}

static void
CG_DrawSelectedPlayerName(rectDef_t *rect, float scale, Vec4 color,
			  qbool voice, int textStyle)
{
	Clientinfo *ci;
	ci = cgs.clientinfo +
	     ((voice) ? cgs.currentVoiceClient : sortedTeamPlayers[
		      CG_GetSelectedPlayer()]);
	if(ci)
		CG_Text_Paint(rect->x, rect->y + rect->h, scale, color, ci->name,
			0, 0,
			textStyle);
}

static void
CG_DrawSelectedPlayerLocation(rectDef_t *rect, float scale, Vec4 color,
			      int textStyle)
{
	Clientinfo *ci;
	ci = cgs.clientinfo + sortedTeamPlayers[CG_GetSelectedPlayer()];
...
static void
CG_DrawPlayerLocation(rectDef_t *rect, float scale, Vec4 color, int textStyle)
{
	Clientinfo *ci = &cgs.clientinfo[cg.snap->ps.clientNum];
	if(ci){
		const char *p = CG_ConfigString(CS_LOCATIONS + ci->location);
...
static void
CG_DrawPlayerScore(rectDef_t *rect, float scale, Vec4 color, Handle shader,
		   int textStyle)
{
	char	num[16];
...
CG_DrawPlayerItem(rectDef_t *rect, float scale, qbool draw2D)
{
	...
	if(value){
		...
		if(qtrue){
			...
			CG_Draw3DModel(rect->x, rect->y, rect->w, rect->h,
				cg_items[ value ].models[0], 0, origin,
				angles);
		}
	}
...
CG_DrawSelectedPlayerHead(rectDef_t *rect, qbool draw2D, qbool voice)
{
	...
	if(ci){
		if(cg_draw3dIcons.integer){
			...
			CG_Draw3DModel(rect->x, rect->y, rect->w, rect->h,
				ci->headModel, ci->headSkin, origin,
				angles);
...
static void
CG_DrawPlayerHealth(rectDef_t *rect, float scale, Vec4 color, Handle shader,
		    int textStyle)
{
	Playerstate *ps;
	int	value;
...
static void
CG_DrawRedScore(rectDef_t *rect, float scale, Vec4 color, Handle shader,
		int textStyle)
{
	int	value;
	char	num[16];
...
static void
CG_DrawBlueScore(rectDef_t *rect, float scale, Vec4 color, Handle shader,
		 int textStyle)
{
	int	value;
	char	num[16];
...
static void
CG_DrawRedName(rectDef_t *rect, float scale, Vec4 color, int textStyle)
{
	CG_Text_Paint(rect->x, rect->y + rect->h, scale, color,
		cg_redTeamName.string, 0, 0,
		textStyle);
}

static void
CG_DrawBlueName(rectDef_t *rect, float scale, Vec4 color, int textStyle)
{
	CG_Text_Paint(rect->x, rect->y + rect->h, scale, color,
		cg_blueTeamName.string, 0, 0,
		textStyle);
}

static void
CG_DrawBlueFlagName(rectDef_t *rect, float scale, Vec4 color, int textStyle)
{
	int i;
...
static void
CG_DrawBlueFlagStatus(rectDef_t *rect, Handle shader)
{
	...
	if(shader)
		...
		Gitem *item = BG_FindItemForPowerup(PW_BLUEFLAG);
		if(item){
			Vec4 color = {0, 0, 1, 1};
			trap_R_SetColor(color);
...
static void
CG_DrawRedFlagName(rectDef_t *rect, float scale, Vec4 color, int textStyle)
{
	int i;
	for(i = 0; i < cgs.maxclients; i++)
		if(cgs.clientinfo[i].infoValid && cgs.clientinfo[i].team ==
...
static void
CG_DrawRedFlagStatus(rectDef_t *rect, Handle shader)
{
	if(cgs.gametype != GT_CTF && cgs.gametype != GT_1FCTF){
		if(cgs.gametype == GT_HARVESTER){
			Vec4 color = {1, 0, 0, 1};
	...
	if(shader)
		...
		Gitem *item = BG_FindItemForPowerup(PW_REDFLAG);
		if(item){
			Vec4 color = {1, 0, 0, 1};
			trap_R_SetColor(color);
...
static void
CG_HarvesterSkulls(rectDef_t *rect, float scale, Vec4 color, qbool force2D,
		   int textStyle)
{
	char num[16];
	Vec3	origin, angles;
	...
	if(cg_drawIcons.integer){
		if(!force2D && cg_draw3dIcons.integer){
			...
			CG_Draw3DModel(rect->x, rect->y, 35, 35, handle, 0,
				origin,
				angles);
		}else{
			if(cg.snap->ps.persistant[PERS_TEAM] == TEAM_BLUE)
...
CG_OneFlagStatus(rectDef_t *rect)
{
	if(cgs.gametype != GT_1FCTF)
		...
		Gitem *item = BG_FindItemForPowerup(PW_NEUTRALFLAG);
		if(item)
...
static void
CG_DrawTeamColor(rectDef_t *rect, Vec4 color)
{
	...
	if(cgs.gametype == GT_CTF || cgs.gametype == GT_1FCTF){
		...
		if(cgs.gametype == GT_1FCTF){
			if(team == TEAM_RED && cgs.flagStatus == FLAG_TAKEN_BLUE)
				return qtrue;
			else if(team == TEAM_BLUE && cgs.flagStatus ==
				FLAG_TAKEN_RED)
				return qtrue;
			else
				return qfalse;
	...
	if(flags &
	   (CG_SHOW_BLUE_TEAM_HAS_REDFLAG | CG_SHOW_RED_TEAM_HAS_BLUEFLAG)){
		if(flags & CG_SHOW_BLUE_TEAM_HAS_REDFLAG &&
		   (cgs.redflag == FLAG_TAKEN || cgs.flagStatus ==
		    FLAG_TAKEN_RED))
			return qtrue;
		else if(flags & CG_SHOW_RED_TEAM_HAS_BLUEFLAG &&
			(cgs.blueflag == FLAG_TAKEN || cgs.flagStatus ==
			 FLAG_TAKEN_BLUE))
			return qtrue;
		return qfalse;
	}
...
CG_DrawPlayerHasFlag(rectDef_t *rect, qbool force2D)
{
	...
	if(cg.predictedPlayerState.powerups[PW_REDFLAG])
		...
		CG_DrawFlagModel(rect->x + adj, rect->y + adj, rect->w - adj,
			rect->h - adj, TEAM_BLUE,
			force2D);
	else if(cg.predictedPlayerState.powerups[PW_NEUTRALFLAG])
		CG_DrawFlagModel(rect->x + adj, rect->y + adj, rect->w - adj,
			rect->h - adj, TEAM_FREE,
			force2D);
}

static void
CG_DrawAreaSystemChat(rectDef_t *rect, float scale, Vec4 color,
		      Handle shader)
{
...
static void
CG_DrawAreaTeamChat(rectDef_t *rect, float scale, Vec4 color, Handle shader)
{
	CG_Text_Paint(rect->x, rect->y + rect->h, scale, color,teamChat1, 0, 0,
		0);
}

static void
CG_DrawAreaChat(rectDef_t *rect, float scale, Vec4 color, Handle shader)
{
	CG_Text_Paint(rect->x, rect->y + rect->h, scale, color, teamChat2, 0, 0,
		0);
}
...
static void
CG_DrawKiller(rectDef_t *rect, float scale, Vec4 color, Handle shader,
	      int textStyle)
{
	/* fragged by ... line */
	if(cg.killerName[0]){
		int x = rect->x + rect->w / 2;
		CG_Text_Paint(x - CG_Text_Width(
				CG_GetKillerText(), scale,
				0) / 2, rect->y + rect->h, scale, color,
...
static void
CG_DrawCapFragLimit(rectDef_t *rect, float scale, Vec4 color, Handle shader,
		    int textStyle)
{
	int limit = (cgs.gametype >= GT_CTF) ? cgs.capturelimit : cgs.fraglimit;
	CG_Text_Paint(rect->x, rect->y, scale, color, va("%2i",
			limit),0, 0, textStyle);
}

static void
CG_Draw1stPlace(rectDef_t *rect, float scale, Vec4 color, Handle shader,
		int textStyle)
{
	if(cgs.scores1 != SCORE_NOT_PRESENT)
		CG_Text_Paint(rect->x, rect->y, scale, color,
			va("%2i", cgs.scores1),0, 0, textStyle);
}

static void
CG_Draw2ndPlace(rectDef_t *rect, float scale, Vec4 color, Handle shader,
		int textStyle)
{
	if(cgs.scores2 != SCORE_NOT_PRESENT)
		CG_Text_Paint(rect->x, rect->y, scale, color,
			va("%2i", cgs.scores2),0, 0, textStyle);
}
...
CG_GetGameStatusText(void)
{
	...
	if(cgs.gametype < GT_TEAM){
		...
		if(cg.teamScores[0] == cg.teamScores[1])
			...
			s = va("Red leads Blue, %i to %i", cg.teamScores[0],
				cg.teamScores[1]);
		else
			s = va("Blue leads Red, %i to %i", cg.teamScores[1],
				cg.teamScores[0]);
	}
	return s;
}
...
static void
CG_DrawGameStatus(rectDef_t *rect, float scale, Vec4 color, Handle shader,
		  int textStyle)
{
	CG_Text_Paint(rect->x, rect->y + rect->h, scale, color,
...
static void
CG_DrawGameType(rectDef_t *rect, float scale, Vec4 color, Handle shader,
		int textStyle)
{
	CG_Text_Paint(rect->x, rect->y + rect->h, scale, color,
...
static void
CG_Text_Paint_Limit(float *maxX, float x, float y, float scale, Vec4 color,
...
/* weapon used is not that useful, use the space for task */
			...
			CG_Text_Paint_Limit(&maxx, xx, y + text_y, scale, color,
				ci->name, 0,
				0);
	...
	if(cg.spectatorLen){
		...
		if(cg.time > cg.spectatorTime){
			...
			if(cg.spectatorPaintX <= rect->x + 2){
				if(cg.spectatorOffset < cg.spectatorLen){
					cg.spectatorPaintX += CG_Text_Width(
		...
		CG_Text_Paint_Limit(&maxX, cg.spectatorPaintX, rect->y +
			rect->h - 3, scale, color,
...
void
CG_DrawMedal(int ownerDraw, rectDef_t *rect, float scale, Vec4 color,
	     Handle shader)
{
	Score *score	= &cg.scores[cg.selectedScore];
	float	value	= 0;
	char	*text	= NULL;
	color[3] = 0.25;
	...
	if(text){
		...
		CG_Text_Paint(rect->x +
			(rect->w - value) / 2, rect->y + rect->h + 10, scale,
			color, text,
			0, 0,
			0);
	}
	     ...
	     int textStyle)
{
	...
	case CG_SELECTEDPLAYER_SHIELD:
		...
		break;
	case CG_SELECTEDPLAYER_HEALTH:
		CG_DrawSelectedPlayerHealth(&rect, scale, color, shader,
			textStyle);
		break;
	...
	case CG_TEAMINFO:
		if(cg_currentSelectedPlayer.integer == numSortedTeamPlayers)
			CG_DrawNewTeamInfo(&rect, text_x, text_y, scale, color,
				shader);
		break;
...
CG_KeyEvent(int key, qbool down)
{
	if(!down)
		return;
	if(cg.predictedPlayerState.pm_type == PM_NORMAL ||
	   (cg.predictedPlayerState.pm_type == PM_SPECTATOR && cg.showScores ==
	    qfalse)){
