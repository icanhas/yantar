...
void
CG_TraceCapsule(Trace *result, const Vec3 start, const Vec3 mins,
	 const Vec3 maxs, const Vec3 end,
	 int skipNumber,
	 int mask)
{
...
CG_PointContents(const Vec3 point, int passEntityNum)
{
	...
	for(i = 0; i < cg_numSolidEntities; i++){
		...
		contents |=
			trap_CM_TransformedPointContents(point, cmodel,
				cent->lerpOrigin,
				cent->lerpAngles);
	}
	...
	*out = cg.snap->ps;
	...
	for(i = 0; i < 3; i++){
		...
		out->velocity[i] = prev->ps.velocity[i] +
				   f *
				   (next->ps.velocity[i] - prev->ps.velocity[i]);
	}
...
CG_TouchTriggerPrediction(void)
{
	...
	for(i = 0; i < cg_numTriggerEntities; i++){
		...
		trap_CM_BoxTrace(&trace, cg.predictedPlayerState.origin,
			...
			cg_pmove.mins, cg_pmove.maxs, cmodel,
			-1);
