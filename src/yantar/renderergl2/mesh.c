...
ProjectRadius(float r, Vec3 location)
{
	...
	c = dotv3(tr.viewParms.or.axis[0], tr.viewParms.or.origin);
	dist = dotv3(tr.viewParms.or.axis[0], location) - c;
...
R_CullModel(mdvModel_t *model, trRefEntity_t *ent)
{
	...
	for(i = 0; i < 3; i++){
		bounds[0][i] = oldFrame->bounds[0][i] <
			       newFrame->bounds[0][i] ? oldFrame->bounds[0][i] : newFrame->bounds[0][i];
		bounds[1][i] = oldFrame->bounds[1][i] >
			       newFrame->bounds[1][i] ? oldFrame->bounds[1][i] : newFrame->bounds[1][i];
	}
...
R_ComputeFogNum(mdvModel_t *model, trRefEntity_t *ent)
{
	...
	addv3(ent->e.origin, mdvFrame->localOrigin, localOrigin);
...
R_AddMD3Surfaces(trRefEntity_t *ent)
{
	...
	personalModel = (ent->e.renderfx & RF_THIRD_PERSON) &&
			!(tr.viewParms.isPortal || tr.viewParms.isShadowmap);
	...
	for(i = 0; i < model->numSurfaces; i++){
		...
		if(ent->e.customShader){
			...
			shader =
				tr.shaders[ surface->shaderIndexes[ ent->e.skinNum %
