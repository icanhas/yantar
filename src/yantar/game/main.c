...
typedef struct Cvartable	Cvartable;
struct Cvartable {
	Vmcvar	*vmCvar;
	char		*cvarName;
	char		*defaultString;
...
Vmcvar	g_speed;
Vmcvar	g_gravity;
Vmcvar	g_hookspeed;	/* hook travel time in metres per second */
...
static Cvartable gameCvarTable[] = {
	...
	/* latched vars */
	{ &g_gametype, "g_gametype", "0", CVAR_SERVERINFO | CVAR_USERINFO |
	  CVAR_LATCH, 0, qfalse  },

	{ &g_maxclients, "sv_maxclients", "8", CVAR_SERVERINFO | CVAR_LATCH |
	  CVAR_ARCHIVE, 0, qfalse  },
	{ &g_maxGameClients, "g_maxGameClients", "0", CVAR_SERVERINFO |
	  CVAR_LATCH | CVAR_ARCHIVE, 0, qfalse  },
	...
	{ &g_gravity, "g_gravity", "0", CVAR_CHEAT, 0, qtrue  },
	{ &g_hookspeed, "g_hookspeed", "30", CVAR_CHEAT, 0, qtrue },
...
G_InitGame(int levelTime, int randomSeed, int restart)
{
	...
	level.snd_fry = G_SoundIndex(Pplayersounds "/fry.wav");	/* FIXME standing in lava / slime */
...
CalculateRanks(void)
{
	...
	if(g_gametype.integer >= GT_TEAM){
		...
		for(i = 0; i < level.numPlayingClients; i++){
			...
			if(i == 0 || newScore != score){
				...
				level.clients[ level.sortedClients[i] ].ps.
				persistant[PERS_RANK] = rank;
			}else{
	...
	if(g_gametype.integer >= GT_TEAM){
		trap_SetConfigstring(CS_SCORES1,
			va("%i", level.teamScores[TEAM_RED]));
		trap_SetConfigstring(CS_SCORES2,
			va("%i", level.teamScores[TEAM_BLUE]));
	}else{
		if(level.numConnectedClients == 0){
			trap_SetConfigstring(CS_SCORES1,
				va("%i", SCORE_NOT_PRESENT));
			trap_SetConfigstring(CS_SCORES2,
				va("%i", SCORE_NOT_PRESENT));
...
MoveClientToIntermission(Gentity *ent)
{
	...
	/* move to the spot */
	copyv3(level.intermission_origin, ent->s.origin);
	copyv3(level.intermission_origin, ent->client->ps.origin);
	copyv3 (level.intermission_angle, ent->client->ps.viewangles);
...
FindIntermissionPoint(void)
{
	...
	if(!ent)	/* the map creator forgot to put in an intermission point */
		...
		copyv3(ent->s.origin, level.intermission_origin);
		copyv3(ent->s.angles, level.intermission_angle);
		/* if it has a target, look towards it */
		if(ent->target){
			target = G_PickTarget(ent->target);
			if(target){
				subv3(target->s.origin, level.intermission_origin, dir);
				v3toeuler(dir, level.intermission_angle);
			}
		}
	}
}
...
CheckExitRules(void)
{
	...
	if(g_gametype.integer < GT_CTF && g_fraglimit.integer){
		...
		if(level.teamScores[TEAM_BLUE] >= g_fraglimit.integer){
			trap_SendServerCommand(
				-1, "print \"Blue hit the fraglimit.\n\"");
			LogExit("Fraglimit hit.");
			return;
		}
	...
	if(g_gametype.integer >= GT_CTF && g_capturelimit.integer){
		...
		if(level.teamScores[TEAM_BLUE] >= g_capturelimit.integer){
			trap_SendServerCommand(
				-1, "print \"Blue hit the capturelimit.\n\"");
			LogExit("Capturelimit hit.");
			return;
		}
	}
		...
		if(level.teamVoteYes[cs_offset] >
		   level.numteamVotingClients[cs_offset]/2){
			...
			if(!Q_strncmp("leader", level.teamVoteString[cs_offset],
				...
				/* set the team leader */
				SetLeader(team,
					atoi(level.teamVoteString[cs_offset] + 7));
			else
				trap_SendConsoleCommand(EXEC_APPEND,
					va("%s\n",
...
G_RunFrame(int levelTime)
{
	...
	for(i=0; i < level.maxclients; i++, ent++)
		if(ent->inuse)
			ClientEndFrame(ent);
	CheckTournament();	/* see if it is time to do a tournement restart */
	CheckExitRules();	/* see if it is time to end the level */
	CheckTeamStatus();	/* update to team status? */
	CheckVote();	/* cancel vote if timed out */
	CheckTeamVote(TEAM_RED);	/* check team votes */
	CheckTeamVote(TEAM_BLUE);
	CheckCvars();	/* for tracking changes */
