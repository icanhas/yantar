	    ...
	    int contentmask)
{
	...
	trap_Trace(&trace, start, mins, maxs, end, nil, nil, passent,
		   contentmask);
...
BotChangeViewAngles(bot_state_t *bs, float thinktime)
{
	...
	for(i = 0; i < 2; i++){
		if(bot_challenge.integer){
			...
			diff =
				abs(AngleDifference(bs->viewangles[i],
			...
			bs->viewanglespeed[i] +=
				(bs->viewanglespeed[i] - disired_speed);
			if(bs->viewanglespeed[i] > 180) bs->viewanglespeed[i] =
					maxchange;
			if(bs->viewanglespeed[i] < -180) bs->viewanglespeed[i] =
					-maxchange;
		      ...
		      int time)
{
	...
	if(bi->actionflags & ACTION_AFFIRMATIVE) ucmd->buttons |=
			BUTTON_AFFIRMATIVE;
 ...
 *      trap_cvarsetstr("bot_report", "0"); */
	...
	if(botlib_residual >= thinktime){
		...
		for(i = 0; i < MAX_GENTITIES; i++){
			...
			if(ent->s.eType == ET_MISSILE && ent->s.weap[WSpri] !=
			   Whook){
				trap_BotLibUpdateEntity(i, NULL);
				continue;
			}
