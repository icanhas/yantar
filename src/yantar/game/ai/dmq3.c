...
#define AREACONTENTS_MODELNUM		(AREACONTENTS_MAXMODELNUM << \
...
BotCTFSeekGoals(bot_state_t *bs)
{
	...
	if(BotCTFCarryingFlag(bs)){
		...
		if(bs->ltgtype != LTG_RUSHBASE){
			...
			/*  */
			switch(BotTeam(bs)){
			case TEAM_RED: subv3(bs->origin,
					ctf_blueflag.origin,
					dir); break;
			case TEAM_BLUE: subv3(bs->origin,
					ctf_redflag.origin,
					dir); break;
			default: setv3(dir, 999, 999, 999); break;
			}
	...
	/*  */
	if(BotTeam(bs) == TEAM_RED) flagstatus = bs->redflagstatus * 2 +
						 bs->blueflagstatus;
	...
	/* if both flags Not at their bases */
		...
		if(bs->owndecision_time < FloatTime())
			...
			if(bs->ltgtype != LTG_RETURNFLAG && bs->ltgtype !=
			   LTG_TEAMACCOMPANY){
				/*  */
				c = BotTeamFlagCarrierVisible(bs);
				/* if there is a visible team mate flag carrier */
				if(c >= 0){
					...
					bs->ordered = qfalse;
					/* get the enemy flag */
					bs->teammessage_time = FloatTime() + 2 *
							       random();
					/* get the flag */
					bs->ltgtype = LTG_RETURNFLAG;
					/* set the time the bot will stop getting the flag */
	...
	if(rnd < l1 && ctf_redflag.areanum && ctf_blueflag.areanum){
		...
		bs->ordered = qfalse;
		/*  */
		if(BotTeam(bs) == TEAM_RED) memcpy(&bs->teamgoal, &ctf_redflag,
				sizeof(bot_goal_t));
	...
	if(bs->neutralflagstatus == 1){
		if(bs->owndecision_time < FloatTime()){
			...
			if(bs->ltgtype != LTG_ATTACKENEMYBASE){
				...
				bs->ordered = qfalse;
				/*  */
				if(BotTeam(bs) == TEAM_RED) memcpy(
						&bs->teamgoal, &ctf_blueflag,
						sizeof(bot_goal_t));
				else memcpy(&bs->teamgoal, &ctf_redflag,
						sizeof(bot_goal_t));
				/* set the ltg type */
				bs->ltgtype = LTG_ATTACKENEMYBASE;
				/* set the time the bot will stop getting the flag */
	...
	/* enemy team has the flag */
		...
		if(bs->owndecision_time < FloatTime()){
			...
			if(bs->ltgtype != LTG_DEFENDKEYAREA){
				...
				/*  */
				if(BotTeam(bs) == TEAM_RED) memcpy(
						&bs->teamgoal, &ctf_redflag,
						sizeof(bot_goal_t));
				else memcpy(&bs->teamgoal, &ctf_blueflag,
						sizeof(bot_goal_t));
				/* set the ltg type */
				bs->ltgtype = LTG_DEFENDKEYAREA;
				/* set the time the bot stops defending the base */
	...
	if(bs->cur_ps.weaponstate == WEAPON_RAISING ||
	   bs->cur_ps.weaponstate == WEAPON_DROPPING)
		...
		newweaponnum = trap_BotChooseBestFightWeapon(bs->ws,
			bs->inventory);
		if(bs->weaponnum != newweaponnum) bs->weaponchange_time =
				FloatTime();
...
BotSetupForMovement(bot_state_t *bs)
{
	...
	copyv3(bs->cur_ps.origin, initmove.origin);
	copyv3(bs->cur_ps.velocity, initmove.velocity);
	clearv3(initmove.viewoffset);
	...
	if(bs->cur_ps.groundEntityNum !=
	   ENTITYNUM_NONE) initmove.or_moveflags |= MFL_ONGROUND;
	...
	/* set presence type */
	/* if(bs->cur_ps.pm_flags & PMF_DUCKED) initmove.presencetype =
			PRESENCE_CROUCH;
	*/
	...
	/*  */
	copyv3(bs->viewangles, initmove.viewangles);
	/*  */
...
BotCheckItemPickup(bot_state_t *bs, int *oldinventory)
{
	...
	if(!bs->inventory[INVENTORY_KAMIKAZE] &&
	   !bs->inventory[INVENTORY_INVULNERABILITY]){
		if(!oldinventory[INVENTORY_SCOUT] &&
		   bs->inventory[INVENTORY_SCOUT] >= 1)
			offence = qtrue;
		if(!oldinventory[INVENTORY_GUARD] &&
		   bs->inventory[INVENTORY_GUARD] >= 1)
			offence = qtrue;
...
BotUpdateInventory(bot_state_t *bs)
{
	...
	bs->inventory[INVENTORY_GAUNTLET] =
		(bs->cur_ps.stats[STAT_PRIWEAPS] & (1 << W1_GAUNTLET)) != 0;
	bs->inventory[INVENTORY_SHOTGUN] =
		(bs->cur_ps.stats[STAT_PRIWEAPS] & (1 << W1_SHOTGUN)) != 0;
	bs->inventory[INVENTORY_MACHINEGUN] =
		(bs->cur_ps.stats[STAT_PRIWEAPS] & (1 << W1_MACHINEGUN)) != 0;
	bs->inventory[INVENTORY_GRENADELAUNCHER] =
		(bs->cur_ps.stats[STAT_PRIWEAPS] & (1 << W1_GRENADE_LAUNCHER)) !=
		0;
	bs->inventory[INVENTORY_ROCKETLAUNCHER] =
		(bs->cur_ps.stats[STAT_PRIWEAPS] & (1 << W1_ROCKET_LAUNCHER)) !=
		0;
	bs->inventory[INVENTORY_LIGHTNING] =
		(bs->cur_ps.stats[STAT_PRIWEAPS] & (1 << W1_LIGHTNING)) != 0;
	bs->inventory[INVENTORY_RAILGUN] =
		(bs->cur_ps.stats[STAT_PRIWEAPS] & (1 << W1_RAILGUN)) != 0;
	bs->inventory[INVENTORY_PLASMAGUN] =
		(bs->cur_ps.stats[STAT_PRIWEAPS] & (1 << W1_PLASMAGUN)) != 0;
	bs->inventory[INVENTORY_BFG10K] =
		(bs->cur_ps.stats[STAT_PRIWEAPS] & (1 << W1_BFG)) != 0;
	bs->inventory[INVENTORY_GRAPPLINGHOOK] =
		(bs->cur_ps.stats[STAT_PRIWEAPS] & (1 << W1_GRAPPLING_HOOK)) != 0;
#ifdef MISSIONPACK
	bs->inventory[INVENTORY_NAILGUN] =
		(bs->cur_ps.stats[STAT_PRIWEAPS] & (1 << W1_NAILGUN)) != 0;;
	bs->inventory[INVENTORY_PROXLAUNCHER] =
		(bs->cur_ps.stats[STAT_PRIWEAPS] & (1 << W1_PROX_LAUNCHER)) != 0;;
	bs->inventory[INVENTORY_CHAINGUN] =
		(bs->cur_ps.stats[STAT_PRIWEAPS] & (1 << W1_CHAINGUN)) != 0;;
#endif
	/* ammo */
	bs->inventory[INVENTORY_SHELLS]		= bs->cur_ps.ammo[W1_SHOTGUN];
	bs->inventory[INVENTORY_BULLETS]	= bs->cur_ps.ammo[W1_MACHINEGUN];
	bs->inventory[INVENTORY_GRENADES]	=
		bs->cur_ps.ammo[W1_GRENADE_LAUNCHER];
	bs->inventory[INVENTORY_CELLS] = bs->cur_ps.ammo[W1_PLASMAGUN];
	bs->inventory[INVENTORY_LIGHTNINGAMMO]	= bs->cur_ps.ammo[W1_LIGHTNING];
	bs->inventory[INVENTORY_ROCKETS]	=
		bs->cur_ps.ammo[W1_ROCKET_LAUNCHER];
	bs->inventory[INVENTORY_SLUGS]		= bs->cur_ps.ammo[W1_RAILGUN];
	bs->inventory[INVENTORY_BFGAMMO]	= bs->cur_ps.ammo[W1_BFG];
#ifdef MISSIONPACK
	bs->inventory[INVENTORY_NAILS]	= bs->cur_ps.ammo[W1_NAILGUN];
	bs->inventory[INVENTORY_MINES]	= bs->cur_ps.ammo[W1_PROX_LAUNCHER];
	bs->inventory[INVENTORY_BELT]	= bs->cur_ps.ammo[W1_CHAINGUN];
#endif
	/* powerups */
	...
	bs->inventory[INVENTORY_REDFLAG] =
		bs->cur_ps.powerups[PW_REDFLAG] != 0;
	bs->inventory[INVENTORY_BLUEFLAG] =
...
BotUpdateBattleInventory(bot_state_t *bs, int enemy)
{
	...
	subv3(entinfo.origin, bs->origin, dir);
	bs->inventory[ENEMY_HEIGHT] = (int)dir[2];
	dir[2] = 0;
	bs->inventory[ENEMY_HORIZONTAL_DIST] = (int)lenv3(dir);
 ...
 */
bot_waypoint_t *
BotCreateWayPoint(char *name, Vec3 origin, int areanum)
{
	bot_waypoint_t *wp;
	Vec3 waypointmins = {-8, -8, -8}, waypointmaxs = {8, 8, 8};
	...
	copyv3(origin, wp->goal.origin);
	copyv3(waypointmins, wp->goal.mins);
	copyv3(waypointmaxs, wp->goal.maxs);
...
{
	return qfalse; /* no persistent powerups in the game at present */
...
BotGoCamp(bot_state_t *bs, bot_goal_t *goal)
{
	...
	/* get the team goal time */
	camper =
		trap_Characteristic_BFloat(bs->character, CHARACTERISTIC_CAMPER,
			0,
			1);
	if(camper > 0.99) bs->teamgoal_time = FloatTime() + 99999;
	else bs->teamgoal_time = FloatTime() + 120 + 180 * camper + random() *
				 15;
...
BotWantsToCamp(bot_state_t *bs)
{
	...
	for(cs = trap_BotGetNextCampSpotGoal(0, &goal); cs;
		...
		traveltime =
			trap_AAS_AreaTravelTimeToGoalArea(bs->areanum,
				bs->origin,
				goal.areanum,
				TFL_DEFAULT);
 ...
 */
void
BotRoamGoal(bot_state_t *bs, Vec3 goal)
{
	int pc, i;
	float	len, rnd;
	...
	for(i = 0; i < 10; i++){
		...
		BotAI_Trace(&trace, bs->origin, NULL, NULL, bestorg,
			bs->entitynum,
			MASK_SOLID);
		/* direction and length towards the roam target */
		subv3(trace.endpos, bs->origin, dir);
		len = normv3(dir);
		/* if the roam target is far away anough */
		if(len > 200){
			/* the roam target is in the given direction before walls */
			scalev3(dir, len * trace.fraction - 40, dir);
			addv3(bs->origin, dir, bestorg);
...
BotAttackMove(bot_state_t *bs, int tfl)
{
	...
	Vec3	forward, backward, sideward, hordir, up = {0, 0, 1};
	aas_entityinfo_t	entinfo;
	...
	if(bs->attackchase_time > FloatTime()){
		...
		copyv3(bs->lastenemyorigin, goal.origin);
		setv3(goal.mins, -8, -8, -8);
		setv3(goal.maxs, 8, 8, 8);
	...
	attack_skill =
		trap_Characteristic_BFloat(bs->character,
			...
			1);
	jumper =
		trap_Characteristic_BFloat(bs->character, CHARACTERISTIC_JUMPER,
			0,
			1);
	croucher =
		trap_Characteristic_BFloat(bs->character,
	...
	subv3(entinfo.origin, bs->origin, forward);
	/* the distance towards the enemy */
	dist = normv3(forward);
	negv3(forward, backward);
	/* walk, crouch or jump */
	movetype = MOVE_WALK;
	/*  */
	if(bs->attackcrouch_time < FloatTime() - 1){
		...
		else if(bs->attackcrouch_time < FloatTime() - 1 && random() <
			croucher)
	...
	if(bs->cur_ps.weapon == W1_GAUNTLET){
		attack_dist	= 0;
		attack_range	= 0;
	}else{
	...
	for(i = 0; i < 2; i++){
		...
		crossv3(hordir, up, sideward);
		/* reverse the vector depending on the strafe direction */
		if(bs->flags & BFL_STRAFERIGHT) negv3(sideward, sideward);
		/* randomly go back a little */
		if(random() > 0.9)
			addv3(sideward, backward, sideward);
		else{
			/* walk forward or backward to get at the ideal attack distance */
			if(dist > attack_dist + attack_range)
				addv3(sideward, forward, sideward);
 ...
 */
qbool
InFieldOfVision(Vec3 viewangles, float fov, Vec3 angles)
{
	int i;
	float diff, angle;

	for(i = 0; i < 2; i++){
		angle = modeuler(viewangles[i]);
		angles[i] = modeuler(angles[i]);
		diff = angles[i] - angle;
		if(angles[i] > angle){
 ...
 */
float
BotEntityVisible(int viewer, Vec3 eye, Vec3 viewangles, float fov, int ent)
{
	...
	addv3(entinfo.mins, entinfo.maxs, middle);
	scalev3(middle, 0.5, middle);
	addv3(entinfo.origin, middle, middle);
	/* check if entity is within field of vision */
	subv3(middle, eye, dir);
	v3toeuler(dir, entangles);
	...
	for(i = 0; i < 3; i++){
		...
		if(trace.fraction >= 1 || trace.ent == hitent){
			...
			vis = 1 /
			      ((squaredfogdist * 0.001) <
			       1 ? 1 : (squaredfogdist * 0.001));
...
BotFindEnemy(bot_state_t *bs, int curenemy)
{
	...
	if(curenemy >= 0){
		...
		subv3(curenemyinfo.origin, bs->origin, dir);
		cursquaredist = lensqrv3(dir);
	}else
		cursquaredist = 0;
	...
	for(i = 0; i < maxclients && i < MAX_CLIENTS; i++){
		...
		if(lastteleport_time > FloatTime() - 3){
			subv3(entinfo.origin, lastteleport_origin, dir);
			if(lensqrv3(dir) < Square(70)) continue;
		}
		/* calculate the distance towards the enemy */
		subv3(entinfo.origin, bs->origin, dir);
		squaredist = lensqrv3(dir);
		...
		vis = BotEntityVisible(bs->entitynum, bs->eye, bs->viewangles, f,
			i);
		if(vis <= 0) continue;
		...
		if(curenemy < 0 && squaredist > Square(100) &&
		   !healthdecrease && !EntityIsShooting(&entinfo)){
			...
			subv3(bs->origin, entinfo.origin, dir);
...
BotTeamFlagCarrierVisible(bot_state_t *bs)
{
	...
	for(i = 0; i < maxclients && i < MAX_CLIENTS; i++){
		...
		vis =
			BotEntityVisible(bs->entitynum, bs->eye, bs->viewangles,
				360,
				i);
		if(vis <= 0)
			continue;
...
BotAimAtEnemy(bot_state_t *bs)
{
	...
	Vec3	dir, bestorigin, end, start, groundtarget, cmdmove,
		enemyvelocity;
	Vec3	mins = {-4,-4,-4}, maxs = {4, 4, 4};
	weaponinfo_t	wi;
	aas_entityinfo_t entinfo;
	bot_goal_t	goal;
	bsp_trace_t	trace;
	...
	if(bs->enemy >= MAX_CLIENTS){
		...
		v3toeuler(dir, bs->ideal_viewangles);
		/* set the aim target before trying to attack */
		copyv3(target, bs->aimtarget);
		return;
	}
	/*
	...
	aim_skill =
		trap_Characteristic_BFloat(bs->character,
			CHARACTERISTIC_AIM_SKILL, 0,
			1);
	aim_accuracy =
		trap_Characteristic_BFloat(bs->character,
			CHARACTERISTIC_AIM_ACCURACY, 0,
			1);
	/*  */
	if(aim_skill > 0.95){
		/* don't aim too early */
	...
	if(wi.number == W1_MACHINEGUN)
		aim_accuracy = trap_Characteristic_BFloat(
			bs->character, CHARACTERISTIC_AIM_ACCURACY_MACHINEGUN, 0,
			1);
	else if(wi.number == W1_SHOTGUN)
		aim_accuracy = trap_Characteristic_BFloat(
			bs->character, CHARACTERISTIC_AIM_ACCURACY_SHOTGUN, 0, 1);
	else if(wi.number == W1_GRENADE_LAUNCHER){
		aim_accuracy = trap_Characteristic_BFloat(
			bs->character,
		...
		aim_skill = trap_Characteristic_BFloat(
			...
			0, 1);
	}else if(wi.number == W1_ROCKET_LAUNCHER){
		aim_accuracy = trap_Characteristic_BFloat(
			bs->character,
			CHARACTERISTIC_AIM_ACCURACY_ROCKETLAUNCHER, 0, 1);
		aim_skill = trap_Characteristic_BFloat(
			bs->character, CHARACTERISTIC_AIM_SKILL_ROCKETLAUNCHER,
			0, 1);
	}else if(wi.number == W1_LIGHTNING)
		aim_accuracy = trap_Characteristic_BFloat(
			bs->character, CHARACTERISTIC_AIM_ACCURACY_LIGHTNING, 0,
			1);
	else if(wi.number == W1_RAILGUN)
		aim_accuracy = trap_Characteristic_BFloat(
			bs->character, CHARACTERISTIC_AIM_ACCURACY_RAILGUN, 0, 1);
	else if(wi.number == W1_PLASMAGUN){
		aim_accuracy = trap_Characteristic_BFloat(
			bs->character, CHARACTERISTIC_AIM_ACCURACY_PLASMAGUN, 0,
			1);
		aim_skill = trap_Characteristic_BFloat(
			bs->character, CHARACTERISTIC_AIM_SKILL_PLASMAGUN, 0, 1);
	}else if(wi.number == W1_BFG){
		aim_accuracy = trap_Characteristic_BFloat(
	...
	/*  */
	subv3(entinfo.origin, entinfo.lastvisorigin, enemyvelocity);
	scalev3(enemyvelocity, 1 / entinfo.update_time, enemyvelocity);
	...
	if(bs->enemyposition_time < FloatTime()){
		...
		copyv3(enemyvelocity, bs->enemyvelocity);
		copyv3(entinfo.origin, bs->enemyorigin);
	}
	/* if not extremely skilled */
	if(aim_skill < 0.9){
		subv3(entinfo.origin, bs->enemyorigin, dir);
		/* if the enemy moved a bit */
		if(lensqrv3(dir) > Square(48))
			/* if the enemy changed direction */
			if(dotv3(bs->enemyvelocity, enemyvelocity) < 0)
	...
	if(enemyvisible){
		/*  */
		copyv3(entinfo.origin, bestorigin);
		bestorigin[2] += 8;
		...
		if(wi.speed){
			/*  */
			subv3(bestorigin, bs->origin, dir);
			dist = lenv3(dir);
			subv3(entinfo.origin, bs->enemyorigin, dir);
			/* if the enemy is NOT pretty far away and strafing just small steps left and right */
			if(!(dist > 100 && lensqrv3(dir) <
			     Square(32))){
				...
				if(aim_skill > 0.8 &&
				   ...
				   bs->cur_ps.weaponstate == WEAPON_READY){
					...
					subv3(entinfo.origin,
						...
						dir);
					/*  */
					scalev3(dir, 1 / entinfo.update_time,
						dir);
					/*  */
					copyv3(entinfo.origin, origin);
					origin[2] += 1;
					/*  */
					clearv3(cmdmove);
					/* AAS_ClearShownDebugLines(); */
					trap_AAS_PredictClientMovement(
						&move, bs->enemy, origin,
						PRESENCE_CROUCH, qfalse,
						dir, cmdmove, 0,
						dist * 10 / wi.speed, 0.1f, 0, 0,
						qfalse);
					copyv3(move.endpos, bestorigin);
					/* BotAI_Print(PRT_MESSAGE, "%1.1f predicted speed = %f, frames = %f\n", FloatTime(), lenv3(dir), dist * 10 / wi.speed); */
				}
		...
		if(aim_skill > 0.6 && wi.proj.damagetype & DAMAGETYPE_RADIAL)
			...
			if(entinfo.origin[2] < bs->origin[2] + 16){
				...
				copyv3(entinfo.origin, end);
				end[2] -= 64;
				BotAI_Trace(&trace, entinfo.origin, NULL, NULL,
					end, entinfo.number,
					MASK_SHOT);
				/*  */
				copyv3(bestorigin, groundtarget);
				...
				BotAI_Trace(&trace, start, NULL, NULL,
					groundtarget, bs->entitynum,
					MASK_SHOT);
		...
		/*  */
		copyv3(bs->lastenemyorigin, bestorigin);
		bestorigin[2] += 8;
		/* if the bot is skilled anough */
		if(aim_skill > 0.5)
			/* do prediction shots around corners */
			if(wi.number == W1_BFG ||
			   wi.number == W1_ROCKET_LAUNCHER ||
			   wi.number == W1_GRENADE_LAUNCHER){
				/* create the chase goal */
				goal.entitynum	= bs->client;
				goal.areanum	= bs->areanum;
				copyv3(bs->eye, goal.origin);
				setv3(goal.mins, -8, -8, -8);
				setv3(goal.maxs, 8, 8, 8);
				/*  */
				if(trap_BotPredictVisiblePosition(bs->
	...
	if(enemyvisible){
		BotAI_Trace(&trace, bs->eye, NULL, NULL, bestorigin,
			bs->entitynum,
			MASK_SHOT);
		copyv3(trace.endpos, bs->aimtarget);
	}else
		copyv3(bestorigin, bs->aimtarget);
	/* get aim direction */
	subv3(bestorigin, bs->eye, dir);
	/*  */
	if(wi.number == W1_MACHINEGUN ||
	   wi.number == W1_SHOTGUN ||
	   wi.number == W1_LIGHTNING ||
	   wi.number == W1_RAILGUN){
		/* distance towards the enemy */
		dist = lenv3(dir);
	...
	bs->ideal_viewangles[PITCH] += 6 * wi.vspread*crandom() *
				       (1 - aim_accuracy);
	bs->ideal_viewangles[PITCH]	= modeuler(bs->ideal_viewangles[PITCH]);
	bs->ideal_viewangles[YAW]	+= 6 * wi.hspread*crandom() *
					   (1 - aim_accuracy);
	bs->ideal_viewangles[YAW] = modeuler(bs->ideal_viewangles[YAW]);
	...
	if(bot_challenge.integer)
		...
		if(aim_accuracy > 0.9 && bs->enemysight_time < FloatTime() - 1){
			...
			copyv3(bs->ideal_viewangles, bs->viewangles);
...
BotCheckAttack(bot_state_t *bs)
{
	...
	if(bs->firethrottleshoot_time < FloatTime()){
		if(random() > firethrottle){
			...
			bs->firethrottleshoot_time = FloatTime() + 1 -
						     firethrottle;
	...
	/*
	 *  */
	subv3(bs->aimtarget, bs->eye, dir);
	/*  */
	if(bs->weaponnum == W1_GAUNTLET)
		if(lensqrv3(dir) > Square(60))
			return;
	if(lensqrv3(dir) < Square(100))
		fov = 120;
	else
		fov = 50;
	/*  */
	...
	copyv3(bs->origin, start);
	start[2] += bs->cur_ps.viewheight;
	anglev3s(bs->viewangles, forward, right, NULL);
	...
	if(trace.ent != attackentity || attackentity >= MAX_CLIENTS){
		...
		if(wi.proj.damagetype & DAMAGETYPE_RADIAL)
			if(trace.fraction * 1000 < wi.proj.radius){
				points =
					(wi.proj.damage - 0.5 * trace.fraction *
					 1000) * 0.5;
				if(points > 0)
					return;
			}
...
BotMapScripts(bot_state_t *bs)
{
	...
	if(!Q_stricmp(mapname, "q3tourney6")){
		Vec3	mins = {700, 204, 672}, maxs = {964, 468, 680};
		Vec3	buttonorg = {304, 352, 920};
		...
		for(i = 0; i < maxclients && i < MAX_CLIENTS; i++){
			...
			if(EntityIsDead(&entinfo) || entinfo.number ==
			   bs->entitynum) continue;
			/*  */
			if(entinfo.origin[0] > mins[0] && entinfo.origin[0] <
			   maxs[0]){
				if(entinfo.origin[1] > mins[1] &&
				   entinfo.origin[1] < maxs[1])
		...
		if(shootbutton){
			...
			subv3(buttonorg, bs->eye, dir);
			v3toeuler(dir, bs->ideal_viewangles);
			aim_accuracy = trap_Characteristic_BFloat(
				bs->character, CHARACTERISTIC_AIM_ACCURACY, 0, 1);
			bs->ideal_viewangles[PITCH] += 8 * crandom() *
						       (1 - aim_accuracy);
			bs->ideal_viewangles[PITCH] = modeuler(
				bs->ideal_viewangles[PITCH]);
			bs->ideal_viewangles[YAW] += 8 * crandom() *
						     (1 - aim_accuracy);
			bs->ideal_viewangles[YAW] = modeuler(
...
void
BotSetMovedir(Vec3 angles, Vec3 movedir)
{
		 ...
		 Vec3 maxs)
{
	...
	for(i = 0; i < level.num_entities; i++, ent++){
		...
		if(ent->s.modelindex == modelindex){
			if(mins)
				addv3(ent->r.currentOrigin, ent->r.mins,
					mins);
			if(maxs)
				addv3(ent->r.currentOrigin, ent->r.maxs,
					maxs);
			return i;
		}
	}
			  ...
			  bot_activategoal_t *activategoal)
{
	...
	if(health){
		...
		if(bsptrace.fraction >= 1.0 || bsptrace.ent == entitynum){
			...
			copyv3(bs->origin, activategoal->goal.origin);
			activategoal->goal.areanum = bs->areanum;
			setv3(activategoal->goal.mins, -8, -8, -8);
			setv3(activategoal->goal.maxs, 8, 8, 8);
			/*  */
			return qtrue;
		}else{
			/* create a goal from where the button is visible and shoot at the button from there
			 * add bounding box size to the dist */
			trap_AAS_PresenceTypeBoundingBox(PRESENCE_CROUCH,
				bboxmins,
				bboxmaxs);
			for(i = 0; i < 3; i++){
				if(movedir[i] < 0) dist += fabs(movedir[i]) *
							   fabs(bboxmaxs[i]);
			...
			if(i >= 0){
				/*  */
				copyv3(points[i], activategoal->goal.origin);
				...
				for(i = 0; i < 3; i++){
					if(movedir[i] <
					   0) activategoal->goal.maxs[i] += fabs(
							movedir[i]) * fabs(
							extramaxs[i]);
		...
		trap_AAS_PresenceTypeBoundingBox(PRESENCE_CROUCH, bboxmins,
			bboxmaxs);
		for(i = 0; i < 3; i++){
			if(movedir[i] < 0) dist += fabs(movedir[i]) * fabs(
					bboxmaxs[i]);
			else dist += fabs(movedir[i]) * fabs(bboxmins[i]);
		}
		/* calculate the goal origin */
		...
		if(i < numareas){
			/*  */
			copyv3(origin, activategoal->goal.origin);
			activategoal->goal.areanum = areas[i];
			subv3(mins, origin, activategoal->goal.mins);
			subv3(maxs, origin, activategoal->goal.maxs);
			/*  */
			for(i = 0; i < 3; i++){
			...
			bot_activategoal_t *activategoal)
{
	...
	addv3(mins, maxs, origin);
	scalev3(origin, 0.5, origin);
	copyv3(origin, activategoal->target);
	activategoal->shoot = qtrue;
	/*  */
			       ...
			       bot_activategoal_t *activategoal)
{
	int	i, areas[10], numareas, modelindex, entitynum;
	char	model[128];
	Vec3	start, end, mins, maxs, angles;
	Vec3	origin, goalorigin;
...
BotIsGoingToActivateEntity(bot_state_t *bs, int entitynum)
{
	...
	for(i = 0; i < MAX_ACTIVATESTACK; i++){
		...
		if(bs->activategoalheap[i].goal.entitynum == entitynum)
			...
			if(bs->activategoalheap[i].justused_time > FloatTime() -
			   2)
				return qtrue;
	}
	return qfalse;
		   ...
		   bot_activategoal_t *activategoal)
{
	...
	Vec3 origin, angles, absmins, absmaxs;
	...
	if(!strcmp(classname, "func_door")){
		...
		if(!cmpv3(origin, entinfo.origin))
			return 0;
		...
		if(*model){
			...
			if(modelindex){
				clearv3(angles);
				BotModelMinsMaxs(modelindex, ET_MOVER, 0,
					absmins,
					absmaxs);
				/*  */
				numareas = trap_AAS_BBoxAreas(
					absmins, absmaxs, areas,
					MAX_ACTIVATEAREAS*2);
				/* store the areas with reachabilities first */
				for(i = 0; i < numareas; i++){
					if(activategoal->numareas >=
					   MAX_ACTIVATEAREAS)
						break;
					if(!trap_AAS_AreaReachability(areas[i]))
						continue;
					...
					if(areainfo.contents &
					   AREACONTENTS_MOVER)
						activategoal->areas[activategoal
								    ->numareas++
						] = areas[i];
				}
	...
	for(i = 0; i >= 0 && i < 10; ){
		...
		/* the actual button or trigger might be linked through a target_relay or target_delay */
			...
			if(trap_AAS_ValueForBSPEpairKey(ent, "targetname",
				   targetname[i+1], sizeof(targetname[0]))){
				i++;
...
BotGoForActivateGoal(bot_state_t *bs, bot_activategoal_t *activategoal)
{
	...
	copyv3(activateinfo.origin, activategoal->origin);
	/*  */
...
{
	int movetype, bspent;
	Vec3 hordir, sideward, angles, up = {0, 0, 1};
	...
	if(normv3(hordir) < 0.1){
		setv3(angles, 0, 360 * random(), 0);
		anglev3s(angles, hordir, NULL, NULL);
	}
	/*
	...
	crossv3(hordir, up, sideward);
	/*  */
	if(bs->flags & BFL_AVOIDRIGHT) negv3(sideward, sideward);
...
BotAIPredictObstacles(bot_state_t *bs, bot_goal_t *goal)
{
	...
	if(route.stopevent & RSE_ENTERCONTENTS){
		...
		if(route.endcontents & AREACONTENTS_MOVER){
			...
			modelnum =
				(route.endcontents &
				 AREACONTENTS_MODELNUM) >>
				AREACONTENTS_MODELNUMSHIFT;
			if(modelnum){
				/*  */
				entitynum =
					BotModelMinsMaxs(modelnum, ET_MOVER, 0,
						NULL,
						NULL);
				if(entitynum){
					/* NOTE: BotGetActivateGoal already checks if the door is open or not */
					bspent = BotGetActivateGoal(
						bs, entitynum, &activategoal);
					if(bspent){
						/*  */
						if(bs->activatestack &&
						   !bs->activatestack->inuse)
							bs->activatestack = NULL;
						/* if not already trying to activate this entity */
...
{
	char	botname[MAX_NETNAME], message[MAX_MESSAGE_SIZE],
		netname[MAX_NETNAME], *ptr;
	float	chat_reply;
	int	context, handle;
	...
	while((handle = trap_BotNextConsoleMessage(bs->cs, &m)) != 0){
		...
		if(trap_BotNumConsoleMessages(bs->cs) < 10)
			...
			if(m.type == CMS_CHAT && m.time > FloatTime() -
			   (1 + random())) break;
		/*  */
		ptr = m.message;
		...
		if(m.type == CMS_CHAT)
			...
			if(trap_BotFindMatch(m.message, &match,
				   MTCONTEXT_REPLYCHAT))
				ptr = m.message +
				      match.variables[MESSAGE].offset;
		...
		if(!BotMatchMessage(bs, m.message))
			...
			if(m.type == CMS_CHAT && !bot_nochat.integer){
				...
				if(!trap_BotFindMatch(m.message, &match,
					   MTCONTEXT_REPLYCHAT)){
					trap_BotRemoveConsoleMessage(bs->cs,
						handle);
					continue;
				}
				...
				if(bot_testrchat.integer){
					...
					if(trap_BotReplyChat(bs->cs, message,
						   context, CONTEXT_REPLY,
						   NULL, NULL,
						   NULL, NULL,
						   NULL, NULL,
						   botname, netname))
...
BotCheckForGrenades(bot_state_t *bs, entityState_t *state)
{
	...
	if(state->eType != ET_MISSILE || state->weapon != W1_GRENADE_LAUNCHER)
		return;
...
BotCheckForProxMines(bot_state_t *bs, entityState_t *state)
{
	...
	if(state->eType != ET_MISSILE || state->weapon != W1_PROX_LAUNCHER)
		return;
...
BotCheckEvents(bot_state_t *bs, entityState_t *state)
{
	...
	case EV_GLOBAL_TEAM_SOUND:
	{
		if(gametype == GT_CTF){
			...
			case GTS_BLUE_RETURN:
				...
				break;
			case GTS_RED_TAKEN:
				bs->neutralflagstatus = BotTeam(bs) ==
							TEAM_RED ? 2 : 1;	/* FIXME: check Team_TakeFlagSound in g_team.c */
				bs->flagstatuschanged = qtrue;
				break;
			case GTS_BLUE_TAKEN:
				bs->neutralflagstatus = BotTeam(bs) ==
							TEAM_BLUE ? 2 : 1;	/* FIXME: check Team_TakeFlagSound in g_team.c */
				bs->flagstatuschanged = qtrue;
				break;
			}
		}
	...
	{
		copyv3(state->origin, lastteleport_origin);
	...
	case EV_GENERAL_SOUND:
	{
		...
		if(state->number == bs->client){
			if(state->eventParm < 0 || state->eventParm >
			   MAX_SOUNDS){
				BotAI_Print(
					PRT_ERROR,
...
BotGetAlternateRouteGoal(bot_state_t *bs, int base)
{
	...
	copyv3(altroutegoals[rnd].origin, goal->origin);
	setv3(goal->mins, -8, -8, -8);
	setv3(goal->maxs, 8, 8, 8);
	goal->entitynum = 0;
	goal->iteminfo	= 0;
...
BotSetupAlternativeRouteGoals(void)
{
	...
	if(altroutegoals_setup)
		return;
	if(gametype == GT_CTF){
		if(trap_BotGetLevelItemGoal(-1, "Neutral Flag",
			   &ctf_neutralflag) < 0)
			BotAI_Print(PRT_WARNING,
				"No alt routes without Neutral Flag\n");
		...
		if(trap_BotGetLevelItemGoal(-1, "Neutral Obelisk",
			   &neutralobelisk) < 0)
			BotAI_Print(PRT_WARNING,
				"One Flag CTF without Neutral Obelisk\n");
		red_numaltroutegoals = trap_AAS_AlternativeRouteGoals(
			ctf_neutralflag.origin, ctf_neutralflag.areanum,
...
BotDeathmatchAI(bot_state_t *bs, float thinktime)
{
	...
	if(bs->setupcount > 0){
		...
		if(gender[0] == 'm') trap_BotSetChatGender(bs->cs,
				CHAT_GENDERMALE);
		else if(gender[0] == 'f') trap_BotSetChatGender(
...
BotSetEntityNumForGoalWithModel(bot_goal_t *goal, int eType, char *modelname)
{
	...
	for(i = 0; i < level.num_entities; i++, ent++){
		...
		if(ent->s.modelindex != modelindex)
			continue;
		subv3(goal->origin, ent->s.origin, dir);
		if(lensqrv3(dir) < Square(10)){
			goal->entitynum = i;
			return;
		}
	}
...
BotSetupDeathmatchAI(void)
{
	...
	if(gametype == GT_CTF){
		...
		if(trap_BotGetLevelItemGoal(-1, "Neutral Flag",
			   &ctf_neutralflag) < 0)
			BotAI_Print(PRT_WARNING,
				"One Flag CTF without Neutral Flag\n");
		if(trap_BotGetLevelItemGoal(-1, "Red Flag", &ctf_redflag) < 0)
			BotAI_Print(PRT_WARNING,
				"One Flag CTF without Red Flag\n");
		if(trap_BotGetLevelItemGoal(-1, "Blue Flag", &ctf_blueflag) < 0)
			BotAI_Print(PRT_WARNING,
				"One Flag CTF without Blue Flag\n");
	}

	max_bspmodelindex = 0;
	for(ent = trap_AAS_NextBSPEntity(0); ent;
	    ent = trap_AAS_NextBSPEntity(ent)){
		if(!trap_AAS_ValueForBSPEpairKey(ent, "model", model,
			   sizeof(model))) continue;
		if(model[0] == '*'){
