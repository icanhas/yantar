...
G_LoadArenasFromFile(char *filename)
{
	...
	g_numArenas += G_ParseInfos(buf, MAX_ARENAS - g_numArenas,
		&g_arenaInfos[g_numArenas]);
}
...
G_LoadArenas(void)
{
	...
	numdirs = trap_fsgetfilelist("scripts", ".arena", dirlist, 1024);
	dirptr	= dirlist;
...
G_GetArenaInfoByMap(const char *map)
{
	...
	for(n = 0; n < g_numArenas; n++)
		if(Q_stricmp(Info_ValueForKey(g_arenaInfos[n],
				   "map"), map) == 0)
...
G_BotConnect(int clientNum, qbool restart)
{
	...
	Q_strncpyz(settings.team,
		Info_ValueForKey(userinfo, "team"), sizeof(settings.team));
...
G_InitBots(qbool restart)
{
	...
	if(g_gametype.integer == GT_SINGLE_PLAYER){
		...
		Q_strncpyz(map,
			Info_ValueForKey(serverinfo, "mapname"), sizeof(map));
		...
		if(!restart)
			G_SpawnBots(Info_ValueForKey(arenainfo,
					"bots"), basedelay);
	}
}
