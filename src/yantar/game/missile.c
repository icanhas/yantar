...
G_RunMissile(gentity_t *ent)
{
	...
	else if(ent->s.parentweap == W2proxlauncher && ent->count)
	...
	if(ent->s.parentweap == W2proxlauncher && !ent->count){
...
fire_grenade(gentity_t *self, Vec3 start, Vec3 dir)
{
	...
	bolt->s.eType = ET_MISSILE;
	bolt->r.svFlags = SVF_USE_CURRENT_ORIGIN;
	bolt->s.parentweap = W2grenadelauncher;
...
fire_rocket(gentity_t *self, Vec3 start, Vec3 dir)
{
	...
	bolt->s.eType = ET_MISSILE;
	bolt->r.svFlags = SVF_USE_CURRENT_ORIGIN;
	bolt->s.parentweap = W2rocketlauncher;
	bolt->r.ownerNum = self->s.number;
	bolt->parent = self;
	bolt->damage = 100;
	bolt->splashDamage = 100;
...
fire_grapple(gentity_t *self, Vec3 start, Vec3 dir)
{
	...
	hook->s.parentweap = W1_GRAPPLING_HOOK;
...
fire_prox(gentity_t *self, Vec3 start, Vec3 dir)
{
	...
	bolt->s.eType = ET_MISSILE;
	bolt->r.svFlags = SVF_USE_CURRENT_ORIGIN;
	bolt->s.parentweap = W2proxlauncher;
	bolt->s.eFlags = 0;
	bolt->r.ownerNum = self->s.number;
	bolt->parent = self;
	bolt->damage = 0;
