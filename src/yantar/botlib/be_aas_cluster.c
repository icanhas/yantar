...
AAS_UpdatePortal(int areanum, int clusternum)
{
	...
	if(!portal->frontcluster)
		...
		Log_Write(
			"portal area %d is seperating more than two clusters\r\n",
			areanum);
		return qfalse;
	}
	...
	aasworld.portalindex[cluster->firstportal +
			     cluster->numportals] = portalnum;
	...
	if(!nofaceflood)
		for(i = 0; i < area->numfaces; i++){
			...
			if(face->frontarea == areanum){
				if(face->backarea) if(!AAS_FloodClusterAreas_r(
							      face->backarea,
							      clusternum))
						return qfalse;
			}else if(face->frontarea) if(!AAS_FloodClusterAreas_r(
							     face->frontarea,
							     clusternum))
					return qfalse;
		}
...
AAS_FindClusters(void)
{
	...
	for(i = 1; i < aasworld.numareas; i++){
		...
		if(!AAS_FloodClusterAreas_r(i, aasworld.numclusters))
			return qfalse;
		if(!AAS_FloodClusterAreasUsingReachabilities(aasworld.
			   numclusters))
			return qfalse;
		     ...
		     int curarea)
{
	...
	for(i = 0; i < area->numfaces; i++){
		...
		if(face->frontarea != areanums[curarea]) otherareanum =
				face->frontarea;
...
AAS_ConnectedAreas(int *areanums, int numareas)
{
	...
	Q_Memset(connectedareas, 0, sizeof(connectedareas));
					    ...
					    int curareanum)
{
	...
	for(i = 0; i < area->numfaces; i++){
		...
		otherpresencetype =
			aasworld.areasettings[otherareanum].presencetype;
...
AAS_CheckAreaForPossiblePortals(int areanum)
{
	...
	int	numareafrontfaces[MAX_PORTALAREAS],
		numareabackfaces[MAX_PORTALAREAS];
	...
	if(aasworld.areasettings[areanum].contents &
	   AREACONTENTS_CLUSTERPORTAL) return 0;
	/* it must be a grounded area */
	if(!(aasworld.areasettings[areanum].areaflags & AREA_GROUNDED)) return 0;
	Q_Memset(numareafrontfaces, 0, sizeof(numareafrontfaces));
	Q_Memset(numareabackfaces, 0, sizeof(numareabackfaces));
	...
	numareas = AAS_GetAdjacentAreasWithLessPresenceTypes_r(areanums, 0,
		areanum);
	for(i = 0; i < numareas; i++){
			...
			if(frontplanenum < 0 || faceplanenum == frontplanenum){
				...
				for(k = 0; k < numfrontareas; k++)
					if(frontareanums[k] ==
					   otherareanum) break;
				if(k ==
				   numfrontareas) frontareanums[numfrontareas++]
						=
							otherareanum;
				numareafrontfaces[i]++;
			}else if(backplanenum < 0 || faceplanenum ==
				 backplanenum){
				...
				for(k = 0; k < numbackareas; k++)
					if(backareanums[k] ==
					   otherareanum) break;
				if(k ==
				   numbackareas) backareanums[numbackareas++] =
						otherareanum;
				numareabackfaces[i]++;
			}else
				return 0;
...
AAS_FloodCluster_r(int areanum, int clusternum)
{
	...
	for(i = 0; i < aasworld.areasettings[areanum].numreachableareas; i++){
		...
		if(!otherareanum){
			continue;
			AAS_Error(
				"reachability %d has zero area\n",
				aasworld.areasettings[areanum].
				firstreachablearea + i);
		}
		/* if the area is a portal */
		if(aasworld.areasettings[otherareanum].contents &
		   AREACONTENTS_CLUSTERPORTAL) continue;
		...
		for(j = 0; j < aasworld.areasettings[i].numreachableareas; j++){
			areanum =
				aasworld.reachability[aasworld.areasettings[i].
						      firstreachablearea +
						      j].areanum;
			if(aasworld.reachability[aasworld.areasettings[i].
						 firstreachablearea +
						 j].traveltype ==
			   TRAVEL_TELEPORT){
	...
	for(i = 1; i < aasworld.numareas; i++){
		...
		for(j = 0; j < area->numfaces; j++){
			...
			for(k = 0; k < area->numfaces; k++){
				facenum =
					abs(aasworld.faceindex[area->firstface +
							       k]);
				face = &aasworld.faces[facenum];
...
AAS_AddTeleporterPortals(void)
{
	...
	Vec3	origin, destorigin, mins, maxs, end;
	Vec3	bbmins, bbmaxs;
	aas_area_t	*area;
	aas_face_t	*face;
	aas_Trace	trace;
		...
		if(classname && !strcmp(classname, "misc_teleporter")){
			...
			for(dest = entities; dest; dest = dest->next){
				classname = AAS_ValueForBSPEpairKey(dest,
					"classname");
				if(classname &&
				   !strcmp(classname, "misc_teleporter_dest")){
					targetname = AAS_ValueForBSPEpairKey(
						dest, "targetname");
					if(targetname &&
			...
			if(!AAS_VectorForBSPEpairKey(dest, "origin",
				   destorigin)){
				botimport.Print(
					PRT_ERROR,
			...
			for(link = areas; link; link = link->next_area){
				...
				for(j = 0; j < area->numfaces; j++){
					facenum =
						abs(aasworld.faceindex[area->
								       firstface
								       +
								       j]);
					face = &aasworld.faces[facenum];
					if(face->frontarea !=
					   link->areanum) otherareanum =
							face->frontarea;
					else otherareanum = face->backarea;
					if(!otherareanum) continue;
			...
			if(aasworld.areasettings[area2num].cluster)
				for(link = areas; link; link = link->next_area){
					if(!AAS_AreaGrounded(link->areanum))
						continue;
					/* add the teleporter portal mark */
					aasworld.areasettings[link->areanum].
					contents &=
						~(AREACONTENTS_CLUSTERPORTAL |
						  AREACONTENTS_TELEPORTAL);
				}
...
AAS_InitClustering(void)
{
	...
	AAS_CreateViewPortals();	/* create portals to for the bot view */
	...
	aasworld.savefile = qtrue;	/* the AAS file should be saved */
	...
	for(i = 1; i < aasworld.numclusters; i++)
		botimport.Print(PRT_MESSAGE,
			"cluster %d has %d reachability areas\n", i,
