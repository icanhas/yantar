...
typedef struct aas_tracestack_s {
	Vec3	start;		/* start point of the piece of line to trace */
	Vec3	end;		/* end point of the piece of line to trace */
 ...
 * =========================================================================== */
void
AAS_PresenceTypeBoundingBox(int presencetype, Vec3 mins, Vec3 maxs)
{
	int index;
	/* bounding box size for each presence type */
	Vec3	boxmins[3] = {{0, 0, 0}, {-15, -15, -24}, {-15, -15, -24}};
	Vec3	boxmaxs[3] = {{0, 0, 0}, { 15,  15,  32}, { 15,  15,   8}};
...
AAS_InitAASLinkHeap(void)
{
	...
	if(!aasworld.linkheap){
		...
		aasworld.linkheap = (aas_link_t*)GetHunkMemory(
			max_aaslinks * sizeof(aas_link_t));
	}
	...
	aasworld.linkheap[max_aaslinks-
			  1].prev_ent = &aasworld.linkheap[max_aaslinks-2];
 ...
 * =========================================================================== */
int
AAS_PointReachabilityAreaIndex(Vec3 origin)
{
...
AAS_AreaCluster(int areanum)
{
	if(areanum <= 0 || areanum >= aasworld.numareas){
		botimport.Print(PRT_ERROR,
			"AAS_AreaCluster: invalid area number\n");
		return 0;
	}
 ...
 * =========================================================================== */
int
AAS_PointPresenceType(Vec3 point)
{
	int areanum;
...
AAS_BoxOriginVec3distv3FromPlane(Vec3 normal, Vec3 mins, Vec3 maxs, int side)
{
	...
	if(side)
		...
		for(i = 0; i < 3; i++){
			...
			else if(normal[i] < -BBOX_NORMAL_EPSILON) v1[i] =
					mins[i];
			else v1[i] = 0;
		}
	else
		    ...
		    int passent)
{
	...
	Vec3	cur_start, cur_end, cur_mid, v1, v2;
...
/*				subv3(end, start, v1); */
				...
				if(passent >= 0)
					if(AAS_AreaEntityCollision(-nodenum,
						   tstack_p->start,
...
/*			subv3(end, start, v1); */
	 ...
	 * the node to test against */
		...
		/* go down the tree both at the front and back of the node */
			 ...
			 *  */
			cur_mid[0] = cur_start[0] +
				     (cur_end[0] - cur_start[0]) * frac;
			cur_mid[1] = cur_start[1] +
				     (cur_end[1] - cur_start[1]) * frac;
			cur_mid[2] = cur_start[2] +
				     (cur_end[2] - cur_start[2]) * frac;
 ...
 * =========================================================================== */
int
AAS_TraceAreas(Vec3 start, Vec3 end, int *areas, Vec3 *points,
	       int maxareas)
{
	int	side, nodenum, tmpplanenum;
	int	numareas;
	float	front, back, frac;
	Vec3	cur_start, cur_end, cur_mid;
	aas_tracestack_t tracestack[127];
	aas_tracestack_t *tstack_p;
	aas_node_t *aasnode;
	aas_plane_t *plane;
 ...
 * Returns:					-
 * Changes Globals:		-
 * void AAS_OrthogonalToVectors(Vec3 v1, Vec3 v2, Vec3 res) */
 ...
 * =========================================================================== */
qbool
AAS_InsideFace(aas_face_t *face, Vec3 pnormal, Vec3 point, float epsilon)
{
	int i, firstvertex, edgenum;
	Vec3	v0;
	Vec3	edgevec, pointvec, sepnormal;
	aas_edge_t *edge;
#ifdef AAS_SAMPLE_DEBUG
	int lastvertex = 0;
#endif	/* AAS_SAMPLE_DEBUG */
 ...
 * =========================================================================== */
qbool
AAS_PointInsideFace(int facenum, Vec3 point, float epsilon)
{
	int i, firstvertex, edgenum;
	Scalar	*v1, *v2;
	Vec3	edgevec, pointvec, sepnormal;
 ...
 * =========================================================================== */
aas_face_t *
AAS_AreaGroundFace(int areanum, Vec3 point)
{
	int i, facenum;
	Vec3	up = {0, 0, 1};
	Vec3	normal;
 ...
 * =========================================================================== */
void
AAS_FacePlane(int facenum, Vec3 normal, float *dist)
{
	aas_plane_t *plane;
 ...
 * =========================================================================== */
int
AAS_BoxOnPlaneSide2(Vec3 absmins, Vec3 absmaxs, aas_plane_t *p)
{
	int i, sides;
	float	dist1, dist2;
	Vec3	corners[2];
 ...
 * Returns:					-
 * Changes Globals:		-
 * int AAS_BoxOnPlaneSide(Vec3 absmins, Vec3 absmaxs, aas_plane_t *p) */
...
aas_link_t *
AAS_AASLinkEntity(Vec3 absmins, Vec3 absmaxs, int entnum)
{
	int side, nodenum;
 ...
 * =========================================================================== */
aas_link_t *
AAS_LinkEntityClientBBox(Vec3 absmins, Vec3 absmaxs, int entnum,
			 int presencetype)
{
	Vec3	mins, maxs;
 ...
 * =========================================================================== */
int
AAS_BBoxAreas(Vec3 absmins, Vec3 absmaxs, int *areas, int maxareas)
{
...
AAS_AreaInfo(int areanum, aas_areainfo_t *info)
{
	...
	if(areanum <= 0 || areanum >= aasworld.numareas){
		botimport.Print(PRT_ERROR,
			"AAS_AreaInfo: areanum %d out of range\n",
			areanum);
		return 0;
	}
