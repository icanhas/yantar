...
typedef struct bot_moveresult_s {
	...
	Vec3	ideal_viewangles;	/* ideal viewangles for the movement */
} bot_moveresult_t;

#define bot_moveresult_t_cleared(x) bot_moveresult_t (x) = \
...
int BotMoveInDirection(int movestate, Vec3 dir, float speed, int type);
/* reset avoid reachability */
void BotResetAvoidReach(int movestate);
/* resets the last avoid reachability */
void BotResetLastAvoidReach(int movestate);
/* returns a reachability area if the origin is in one */
int BotReachabilityArea(Vec3 origin, int client);
/* view target based on movement */
int BotMovementViewTarget(int movestate, bot_goal_t *goal, int travelflags,
			  float lookahead,
			  Vec3 target);
/* predict the position of a player based on movement towards a goal */
int BotPredictVisiblePosition(Vec3 origin, int areanum, bot_goal_t *goal,
			      int travelflags,
			      Vec3 target);
...
void BotAddAvoidSpot(int movestate, Vec3 origin, float radius, int type);
