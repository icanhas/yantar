...
AAS_InitAreaContentsTravelFlags(void)
{
	...
	aasworld.areacontentstravelflags = (int*)GetClearedMemory(
		aasworld.numareas * sizeof(int));
	for(i = 0; i < aasworld.numareas; i++)
...
AAS_CreateReversedReachability(void)
{
	...
	for(i = 1; i < aasworld.numareas; i++){
		...
		for(n = 0; n < settings->numreachableareas && n < 128; n++){
			/* reachability link */
			reach =
				&aasworld.reachability[settings->
						       firstreachablearea + n];
			revlink = (aas_reversedlink_t*)ptr;
			ptr += sizeof(aas_reversedlink_t);
			revlink->areanum	= i;
			revlink->linknum	= settings->firstreachablearea +
						  n;
			revlink->next =
				aasworld.reversedreachability[reach->areanum].
				first;
			aasworld.reversedreachability[reach->areanum].first =
				revlink;
...
ushort
AAS_AreaTravelTime(int areanum, Vec3 start, Vec3 end)
{
	int intdist;
	float	dist;
	Vec3	dir;
...
AAS_InitPortalMaxTravelTimes(void)
{
	...
	if(aasworld.portalmaxtraveltimes) FreeMemory(
			aasworld.portalmaxtraveltimes);

	aasworld.portalmaxtraveltimes = (int*)GetClearedMemory(
		aasworld.numportals * sizeof(int));
...
AAS_FreeOldestCache(void)
{
	...
	if(cache){
		...
		if(cache->type == CACHETYPE_AREA){
			...
			clusterareanum = AAS_ClusterAreaNum(cache->cluster,
				cache->areanum);
			/* unlink from cluster area cache */
			if(cache->prev) cache->prev->next = cache->next;
			else aasworld.clusterareacache[cache->cluster][
					clusterareanum] = cache->next;
...
AAS_CreateAllRoutingCache(void)
{
	...
	for(i = 1; i < aasworld.numareas; i++){
		...
		for(j = 1; j < aasworld.numareas; j++){
			...
			AAS_AreaTravelTimeToGoalArea(i, aasworld.areas[i].center,
				j,
				TFL_DEFAULT);
...
AAS_WriteRouteCache(void)
{
	...
	Q_sprintf(filename, MAX_QPATH, Pmaps "/%s.rcd", aasworld.mapname);
	...
	routecacheheader.areacrc = CRC_ProcessString(
		(unsigned char*)aasworld.areas, sizeof(aas_area_t) *
		aasworld.numareas);
	routecacheheader.clustercrc = CRC_ProcessString(
		(unsigned char*)aasworld.clusters, sizeof(aas_cluster_t) *
		aasworld.numclusters);
	routecacheheader.numportalcache = numportalcache;
	routecacheheader.numareacache	= numareacache;
	/* write the header */
	botimport.fswrite(&routecacheheader, sizeof(routecacheheader_t), fp);
	totalsize = 0;
	/* write all the cache */
	for(i = 0; i < aasworld.numareas; i++)
		for(cache = aasworld.portalcache[i]; cache; cache =
			    cache->next){
			botimport.fswrite(cache, cache->size, fp);
	 ...
	 *              size = 0;
	 *              botimport.fswrite(&size, sizeof(int), fp);
	 *              continue;
	 *      }
	 ...
	 *      botimport.fswrite(&size, sizeof(int), fp);
	 *      botimport.fswrite(aasworld.decompressedvis, size, fp);
	 * }
	 */
...
AAS_ReadCache(Fhandle fp)
{
	...
	botimport.fsread(&size, sizeof(size), fp);
	cache = (aas_routingcache_t*)GetMemory(size);
	cache->size = size;
	botimport.fsread((unsigned char*)cache + sizeof(size), size -
		sizeof(size), fp);
	cache->reachabilities = (unsigned char*)cache +
				sizeof(aas_routingcache_t) -
...
AAS_ReadRouteCache(void)
{
	...
	if(!fp)
		return qfalse;
	botimport.fsread(&routecacheheader, sizeof(routecacheheader_t), fp);
	 ...
	 *              botimport.fsread(aasworld.areavisibility[i], size, fp );
	 *      }
	 * }
	 */
	botimport.fsclose(fp);
...
AAS_InitReachabilityAreas(void)
{
	...
	aasworld.reachabilityareaindex = (int*)
					 GetClearedMemory(
		aasworld.reachabilitysize * MAX_REACHABILITYPASSAREAS *
		sizeof(int));
	numreachareas = 0;
	for(i = 0; i < aasworld.reachabilitysize; i++){
		...
		case TRAVEL_GRAPPLEHOOK:
			numareas =
				AAS_TraceAreas(reach->start, reach->end, areas,
					NULL,
					MAX_REACHABILITYPASSAREAS);
			break;
...
#endif	/* ROUTING_DEBUG */
	routingcachesize = 0;
	max_routingcachesize = 1024 * (int)LibVarValue("max_routingcache",
		"4096");
...
AAS_FreeRoutingCaches(void)
{
	...
	if(aasworld.reachabilityareaindex) FreeMemory(
...
/*	Q_Memset(aasworld.areaupdate, 0, aasworld.numareas * sizeof(aas_routingupdate_t)); */
	...
	clusterareanum = AAS_ClusterAreaNum(areacache->cluster,
		areacache->areanum);
	...
	while(updateliststart){
		...
		for(i = 0, revlink = revreach->first; revlink;
			...
			if(aasworld.areasettings[reach->areanum].areaflags &
			   AREA_DISABLED) continue;
			...
			if(!areacache->traveltimes[clusterareanum] ||
			   areacache->traveltimes[clusterareanum] > t){
				...
				if(!nextupdate->inlist){
					...
					if(updatelistend) updatelistend->next =
							nextupdate;
...
/*	Q_Memset(aasworld.portalupdate, 0, (aasworld.numportals+1) * sizeof(aas_routingupdate_t)); */
	...
	if(clusternum < 0)
		portalcache->traveltimes[-clusternum] =
			portalcache->starttraveltime;
	/* put the area to start with in the current read list */
	curupdate->next = NULL;
	curupdate->prev = NULL;
	updateliststart = curupdate;
	...
	while(updateliststart){
		...
		for(i = 0; i < cluster->numportals; i++){
			portalnum =
				aasworld.portalindex[cluster->firstportal + i];
			portal = &aasworld.portals[portalnum];
			/* if this is the portal of the current update continue */
			if(portal->areanum == curupdate->areanum) continue;
			clusterareanum = AAS_ClusterAreaNum(curupdate->cluster,
				portal->areanum);
			if(clusterareanum >=
			   cluster->numreachabilityareas) continue;
...
AAS_GetPortalRoutingCache(int clusternum, int areanum, int travelflags)
{
	...
	if(!cache){
		...
		if(aasworld.portalcache[areanum]) aasworld.portalcache[areanum]
			->prev = cache;
...
int
AAS_AreaRouteToGoalArea(int areanum, Vec3 origin, int goalareanum,
			int travelflags, int *traveltime,
			int *reachnum)
{
	...
	if(clusternum > 0 && goalclusternum > 0 && clusternum ==
	   goalclusternum){
		areacache = AAS_GetAreaRoutingCache(clusternum, goalareanum,
			travelflags);
	...
	portalcache = AAS_GetPortalRoutingCache(goalclusternum, goalareanum,
		travelflags);
...
int
AAS_AreaTravelTimeToGoalArea(int areanum, Vec3 origin, int goalareanum,
			     int travelflags)
{
...
int
AAS_AreaReachabilityToGoalArea(int areanum, Vec3 origin, int goalareanum,
			       int travelflags)
{
	int traveltime, reachnum;
 ...
 */
int
AAS_PredictRoute(struct aas_predictroute_s *route, int areanum, Vec3 origin,
		 ...
		 int stopevent, int stopcontents, int stoptfl, int stopareanum)
{
	...
	for(i = 0;
	    ...
	    (!maxareas || i < maxareas) && i < aasworld.numareas; i++){
		reachnum =
			AAS_AreaReachabilityToGoalArea(curareanum, curorigin,
				goalareanum,
				travelflags);
		if(!reachnum){
		...
		if(stopevent & RSE_USETRAVELTYPE){
			if(AAS_TravelFlagForType_inline(reach->traveltype) &
			   stoptfl){
				...
				route->endcontents	=
					aasworld.areasettings[curareanum].
					contents;
			...
			if(AAS_AreaContentsTravelFlags_inline(reach->areanum) &
			   stoptfl){
				...
				route->endcontents	=
					aasworld.areasettings[reach->areanum].
					contents;
				route->endtravelflags =
		...
		route->time += AAS_AreaTravelTime(areanum, origin,
			reach->start);
		route->time += reach->traveltime;
		route->endarea = reach->areanum;
		route->endcontents =
...
AAS_NextModelReachability(int num, int modelnum)
{
	...
	for(i = num; i < aasworld.reachabilitysize; i++){
		if((aasworld.reachability[i].traveltype & TRAVELTYPE_MASK) ==
		   TRAVEL_ELEVATOR){
			if(aasworld.reachability[i].facenum ==
			   modelnum) return i;
		}else if((aasworld.reachability[i].traveltype &
			  TRAVELTYPE_MASK) == TRAVEL_FUNCBOB)
			if((aasworld.reachability[i].facenum & 0x0000FFFF) ==
			   modelnum) return i;
	}
	return 0;
}
...
int
AAS_RandomGoalArea(int areanum, int travelflags, int *goalareanum,
		   Vec3 goalorigin)
{
	int i, n, t;
	Vec3 start, end;
	...
	for(i = 0; i < aasworld.numareas; i++){
		...
		if(AAS_AreaReachability(n)){
			t = AAS_AreaTravelTimeToGoalArea(
				areanum, aasworld.areas[areanum].center, n,
				travelflags);
...
int
AAS_NearestHideArea(int srcnum, Vec3 origin, int areanum, int enemynum,
		    Vec3 enemyorigin, int enemyareanum,
		    int travelflags)
{
	...
	*nextupdate;
	...
	while(updateliststart){
		...
		/* check all reversed reachability links */
		numreach =
			aasworld.areasettings[curupdate->areanum].
			numreachableareas;
		reach =
			&aasworld.reachability[aasworld.areasettings[curupdate->
								     areanum].
					       firstreachablearea];
		for(i = 0; i < numreach; i++, reach++){
			...
			AAS_ProjectPointOntoVector(enemyorigin, curupdate->start,
				...
				p);
			for(j = 0; j < 3; j++)
				if((p[j] > curupdate->start[j] && p[j] >
				    reach->end[j]) ||
				   (p[j] < curupdate->start[j] && p[j] <
				    reach->end[j]))
					break;
			if(j < 3)
