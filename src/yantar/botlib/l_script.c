...
PS_CreatePunctuationTable(script_t *script, punctuation_t *punctuations)
{
	...
	if(!script->punctuationtable) script->punctuationtable =
			(punctuation_t**)
			GetMemory(
				256 * sizeof(punctuation_t *));
	Q_Memset(script->punctuationtable, 0, 256 * sizeof(punctuation_t *));
...
PS_ReadEscapeCharacter(script_t *script, char *ch)
{
	...
	{
		if(*script->script_p < '0' || *script->script_p >
		   '9') ScriptError(script, "unknown escape char");
		for(i = 0, val = 0;; i++, script->script_p++){
			c = *script->script_p;
			if(c >= '0' && c <= '9') c = c - '0';
			else break;
	...
	while(1){
		...
		if(*script->script_p == '\\' &&
		   !(script->flags & SCFL_NOSTRINGESCAPECHARS)){
			if(!PS_ReadEscapeCharacter(script,
				   &token->string[len])){
				token->string[len] = 0;
				return 0;
			}
	...
	*floatvalue	= 0;
	...
	if(subtype & TT_FLOAT){
		...
		*floatvalue = *intvalue;
		...
		while(*string){
			*intvalue <<= 4;
			if(*string >= 'a' && *string <= 'f') *intvalue +=
					*string - 'a' + 10;
			else if(*string >= 'A' && *string <= 'F') *intvalue +=
					*string - 'A' + 10;
...
PS_ReadPrimitive(script_t *script, token_t *token)
{
	...
	Q_Memcpy(&script->token, token, sizeof(token_t));
...
PS_ReadToken(script_t *script, token_t *token)
{
	...
	if(script->tokenavailable){
		...
		Q_Memcpy(token, &script->token, sizeof(token_t));
		return 1;
	}
	/* save script pointer */
	...
	Q_Memset(token, 0, sizeof(token_t));
...
ResetScript(script_t *script)
{
	...
	Q_Memset(&script->token, 0, sizeof(token_t));
...
LoadScriptFile(const char *filename)
{
	...
	if(strlen(basefolder))
		Q_sprintf(pathname, sizeof(pathname), "%s/%s", basefolder,
			filename);
	else
	...
	Q_Memset(script, 0, sizeof(script_t));
...
#else
	Q_sprintf(basefolder, sizeof(basefolder), "%s", path);
#endif
