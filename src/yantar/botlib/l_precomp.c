...
PC_PushScript(source_t *source, script_t *script)
{
	...
	for(s = source->scriptstack; s; s = s->next)
		if(!Q_stricmp(s->filename, script->filename)){
			SourceError(source, "%s recursively included",
				script->filename);
			return;
		}
	/* end for
...
#else
		comerrorf(ERR_FATAL, "out of token space");
#endif
		return NULL;
	}
		   ...
		   int maxparms)
{
	...
	if(define->numparms > maxparms){
		SourceError(source, "define with more than %d parameters",
			maxparms);
		return qfalse;
	}
	...
	for(done = 0, numparms = 0, indent = 0; !done; ){
		...
		while(!done){
			if(!PC_ReadSourceToken(source, &token)){
				SourceError(source, "define %s incomplete",
					define->name);
				return qfalse;
			}
			if(!strcmp(token.string, ","))
				if(indent <= 0){
					if(lastcomma) SourceWarning(source,
							"too many commas");
					lastcomma = 1;
					break;
				}
...
PC_StringizeTokens(token_t *tokens, token_t *token)
{
	...
	for(t = tokens; t; t = t->next)
		strncat(token->string, t->string, MAX_TOKEN -
...
PC_MergeTokens(token_t *t1, token_t *t2)
{
	...
	if(t1->type == TT_NAME &&
	   (t2->type == TT_NAME || t2->type == TT_NUMBER)){
...
/* takes number of the parm, returns -1 if none found with the given name */
int
		...
		token_t **firsttoken, token_t **lasttoken)
{
	...
	if(define->builtin)
		return PC_ExpandBuiltinDefine(source, deftoken, define,
			firsttoken,
			lasttoken);
	/* if the define has parameters */
	if(define->numparms){
		if(!PC_ReadDefineParms(source, define, parms,
			   MAX_DEFINEPARMS)) return qfalse;
#ifdef DEBUG_EVAL
		...
		if(parmnum >= 0)
			...
			if(dt->string[0] == '#' && dt->string[1] == '\0'){
				...
				if(parmnum >= 0){
					...
					if(!PC_StringizeTokens(parms[parmnum],
						   &token)){
						SourceError(
							source,
							"can't stringize tokens");
						return qfalse;
					}
					t = PC_CopyToken(&token);
				}else{
...
PC_ExpandDefineIntoSource(source_t *source, token_t *deftoken, define_t *define)
{
	...
	if(!PC_ExpandDefine(source, deftoken, define, &firsttoken,
		   &lasttoken)) return qfalse;
	...
	for(lastdefine = NULL, define = source->definehash[hash]; define;
		...
		if(!strcmp(define->name, token.string)){
			if(define->flags & DEFINE_FIXED)
				SourceWarning(source, "can't undef %s",
					token.string);
			else{
				if(lastdefine) lastdefine->hashnext =
						define->hashnext;
...
PC_Directive_define(source_t *source)
{
	...
	do {
		...
		if(t->type == TT_NAME && !strcmp(t->string, define->name)){
			SourceError(source,
				"recursive define (removed recursion)");
			continue;
		}
		PC_ClearTokenWhiteSpace(t);
		t->next = NULL;
...
PC_CopyDefine(source_t *source, define_t *define)
{
	...
	for(lasttoken = NULL, token = define->tokens; token; token =
		    token->next){
	...
	for(lasttoken = NULL, token = define->parms; token; token =
		    token->next){
		newtoken = PC_CopyToken(token);
		newtoken->next = NULL;
		if(lasttoken) lasttoken->next = newtoken;
		  ...
		  float *floatvalue, int integer)
{
	...
	for(t = tokens; t; t = t->next){
		...
		case TT_NAME:
		{
			...
			if(!t || t->type != TT_NAME){
				SourceError(source,
					"defined without name in #if/#elif");
				error = 1;
				break;
			}	/* end if
		...
		case TT_PUNCTUATION:
		{
			if(negativevalue){
				SourceError(source,
					"misplaced minus sign in #if/#elif");
				error = 1;
				break;
			}
			...
			if(!integer)
				if(t->subtype == P_BIN_NOT || t->subtype ==
				   P_MOD ||
				   t->subtype == P_RSHIFT || t->subtype ==
				   P_LSHIFT ||
				   t->subtype == P_BIN_AND || t->subtype ==
				   P_BIN_OR ||
				   t->subtype == P_BIN_XOR){
					SourceError(
						source,
	...
	while(!error && firstoperator){
		...
		if(integer){
			Log_Write("operator %s, value1 = %d",
				PunctuationFromNum(source->scriptstack,
					o->operator), v1->intvalue);
			if(v2) Log_Write("value2 = %d", v2->intvalue);
		}else{
			Log_Write("operator %s, value1 = %f",
				PunctuationFromNum(source->scriptstack,
					o->operator), v1->floatvalue);
		...
		case P_BIN_NOT:                 v1->intvalue = ~v1->intvalue;
			break;
		case P_MUL:                             v1->intvalue *=
			v2->intvalue;
			v1->floatvalue *= v2->floatvalue; break;
		case P_DIV:                             if(!v2->intvalue ||
							   !v2->floatvalue){
				SourceError(source,
		...
		case P_ADD:                             v1->intvalue +=
			v2->intvalue;
			v1->floatvalue += v2->floatvalue; break;
		case P_SUB:                             v1->intvalue -=
			v2->intvalue;
			v1->floatvalue -= v2->floatvalue; break;
		case P_LOGIC_AND:               v1->intvalue = v1->intvalue &&
							       v2->intvalue;
			v1->floatvalue = v1->floatvalue && v2->floatvalue; break;
		case P_LOGIC_OR:                v1->intvalue = v1->intvalue ||
							       v2->intvalue;
			v1->floatvalue = v1->floatvalue || v2->floatvalue; break;
		case P_LOGIC_GEQ:               v1->intvalue = v1->intvalue >=
							       v2->intvalue;
			v1->floatvalue = v1->floatvalue >= v2->floatvalue; break;
		case P_LOGIC_LEQ:               v1->intvalue = v1->intvalue <=
							       v2->intvalue;
			v1->floatvalue = v1->floatvalue <= v2->floatvalue; break;
		case P_LOGIC_EQ:                v1->intvalue = v1->intvalue ==
							       v2->intvalue;
			v1->floatvalue = v1->floatvalue == v2->floatvalue; break;
		case P_LOGIC_UNEQ:              v1->intvalue = v1->intvalue !=
							       v2->intvalue;
			v1->floatvalue = v1->floatvalue != v2->floatvalue; break;
		case P_LOGIC_GREATER:   v1->intvalue = v1->intvalue >
						       v2->intvalue;
			v1->floatvalue = v1->floatvalue > v2->floatvalue; break;
		case P_LOGIC_LESS:              v1->intvalue = v1->intvalue <
							       v2->intvalue;
			v1->floatvalue = v1->floatvalue < v2->floatvalue; break;
		...
		case P_COLON:
		{
			...
			if(integer){
				if(!questmarkintvalue) v1->intvalue =
						v2->intvalue;
			}else if(!questmarkfloatvalue) v1->floatvalue =
					v2->floatvalue;
			gotquestmarkvalue = qfalse;
			break;
		}	/* end case */
	...
	do {
		...
		if(token.type == TT_NAME){
			if(defined){
...
#if DEFINEHASHING
				define = PC_FindHashedDefine(source->definehash,
					token.string);
#else
				define = PC_FindDefine(source->defines,
					token.string);
#endif	/* DEFINEHASHING */
				if(!define){
					SourceError(
				...
				if(!PC_ExpandDefineIntoSource(source, &token,
					   define)) return qfalse;
			}
		}
		/* if the token is a number or a punctuation */
		else if(token.type == TT_NUMBER || token.type ==
			TT_PUNCTUATION){
			t = PC_CopyToken(&token);
			t->next = NULL;
			if(lasttoken) lasttoken->next = t;
...
PC_ReadToken(source_t *source, token_t *token)
{
	...
	while(1){
		...
		if(token->type == TT_PUNCTUATION && *token->string == '$'){
			...
			{
				/* read the precompiler directive */
				if(!PC_ReadDollarDirective(source)) return
						qfalse;
				continue;
			}
		}
...
PC_ExpectTokenString(source_t *source, char *string)
{
	...
	if(strcmp(token.string, string)){
		SourceError(source, "expected %s, found %s", string,
			token.string);
		return qfalse;
	}
	return qtrue;
...
PC_ExpectTokenType(source_t *source, int type, int subtype, token_t *token)
{
	...
	if(token->type != type){
		...
		SourceError(source, "expected a %s, found %s", str,
			token->string);
		return qfalse;
	}
	if(token->type == TT_NUMBER){
		if((token->subtype & subtype) != subtype){
			...
			SourceError(source, "expected %s, found %s", str,
				token->string);
			return qfalse;
		}
	...
	for(i = 1; i < MAX_SOURCEFILES; i++){
		if(sourceFiles[i]){
			...
			botimport.Print(PRT_ERROR,
				"file %s still open in precompiler\n",
