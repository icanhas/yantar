...
AAS_FaceArea(aas_face_t *face)
{
	...
	for(i = 1; i < face->numedges - 1; i++){
		...
		side	= edgenum < 0;
		edge	= &aasworld.edges[abs(edgenum)];
		subv3(aasworld.vertexes[edge->v[side]], v, d1);
		subv3(aasworld.vertexes[edge->v[!side]], v, d2);
		crossv3(d1, d2, cross);
...
AAS_AreaVolume(int areanum)
{
	...
	edgenum = aasworld.edgeindex[face->firstedge];
	edge = &aasworld.edges[abs(edgenum)];
	copyv3(aasworld.vertexes[edge->v[0]], corner);
	...
	for(i = 0; i < area->numfaces; i++){
		...
		d	= -(dotv3 (corner, plane->normal) - plane->dist);
		a	= AAS_FaceArea(face);
		volume	+= d * a;
	}
 ...
 * =========================================================================== */
int
AAS_GetJumpPadInfo(int ent, Vec3 areastart, Vec3 absmins, Vec3 absmaxs,
		   Vec3 velocity)
{
	int modelnum, ent2;
	...
	addv3(origin, absmins, absmins);
	addv3(origin, absmaxs, absmaxs);
	addv3(absmins, absmaxs, origin);
	scalev3 (origin, 0.5, origin);

	/* get the start areas */
	copyv3(origin, teststart);
	teststart[2] += 64;
	trace = AAS_TraceClientBBox(teststart, origin, PRESENCE_CROUCH, -1);
	if(trace.startsolid){
		botimport.Print(PRT_MESSAGE, "trigger_push start solid\n");
		copyv3(origin, areastart);
	}else
		copyv3(trace.endpos, areastart);
	areastart[2] += 0.125;
	/*
	...
	subv3 (ent2origin, origin, velocity);
	dist = normv3(velocity);
	forward = dist / time;
	/* FIXME: why multiply by 1.1 */
	forward *= 1.1f;
	scalev3(velocity, forward, velocity);
 ...
 * =========================================================================== */
int
AAS_BestReachableFromJumpPadArea(Vec3 origin, Vec3 mins, Vec3 maxs)
{
	int ent, bot_visualizejumppads, bestareanum;
	float	volume, bestareavolume;
	Vec3	areastart, cmdmove, bboxmins, bboxmaxs;
	Vec3	absmins, absmaxs, velocity;
...
#endif
	addv3(origin, mins, bboxmins);
	addv3(origin, maxs, bboxmaxs);
	for(ent = AAS_NextBSPEntity(0); ent; ent = AAS_NextBSPEntity(ent)){
		if(!AAS_ValueForBSPEpairKey(ent, "classname", classname,
			   MAX_EPAIRKEY)) continue;
		if(strcmp(classname, "trigger_push")) continue;
		if(!AAS_GetJumpPadInfo(ent, areastart, absmins, absmaxs,
			   velocity)) continue;
 ...
 * =========================================================================== */
int
AAS_BestReachableArea(Vec3 origin, Vec3 mins, Vec3 maxs, Vec3 goalorigin)
{
	int areanum, i, j, k, l;
	aas_link_t *areas;
	...
	if(!aasworld.loaded){
		botimport.Print(PRT_ERROR,
			"AAS_BestReachableArea: aas not loaded\n");
		return 0;
	}
	...
	if(areanum){
		...
		if(!trace.startsolid){
			...
			copyv3(trace.endpos, goalorigin);
			...
			if(AAS_PointAreaNum(start)){
				Log_Write(
					"point %f %f %f in area %d but trace startsolid",
	...
	copyv3(origin, goalorigin);
	addv3(origin, mins, absmins);
	addv3(origin, maxs, absmaxs);
	/* add bounding box size
	 * subv3(absmins, bbmaxs, absmins);
	 * subv3(absmaxs, bbmins, absmaxs);
...
{
	Q_Memset(lreach, 0, sizeof(aas_lreachability_t));
 ...
 * =========================================================================== */
void
AAS_FaceCenter(int facenum, Vec3 center)
{
	int i;
	float scale;
	aas_face_t *face;
	...
	for(i = 0; i < face->numedges; i++){
		edge =
			&aasworld.edges[abs(aasworld.edgeindex[face->firstedge +
							       i])];
		addv3(center, aasworld.vertexes[edge->v[0]], center);
		addv3(center, aasworld.vertexes[edge->v[1]], center);
	}
...
}	/* end of the function AAS_FallDamageVec3distv3 */
/* ===========================================================================
...
AAS_MaxJumpHeight(float phys_jumpvel)
{
	...
	return 0.5 * phys_gravity *
	       (phys_jumpvel / phys_gravity) * (phys_jumpvel / phys_gravity);
 ...
 * =========================================================================== */
float
AAS_MaxJumpVec3distv3(float phys_jumpvel)
{
...
}	/* end of the function AAS_MaxJumpVec3distv3 */
/* ===========================================================================
 * returns true if a player can only crouch in the area
 *
 * Parameter:				-
 ...
 * =========================================================================== */
int
AAS_NearbySolidOrGap(Vec3 start, Vec3 end)
{
	Vec3	dir, testpoint;
	int	areanum;
	...
	if(areanum)
		if(!AAS_AreaSwim(areanum) &&
		   !AAS_AreaGrounded(areanum)) return qtrue;
	return qfalse;
...
AAS_Reachability_Swim(int area1num, int area2num)
{
	...
	if(!(aasworld.areasettings[area2num].presencetype &
...
AAS_Reachability_EqualFloorHeight(int area1num, int area2num)
{
	...
	Vec3	dir, start, end, normal, invgravity, gravitydirection =
	{0, 0, -1};
	Vec3	edgevec;
	aas_area_t *area1, *area2;
	...
	if(!AAS_AreaGrounded(area1num) ||
	   !AAS_AreaGrounded(area2num)) return qfalse;
	...
	copyv3(gravitydirection, invgravity);
	invv3(invgravity);
	bestheight = 99999;
	bestlength = 0;
	foundreach = qfalse;
	Q_Memset(&lr, 0, sizeof(aas_lreachability_t));	/* make the compiler happy */
	/*
	 * check if the areas have ground faces with a common edge
	 * if existing use the lowest common edge for a reachability link */
	for(i = 0; i < area1->numfaces; i++){
		face1 =
			&aasworld.faces[abs(aasworld.faceindex[area1->firstface
							       + i])];
		if(!(face1->faceflags & FACE_GROUND)) continue;
		for(j = 0; j < area2->numfaces; j++){
			face2 =
				&aasworld.faces[abs(aasworld.faceindex[area2->
								       firstface
								       +
								       j])];
			...
			for(edgenum1 = 0; edgenum1 < face1->numedges; edgenum1++)
				for(edgenum2 = 0; edgenum2 < face2->numedges;
				    edgenum2++){
					if(abs(aasworld.edgeindex[face1->
								  firstedge +
								  edgenum1]) !=
					   abs(aasworld.edgeindex[face2->
								  firstedge +
								  edgenum2]))
						continue;
					edgenum =
						aasworld.edgeindex[face1->
								   firstedge +
								   edgenum1];
					side = edgenum < 0;
					edge = &aasworld.edges[abs(edgenum)];
					...
					length = lenv3(dir);
					/* get the start point */
					addv3(aasworld.vertexes[edge->v[0]],
						aasworld.vertexes[edge->v[1]],
						start);
					scalev3(start, 0.5, start);
					copyv3(start, end);
	...
	if(foundreach){
		...
		copyv3(lr.start, lreach->start);
		copyv3(lr.end, lreach->end);
...
AAS_Reachability_Step_Barrier_WaterJump_WalkOffLedge(int area1num, int area2num)
{
	...
	float	length, ground_bestlength, water_bestlength, ground_bestdist,
		water_bestdist;
	Vec3	v1, v2, v3, v4, tmpv, p1area1, p1area2, p2area1, p2area2;
	Vec3	normal, ort, edgevec, start, end, dir;
	Vec3	ground_beststart =
	{0, 0,
	 0}, ground_bestend = {0, 0, 0}, ground_bestnormal = {0, 0, 0};
	Vec3	water_beststart		=
	{0, 0, 0}, water_bestend	= {0, 0, 0}, water_bestnormal = {0, 0, 0};
	Vec3	invgravity = {0, 0, 1};
	Vec3	testpoint;
	aas_plane_t *plane;
	aas_area_t *area1, *area2;
	...
	for(i = 0; i < area1->numfaces; i++){
		...
		for(k = 0; k < groundface1->numedges; k++){
			edge1num =
				aasworld.edgeindex[groundface1->firstedge + k];
			side1 = (edge1num < 0);
			...
			for(j = 0; j < area2->numfaces; j++){
				...
				for(l = 0; l < groundface2->numedges; l++){
					...
					diff = dotv3(normal, v3) - dist;
					if(diff < -0.1 || diff > 0.1) continue;
					diff = dotv3(normal, v4) - dist;
					if(diff < -0.1 || diff > 0.1) continue;
					/*
					...
					crossv3(invgravity, normal, ort);
					/* invgravitydot = dotv3(invgravity, invgravity); */
					ortdot = dotv3(ort, ort);
					/* projection into the step plane
					 * NOTE: since gravity is vertical this is just the z coordinate */
					y1 = v1[2];	/* dotv3(v1, invgravity) / invgravitydot; */
					y2 = v2[2];	/* dotv3(v2, invgravity) / invgravitydot; */
					y3 = v3[2];	/* dotv3(v3, invgravity) / invgravitydot; */
					y4 = v4[2];	/* dotv3(v4, invgravity) / invgravitydot; */
					x1 = dotv3(v1, ort) / ortdot;
					x2 = dotv3(v2, ort) / ortdot;
					x3 = dotv3(v3, ort) / ortdot;
					x4 = dotv3(v4, ort) / ortdot;
					if(x1 > x2){
						tmp = x1; x1 = x2; x2 = tmp;
						tmp = y1; y1 = y2; y2 = tmp;
						copyv3(v1, tmpv); copyv3(
							v2, v1); copyv3(tmpv,
							v2);
					}
					if(x3 > x4){
...
/*						Log_Write("lines no overlap: from area %d to %d\r\n", area1num, area2num); */
					...
					if(dist1 > dist2 - 1 && dist1 < dist2 +
					   1){
						dist = dist1;
						addv3(p1area1, p2area1,
							start);
						scalev3(start, 0.5, start);
						addv3(p1area2, p2area2, end);
						scalev3(end, 0.5, end);
					}else if(dist1 < dist2){
						dist = dist1;
						copyv3(p1area1, start);
						copyv3(p1area2, end);
					}	/* end else if */
					else{
						dist = dist2;
						copyv3(p2area1, start);
						copyv3(p2area2, end);
					}
					/* get the length of the overlapping part of the edges of the two areas */
					subv3(p2area2, p1area2, dir);
					length = lenv3(dir);
					if(groundface1->faceflags &
					   FACE_GROUND){
	...
	if(ground_foundreach)
		...
		if(ground_bestdist >= 0 && ground_bestdist <
			...
			if(!aassettings.rs_maxfallheight ||
			   ...
			   aassettings.rs_maxfallheight){
				...
				if(!trace.startsolid && trace.fraction >= 1.0){
					...
					trace.endpos[2] += 1;
					if(AAS_PointAreaNum(trace.endpos) ==
					   area2num){
						/* if not going through a cluster portal */
						numareas = AAS_TraceAreas(
							start, end, areas, NULL,
							ARRAY_LEN(areas));
						for(i = 0; i < numareas; i++)
							if(AAS_AreaClusterPortal(
								   areas[i]))
								break;
						if(i >= numareas){
 ...
 * =========================================================================== */
float
VectorVec3distv3(Vec3 v1, Vec3 v2)
{
	Vec3 dir;

	subv3(v2, v1, dir);
	return lenv3(dir);
}	/* end of the function VectorVec3distv3 */
/* ===========================================================================
 ...
 * =========================================================================== */
int
VectorBetweenVectors(Vec3 v, Vec3 v1, Vec3 v2)
{
	Vec3 dir1, dir2;

	subv3(v, v1, dir1);
	subv3(v, v2, dir2);
	return (dotv3(dir1, dir2) <= 0);
 ...
 * =========================================================================== */
void
VectorMiddle(Vec3 v1, Vec3 v2, Vec3 middle)
{
	addv3(v1, v2, middle);
	scalev3(middle, 0.5, middle);
 ...
 * =========================================================================== */
/*
 * float AAS_ClosestEdgePoints(Vec3 v1, Vec3 v2, Vec3 v3, Vec3 v4,
 *                                                      aas_plane_t *plane1, aas_plane_t *plane2,
 *                                                      Vec3 beststart, Vec3 bestend, float bestdist)
 * {
 *      Vec3 dir1, dir2, p1, p2, p3, p4;
 ...
 *              p1[0] = (dotv3(v1, dir2) - (a2 * dir2[0] + b2 * dir2[1])) / dir2[0];
 *              p1[1] = a2 * p1[0] + b2;
 *              //point on the edge vector of area2 closest to v2
 *              p2[0] = (dotv3(v2, dir2) - (a2 * dir2[0] + b2 * dir2[1])) / dir2[0];
 *              p2[1] = a2 * p2[0] + b2;
 *      }
 *      else
 *      {
 ...
 *              p3[0] = (dotv3(v3, dir1) - (a1 * dir1[0] + b1 * dir1[1])) / dir1[0];
 *              p3[1] = a1 * p3[0] + b1;
 *              //point on the edge vector of area1 closest to v4
 *              p4[0] = (dotv3(v4, dir1) - (a1 * dir1[0] + b1 * dir1[1])) / dir1[0];
 *              p4[1] = a1 * p4[0] + b1;
 *      }
 *      else
 *      {
 ...
 *      p1[2] = (plane2->dist - dotv3(plane2->normal, p1)) / plane2->normal[2];
 *      p2[2] = (plane2->dist - dotv3(plane2->normal, p2)) / plane2->normal[2];
 *      p3[2] = (plane1->dist - dotv3(plane1->normal, p3)) / plane1->normal[2];
 *      p4[2] = (plane1->dist - dotv3(plane1->normal, p4)) / plane1->normal[2];
 *      //
 *      founddist = qfalse;
 *      //
 *      if (VectorBetweenVectors(p1, v3, v4))
 *      {
 *              dist = VectorVec3distv3(v1, p1);
...
float
AAS_ClosestEdgePoints(Vec3 v1, Vec3 v2, Vec3 v3, Vec3 v4,
		      aas_plane_t *plane1, aas_plane_t *plane2,
		      Vec3 beststart1, Vec3 bestend1,
		      Vec3 beststart2, Vec3 bestend2, float bestdist)
{
	Vec3	dir1, dir2, p1, p2, p3, p4;
	...
	p1[2] =
		(plane2->dist -
		 dotv3(plane2->normal, p1)) / plane2->normal[2];
	p2[2] =
		(plane2->dist -
		 dotv3(plane2->normal, p2)) / plane2->normal[2];
	p3[2] =
		(plane1->dist -
		 dotv3(plane1->normal, p3)) / plane1->normal[2];
	p4[2] =
		(plane1->dist -
		 dotv3(plane1->normal, p4)) / plane1->normal[2];
	founddist = qfalse;
	if(VectorBetweenVectors(p1, v3, v4)){
		dist = VectorVec3distv3(v1, p1);
		if(dist > bestdist - 0.5 && dist < bestdist + 0.5){
			dist1	= VectorVec3distv3(beststart1, v1);
			dist2	= VectorVec3distv3(beststart2, v1);
			if(dist1 > dist2){
				if(dist1 >
				   VectorVec3distv3(beststart1,
					   beststart2)) copyv3(
						v1, beststart2);
			}else if(dist2 >
				 VectorVec3distv3(beststart1,
					 beststart2)) copyv3(
					v1, beststart1);
			dist1	= VectorVec3distv3(bestend1, p1);
			dist2	= VectorVec3distv3(bestend2, p1);
			if(dist1 > dist2){
				if(dist1 >
				   VectorVec3distv3(bestend1,
					   bestend2)) copyv3(
						p1, bestend2);
			}else if(dist2 >
				 VectorVec3distv3(bestend1,
			...
			bestdist = dist;
			copyv3(v1, beststart1);
			copyv3(v1, beststart2);
			copyv3(p1, bestend1);
			copyv3(p1, bestend2);
		}
		founddist = qtrue;
	...
	if(VectorBetweenVectors(p2, v3, v4)){
		dist = VectorVec3distv3(v2, p2);
		if(dist > bestdist - 0.5 && dist < bestdist + 0.5){
			dist1	= VectorVec3distv3(beststart1, v2);
			dist2	= VectorVec3distv3(beststart2, v2);
			if(dist1 > dist2){
				if(dist1 >
				   VectorVec3distv3(beststart1,
					   beststart2)) copyv3(
				 ...
				 VectorVec3distv3(beststart1,
					 beststart2)) copyv3(
					v2, beststart1);
			dist1	= VectorVec3distv3(bestend1, p2);
			dist2	= VectorVec3distv3(bestend2, p2);
			if(dist1 > dist2){
				if(dist1 >
				   VectorVec3distv3(bestend1,
					   bestend2)) copyv3(
			...
			bestdist = dist;
			copyv3(v2, beststart1);
			copyv3(v2, beststart2);
			copyv3(p2, bestend1);
			copyv3(p2, bestend2);
		}
		founddist = qtrue;
	...
	if(VectorBetweenVectors(p3, v1, v2)){
		dist = VectorVec3distv3(v3, p3);
		if(dist > bestdist - 0.5 && dist < bestdist + 0.5){
			dist1	= VectorVec3distv3(beststart1, p3);
			dist2	= VectorVec3distv3(beststart2, p3);
			if(dist1 > dist2){
				if(dist1 >
				   VectorVec3distv3(beststart1,
					   beststart2)) copyv3(
				 ...
				 VectorVec3distv3(beststart1,
					 beststart2)) copyv3(
					p3, beststart1);
			dist1	= VectorVec3distv3(bestend1, v3);
			dist2	= VectorVec3distv3(bestend2, v3);
			if(dist1 > dist2){
				if(dist1 >
				   VectorVec3distv3(bestend1,
					   bestend2)) copyv3(
			...
			bestdist = dist;
			copyv3(p3, beststart1);
			copyv3(p3, beststart2);
			copyv3(v3, bestend1);
			copyv3(v3, bestend2);
		}
		founddist = qtrue;
	...
	if(VectorBetweenVectors(p4, v1, v2)){
		dist = VectorVec3distv3(v4, p4);
		if(dist > bestdist - 0.5 && dist < bestdist + 0.5){
			dist1	= VectorVec3distv3(beststart1, p4);
			dist2	= VectorVec3distv3(beststart2, p4);
			if(dist1 > dist2){
				if(dist1 >
				   VectorVec3distv3(beststart1,
					   beststart2)) copyv3(
				 ...
				 VectorVec3distv3(beststart1,
					 beststart2)) copyv3(
					p4, beststart1);
			dist1	= VectorVec3distv3(bestend1, v4);
			dist2	= VectorVec3distv3(bestend2, v4);
			if(dist1 > dist2){
				if(dist1 >
				   VectorVec3distv3(bestend1,
					   bestend2)) copyv3(
			...
			bestdist = dist;
			copyv3(p4, beststart1);
			copyv3(p4, beststart2);
			copyv3(v4, bestend1);
			copyv3(v4, bestend2);
		}
		founddist = qtrue;
...
AAS_Reachability_Jump(int area1num, int area2num)
{
	...
	float phys_jumpvel, maxjumpdistance, maxjumpheight, height, bestdist,
	      speed;
	Scalar	*v1, *v2, *v3, *v4;
	Vec3	beststart, beststart2, bestend, bestend2;
	Vec3	teststart, testend, dir, velocity, cmdmove, up =
	{0, 0, 1}, sidewards;
	aas_area_t *area1, *area2;
	aas_face_t *face1, *face2;
	aas_edge_t	*edge1, *edge2;
	...
	maxjumpdistance = 2 * AAS_MaxJumpVec3distv3(phys_jumpvel);
	/* maximum height a player can jump with the given initial z velocity */
	maxjumpheight = AAS_MaxJumpHeight(phys_jumpvel);

	/* if the areas are not near anough in the x-y direction */
	for(i = 0; i < 2; i++){
		if(area1->mins[i] > area2->maxs[i] +
		   maxjumpdistance) return qfalse;
		if(area1->maxs[i] < area2->mins[i] -
		   maxjumpdistance) return qfalse;
	}
	...
	for(i = 0; i < area1->numfaces; i++){
		...
		for(j = 0; j < area2->numfaces; j++){
			...
			for(k = 0; k < face1->numedges; k++){
				edge1num =
					abs(aasworld.edgeindex[face1->firstedge
							       + k]);
				edge1 = &aasworld.edges[edge1num];
...
/*		Log_Write("shortest distance between %d and %d is %f\r\n", area1num, area2num, bestdist); */
		...
		if(bestdist <= 48 && fabs(beststart[2] - bestend[2]) < 8){
		...
		}else if(AAS_HorizontalVelocityForJump(0, beststart, bestend,
				 &speed)){
			...
			if(!AAS_HorizontalVelocityForJump(phys_jumpvel,
				   beststart, bestend, &speed))
				return qfalse;
			speed *= 1.05f;
		...
		for(i = 0; i < 3; i++){
			if(i == 1)
				addv3(testend, sidewards, testend);
			else if(i == 2)
				subv3(bestend, sidewards, testend);
			else
				copyv3(bestend, testend);
			subv3(testend, beststart, dir);
			dir[2] = 0;
			normv3(dir);
			scalev3(dir, speed, velocity);
			AAS_PredictClientMovement(&move, -1, beststart,
				PRESENCE_NORMAL, qtrue,
	 ...
	 * create a new reachability link */
		...
		lreach->facenum = 0;
		lreach->edgenum = 0;
		copyv3(beststart, lreach->start);
		copyv3(bestend, lreach->end);
...
AAS_Reachability_Ladder(int area1num, int area2num)
{
	...
	float	face1area, face2area, bestface1area = -9999, bestface2area =
		-9999;
	float	phys_jumpvel, maxjumpheight;
	Vec3	area1point, area2point, v1, v2, up = {0, 0, 1};
	Vec3	mid, lowestpoint = {0, 0}, start, end, sharededgevec, dir;
	aas_area_t *area1, *area2;
	...
	for(i = 0; i < area1->numfaces; i++){
		...
		for(j = 0; j < area2->numfaces; j++){
			...
			for(k = 0; k < face1->numedges; k++){
				edge1num =
					aasworld.edgeindex[face1->firstedge + k];
				for(l = 0; l < face2->numedges; l++){
					edge2num =
						aasworld.edgeindex[face2->
								   firstedge +
								   l];
					if(abs(edge1num) == abs(edge2num)){
						/* get the face with the largest area */
						face1area = AAS_FaceArea(
							face1);
						face2area = AAS_FaceArea(
							face2);
	...
	if(ladderface1 && ladderface2){
		...
		copyv3(aasworld.vertexes[sharededge->v[firstv]], v1);
		copyv3(aasworld.vertexes[sharededge->v[!firstv]], v2);
		addv3(v1, v2, area1point);
		scalev3(area1point, 0.5, area1point);
		copyv3(area1point, area2point);
		/*
		 * if the face plane in area 1 is pretty much vertical */
		plane1 =
			&aasworld.planes[ladderface1->planenum ^
					 (ladderface1num < 0)];
		plane2 =
			&aasworld.planes[ladderface2->planenum ^
					 (ladderface2num < 0)];
		/*
		 * get the points really into the areas */
		subv3(v2, v1, sharededgevec);
		...
		ladderface2vertical =
			abs(dotv3(plane2->normal, up)) < 0.1;
		...
		if(ladderface1vertical && ladderface2vertical
		   ...
		   && dotv3(plane1->normal, plane2->normal) > 0.7
			/* and the shared edge is not too vertical */
		   && abs(dotv3(sharededgevec, up)) < 0.7){
			/* create a new reachability link */
			lreach = AAS_AllocReachability();
			if(!lreach) return qfalse;
			lreach->areanum = area2num;
		...
		if(ladderface1vertical &&
		   (ladderface2->faceflags & FACE_GROUND)){
			/* create a new reachability link */
			lreach = AAS_AllocReachability();
			if(!lreach) return qfalse;
			lreach->areanum = area2num;
		...
		if(ladderface1vertical){
			...
			for(i = 0; i < ladderface1->numedges; i++){
				edge1num =
					abs(
						aasworld.edgeindex[ladderface1->
								   firstedge +
								   i]);
				edge1 = &aasworld.edges[edge1num];
				copyv3(aasworld.vertexes[edge1->v[0]], v1);
				copyv3(aasworld.vertexes[edge1->v[1]], v2);
				addv3(v1, v2, mid);
				scalev3(mid, 0.5, mid);
				if(mid[2] < lowestpoint[2]){
					copyv3(mid, lowestpoint);
			...
			if(trace.startsolid)
				Log_Write(
					"trace from area %d started in solid\r\n",
					area1num);
#endif	/* REACH_DEBUG
	 *  */
			trace.endpos[2] += 1;
			...
			for(i = 0; i < area2->numfaces; i++){
				face2num =
					aasworld.faceindex[area2->firstface + i];
				face2 = &aasworld.faces[abs(face2num)];
				if(face2->faceflags & FACE_LADDER){
					plane2 =
						&aasworld.planes[face2->planenum
						];
					if(abs(dotv3(plane2->normal,
							   up)) < 0.1) break;
				}
			}
			...
			if(i >= area2->numfaces && area2num != area1num &&
			   ...
			   !AAS_ReachabilityExists(area2num, area1num)){
				...
				if(start[2] - trace.endpos[2] < maxjumpheight){
					...
					copyv3(lowestpoint, lreach->start);
					copyv3(trace.endpos, lreach->end);
					lreach->traveltype = TRAVEL_LADDER;
					lreach->traveltime = 10;
					lreach->next =
						areareachability[area1num];
					...
					lreach->traveltime = 10;
					lreach->next =
						areareachability[area2num];
					areareachability[area2num] = lreach;
					reach_jump++;
					return qtrue;
#ifdef REACH_DEBUG
					Log_Write(
						"jump up to ladder reach between %d and %d\r\n",
						area2num, area1num);
#endif	/* REACH_DEBUG */
				}
#ifdef REACH_DEBUG
				else Log_Write(
						"jump too high between area %d and %d\r\n",
						area2num, area1num);
#endif	/* REACH_DEBUG */
			}
			   ...
			   (AREACONTENTS_SLIME
				...
				for(i = 20; i <= 120; i += 20){
					...
					trace = AAS_TraceClientBBox(
						start, end, PRESENCE_NORMAL, -1);
					if(trace.startsolid) break;
					trace.endpos[2] += 1;
					area2num = AAS_PointAreaNum(trace.endpos);
					if(area2num == area1num) continue;
					if(start[2] - trace.endpos[2] >
					   maxjumpheight) continue;
					if(aasworld.areasettings[area2num].
					   contents & (AREACONTENTS_SLIME
						       |
						       AREACONTENTS_LAVA))
						continue;
					/*
					...
					reach_jump++;
					Log_Write(
						"jump far to ladder reach between %d and %d\r\n",
						area2num, area1num);
					break;
				}
			/* end if* / */
...
AAS_Reachability_Teleport(void)
{
	...
	int	ent, dest;
	float	angle;
	Vec3	origin, destorigin, mins, maxs, end, angles;
	...
	for(ent = AAS_NextBSPEntity(0); ent; ent = AAS_NextBSPEntity(ent)){
		...
		if(!strcmp(classname, "trigger_multiple")){
			AAS_ValueForBSPEpairKey(ent, "model", model,
				MAX_EPAIRKEY);
/* #ifdef REACH_DEBUG */
			botimport.Print(PRT_MESSAGE,
...
/* #endif REACH_DEBUG */
			clearv3(angles);
			AAS_BSPModelMinsMaxsOrigin(atoi(
					model+1), angles, mins, maxs, origin);
			if(!AAS_ValueForBSPEpairKey(ent, "target", target,
				   MAX_EPAIRKEY)){
				botimport.Print(
					PRT_ERROR,
			...
			for(dest = AAS_NextBSPEntity(0); dest;
				...
				if(!AAS_ValueForBSPEpairKey(dest, "classname",
					   classname, MAX_EPAIRKEY)) continue;
				if(!strcmp(classname, "target_teleporter")){
					if(!AAS_ValueForBSPEpairKey(dest,
...
/* #endif REACH_DEBUG */
		...
		for(dest = AAS_NextBSPEntity(0); dest;
			...
			if(AAS_ValueForBSPEpairKey(dest, "targetname",
				   targetname, MAX_EPAIRKEY))
				if(!strcmp(targetname, target))
					break;
		if(!dest){
			botimport.Print(
		...
		if(!AAS_AreaTeleporter(area2num) && !AAS_AreaJumpPad(area2num)){
			 ...
			 * else*/
			{
				...
				if(angle){
					setv3(angles, 0, angle, 0);
					anglev3s(angles, velocity, NULL,
						NULL);
					scalev3(velocity, 400, velocity);
				}else
					clearv3(velocity);
				clearv3(cmdmove);
				AAS_PredictClientMovement(
					&move, -1, destorigin, PRESENCE_NORMAL,
					qfalse,
					velocity, cmdmove, 0, 30, 0.1f,
					SE_HITGROUND|SE_ENTERWATER|
					SE_ENTERSLIME|
					SE_ENTERLAVA|SE_HITGROUNDDAMAGE|
					SE_TOUCHJUMPPAD|SE_TOUCHTELEPORTER, 0,
					qfalse);	/* qtrue); */
		...
		if(!areas) botimport.Print(PRT_MESSAGE,
		...
		for(link = areas; link; link = link->next_area){
			...
			lreach->facenum = 0;
			lreach->edgenum = 0;
			copyv3(mid, lreach->start);
			copyv3(destorigin, lreach->end);
...
AAS_Reachability_Elevator(void)
{
	...
	int	ent;
	Vec3	mins, maxs, origin, angles = {0, 0, 0};
	Vec3	pos1, pos2, mids, platbottom, plattop;
	...
	for(ent = AAS_NextBSPEntity(0); ent; ent = AAS_NextBSPEntity(ent)){
		...
		if(!strcmp(classname, "func_plat")){
			...
			AAS_BSPModelMinsMaxsOrigin(modelnum, angles, mins, maxs,
				origin);
			...
			addv3(mins, maxs, mids);
			scalev3(mids, 0.5, mids);
			xvals[0]	= mins[0]; xvals[1] = mids[0];
			xvals[2]	= maxs[0]; xvals[3] = mids[0];
			yvals[0]	= mids[1]; yvals[1] = maxs[1];
			yvals[2]	= mids[1]; yvals[3] = mins[1];
			xvals[4]	= mins[0]; xvals[5] = maxs[0];
			xvals[6]	= maxs[0]; xvals[7] = mins[0];
			yvals[4]	= maxs[1]; yvals[5] = maxs[1];
			yvals[6]	= mins[1]; yvals[7] = mins[1];
			...
			for(i = 0; i < 9; i++){
				if(i < 8){	/* check at the sides of the plat */
					...
					copyv3(plattop, bottomorg);
					bottomorg[2] += 24;
					area1num = AAS_PointAreaNum(bottomorg);
					if(!area1num) continue;
					copyv3(platbottom, bottomorg);
					bottomorg[2] += 24;
				}
				...
				for(n = 0; n < 3; n++){
					...
					xvals_top[1] = mids[0];
					xvals_top[2] =
						maxs[0]; xvals_top[3] = mids[0];
					yvals_top[0] = mids[1];
					yvals_top[1] = maxs[1];
					yvals_top[2] =
						mids[1]; yvals_top[3] = mins[1];
					xvals_top[4] = mins[0];
					xvals_top[5] = maxs[0];
					xvals_top[6] =
						maxs[0]; xvals_top[7] = mins[0];
					yvals_top[4] = maxs[1];
					yvals_top[5] = maxs[1];
					yvals_top[6] =
						mins[1]; yvals_top[7] = mins[1];
					for(j = 0; j < 8; j++){
						toporg[0] = origin[0] +
							    xvals_top[j];
						toporg[1] = origin[1] +
						...
						copyv3(dir, lreach->start);
						copyv3(toporg, lreach->end);
						lreach->traveltype =
							TRAVEL_ELEVATOR;
						lreach->traveltype |=
 ...
 * =========================================================================== */
aas_lreachability_t *
AAS_FindFaceReachabilities(Vec3 *facepoints, int numpoints, aas_plane_t *plane,
			   int towardsface)
{
	int	i, j, k, l;
	...
	Vec3	beststart, beststart2, bestend, bestend2, tmp, hordir, testpoint;
	...
	for(i = 1; i < aasworld.numareas; i++){
		...
		for(j = 0; j < area->numfaces; j++){
			...
			for(k = 0; k < face->numedges; k++){
				edgenum =
					abs(aasworld.edgeindex[face->firstedge +
							       k]);
		...
		VectorMiddle(bestend, bestend2, bestend);
		if(!towardsface){
			copyv3(beststart, tmp);
			copyv3(bestend, beststart);
			copyv3(tmp, bestend);
		}
		subv3(bestend, beststart, hordir);
		hordir[2] = 0;
		hordist = lenv3(hordir);
		if(hordist > 2 *
		   AAS_MaxJumpVec3distv3(aassettings.phys_jumpvel)) continue;
		...
		if(hordist > 32)
			...
			if(!AAS_HorizontalVelocityForJump(0, beststart, bestend,
				   &speed)) continue;
		beststart[2] += 1;
		bestend[2] += 1;
		if(towardsface) copyv3(bestend, testpoint);
		else copyv3(beststart, testpoint);
		testpoint[2] = 0;
		testpoint[2] =
			(bestfaceplane->dist -
			 dotv3(bestfaceplane->normal,
...
AAS_Reachability_FuncBobbing(void)
{
	...
	char	classname[MAX_EPAIRKEY], model[MAX_EPAIRKEY];
	Vec3	origin, move_end, move_start, move_start_top, move_end_top;
	Vec3	mins, maxs, angles = {0, 0, 0};
	Vec3	start_edgeverts[4], end_edgeverts[4], mid;
	Vec3	org, start, end, dir, points[10];
	float	height;
	aas_plane_t start_plane, end_plane;
	aas_lreachability_t *startreach, *endreach, *nextstartreach,
	*nextendreach, *lreach;
	...
	for(ent = AAS_NextBSPEntity(0); ent; ent = AAS_NextBSPEntity(ent)){
		...
		addv3(mins, origin, mins);
		addv3(maxs, origin, maxs);
		addv3(mins, maxs, mid);
		scalev3(mid, 0.5, mid);
		copyv3(mid, origin);
		copyv3(origin, move_end);
		copyv3(origin, move_start);
...
#endif
		for(i = 0; i < 4; i++){
			copyv3(move_start, start_edgeverts[i]);
		...
		setv3(start_plane.normal, 0, 0, 1);
		for(i = 0; i < 4; i++){
			copyv3(move_end, end_edgeverts[i]);
		...
		setv3(end_plane.normal, 0, 0, 1);
#ifndef BSPC
#if 0
		for(i = 0; i < 4; i++){
			AAS_PermanentLine(start_edgeverts[i],
				start_edgeverts[(i+1)%4], 1);
			AAS_PermanentLine(end_edgeverts[i],
				end_edgeverts[(i+1)%4], 1);
		}
#endif
#endif
		copyv3(move_start, move_start_top);
		move_start_top[2] += maxs[2] - mid[2] + 24;	/* + bbox maxs z */
		copyv3(move_end, move_end_top);
		...
		for(i = 0; i < 2; i++){
			if(i == 0){
				...
				firstendreach = AAS_FindFaceReachabilities(
					end_edgeverts, 4, &end_plane, qfalse);
			}else{
				firststartreach = AAS_FindFaceReachabilities(
					end_edgeverts, 4, &end_plane, qtrue);
				firstendreach = AAS_FindFaceReachabilities(
			...
			for(startreach = firststartreach; startreach;
				...
				for(endreach = firstendreach; endreach;
					 ...
					 *  */
					Log_Write(
						"funcbob reach from area %d to %d\n",
						startreach->areanum,
						endreach->areanum);
					/*
					 *  */
					if(i == 0) copyv3(move_start_top,
							org);
					else copyv3(move_end_top, org);
					subv3(startreach->start, org,
						dir);
					dir[2] = 0;
					normv3(dir);
					...
					if(numareas > 1) copyv3(
							points[1],
							startreach->start);
					else copyv3(end, startreach->start);
					...
					copyv3(startreach->start,
						lreach->start);
					copyv3(endreach->end, lreach->end);
#ifndef BSPC
 ...
 *                              AAS_PermanentLine(lreach->start, lreach->end, 1); */
					...
					reach_funcbob++;
					lreach->next =
						areareachability[startreach->
								 areanum];
					areareachability[startreach->areanum] =
						lreach;
				}
			}
			for(startreach = firststartreach; startreach;
...
AAS_Reachability_JumpPad(void)
{
	...
	aas_area_t *area2;
	aas_lreachability_t *lreach;
	Vec3	areastart, facecenter, dir, cmdmove;
	...
	for(ent = AAS_NextBSPEntity(0); ent; ent = AAS_NextBSPEntity(ent)){
		 ...
		 * //		scalev3(velocity, speed, velocity);
		 * clearv3(angles);
		 * //get the mins, maxs and origin of the model
		 * AAS_ValueForBSPEpairKey(ent, "model", model, MAX_EPAIRKEY);
		 * if (model[0]) modelnum = atoi(model+1);
		...
		if(velocity[0] || velocity[1]){
			setv3(cmdmove, 0, 0, 0);
			/* copyv3(velocity, cmdmove);
			 * cmdmove[2] = 0; */
			Q_Memset(&move, 0, sizeof(aas_clientmove_t));
			area2num = 0;
			for(i = 0; i < 20; i++){
				AAS_PredictClientMovement(
					&move, -1, areastart, PRESENCE_NORMAL,
					qfalse,
					velocity, cmdmove, 0, 30, 0.1f,
					SE_HITGROUND|SE_ENTERWATER|
					SE_ENTERSLIME|
				...
				if(!link) break;
				copyv3(move.endpos, areastart);
				copyv3(move.velocity, velocity);
			}
			if(area2num && i < 20)
				for(link = areas; link; link = link->next_area){
					if(!AAS_AreaJumpPad(link->areanum))
					...
					lreach->edgenum = sqrt(
						...
						velocity[1] *
						velocity[1]);
					copyv3(areastart, lreach->start);
					copyv3(move.endpos, lreach->end);
...
AAS_Reachability_Grapple(int area1num, int area2num)
{
	...
	aas_area_t *area1, *area2;
	aas_lreachability_t *lreach;
	Vec3 areastart, facecenter, start, end, dir, down = {0, 0, -1};
	Scalar *v;
	...
	copyv3(aasworld.areas[area1num].center, start);
	/* if not a swim area */
	...
	for(i = 0; i < area2->numfaces; i++){
		...
		v =
			aasworld.vertexes[aasworld.edges[abs(aasworld.edgeindex[
									 face2->
									 firstedge
								 ])].v[0]];
		subv3(v, areastart, dir);
		/* if the face plane is facing away */
		if(dotv3(aasworld.planes[face2->planenum].normal,
			   dir) > 0) continue;
		/* get the center of the face */
		AAS_FaceCenter(face2num, facecenter);
		...
		if(lenv3(dir) > 24) continue;
		copyv3(trace.endpos, start);
		copyv3(trace.endpos, end);
		end[2]	-= AAS_FallDamageVec3distv3();
		trace	= AAS_TraceClientBBox(start, end, PRESENCE_NORMAL, -1);
		if(trace.fraction >= 1) continue;
		/* area to end in */
		areanum = AAS_PointAreaNum(trace.endpos);
		...
		numareas =
			AAS_TraceAreas(areastart, bsptrace.endpos, areas, NULL,
				20);
		if(numareas >= 20) continue;
		for(j = 0; j < numareas; j++)
			if(aasworld.areasettings[areas[j]].contents &
			   AREACONTENTS_CLUSTERPORTAL) break;
		...
		lreach->edgenum = 0;
		copyv3(areastart, lreach->start);
		/* copyv3(facecenter, lreach->end); */
		copyv3(bsptrace.endpos, lreach->end);
		lreach->traveltype = TRAVEL_GRAPPLEHOOK;
		subv3(lreach->end, lreach->start, dir);
		lreach->traveltime = aassettings.rs_startgrapple + lenv3(
			dir) * 0.25;
		lreach->next = areareachability[area1num];
		areareachability[area1num] = lreach;
		reach_grapple++;
...
{
	int ent, i;
	Vec3	mins = {-15, -15, -15}, maxs = {15, 15, 15};
	Vec3	origin;
	...
	for(ent = AAS_NextBSPEntity(0); ent; ent = AAS_NextBSPEntity(ent)){
		...
		if(
			...
			if(AAS_VectorForBSPEpairKey(ent, "origin", origin)){
				...
				areanum =
					AAS_BestReachableArea(origin, mins, maxs,
						origin);
 ...
 * } */
	...
	if(!(aasworld.areasettings[area2num].areaflags &
	     AREA_WEAPONJUMP)) return qfalse;
	area1	= &aasworld.areas[area1num];
	area2	= &aasworld.areas[area2num];
	...
	for(i = 0; i < area2->numfaces; i++){
		...
		for(n = 0; n < 1 /*2*/; n++){
			 ...
			 * speed (the jump is not possible) then there's no jump reachability created */
			ret =
				AAS_HorizontalVelocityForJump(zvel, areastart,
					facecenter,
					&speed);
			if(ret && speed < 300){
				/* direction towards the face center */
...
AAS_Reachability_WalkOffLedge(int areanum)
{
	...
	for(i = 0; i < area->numfaces; i++){
		...
		for(k = 0; k < face1->numedges; k++){
			...
			for(j = 0; j < area->numfaces; j++){
				face2num =
					aasworld.faceindex[area->firstface + j];
				face2 = &aasworld.faces[abs(face2num)];
				...
				for(l = 0; l < face2->numedges; l++){
					...
					if(abs(edge1num) == abs(edge2num)){
						...
						plane =
							&aasworld.planes[face1->
									 planenum
							];
						/* get the points really into the areas */
						subv3(v2, v1,
							sharededgevec);
						crossv3(plane->normal,
							sharededgevec,
							dir);
						normv3(dir);
						addv3(v1, v2, mid);
...
AAS_StoreReachability(void)
{
	...
	aasworld.reachability =
		(aas_reachability_t*)GetClearedMemory(
	...
	for(i = 0; i < aasworld.numareas; i++){
		...
		for(lreach = areareachability[i]; lreach; lreach =
			    lreach->next){
			reach =
				&aasworld.reachability[areasettings->
						       firstreachablearea +
						       areasettings->
						       numreachableareas];
			...
			copyv3(lreach->start, reach->start);
			copyv3(lreach->end, reach->end);
...
AAS_ContinueInitReachability(float time)
{
	...
	/* if this is the last step in the reachability calculations */
		...
		lastpercentage = aasworld.numreachabilityareas * 1000 /
				 aasworld.numareas;
		botimport.Print(PRT_MESSAGE, "\r%6.1f%%",
			(float)lastpercentage / 10);
	}
	/* not yet finished */
	return qtrue;
