...
{
	Q_Memcpy(&buffer[ bufIndex ], s, strlen(s));
	bufIndex += strlen(s);
}
...
CL_OpenAVIForWriting(const char *fileName)
{
	...
	Q_Memset(&afd, 0, sizeof(aviFileData_t));
  ...
  #define MAX_PACK_LEN 16
	...
	afd.eBuffer = zalloc(PAD(afd.width * 3, AVI_LINE_PADDING) * afd.height);
	...
	Safefswrite(buffer, bufIndex, afd.f);
	afd.fileSize = bufIndex;

	bufIndex = 0;
	START_CHUNK("idx1");
	Safefswrite(buffer, bufIndex, afd.idxF);
...
CL_WriteAVIVideoFrame(const byte *imageBuffer, int size)
{
	...
	Safefswrite(buffer, 8, afd.f);
	Safefswrite(imageBuffer, size, afd.f);
	Safefswrite(padding, paddingSize, afd.f);
	...
	Safefswrite(buffer, 16, afd.idxF);
...
CL_WriteAVIAudioFrame(const byte *pcmBuffer, int size)
{
	...
	Q_Memcpy(&pcmCaptureBuffer[ bytesInBuffer ], pcmBuffer, size);
	bytesInBuffer += size;
	...
	if(bytesInBuffer >=
	   ...
	   afd.a.sampleSize){
		...
		Safefswrite(buffer, 8, afd.f);
		Safefswrite(pcmCaptureBuffer, bytesInBuffer, afd.f);
		Safefswrite(padding, paddingSize, afd.f);
		afd.fileSize += (chunkSize + paddingSize);
...
CL_CloseAVI(void)
{
	...
	if((indexSize = fsopenr(idxFileName,
		    &afd.idxF, qtrue)) <= 0){
		fsclose(afd.f);
		return qfalse;
	}
	...
	while(indexRemainder > MAX_AVI_BUFFER){
		fsread(buffer, MAX_AVI_BUFFER, afd.idxF);
		Safefswrite(buffer, MAX_AVI_BUFFER, afd.f);
		afd.fileSize += MAX_AVI_BUFFER;
		indexRemainder -= MAX_AVI_BUFFER;
	}
	fsread(buffer, indexRemainder, afd.idxF);
	Safefswrite(buffer, indexRemainder, afd.f);
	afd.fileSize += indexRemainder;
	fsclose(afd.idxF);

	/* Remove temp index file */
	fshomeremove(idxFileName);

	/* Write the real header */
	fsseek(afd.f, 0, FS_SEEK_SET);
	CL_WriteAVIHeader( );
