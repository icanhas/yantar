...
CL_GetUserCmd(int cmdNumber, Usrcmd *ucmd)
{
	...
	if(cmdNumber > cl.cmdNumber)
		comerrorf(ERR_DROP, "CL_GetUserCmd: %i >= %i", cmdNumber,
			cl.cmdNumber);
...
CL_GetParseEntityState(int parseEntityNumber, Entstate *state)
{
	...
	if(parseEntityNumber >= cl.parseEntitiesNum)
		comerrorf(ERR_DROP, "CL_GetParseEntityState: %i >= %i",
...
CL_GetSnapshot(int snapshotNumber, Snap *snapshot)
{
	...
	Q_Memcpy(snapshot->areamask, clSnap->areamask,
		sizeof(snapshot->areamask));
	snapshot->ps = clSnap->ps;
	count = clSnap->numEntities;
	if(count > MAX_ENTITIES_IN_SNAPSHOT){
		comdprintf("CL_GetSnapshot: truncated %i entities to %i\n",
			count,
	...
	for(i = 0; i < count; i++)
		snapshot->entities[i] =
			cl.parseEntities[ (clSnap->parseEntitiesNum +
					   i) & (MAX_PARSE_ENTITIES-1) ];
...
CL_ConfigstringModified(void)
{
	...
	if(index < 0 || index >= MAX_CONFIGSTRINGS)
		comerrorf(ERR_DROP, "configstring > MAX_CONFIGSTRINGS");
	...
	for(i = 0; i < MAX_CONFIGSTRINGS; i++){
		...
		if(len + 1 + cl.gameState.dataCount > MAX_GAMESTATE_CHARS)
			comerrorf(ERR_DROP, "MAX_GAMESTATE_CHARS exceeded");

		/* append it to the gameState string buffer */
		cl.gameState.stringOffsets[ i ] = cl.gameState.dataCount;
		Q_Memcpy(cl.gameState.stringData + cl.gameState.dataCount, dup,
			len + 1);
...
CL_GetServerCommand(int serverCommandNumber)
{
	...
	if(serverCommandNumber <= clc.serverCommandSequence -
	   MAX_RELIABLE_COMMANDS){
		...
		if(clc.demoplaying)
			return qfalse;
		comerrorf(ERR_DROP, "CL_GetServerCommand: a reliable command was cycled out");
		return qfalse;
	}

	if(serverCommandNumber > clc.serverCommandSequence){
		comerrorf(ERR_DROP, "CL_GetServerCommand: requested a command not received");
		return qfalse;
	}
...
Rescan:
	...
	if(!strcmp(cmd, "disconnect")){
		...
		if(argc >= 2)
			comerrorf(ERR_SERVERDISCONNECT,
				"Server disconnected - %s", cmdargv(1));
		else
			comerrorf(ERR_SERVERDISCONNECT, "Server disconnected");
	}
	...
	if(!strcmp(cmd, "bcs1")){
		...
		if(strlen(bigConfigString) + strlen(s) >= BIG_INFO_STRING)
			comerrorf(ERR_DROP, "bcs exceeded BIG_INFO_STRING");
...
/*      comflushevents();	// FIXME: if a server restarts here, BAD THINGS HAPPEN!
 * We can't call comflushevents here, a restart will crash and this _does_ happen
 ...
 * ZOID */
	...
	case CG_CM_TRANSFORMEDPOINTCONTENTS:
		return CM_TransformedPointContents(VMA(1), args[2], VMA(3),
			VMA(4));
	case CG_CM_BOXTRACE:
		CM_BoxTrace(VMA(1), VMA(2), VMA(3), VMA(4), VMA(
	...
	case CG_CM_TRANSFORMEDBOXTRACE:
		CM_TransformedBoxTrace(VMA(1), VMA(2), VMA(3), VMA(4), VMA(
				5), args[6], args[7], VMA(8), VMA(
				9), /*int capsule*/ qfalse);
		return 0;
	...
	case CG_R_MODELBOUNDS:
		...
		return 0;
	case CG_R_LERPTAG:
		return re.LerpTag(VMA(1), args[2], args[3], args[4], VMF(5),
			VMA(6));
	case CG_GETGLCONFIG:
	...
	default:
		assert(0);
		comerrorf(ERR_DROP, "Bad cgame system trap: %ld",
			(long int)args[0]);
	}
	return 0;
}
...
clinitCGame(void)
{
	...
	if(!cgvm)
		comerrorf(ERR_DROP, "vmcreate on cgame failed");
...
CL_AdjustTimeDelta(void)
{
	...
	if(cl_showTimeDelta->integer)
		comprintf("%i ", cl.serverTimeDelta);
}
...
CL_FirstSnapshot(void)
{
	...
	if((cl_useMumble->integer) && !mumble_islinked()){
		...
		comprintf("Mumble: Linking to Mumble application %s\n",
			ret==0 ? "ok" : "failed");
	}
#endif
...
CL_SetCGameTime(void)
{
	...
	if(!cl.snap.valid)
		comerrorf(ERR_DROP, "CL_SetCGameTime: !cl.snap.valid");
	...
	if(cl.snap.serverTime < cl.oldFrameServerTime)
		comerrorf(ERR_DROP, "cl.snap.serverTime < cl.oldFrameServerTime");
