...
DrawSkySide(struct image_s *image, const int mins[2], const int maxs[2])
{
	...
	for(t = 0; t < maxs[1] - mins[1]; t++)
		for(s = 0; s < maxs[0] - mins[0]; s++){
			...
			tess.indexes[tess.numIndexes++] =  s +
							  (t + 1) * (maxs[0] - mins[0] + 1) + firstVertex;
 ...
 */
	...
	qglDrawElements(GL_TRIANGLES, tess.numIndexes - tess.firstIndex, GL_INDEX_TYPE,
		BUFFER_OFFSET(tess.firstIndex * sizeof(GL_INDEX_TYPE)));
...
DrawSkyBox(material_t *shader)
{
	...
	for(i=0; i<6; i++){
		 ...
		 *  */
		for(t = sky_mins_subd[1]+HALF_SKY_SUBDIVISIONS; t <= sky_maxs_subd[1]+HALF_SKY_SUBDIVISIONS;
		    t++)
...
RB_DrawSun(void)
{
	 ...
	 * qglTranslatef (backEnd.viewParms.or.origin[0], backEnd.viewParms.or.origin[1], backEnd.viewParms.or.origin[2]); */
	{
		...
		mat44translation(backEnd.viewParms.or.origin, trans);
		mat44mul(backEnd.viewParms.world.modelMatrix, trans, product);
...
RB_StageIteratorSky(void)
{
	...
	if(tess.shader->sky.outerbox[0] && tess.shader->sky.outerbox[0] != tr.defaultImage){
...

		{
			...
			mat44copy(glState.modelview, oldmodelview);
			mat44translation(backEnd.viewParms.or.origin, trans);
			mat44mul(glState.modelview, trans, product);
			GL_SetModelviewMatrix(product);
