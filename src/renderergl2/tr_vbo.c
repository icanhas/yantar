...
R_InitVBOs(void)
{
	...
	tess.vbo->ofs_xyz = 0;
	tess.vbo->ofs_normal = tess.vbo->ofs_xyz         + sizeof(tess.xyz[0])              *
			       SHADER_MAX_VERTEXES;
	tess.vbo->ofs_tangent = tess.vbo->ofs_normal      + sizeof(tess.normal[0])           *
				SHADER_MAX_VERTEXES;
	tess.vbo->ofs_bitangent = tess.vbo->ofs_tangent     + sizeof(tess.tangent[0])          *
				  SHADER_MAX_VERTEXES;
	...
	tess.vbo->ofs_vertexcolor = tess.vbo->ofs_st          + sizeof(tess.texCoords[0][0]) * 2 *
				    SHADER_MAX_VERTEXES;
	tess.vbo->ofs_lightdir = tess.vbo->ofs_vertexcolor + sizeof(tess.vertexColors[0])     *
				 SHADER_MAX_VERTEXES;
	...
	if(tess.numVertexes > 0 && tess.numVertexes <= SHADER_MAX_VERTEXES){
		...
		if(attribBits & ATTR_BITS){
			if(attribBits & ATTR_POSITION){
				...
				qglBufferSubDataARB(GL_ARRAY_BUFFER_ARB, tess.vbo->ofs_xyz,
			...
			if(attribBits & ATTR_TEXCOORD || attribBits & ATTR_LIGHTCOORD){
				...
				qglBufferSubDataARB(GL_ARRAY_BUFFER_ARB, tess.vbo->ofs_st,
			...
			if(attribBits & ATTR_NORMAL){
				...
				qglBufferSubDataARB(GL_ARRAY_BUFFER_ARB, tess.vbo->ofs_normal,
			...
			if(attribBits & ATTR_TANGENT){
				...
				qglBufferSubDataARB(GL_ARRAY_BUFFER_ARB, tess.vbo->ofs_tangent,
			...
			if(attribBits & ATTR_BITANGENT){
				...
				qglBufferSubDataARB(GL_ARRAY_BUFFER_ARB, tess.vbo->ofs_bitangent,
			...
			if(attribBits & ATTR_COLOR){
				...
				qglBufferSubDataARB(GL_ARRAY_BUFFER_ARB, tess.vbo->ofs_vertexcolor,
			...
			if(attribBits & ATTR_LIGHTDIRECTION){
				...
				qglBufferSubDataARB(GL_ARRAY_BUFFER_ARB, tess.vbo->ofs_lightdir,
