			...
			if(!Q_stricmp(token, "$whiteimage")){
				...
				if(stage->type == ST_DIFFUSEMAP){
					...
					if(r_normalMapping->integer){
						...
						stage->bundle[TB_NORMALMAP].image[0] = R_FindImageFile2(
							filename, flags | IMGFLAG_SWIZZLE |
							IMGFLAG_NORMALIZED |
					...
					if(r_specularMapping->integer){
						...
						stage->bundle[TB_SPECULARMAP].image[0] = R_FindImageFile2(
							filename, flags);
...
CollapseStagesToLightall(shaderStage_t *diffuse,
			 ...
			 qboolean useLightVector, qboolean useLightVertex, qboolean parallax,
...
qhandle_t
RE_RegisterShaderFromImage(const char *name, int lightmapIndex, image_t *image, qboolean mipRawImage)
{
	int i, hash;
	shader_t *sh;
