...
R_CullSurface(msurface_t *surf)
{
	...
	if(surf->cullinfo.type & CULLINFO_PLANE){
		...
		d = dotv3 (tr.or.viewOrigin, surf->cullinfo.plane.normal);
		...
		for(i = 0; i < tr.refdef.num_dlights; i++){
			...
			if(dl->origin[0] - dl->radius > surf->cullinfo.bounds[1][0]
			   || dl->origin[0] + dl->radius < surf->cullinfo.bounds[0][0]
							   || dl->origin[1] -
							   dl->radius > surf->cullinfo.bounds[1][1]
			   || dl->origin[1] + dl->radius < surf->cullinfo.bounds[0][1]
							   || dl->origin[2] -
							   dl->radius > surf->cullinfo.bounds[1][2]
 ...
 */
static int
R_PshadowSurface(msurface_t *surf, int pshadowBits)
{
	float	d;
	int	i;
	pshadow_t *ps;
		...
		for(i = 0; i < tr.refdef.num_pshadows; i++){
			...
			if(ps->lightOrigin[0] - ps->lightRadius > surf->cullinfo.bounds[1][0]
			   || ps->lightOrigin[0] + ps->lightRadius < surf->cullinfo.bounds[0][0]
								     || ps->lightOrigin[1] -
								     ps->lightRadius > surf->cullinfo.bounds[
				   1][1]
	...
	if(surf->cullinfo.type & CULLINFO_SPHERE){
		for(i = 0; i < tr.refdef.num_pshadows; i++){
			...
			if(!SpheresIntersect(ps->viewOrigin, ps->viewRadius, surf->cullinfo.localOrigin,
				   surf->cullinfo.radius)
			   || dotv3(surf->cullinfo.localOrigin,
 ...
 */
static void
R_AddWorldSurface(msurface_t *surf, int dlightBits, int pshadowBits)
{
	/* FIXME: bmodel fog? */
 ...
 */
static void
R_RecursiveWorldNode(mnode_t *node, int planeBits, int dlightBits, int pshadowBits)
{
...
R_PointInLeaf(const Vec3 p)
{
	...
	while(1){
		...
		plane = node->plane;
		d = dotv3 (p,plane->normal) - plane->dist;
		if(d > 0){
