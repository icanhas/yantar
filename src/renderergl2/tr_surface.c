...
void
RB_InstantQuad(vec4_t quadVerts[4])
{
	vec4_t	color;
	vec2_t	texCoords[4];
	vec2_t	invTexRes;
...
static void
DoRailCore(const vec3_t start, const vec3_t end, const vec3_t up, float len, float spanWidth)
{
	float	spanWidth2;
	int	vbase;
	float	t = len / 256.0f;
...
*/
static void
VectorArrayNormalize(vec4_t *normals, unsigned int count)
{
/*    assert(count); */
...
static float
LodErrorForVolume(vec3_t local, float radius)
{
	vec3_t	world;
	float	d;
