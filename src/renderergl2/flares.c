...
R_ClearFlares(void)
{
	...
	Q_Memset(r_flareStructs, 0, sizeof(r_flareStructs));
	r_activeFlares = NULL;
...
RB_AddFlare(void *surface, int fogNum, Vec3 point, Vec3 color, Vec3 normal)
{
	...
	if(normal && (normal[0] || normal[1] || normal[2])){
		subv3(backEnd.viewParms.or.origin, point, local);
		fastnormv3(local);
		d = dotv3(local, normal);
	...
	copyv3(point, f->origin);
	copyv3(color, f->color);

	/* fade the intensity of the flare down as the
	 * light surface turns away from the viewer */
	scalev3(f->color, d, f->color);
	...
	for(i=0; i<backEnd.refdef.num_dlights; i++, l++){
		...
		if(fog){
			...
			for(j = 1; j < tr.world->numfogs; j++){
				fog = &tr.world->fogs[j];
				for(k = 0; k < 3; k++)
					if(l->origin[k] < fog->bounds[0][k] || l->origin[k] >
					   fog->bounds[1][k]){
						break;
					}
				if(k == 3){
...
RB_TestFlare(flare_t *f)
{
	...
	screenZ = backEnd.viewParms.projectionMatrix[14] /
		  ((2*depth -
		    1) * backEnd.viewParms.projectionMatrix[11] - backEnd.viewParms.projectionMatrix[10]);
 ...
 */
	...
	scalev3(f->color, f->drawIntensity * intensity, color);

/* Calculations for fogging */
	if(tr.world && f->fogNum < tr.world->numfogs){
		tess.numVertexes = 1;
		copyv3(f->origin, tess.xyz[0]);
...
RB_RenderFlares(void)
{
	...
	flare_t **prev;
	qbool draw;
	Mat4 oldmodelview, oldprojection, matrix;
...
/*	RB_AddDlightFlares(); */
	...
	copym4(glState.projection, oldprojection);
	copym4(glState.modelview, oldmodelview);
	identm4(matrix);
	GL_SetModelviewMatrix(matrix);
	orthom4(backEnd.viewParms.viewportX, backEnd.viewParms.viewportX +
