...
typedef struct {
	...
	int			maxservers;
	int			refreshtime;
	char			favoriteaddresses[MAX_FAVORITESERVERS][
		MAX_ADDRESSLENGTH];
...
ArenaServers_UpdateMenu(void)
{
	...
	if(g_arenaservers.numqueriedservers > 0){
		...
		if(g_arenaservers.refreshservers &&
		   (g_arenaservers.currentping <=
			...
			qsort(g_arenaservers.serverlist,
				*g_arenaservers.numservers, sizeof(servernode_t),
				ArenaServers_Compare);
		}else{
			...
			/* update status bar */
			if(g_servertype >= UIAS_GLOBAL1 && g_servertype <=
			   UIAS_GLOBAL5)
	...
	for(i = 0, j = 0; i < count; i++, servernodeptr++){
		...
		else if(servernodeptr->maxPing && servernodeptr->pingtime >
			servernodeptr->maxPing)
			pingColor = S_COLOR_BLUE;
	...
	for(i=0; i<g_numfavoriteservers; i++)
		if(&g_favoriteserverlist[i] == servernodeptr){
			...
			if(i < g_numfavoriteservers-1)
				/* shift items up */
				memcpy(
					&g_favoriteserverlist[i],
					&g_favoriteserverlist[i+1],
					(g_numfavoriteservers - i -
					 1)*sizeof(servernode_t));
...
ArenaServers_Insert(char* adrstr, char* info, int pingtime)
{
	...
	if(servernodeptr->nettype < 0 || servernodeptr->nettype >=
	   ARRAY_LEN(netnames) - 1)
		servernodeptr->nettype = 0;
...
ArenaServers_InsertFavorites(void)
{
	...
	for(i=0; i<g_arenaservers.numfavoriteaddresses; i++){
		...
		if(j >= g_numfavoriteservers)
			/* not in list, add it */
			ArenaServers_Insert(g_arenaservers.favoriteaddresses[i],
				info,
				ArenaServers_MaxPing());
	}
}
...
ArenaServers_LoadFavorites(void)
{
	...
	for(i=0; i<MAX_FAVORITESERVERS; i++){
		...
		if(j < numtempitems){
			...
			/* add new server */
			Q_strncpyz(
				g_favoriteserverlist[g_numfavoriteservers].
				adrstr, adrstr,
				MAX_ADDRESSLENGTH);
...
ArenaServers_DoRefresh(void)
{
	...
	for(i=0; i<MAX_PINGREQUESTS; i++){
		...
		if(j < MAX_PINGREQUESTS){
			...
			if(!time){
				time = uis.realtime -
				       g_arenaservers.pinglist[j].start;
				if(time < maxPing)
					/* still waiting */
					continue;
			}
	...
	if(g_servertype == UIAS_FAVORITES)
		...
		g_arenaservers.numqueriedservers = trap_LAN_GetServerCount(
			ArenaServers_SourceForLAN());
 ...
 *      g_arenaservers.numqueriedservers = g_arenaservers.maxservers; */
	...
	for(i=0;
	    i<MAX_PINGREQUESTS && g_arenaservers.currentping <
...
ArenaServers_StartRefresh(void)
{
	...
	memset(g_arenaservers.serverlist, 0, g_arenaservers.maxservers*
		sizeof(table_t));
...
ArenaServers_Event(void* ptr, int event)
{
	...
	case ID_PUNKBUSTER:
		if(g_arenaservers.punkbuster.curvalue)
			UI_ConfirmMenu_Style("Enable Punkbuster?",  UI_CENTER|
				UI_INVERSE|UI_SMALLFONT, 0,
				Punkbuster_ConfirmEnable);
		else
			UI_ConfirmMenu_Style("Disable Punkbuster?", UI_CENTER|
				UI_INVERSE|UI_SMALLFONT, 0,
...
ArenaServers_MenuKey(int key)
{
	...
	if((key == K_DEL ||
	    key == K_KP_DEL) && (g_servertype == UIAS_FAVORITES) &&
...
ArenaServers_MenuInit(void)
{
	...
	g_arenaservers.down.generic.flags = QMF_LEFT_JUSTIFY|
					    QMF_PULSEIFFOCUS|
					    QMF_MOUSEONLY;
	...
	g_arenaservers.remove.generic.flags = QMF_LEFT_JUSTIFY|
					      QMF_PULSEIFFOCUS;
	...
	g_arenaservers.create.generic.flags = QMF_LEFT_JUSTIFY|
					      QMF_PULSEIFFOCUS;
