		...
		if(ch == ' ')
			ax +=
				((float)PROPB_SPACE_WIDTH +
				 (float)PROPB_GAP_WIDTH)* uis.xscale;
		else if(ch >= 'A' && ch <= 'Z'){
			ch	-= 'A';
			   ...
			   qhandle_t charset)
{
	...
	while(*s){
		...
		if(ch == ' ')
			...
			aw	= (float)propMap[ch][2] * uis.xscale *
				  sizeScale;
			...
			*s2 = '\0';
			...
			if(c_bcp == '\0'){
				...
				if(*s2 != '\0')	/* if we are printing an overflowing line we have s2 == s3 */
					UI_DrawProportionalString(x, y, s2,
						style,
						color);
				break;
			}
...
UI_DrawString2(int x, int y, const char* str, vec4_t color, int charw, int charh)
{
	...
	while(*s){
		if(Q_IsColorString(s)){
			if(!forceColor){
				memcpy(tempcolor, g_color_table[ColorIndex(
									s[1])],
					sizeof(tempcolor));
				tempcolor[3] = color[3];
...
UI_DrawString(int x, int y, const char* str, int style, vec4_t color)
{
	...
	if(style & UI_PULSE){
		...
		UI_LerpColor(color,lowlight,newcolor,0.5+0.5*
			sin(uis.realtime/PULSE_DIVISOR));
		drawcolor = newcolor;
	}else
		drawcolor = color;
...
UI_SetActiveMenu(uiMenuCommand_t menu)
{
	...
	default:
#ifndef NDEBUG
		Q_Printf("UI_SetActiveMenu: bad enum %d\n", menu);
#endif
		break;
	}
}
...
UI_MouseEvent(int dx, int dy)
{
	...
	for(i=0; i<uis.activemenu->nitems; i++){
		...
		if(uis.activemenu->cursor != i){
			...
			((menucommon_s*)(uis.activemenu->items[uis.activemenu
							       ->cursor_prev]))
			->flags &= ~QMF_HASMOUSEFOCUS;

			if(!(((menucommon_s*)(uis.activemenu->items[uis.
								    activemenu
								    ->cursor]))
			     ->flags & QMF_SILENT))
		...
		((menucommon_s*)(uis.activemenu->items[uis.activemenu->cursor]))
		->flags |= QMF_HASMOUSEFOCUS;
		return;
	}
...
UI_Refresh(int realtime)
{
		...
		if(uis.activemenu->fullscreen){
			...
			if(0)
				UI_DrawHandlePic(
					...
					uis.whiteShader	/*uis.menuBackShader*/);
			else
				UI_DrawHandlePic(
					0, 0, SCREEN_WIDTH, SCREEN_HEIGHT,
					uis.whiteShader	/*uis.menuBackNoLogoShader*/);
		}
	...
	if(uis.debug)
		/* cursor coordinates */
		UI_DrawString(0, 0, va("(%d,%d)",uis.cursorx,
				uis.cursory), UI_LEFT|UI_SMALLFONT, colorRed);
