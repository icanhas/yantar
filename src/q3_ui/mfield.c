...
MField_Draw(mfield_t *edit, int x, int y, int style, Vec4 color)
{
	...
	UI_DrawChar(x +
		(edit->cursor -
		 prestep) * charw, y, cursorChar, style & ~(UI_CENTER|UI_RIGHT),
		color);
}
...
MField_CharEvent(mfield_t *edit, int ch)
{
	...
	if(!trap_Key_GetOverstrikeMode()){
		...
		if((len == MAX_EDIT_LINE - 1) ||
		   (edit->maxchars && len >= edit->maxchars))
			return;
		memmove(edit->buffer + edit->cursor + 1, edit->buffer +
			edit->cursor,
			len + 1 - edit->cursor);
	}
...
MenuField_Draw(menufield_s *f)
{
	...
	if(focus){
		/* draw cursor */
		UI_FillRect(f->generic.left, f->generic.top, f->generic.right-
			f->generic.left+1, f->generic.bottom-f->generic.top+1,
			listbar_color);
...
MenuField_Key(menufield_s* m, int* key)
{
	...
	default:
		if(keycode & K_CHAR_FLAG){
			...
			if((m->generic.flags & QMF_UPPERCASE) &&
			   Q_islower(keycode))
				keycode -= 'a' - 'A';
			else if((m->generic.flags & QMF_LOWERCASE) &&
				Q_isupper(keycode))
				keycode -= 'A' - 'a';
			else if((m->generic.flags & QMF_NUMBERSONLY) &&
				Q_isalpha(keycode))
