...
PlayerModel_UpdateGrid(void)
{
	...
	for(i=0; i<PLAYERGRID_ROWS*PLAYERGRID_COLS; i++,j++){
		if(j < s_playermodel.nummodels){
			/* model/skin portrait */
			s_playermodel.pics[i].generic.name =
				s_playermodel.modelnames[j];
			s_playermodel.picbuttons[i].generic.flags &=
				~QMF_INACTIVE;
		}else{
			/* dead slot */
...
PlayerModel_UpdateModel(void)
{
	...
	UI_PlayerInfo_SetInfo(&s_playermodel.playerinfo, LEGS_IDLE, TORSO_STAND,
		viewangles, moveangles, WP_MACHINEGUN,
		qfalse);
}
...
PlayerModel_MenuKey(int key)
{
	...
	case K_LEFTARROW:
		...
		if(picnum >= 0 && picnum <= 15){
			if(picnum > 0){
				Menu_SetCursor(&s_playermodel.menu,
	...
	case K_RIGHTARROW:
		...
		if(picnum >= 0 && picnum <= 15){
			if((picnum < 15) &&
			   (s_playermodel.modelpage*MAX_MODELSPERPAGE + picnum+
			    1 <
...
PlayerModel_DrawPlayer(void *self)
{
	...
	if(trap_MemoryRemaining() <= LOW_MEMORY){
		UI_DrawProportionalString(b->generic.x, b->generic.y +
			b->height / 2, "LOW MEMORY", UI_LEFT,
			color_red);
		return;
	}
...
PlayerModel_BuildList(void)
{
	...
	for(i=0; i<numdirs && s_playermodel.nummodels < MAX_PLAYERMODELS;
		...
		fileptr = filelist;
		for(j=0;
		    j<numfiles && s_playermodel.nummodels < MAX_PLAYERMODELS;
		    j++,fileptr+=filelen+1){
...
PlayerModel_SetMenuItems(void)
{
	...
	for(i=0; i<s_playermodel.nummodels; i++){
		...
		if(!Q_stricmp(s_playermodel.modelskin, modelskin)){
			...
			if(maxlen > 16)
				maxlen = 16;
			Q_strncpyz(s_playermodel.modelname.string, buffptr,
				maxlen);
			Q_strupr(s_playermodel.modelname.string);
			...
			if(maxlen > 16)
				maxlen = 16;
			Q_strncpyz(s_playermodel.skinname.string, pdest+5,
				maxlen);
			Q_strupr(s_playermodel.skinname.string);
			break;
		}
...
PlayerModel_MenuInit(void)
{
	...
	for(i=0,k=0; i<PLAYERGRID_ROWS; i++){
		...
		for(j=0; j<PLAYERGRID_COLS; j++,k++){
			...
			s_playermodel.pics[k].generic.flags	=
				QMF_LEFT_JUSTIFY|
				QMF_INACTIVE;
			...
			s_playermodel.picbuttons[k].generic.type =
				MTYPE_BITMAP;
			...
			s_playermodel.picbuttons[k].generic.callback =
				PlayerModel_PicEvent;
	...
	s_playermodel.playername.generic.flags	= QMF_CENTER_JUSTIFY|
						  QMF_INACTIVE;
	...
	s_playermodel.modelname.generic.flags = QMF_CENTER_JUSTIFY|
						QMF_INACTIVE;
	...
	s_playermodel.skinname.generic.flags = QMF_CENTER_JUSTIFY|
					       QMF_INACTIVE;
	...
	s_playermodel.left.generic.flags	= QMF_LEFT_JUSTIFY|
						  QMF_PULSEIFFOCUS;
	...
	s_playermodel.right.generic.flags	= QMF_LEFT_JUSTIFY|
						  QMF_PULSEIFFOCUS;
	...
	for(i=0; i<MAX_MODELSPERPAGE; i++){
		...
		Menu_AddItem(&s_playermodel.menu,
			&s_playermodel.picbuttons[i]);
	}
...
UI_PlayerModelMenu(void)
{
	...
	Menu_SetCursorToItem(&s_playermodel.menu,
		&s_playermodel.pics[s_playermodel.selectedmodel %
				    MAX_MODELSPERPAGE]);
}
