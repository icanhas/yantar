...
UI_TeamOrdersMenu_SetList(int id)
{
	...
	case ID_LIST_BOTS:
		...
		teamOrdersMenuInfo.list.numitems	=
			teamOrdersMenuInfo.numBots;
		teamOrdersMenuInfo.list.itemnames =
			(const char**)teamOrdersMenuInfo.bots;
		break;
...
UI_TeamOrdersMenu_Key(int key)
{
	...
	case K_MOUSE1:
		x	= l->generic.left;
		y	= l->generic.top;
		if(UI_CursorInRect(x, y, l->generic.right - x,
			   l->generic.bottom - y)){
...
UI_TeamOrdersMenu_Init(void)
{
	...
	teamOrdersMenuInfo.list.generic.ownerdraw =
		UI_TeamOrdersMenu_ListDraw;
	teamOrdersMenuInfo.list.generic.callback =
		UI_TeamOrdersMenu_ListEvent;
	...
	teamOrdersMenuInfo.back.generic.flags = QMF_LEFT_JUSTIFY|
						QMF_PULSEIFFOCUS;
	teamOrdersMenuInfo.back.generic.callback =
		UI_TeamOrdersMenu_BackEvent;
	...
	teamOrdersMenuInfo.list.generic.top	=
		teamOrdersMenuInfo.list.generic.y;
