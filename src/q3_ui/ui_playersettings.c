...
#define ART_MODEL0	Pmenuart "/model_0"
#define ART_MODEL1	Pmenuart "/model_1"
...
PlayerSettings_DrawName(void *self)
{
	...
	if(focus){
		...
		UI_DrawChar(basex + f->field.cursor * SMALLCHAR_WIDTH, y, c,
			style,
			color_white);
	}
...
PlayerSettings_DrawHandicap(void *self)
{
	...
	UI_DrawProportionalString(item->generic.x, item->generic.y, "Handicap",
		style,
		color);
	UI_DrawProportionalString(item->generic.x + 64, item->generic.y +
		PROP_HEIGHT, handicap_items[item->curvalue], style,
		color);
}
...
PlayerSettings_DrawEffects(void *self)
{
	...
	UI_DrawProportionalString(item->generic.x, item->generic.y, "Effects",
		style,
		color);

	UI_DrawHandlePic(item->generic.x + 64, item->generic.y + PROP_HEIGHT + 8,
		128, 8,
...
PlayerSettings_DrawPlayer(void *self)
{
	...
	if(strcmp(buf, s_playersettings.playerModel) != 0){
		...
		UI_PlayerInfo_SetInfo(&s_playersettings.playerinfo, LEGS_IDLE,
			TORSO_STAND, viewangles, vec3_origin, WP_MACHINEGUN,
			qfalse);
	}

	b = (menubitmap_s*)self;
	UI_DrawPlayer(b->generic.x, b->generic.y, b->width, b->height,
		&s_playersettings.playerinfo,
		uis.realtime/2);
}
...
PlayerSettings_MenuInit(void)
{
	...
	s_playersettings.model.generic.flags = QMF_RIGHT_JUSTIFY|
					       QMF_PULSEIFFOCUS;
	...
	s_playersettings.item_null.generic.flags = QMF_LEFT_JUSTIFY|
						   QMF_MOUSEONLY|
						   QMF_SILENT;
...
PlayerSettings_Cache(void)
{
	...
	s_playersettings.fxPic[1]	= trap_R_RegisterShaderNoMip(
		ART_FX_YELLOW);
	s_playersettings.fxPic[2] = trap_R_RegisterShaderNoMip(
		ART_FX_GREEN);
	...
	s_playersettings.fxPic[6]	= trap_R_RegisterShaderNoMip(
		ART_FX_WHITE);
}
