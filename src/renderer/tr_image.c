 ...
 */
static void
ResampleTexture(unsigned *in, int inwidth, int inheight, unsigned *out,
 ...
 */
static void
R_MipMap2(unsigned *in, int inWidth, int inHeight)
{
	int i, j, k;
	byte *outpix;
		...
		for(j = 0; j < outWidth; j++){
			...
			for(k = 0; k < 4; k++){
				total =
					...
					((byte*)&in[ ((i*2-
						       1)&
						      inHeightMask)*inWidth + ((j*2-1)&inWidthMask) ])[k] +
					2 *
					((byte*)&in[ ((i*2-
						       1)&inHeightMask)*inWidth + ((j*2)&inWidthMask) ])[k] +
					2 *
					((byte*)&in[ ((i*2-
						       1)&
						      inHeightMask)*inWidth + ((j*2+1)&inWidthMask) ])[k] +
					...
					4 *
					((byte*)&in[ ((i*
						       2)&inHeightMask)*inWidth + ((j*2)&inWidthMask) ])[k] +
					4 *
					((byte*)&in[ ((i*
						       2)&
						      inHeightMask)*inWidth + ((j*2+1)&inWidthMask) ])[k] +
					...
					1 *
					((byte*)&in[ ((i*2+
						       2)&inHeightMask)*inWidth + ((j*2+2)&inWidthMask) ])[k];
				outpix[k] = total / 36;
			}
		}
