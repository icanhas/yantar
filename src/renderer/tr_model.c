...
R_LoadMDR(model_t *mod, void *buffer, int filesize, const char *mod_name)
{
	...
	if(pinmodel->numBones < 0 ||
	   ...
	   (sizeof(*frame) + (pinmodel->numBones - 1) * sizeof(*frame->bones)) > size){
	...
	for(l = 0; l < mdr->numLODs; l++){
		...
		for(i = 0; i < lod->numSurfaces; i++){
			...
			if(surf->numVerts > SHADER_MAX_VERTEXES){
				ri.Printf(PRINT_WARNING,
					"R_LoadMDR: %s has more than %i verts on a surface (%i).\n",
			...
			if(surf->numTriangles*3 > SHADER_MAX_INDEXES){
				ri.Printf(PRINT_WARNING,
					"R_LoadMDR: %s has more than %i triangles on a surface (%i).\n",
					mod_name, SHADER_MAX_INDEXES / 3,
					surf->numTriangles);
				return qfalse;
			}
			...
			for(j = 0; j < surf->numVerts; j++){
				...
				if(curv->numWeights < 0 ||
				   (byte*)(v +
					   1) +
				   (curv->numWeights - 1) * sizeof(*weight) > (byte*)mdr + size){
					ri.Printf(PRINT_WARNING, "R_LoadMDR: %s has broken structure.\n",
						mod_name);
					return qfalse;
				}
...
#ifdef RAVENMD4
void
R_GetAnimTag(mdrHeader_t *mod, int framenum, const char *tagName, md3Tag_t * dest)
{
	int	i, j, k;
	int	frameSize;
	mdrFrame_t *frame;
