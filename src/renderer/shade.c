...
static int c_begins;
static void
R_DrawStripElements(int numIndexes, const glIndex_t *indexes, void (APIENTRY *element)(GLint))
{
	int i;
	int last[3] = { -1, -1, -1 };
	qbool even;
...
ComputeColors(shaderStage_t *pStage)
{
	...
	case CGEN_IDENTITY:
		Q_Memset(tess.svars.colors, 0xff, tess.numVertexes * 4);
		break;
	default:
	case CGEN_IDENTITY_LIGHTING:
		Q_Memset(tess.svars.colors, tr.identityLightByte, tess.numVertexes * 4);
		break;
	case CGEN_LIGHTING_DIFFUSE:
		RB_CalcDiffuseColor(( unsigned char* )tess.svars.colors);
		break;
	case CGEN_EXACT_VERTEX:
		Q_Memcpy(tess.svars.colors, tess.vertexColors, tess.numVertexes *
...
ComputeTexCoords(shaderStage_t *pStage)
{
	...
	for(b = 0; b < NUM_TEXTURE_BUNDLES; b++){
		...
		case TCGEN_IDENTITY:
			Q_Memset(tess.svars.texcoords[b], 0, sizeof(float) * 2 * tess.numVertexes);
			break;
		case TCGEN_TEXTURE:
			for(i = 0; i < tess.numVertexes; i++){
