...
static struct BufferedFile *
ReadBufferedFile(const char *name)
{
...
static void
CloseBufferedFile(struct BufferedFile *BF)
{
	if(BF){
		if(BF->Buffer){
...
static void *
BufferedFileRead(struct BufferedFile *BF, unsigned Length)
{
	void *RetVal;
...
static uint32_t
DecompressIDATs(struct BufferedFile *BF, uint8_t **Buffer)
{
...
static uint8_t
PredictPaeth(uint8_t a, uint8_t b, uint8_t c)
{
	/*
	 *  a == Left
	 *  b == Up
	...
	for(h = 0; h < ImageHeight; h++){
		...
		for(w = 0; w < (BytesPerScanline / BytesPerPixel); w++){
			...
			for(p = 0; p < BytesPerPixel; p++){
				...
				{
					DecompPtr[p] +=
						((uint8_t)((((uint16_t)PixelLeft[p]) +
							    ((uint16_t)PixelUp[p])) / 2));
			 ...
			 uint8_t               *OutPal)
{
	...
	if(!(IHDR && OutBuffer && DecompressedData && DecompressedDataLength && TransparentColour &&
	     OutPal)){
		return(qfalse);
	}
						...
						if(CurrPixel < PassWidth[a]){
							...
							OutPtr = OutBuffer +
								 (((((h *
								   ...
								   ((CurrPixel *
								     WSkip[a]) +
								    WOffset[a])) * Q3IMAGE_BYTESPERPIXEL);
...
void
R_LoadPNG(const char *name, byte **pic, int *width, int *height)
{
	...
	*pic = NULL;
	...
	{
		if(!DecodeImageNonInterlaced(IHDR, OutBuffer, DecompressedData, DecompressedDataLength,
			   HasTransparentColour, TransparentColour, OutPal)){
			ri.Free(OutBuffer);
			ri.Free(DecompressedData);
			CloseBufferedFile(ThePNG);
	...
	{
		if(!DecodeImageInterlaced(IHDR, OutBuffer, DecompressedData, DecompressedDataLength,
			   HasTransparentColour, TransparentColour, OutPal)){
			ri.Free(OutBuffer);
			ri.Free(DecompressedData);
			CloseBufferedFile(ThePNG);
