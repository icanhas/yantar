...
static int
R_MDRCullModel(mdrHeader_t *header, trRefEntity_t *ent)
{
	vec3_t	bounds[2];
...
int
R_MDRComputeFogNum(mdrHeader_t *header, trRefEntity_t *ent)
{
	int i, j;
	fog_t *fog;
	mdrFrame_t	*mdrFrame;
...
void
R_MDRAddAnimSurfaces(trRefEntity_t *ent)
{
	mdrHeader_t	*header;
...
RB_MDRSurfaceAnim(md4Surface_t *surface)
{
	...
	if(!backlerp){
		...
		for(i = 0; i < header->numBones*12; i++)
			((float*)bonePtr)[i] = frontlerp * ((float*)frame->bones)[i] + backlerp *
					       ((float*)oldFrame->bones)[i];
	}
