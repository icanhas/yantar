	...
	for( k = 0; k < facet->numBorders; k++ ){
		...
		if( front && !back )
			...
			if( !debugBlock ){
				...
				VectorCopy(grid->points[i][j],
					debugBlockPoints[0]);
				VectorCopy(grid->points[i+1][j],
					debugBlockPoints[1]);
				VectorCopy(grid->points[i+1][j+1],
					debugBlockPoints[2]);
				VectorCopy(grid->points[i][j+1],
					debugBlockPoints[3]);
			}
		}
	}
				 ...
				 const struct patchCollide_s *pc)
{
	...
	for( i = 0; i < pc->numPlanes; i++, planes++ ){
		offset	= DotProduct(tw->offsets[ planes->signbits ],
			planes->plane);
		d1	=
			DotProduct(tw->start,
...
CM_TraceThroughPatchCollide(traceWork_t *tw, const struct patchCollide_s *pc)
{
	...
	for( i = 0; i < pc->numFacets; i++, facet++ ){
		...
		if( tw->sphere.use ){
			...
			if( t > 0.0f ){
				VectorSubtract(tw->start, tw->sphere.offset,
					startp);
			...
			offset =
				DotProduct(tw->offsets[ planes->signbits ],
					plane);
			plane[3] -= offset;
		...
		for( j = 0; j < facet->numBorders; j++ ){
			...
			if( tw->sphere.use ){
				...
				t = DotProduct(plane, tw->sphere.offset);
				if( t > 0.0f ){
					VectorSubtract(tw->start,
						tw->sphere.offset,
						startp);
					VectorSubtract(tw->end,
						tw->sphere.offset,
						endp);
				}else{
					VectorAdd(tw->start, tw->sphere.offset,
						startp);
					VectorAdd(tw->end, tw->sphere.offset,
						endp);
				}
			}else{
	...
	for( i = 0, facet = pc->facets; i < pc->numFacets; i++, facet++ )
		...
		for( k = 0; k < facet->numBorders + 1; k++ ){
			...
			for( j = 0; j < facet->numBorders + 1 && w; j++ ){
				...
				Vector4Copy(pc->planes[ curplanenum ].plane,
					plane);
				if( !curinward ){
					VectorSubtract(vec3_origin, plane, plane);
					plane[3] = -plane[3];
