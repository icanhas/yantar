...
MSG_ReadBits(msg_t *msg, int bits)
{
	...
	if(msg->oob){
		if(bits==8){
			...
			Q_Error(ERR_DROP, "can't read %d bits", bits);
	}else{
		nbits = 0;
		if(bits&7){
			nbits = bits&7;
...
MSG_WriteString(msg_t *sb, const char *s)
{
	if(!s)
		...
		if(l >= MAX_STRING_CHARS){
			Q_Printf("MSG_WriteString: MAX_STRING_CHARS");
			MSG_WriteData (sb, "", 1);
			return;
		}
...
MSG_WriteBigString(msg_t *sb, const char *s)
{
	if(!s)
		...
		if(l >= BIG_INFO_STRING){
			Q_Printf("MSG_WriteString: BIG_INFO_STRING");
			MSG_WriteData (sb, "", 1);
			return;
		}
		Q_strncpyz(string, s, sizeof(string));
