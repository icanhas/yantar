		     ...
		     sa_family_t family)
{
	...
	if(!retval){
		...
		Q_Printf("Sys_StringToSockaddr: Error resolving %s: %s\n", s,
			gai_strerror(
				retval));
...
Sys_SockaddrToString(char *dest, int destlen, struct sockaddr *input)
{
	...
	if(getnameinfo(input, inputlen, dest, destlen, NULL, 0,
		   NI_NUMERICHOST) && destlen > 0)
		*dest = '\0';
}
...
Sys_StringToAdr(const char *s, netadr_t *a, netadrtype_t family)
{
	...
	if(!Sys_StringToSockaddr(s, (struct sockaddr*)&sadr, sizeof(sadr),
		   fam))
		return qfalse;
...
NET_CompareBaseAdrMask(netadr_t a, netadr_t b, int netmask)
{
	...
	if(a.type == NA_IP){
		...
		Q_Printf ("NET_CompareBaseAdr: bad address type\n");
		return qfalse;
	}
...
NET_GetPacket(netadr_t *net_from, msg_t *net_message, fd_set *fdr)
{
	...
	if(ip_socket != INVALID_SOCKET && FD_ISSET(ip_socket, fdr)){
		fromlen = sizeof(from);
		ret =
			recvfrom(ip_socket, (void*)net_message->data,
		...
		if(ret == SOCKET_ERROR){
			...
			if(usingSocks &&
			   memcmp(&from, &socksRelayAddr, fromlen) == 0){
				...
				net_from->port	=
					*(short*)&net_message->data[8];
				net_message->readcount = 10;
			}else{
				SockadrToNetadr((struct sockaddr*)&from,
					net_from);
				net_message->readcount = 0;
			}

			if(ret >= net_message->maxsize){
				Q_Printf("Oversize packet from %s\n",
					NET_AdrToString (
						*net_from));
				return qfalse;
			}
	...
	if(multicast6_socket != INVALID_SOCKET && multicast6_socket !=
	   ip6_socket && FD_ISSET(multicast6_socket, fdr)){
		fromlen = sizeof(from);
		ret =
...
Sys_SendPacket(int length, const void *data, netadr_t to)
{
	...
	if(to.type != NA_BROADCAST && to.type != NA_IP && to.type != NA_IP6 &&
	   to.type != NA_MULTICAST6){
		Q_Error(ERR_FATAL, "Sys_SendPacket: bad address type");
		return;
	}
	...
	if(usingSocks && to.type == NA_IP){
		...
		*(short*)&socksBuf[8] =
		...
		if(addr.ss_family == AF_INET)
			ret =
				sendto(ip_socket, data, length, 0,
					(struct sockaddr*)&addr,
					sizeof(struct sockaddr_in));
		else if(addr.ss_family == AF_INET6)
			ret =
				sendto(ip6_socket, data, length, 0,
					(struct sockaddr*)&addr,
...
NET_IPSocket(char *net_interface, int port, int *err)
{
	...
	*err = 0;
	...
	if(net_interface)
		Q_Printf("Opening IP socket: %s:%i\n", net_interface, port);
	else
		Q_Printf("Opening IP socket: 0.0.0.0:%i\n", port);

	if((newsocket =
		    socket(PF_INET, SOCK_DGRAM,
			    IPPROTO_UDP)) == INVALID_SOCKET){
		*err = socketError;
		Q_Printf("WARNING: NET_IPSocket: socket: %s\n",
			NET_ErrorString());
		return newsocket;
	}
	/* make it non-blocking */
	if(ioctlsocket(newsocket, FIONBIO, &_true) == SOCKET_ERROR){
		Q_Printf("WARNING: NET_IPSocket: ioctl FIONBIO: %s\n",
			NET_ErrorString());
		*err = socketError;
	...
	}else if(!Sys_StringToSockaddr(net_interface,
	...
	if(bind(newsocket, (void*)&address,
		...
		Q_Printf("WARNING: NET_IPSocket: bind: %s\n", NET_ErrorString());
		*err = socketError;
		closesocket(newsocket);
		return INVALID_SOCKET;
	}
	...
	*err = 0;
	...
	if(net_interface){
		...
		if(Q_CountChar(net_interface, ':'))
			Q_Printf("Opening IP6 socket: [%s]:%i\n",
				net_interface,
				port);
		else
			Q_Printf("Opening IP6 socket: %s:%i\n", net_interface,
		...
		Q_Printf("Opening IP6 socket: [::]:%i\n", port);

	if((newsocket =
		    socket(PF_INET6, SOCK_DGRAM,
			    IPPROTO_UDP)) == INVALID_SOCKET){
		*err = socketError;
		Q_Printf("WARNING: NET_IP6Socket: socket: %s\n",
			NET_ErrorString());
		return newsocket;
	}
	...
	/* make it non-blocking */
	if(ioctlsocket(newsocket, FIONBIO, &_true) == SOCKET_ERROR){
		Q_Printf("WARNING: NET_IP6Socket: ioctl FIONBIO: %s\n",
			NET_ErrorString());
		*err = socketError;
		closesocket(newsocket);
		return INVALID_SOCKET;
	...
	if(bind(newsocket, (void*)&address,
		   sizeof(address)) == SOCKET_ERROR){
		Q_Printf("WARNING: NET_IP6Socket: bind: %s\n", NET_ErrorString());
		*err = socketError;
		closesocket(newsocket);
		return INVALID_SOCKET;
	}
...
#else
		curgroup.ipv6mr_interface = if_nametoindex(
			net_mcast6iface->string);
#endif
	}else
...
NET_JoinMulticast6(void)
{
	...
	if(ip6_socket == INVALID_SOCKET || multicast6_socket !=
	   INVALID_SOCKET || (net_enabled->integer & NET_DISABLEMCAST))
		return;
	...
	if(multicast6_socket != INVALID_SOCKET){
		if(multicast6_socket != ip6_socket)
			...
			setsockopt(multicast6_socket, IPPROTO_IPV6,
				IPV6_LEAVE_GROUP, (char*)&curgroup,
				sizeof(curgroup));
...
NET_OpenSocks(int port)
{
	...
	Q_Printf("Opening connection to SOCKS server.\n");

	if((socks_socket =
		    socket(AF_INET, SOCK_STREAM,
			    IPPROTO_TCP)) == INVALID_SOCKET){
		Q_Printf("WARNING: NET_OpenSocks: socket: %s\n",
			NET_ErrorString());
		return;
	}
	...
	if(h == NULL){
		Q_Printf("WARNING: NET_OpenSocks: gethostbyname: %s\n",
			NET_ErrorString());
		return;
	}
	...
	if(connect(socks_socket, (struct sockaddr*)&address,
		   sizeof(address)) == SOCKET_ERROR){
		Q_Printf("NET_OpenSocks: connect: %s\n", NET_ErrorString());
		return;
	}
	...
	if(len == SOCKET_ERROR){
		Q_Printf("NET_OpenSocks: recv: %s\n", NET_ErrorString());
		return;
	}
	if(len != 2 || buf[0] != 5){
		Q_Printf("NET_OpenSocks: bad response\n");
		return;
	}
	switch(buf[1]){
	...
	case 2:	/* username/password authentication */
		break;
	default:
		Q_Printf("NET_OpenSocks: request denied\n");
		return;
	}
	...
	if(buf[1] == 2){
		...
		if(buf[1] != 0){
			Q_Printf("NET_OpenSocks: authentication failed\n");
			return;
		}
	}
	...
	*(short*)&buf[8] = htons((short)port);	/* port */
	...
	if(buf[1] != 0){
		Q_Printf("NET_OpenSocks: request denied: %i\n", buf[1]);
		return;
	}
	if(buf[3] != 1){
		Q_Printf("NET_OpenSocks: relay address is not IPV4: %i\n",
			buf[3]);
		return;
	}
...
NET_GetLocalAddress(void)
{
	...
	if(!getaddrinfo(hostname, NULL, &hint, &res)){
		...
		memset(&mask4.sin_addr.s_addr, 0xFF,
...
NET_OpenIP(void)
{
	...
	if(net_enabled->integer & NET_ENABLEV6){
		for(i = 0; i < 10; i++){
			ip6_socket =
				NET_IP6Socket(net_ip6->string, port6 + i,
					&boundto,
					&err);
		...
		if(NET_GetPacket(&from, &netmsg, fdr)){
			if(net_dropsim->value > 0.0f && net_dropsim->value <=
			   100.0f)
				...
				if(rand() <
				   (int)(((double)RAND_MAX) / 100.0 *
					 (double)net_dropsim->value))
					continue;	/* drop this packet */

			if(com_sv_running->integer)
				Q_RunAndTimeServerPacket(&from, &netmsg);
			else
...
NET_Sleep(int msec)
{
	...
	if(retval < 0)
		Q_Printf("Warning: select() syscall failed: %s\n",
			NET_ErrorString());
	else if(retval > 0)
		NET_Event(&fdr);
}
