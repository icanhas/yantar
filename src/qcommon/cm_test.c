...
CM_StoreBrushes(leafList_t *ll, int nodenum)
{
	...
	for(k = 0; k < leaf->numLeafSurfaces; k++){
		patch =
			cm.surfaces[ cm.leafsurfaces[ leaf->firstleafsurface +
						      k ] ];
		if(!patch)
			continue;
	}
...
CM_FloodArea_r(int areaNum, int floodnum)
{
	...
	if(area->floodvalid == cm.floodvalid){
		if(area->floodnum == floodnum)
			return;
		Q_Error (ERR_DROP, "FloodArea_r: reflooded");
	}
...
CM_AdjustAreaPortalState(int area1, int area2, qbool open)
{
	...
	if(area1 >= cm.numAreas || area2 >= cm.numAreas)
		Q_Error (ERR_DROP, "CM_ChangeAreaPortalState: bad area number");
...
CM_AreasConnected(int area1, int area2)
{
	...
	if(area1 >= cm.numAreas || area2 >= cm.numAreas)
		Q_Error (ERR_DROP, "area >= cm.numAreas");
