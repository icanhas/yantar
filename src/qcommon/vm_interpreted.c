...
/*	memcpy( vm->codeBase, (byte *)header + header->codeOffset, vm->codeLength ); */
	...
	while(instruction < header->instructionCount){
		...
		case OP_GEF:
			...
			codeBase[int_pc] =
				vm->instructionPointers[codeBase[int_pc]];
			int_pc++;
			break;
...
nextInstruction2:
		...
		if((unsigned)programCounter >= vm->codeLength){
			Q_Error(ERR_DROP, "VM pc out of range");
			return 0;
		}

		if(programStack <= vm->stackBottom){
			Q_Error(ERR_DROP, "VM stack overflow");
			return 0;
		}

		if(programStack & 3){
			Q_Error(ERR_DROP, "VM program stack misaligned");
			return 0;
		}
...
#ifdef DEBUG_VM
		default:
			Q_Error(ERR_DROP, "Bad VM instruction");	/* this should be scanned on load! */
			return 0;
#endif
		case OP_BREAK:
			vm->breakCount++;
		...
		case OP_LOAD4:
			...
			if(opStack[opStackOfs] & 3){
				Q_Error(ERR_DROP, "OP_LOAD4 misaligned");
				return 0;
			}
#endif
			r0 = opStack[opStackOfs] =
...
/*				vm->callLevel = temp; */
				...
				programCounter =
					vm->instructionPointers[ programCounter
					];
			goto nextInstruction;
