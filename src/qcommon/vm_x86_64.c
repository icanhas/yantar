...
#define VM_FREEBUFFERS(vm) do {assembler_init(0); VM_Destroy_Compiled(vm); \
} while(0)
...
#define JMPIARG() \
	CHECK_INSTR(iarg); \
	emit("movq $%" PRIu64 ", %%rax", vm->codeBase+ \
	vm->instructionPointers[iarg]);	\
	emit("jmpq *%%rax")
...
#ifdef DEBUG_VM
#define NOTIMPL(x) \
	do { Q_Error(ERR_DROP, "instruction not implemented: %s", opnames[x]); \
	} while(0)
#else
#define NOTIMPL(x) \
	do { Q_Printf(S_COLOR_RED "instruction not implemented: %x\n", x); \
...
eop(void)
{
	Q_Error(ERR_DROP, "End of program reached without return!");
	exit(1);
}
...
{
	Q_Error(ERR_DROP, "Program corrupted the VM opStack");
	exit(1);
}
#endif
...
VM_Compile(vm_t *vm, vmHeader_t *header)
{
	...
	for(pass = 0; pass < 2; ++pass){
		...
		if(pass){
			...
			vm->codeBase =
				mmap(NULL, compiledOfs, PROT_WRITE, MAP_SHARED|
					MAP_ANONYMOUS, -1,
					0);
			...
			vm->codeBase =
				VirtualAlloc(NULL, compiledOfs, MEM_RESERVE|
					MEM_COMMIT,
					PAGE_READWRITE);
			if(!vm->codeBase)
				Q_Error(
...
/*				emit("frstor 4(%%r9, %%rsi, 1)"); */
		...
		if(got_const){
			VM_FREEBUFFERS(vm);
			Q_Error(ERR_DROP, "leftover const");
		}
	...
	if(mprotect(vm->codeBase, compiledOfs, PROT_READ|PROT_EXEC))
		Q_Error(ERR_FATAL, "VM_CompileX86_64: mprotect failed");
	#elif __WIN64__
	{
		DWORD oldProtect = 0;
...
/*	Q_Printf("entering %s level %d, call %d, arg1 = 0x%x\n", vm->name, vm->callLevel, args[0], args[1]); */
...
/*	callMask = vm->dataMask; */
	...
	*opStack = 0xDEADBEEF;
	...
	__asm__ __volatile__ (
		...
		: "g" (entryPoint), "g" (opStack), "g" (vm->dataBase),
		"g" (programStack)
		: "%rsi", "%rax", "%rcx", "%rdx", "%r8", "%r9", "%r10", "%r11",
		"%xmm0"
		);
