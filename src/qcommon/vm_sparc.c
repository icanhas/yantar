...
#define DIE(reason) \
	do { \
		Q_Error(ERR_DROP, "vm_sparc compiler error: " reason); \
		exit(1); \
	} while(0)
...
#define FMT3(op, \
	     op3)		((op) << 30)|((op3)<<19), \
	(OP_MASK | OP3_MASK | IMM)
#define FMT3I(op, \
	      op3)		((op) << 30)|((op3)<<19)|IMM, \
	(OP_MASK | OP3_MASK | IMM)
...
pgreg(int reg_num, int arg_index, int flt)
{
	if(!flt){
		...
		Q_Printf("%s%%f%d", (arg_index ? ", " : ""), reg_num);
}
...
static void
dst_insn_insert_tail(struct func_info * const fp,
		     struct dst_insn *dp)
{
	if(!fp->dst_first)
...
static void
jump_insn_insert_tail(struct func_info * const fp,
		      struct jump_insn *jp)
{
	if(!fp->jump_first)
...
compile_one_insn(vm_t *vm, struct func_info * const fp, struct src_insn *sp)
{
	...
	switch(sp->op){
	default:
		Q_Printf("VM: Unhandled opcode 0x%02x[%s]\n",
			sp->op,
	...
	case OP_CALL:
		if(fp->cached_const){
			...
			if(fp->cached_const->arg.si >= 0)
				...
				in(LDLI, rVMDATA, VM_Data_Offset(
						CallThunk), rTMP);
	...
	case OP_ADD:
		if(fp->cached_const && SIMM13_P(fp->cached_const->arg.si)){
			EMIT_FALSE_CONST(fp);
			in(ADDI, rSECOND(fp), fp->cached_const->arg.si,
				rSECOND(fp));
		}else{
			MAYBE_EMIT_CONST(fp);
	...
	case OP_SUB:
		if(fp->cached_const && SIMM13_P(fp->cached_const->arg.si)){
			EMIT_FALSE_CONST(fp);
			in(SUBI, rSECOND(fp), fp->cached_const->arg.si,
				rSECOND(fp));
		}else{
			MAYBE_EMIT_CONST(fp);
	...
	case OP_LSH:
		if(fp->cached_const){
			EMIT_FALSE_CONST(fp);
			in(SLLI, rSECOND(fp), fp->cached_const->arg.si,
				rSECOND(fp));
		}else{
			MAYBE_EMIT_CONST(fp);
	...
	case OP_RSHI:
		if(fp->cached_const){
			EMIT_FALSE_CONST(fp);
			in(SRAI, rSECOND(fp), fp->cached_const->arg.si,
				rSECOND(fp));
		}else{
			MAYBE_EMIT_CONST(fp);
	...
	case OP_RSHU:
		if(fp->cached_const){
			EMIT_FALSE_CONST(fp);
			in(SRLI, rSECOND(fp), fp->cached_const->arg.si,
				rSECOND(fp));
		}else{
			MAYBE_EMIT_CONST(fp);
...
VM_Compile(vm_t *vm, vmHeader_t *header)
{
	...
	for(i = 0; i < header->instructionCount; i++)
		if(!fi.dst_by_i_count[i]){
			Q_Printf(S_COLOR_RED "Pointer %d not initialized !\n",
				i);
			DIE("sparc JIT bug");
		}
