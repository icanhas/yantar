...
CG_ParseScores(void)
{
	...
	for(i = 0; i < cg.numScores; i++){
		...
		if(cg.scores[i].client < 0 || cg.scores[i].client >=
		   MAX_CLIENTS)
...
CG_ParseTeamInfo(void)
{
	...
	if(numSortedTeamPlayers < 0 || numSortedTeamPlayers >
	   TEAM_MAXOVERLAY){
		CG_Error(
	...
	for(i = 0; i < numSortedTeamPlayers; i++){
		...
		if(client < 0 || client >= MAX_CLIENTS){
			CG_Error("CG_ParseTeamInfo: bad client number: %d",
				client);
			return;
		}
	...
	while(*str){
		if(len > TEAMCHAT_WIDTH - 1){
			...
			*p = 0;
			...
			cgs.teamChatMsgTimes[cgs.teamChatPos %
					     chatHeight] = cg.time;
...
CG_VoiceChatListForClient(int clientNum)
{
	...
	for(k = 0; k < 2; k++){
		...
		for(i = 0; i < MAX_HEADMODELS; i++)
			if(!Q_stricmp(headModelVoiceChat[i].headmodel,
				   headModelName))
				break;
		if(i < MAX_HEADMODELS)
			return &voiceChatLists[headModelVoiceChat[i].
					       voiceChatNum];
		...
		for(i = 0; i < MAX_HEADMODELS; i++)
			if(!strlen(headModelVoiceChat[i].headmodel)){
				...
				Q_sprintf(
					...
					"%s", headModelName);
				headModelVoiceChat[i].voiceChatNum =
					voiceChatNum;
				return &voiceChatLists[headModelVoiceChat[i].
						       voiceChatNum];
			}
	...
	if(!cg_noVoiceChats.integer){
		...
		if(vchat->clientNum != cg.snap->ps.clientNum){
			...
			if(orderTask > 0){
				...
				Q_strncpyz(cgs.acceptVoice, vchat->cmd,
					sizeof(cgs.acceptVoice));
