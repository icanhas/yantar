...
CG_AddFragment(localEntity_t *le)
{
	...
	if(le->pos.trType == TR_STATIONARY){
		...
		if(t < SINK_TIME){
			...
			le->refEntity.origin[2] -= 16 *
						   (1.0 - (float)t / SINK_TIME);
...
CG_AddMoveScaleFade(localEntity_t *le)
{
	...
	if(le->fadeInTime > le->startTime && cg.time < le->fadeInTime)
		...
		c = 1.0 -
		    (float)(le->fadeInTime -
			    cg.time) / (le->fadeInTime - le->startTime);
	else
		/* fade / grow time */
		c = (le->endTime - cg.time) * le->lifeRate;
...
CG_AddExplosion(localEntity_t *ex)
{
	...
	if(ex->light){
		...
		light =
			(float)(cg.time -
				ex->startTime) / (ex->endTime - ex->startTime);
		if(light < 0.5)
			light = 1.0;
		else
...
CG_AddSpriteExplosion(localEntity_t *le)
{
	...
	if(le->light){
		...
		light =
			(float)(cg.time -
				le->startTime) / (le->endTime - le->startTime);
		if(light < 0.5)
			light = 1.0;
		else
			light = 1.0 - (light - 0.5) * 2;
...
CG_AddInvulnerabilityJuiced(localEntity_t *le)
{
	...
	t = cg.time - le->startTime;
	if(t > 3000){
		le->refEntity.axis[0][0] = (float)1.0 + 0.3 *
					   (t - 3000) / 2000;
		le->refEntity.axis[1][1] = (float)1.0 + 0.3 *
					   (t - 3000) / 2000;
		le->refEntity.axis[2][2] = (float)0.7 + 0.3 *
					   (2000 - (t - 3000)) / 2000;
	}
	if(t > 5000){
		le->endTime = 0;
...
CG_AddScorePlum(localEntity_t *le)
{
	...
	for(i = 0; i < numdigits; i++){
		vec3ma(origin,
			...
			re->origin);
		re->customShader =
			cgs.media.numberShaders[digits[numdigits-1-i]];
		trap_R_AddRefEntityToScene(re);
	}
}
