...
CG_ResetEntity(centity_t *cent)
{
	...
	copyv3 (cent->currentState.origin, cent->lerpOrigin);
	copyv3 (cent->currentState.angles, cent->lerpAngles);
...
CG_SetInitialSnapshot(snapshot_t *snap)
{
	...
	BG_PlayerStateToEntityState(
		&snap->ps, &cg_entities[ snap->ps.clientNum ].currentState,
		qfalse);
...
CG_TransitionSnapshot(void)
{
	...
	BG_PlayerStateToEntityState(&cg.snap->ps,
		&cg_entities[ cg.snap->ps.clientNum ].currentState, qfalse);
...
CG_ProcessSnapshots(void)
{
	...
	do {
		...
		if(cg.time >= cg.snap->serverTime && cg.time <
