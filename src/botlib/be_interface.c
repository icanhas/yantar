...
/*	Swap_Init(); */
	...
	botlibglobals.maxentities = (int)LibVarValue("maxentities",
		"1024");
...
float BotGapVec3Vec3Distance(vec3_t origin, vec3_t hordir, int entnum);
...
/*	AAS_ClearShownDebugLines(); */
	...
	if(newarea != area){
		...
		botimport.Print(PRT_MESSAGE,
			...
			area, AAS_AreaCluster(
				area), AAS_PointPresenceType(origin));
 ...
 *      } */
	...
	VectorClear(forward);
	/* BotGapVec3Vec3Distance(origin, forward, 0); */
	/*
	 * if (parm0 & BUTTON_USE)
	 * {
	 *      botimport.Print(PRT_MESSAGE, "test rj from 703 to 716\n");
