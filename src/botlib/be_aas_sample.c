			...
			if(!(aasworld.areasettings[-nodenum].presencetype &
				...
				if(tstack_p->start[0] == start[0] &&
				   ...
				   tstack_p->start[2] == start[2]){
					...
					VectorSubtract(tstack_p->start, start,
						v2);
					trace.fraction = VectorLength(v2) /
							 VectorNormalize(v1);
					VectorMA(tstack_p->start, -0.125, v1,
						tstack_p->start);
				}
...
/*			VectorSubtract(end, start, v1); */
	 ...
	 * the node to test against */
		...
		{
			front =
				DotProduct(cur_start,
					plane->normal) - plane->dist;
			back =
				DotProduct(cur_end, plane->normal) - plane->dist;
			break;
		}	/* end default */
		}	/* end switch */
...
AAS_InsideFace(aas_face_t *face, vec3_t pnormal, vec3_t point, float epsilon)
{
	...
	for(i = 0; i < face->numedges; i++){
		...
		/* edge vector */
		VectorSubtract(aasworld.vertexes[edge->v[!firstvertex]], v0,
			edgevec);
		/*  */
#ifdef AAS_SAMPLE_DEBUG
 ...
 * =========================================================================== */
aas_link_t *
AAS_LinkEntityClientBBox(vec3_t absmins, vec3_t absmaxs, int entnum,
			 int presencetype)
{
	vec3_t	mins, maxs;
