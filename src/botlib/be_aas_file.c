...
AAS_SwapAASData(void)
{
	...
	for(i = 0; i < aasworld.numvertexes; i++)
		for(j = 0; j < 3; j++)
			aasworld.vertexes[i][j] =
				LittleFloat(aasworld.vertexes[i][j]);
	/* planes */
	for(i = 0; i < aasworld.numplanes; i++){
		for(j = 0; j < 3; j++)
			aasworld.planes[i].normal[j] = LittleFloat(
	...
	for(i = 0; i < aasworld.numfaces; i++){
		aasworld.faces[i].planenum = LittleLong(
			aasworld.faces[i].planenum);
		aasworld.faces[i].faceflags = LittleLong(
			aasworld.faces[i].faceflags);
		aasworld.faces[i].numedges = LittleLong(
			aasworld.faces[i].numedges);
		aasworld.faces[i].firstedge = LittleLong(
			aasworld.faces[i].firstedge);
		aasworld.faces[i].frontarea = LittleLong(
			aasworld.faces[i].frontarea);
		aasworld.faces[i].backarea = LittleLong(
			aasworld.faces[i].backarea);
	}
	/* face index */
	...
	for(i = 0; i < aasworld.numareas; i++){
		aasworld.areas[i].areanum = LittleLong(
			aasworld.areas[i].areanum);
		aasworld.areas[i].numfaces = LittleLong(
			aasworld.areas[i].numfaces);
		aasworld.areas[i].firstface = LittleLong(
			aasworld.areas[i].firstface);
		for(j = 0; j < 3; j++){
			aasworld.areas[i].mins[j] = LittleFloat(
				aasworld.areas[i].mins[j]);
			aasworld.areas[i].maxs[j] = LittleFloat(
				aasworld.areas[i].maxs[j]);
			aasworld.areas[i].center[j] = LittleFloat(
				aasworld.areas[i].center[j]);
		}
	}
	/* area settings */
	for(i = 0; i < aasworld.numareasettings; i++){
		aasworld.areasettings[i].contents = LittleLong(
			aasworld.areasettings[i].contents);
		aasworld.areasettings[i].areaflags = LittleLong(
			aasworld.areasettings[i].areaflags);
		aasworld.areasettings[i].presencetype = LittleLong(
			aasworld.areasettings[i].presencetype);
		aasworld.areasettings[i].cluster = LittleLong(
			aasworld.areasettings[i].cluster);
		aasworld.areasettings[i].clusterareanum = LittleLong(
			aasworld.areasettings[i].clusterareanum);
		aasworld.areasettings[i].numreachableareas = LittleLong(
			aasworld.areasettings[i].numreachableareas);
		aasworld.areasettings[i].firstreachablearea = LittleLong(
			aasworld.areasettings[i].firstreachablearea);
	}
	/* area reachability */
	for(i = 0; i < aasworld.reachabilitysize; i++){
		aasworld.reachability[i].areanum = LittleLong(
			aasworld.reachability[i].areanum);
		aasworld.reachability[i].facenum = LittleLong(
			aasworld.reachability[i].facenum);
		aasworld.reachability[i].edgenum = LittleLong(
			aasworld.reachability[i].edgenum);
		for(j = 0; j < 3; j++){
			aasworld.reachability[i].start[j] = LittleFloat(
				aasworld.reachability[i].start[j]);
			aasworld.reachability[i].end[j] = LittleFloat(
				aasworld.reachability[i].end[j]);
		}
		aasworld.reachability[i].traveltype = LittleLong(
			aasworld.reachability[i].traveltype);
		aasworld.reachability[i].traveltime = LittleShort(
			aasworld.reachability[i].traveltime);
	}
	/* nodes */
	for(i = 0; i < aasworld.numnodes; i++){
		aasworld.nodes[i].planenum = LittleLong(
			aasworld.nodes[i].planenum);
		aasworld.nodes[i].children[0] = LittleLong(
			aasworld.nodes[i].children[0]);
		aasworld.nodes[i].children[1] = LittleLong(
			aasworld.nodes[i].children[1]);
	}
	/* cluster portals */
	for(i = 0; i < aasworld.numportals; i++){
		aasworld.portals[i].areanum = LittleLong(
			aasworld.portals[i].areanum);
		aasworld.portals[i].frontcluster = LittleLong(
			aasworld.portals[i].frontcluster);
		aasworld.portals[i].backcluster = LittleLong(
			aasworld.portals[i].backcluster);
		aasworld.portals[i].clusterareanum[0] = LittleLong(
			aasworld.portals[i].clusterareanum[0]);
		aasworld.portals[i].clusterareanum[1] = LittleLong(
	...
	for(i = 0; i < aasworld.numclusters; i++){
		aasworld.clusters[i].numareas = LittleLong(
			aasworld.clusters[i].numareas);
		aasworld.clusters[i].numreachabilityareas = LittleLong(
			aasworld.clusters[i].numreachabilityareas);
		aasworld.clusters[i].numportals = LittleLong(
 ...
 * =========================================================================== */
char *
AAS_LoadAASLump(fileHandle_t fp, int offset, int length, int *lastoffset,
		int size)
{
	char *buf;
	/*  */
...
AAS_LoadAASFile(char *filename)
{
	...
	if(aasworld.numbboxes &&
	   !aasworld.bboxes) return BLERR_CANNOTREADAASLUMP;
	/* vertexes */
	...
	if(aasworld.numvertexes &&
	   !aasworld.vertexes) return BLERR_CANNOTREADAASLUMP;
	/* planes */
	...
	if(aasworld.numplanes &&
	   !aasworld.planes) return BLERR_CANNOTREADAASLUMP;
	/* edges */
	...
	if(aasworld.edgeindexsize &&
	   !aasworld.edgeindex) return BLERR_CANNOTREADAASLUMP;
	/* faces */
	...
	if(aasworld.faceindexsize &&
	   !aasworld.faceindex) return BLERR_CANNOTREADAASLUMP;
	/* convex areas */
	...
	if(aasworld.numareasettings &&
	   !aasworld.areasettings) return BLERR_CANNOTREADAASLUMP;
	/* reachability list */
	...
	if(aasworld.reachabilitysize &&
	   !aasworld.reachability) return BLERR_CANNOTREADAASLUMP;
	/* nodes */
	...
	if(aasworld.numportals &&
	   !aasworld.portals) return BLERR_CANNOTREADAASLUMP;
	/* cluster portal index */
	...
	if(aasworld.portalindexsize &&
	   !aasworld.portalindex) return BLERR_CANNOTREADAASLUMP;
	/* clusters */
	...
	if(aasworld.numclusters &&
	   !aasworld.clusters) return BLERR_CANNOTREADAASLUMP;
	/* swap everything */
	AAS_SwapAASData();
	/* aas file is loaded */
...
int
AAS_WriteAASLump(fileHandle_t fp, aas_header_t *h, int lumpnum, void *data,
		 int length)
{
	aas_lump_t *lump;
...
AAS_WriteAASFile(char *filename)
{
	...
	if(!AAS_WriteAASLump(fp, &header, AASLUMP_EDGEINDEX, aasworld.edgeindex,
		   aasworld.edgeindexsize *
		   sizeof(aas_edgeindex_t))) return qfalse;
	...
	if(!AAS_WriteAASLump(fp, &header, AASLUMP_FACEINDEX, aasworld.faceindex,
		   aasworld.faceindexsize *
		   sizeof(aas_faceindex_t))) return qfalse;
	...
	if(!AAS_WriteAASLump(fp, &header, AASLUMP_AREASETTINGS,
		   ...
		   aasworld.numareasettings *
		   sizeof(aas_areasettings_t))) return qfalse;
	if(!AAS_WriteAASLump(fp, &header, AASLUMP_REACHABILITY,
		   aasworld.reachability,
		   aasworld.reachabilitysize *
		   sizeof(aas_reachability_t))) return qfalse;
	...
	if(!AAS_WriteAASLump(fp, &header, AASLUMP_PORTALINDEX,
		   ...
		   aasworld.portalindexsize *
		   sizeof(aas_portalindex_t))) return qfalse;
