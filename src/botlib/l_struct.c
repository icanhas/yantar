...
ReadNumber(source_t *source, fielddef_t *fd, void *p)
{
	...
	if(token.type == TT_PUNCTUATION){
		...
		if(strcmp(token.string, "-")){
			SourceError(source, "unexpected punctuation %s",
				token.string);
			return 0;
		}	
		negative = qtrue;
	...
	if((fd->type & FT_TYPE) == FT_CHAR || (fd->type & FT_TYPE) == FT_INT){
		...
		if(intval < intmin || intval > intmax){
			SourceError(source, "value %ld out of range [%ld, %ld]",
				intval, intmin,
				intmax);
			return 0;
		}	
	...
	if((fd->type & FT_TYPE) == FT_CHAR){
		...
		if(fd->type & FT_UNSIGNED) *(unsigned int *) p =
				(unsigned int) intval;
...
ReadStructure(source_t *source, structdef_t *def, char *structure)
{
	...
	while(1){
		...
		if(!fd){
			SourceError(source, "unknown structure field %s",
				token.string);
			return qfalse;
		}	
		if(fd->type & FT_ARRAY){
...
WriteStructWithIndent(FILE *fp, structdef_t *def, char *structure, int indent)
{
	...
	for(i = 0; def->fields[i].name; i++){
		...
		while(num-- > 0){
			...
			case FT_STRUCT:
			{
				if(!WriteStructWithIndent(fp, fd->substruct,
					   structure, indent)) return qfalse;
				p = (char *) p + fd->substruct->size;
				break;
			}	/* end case */
			}	/* end switch */
			if(fd->type & FT_ARRAY){
				...
				else    if(fprintf(fp, "}") < 0) return qfalse;
			}	
		}		
		if(fprintf(fp, "\r\n") < 0) return qfalse;
	}	
