...
AAS_OptimizeFace(optimized_t *optimized, int facenum)
{
	...
	Q_Memcpy(optface, face, sizeof(aas_face_t));
	...
	for(i = 0; i < face->numedges; i++){
		...
		if(optedgenum){
			optimized->edgeindex[optface->firstedge +
...
AAS_OptimizeArea(optimized_t *optimized, int areanum)
{
	...
	Q_Memcpy(optarea, area, sizeof(aas_area_t));
	...
	for(i = 0; i < area->numfaces; i++){
		...
		if(optfacenum){
			optimized->faceindex[optarea->firstface +
...
AAS_OptimizeAlloc(optimized_t *optimized)
{
	...
	optimized->vertexoptimizeindex = (int*)GetClearedMemory(
		aasworld.numvertexes * sizeof(int));
