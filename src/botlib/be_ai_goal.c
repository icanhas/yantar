...
typedef struct levelitem_s {
	...
	Vec3	origin;		/* origin of the item */
	int	goalareanum;	/* area the item is in */
	Vec3	goalorigin;	/* goal origin within the area */
...
typedef struct iteminfo_s {
	...
	Vec3	mins;			/* mins of the item */
	Vec3	maxs;			/* maxs of the item */
...
LoadItemConfig(char *filename)
{
	...
	while(PC_ReadToken(source, &token)){
		if(!strcmp(token.string, "iteminfo")){
			...
			strncpy(ii->classname, token.string,
				sizeof(ii->classname)-1);
			if(!ReadStructure(source, &iteminfo_struct,
				   (char*)ii)){
				FreeMemory(ic);
				FreeSource(source);
				return NULL;
	...
	FreeSource(source);
	/*  */
	if(!ic->numiteminfo) botimport.Print(PRT_WARNING,
			"no item info loaded\n");
...
InitLevelItemHeap(void)
{
	...
	levelitemheap	= (levelitem_t*)GetClearedMemory(
		max_levelitems * sizeof(levelitem_t));
...
BotInitInfoEntities(void)
{
	...
	for(ent = AAS_NextBSPEntity(0); ent; ent = AAS_NextBSPEntity(ent)){
		...
		if(!strcmp(classname, "target_location")){
			ml =
				(maplocation_t*)GetClearedMemory(sizeof(
						maplocation_t));
			AAS_VectorForBSPEpairKey(ent, "origin", ml->origin);
			AAS_ValueForBSPEpairKey(ent, "message", ml->name,
				sizeof(ml->name));
		...
		/* camp spots */
			...
			/* cs->origin[2] += 16; */
			AAS_ValueForBSPEpairKey(ent, "message", cs->name,
				sizeof(cs->name));
...
BotInitLevelItems(void)
{
	...
	for(ent = AAS_NextBSPEntity(0); ent; ent = AAS_NextBSPEntity(ent)){
				...
				if(trace.fraction >= 1){
					...
					goalareanum =
						AAS_BestReachableFromJumpPadArea(
							...
							ic->iteminfo[i].maxs);
					Log_Write(
						"item %s reachable from jumppad area %d\r\n",
						ic->iteminfo[i].classname,
						goalareanum);
		...
		li->iteminfo = i;
		/* origin of the item */
		copyv3(origin, li->origin);
		/*  */
		if(goalareanum){
			li->goalareanum = goalareanum;
			copyv3(origin, li->goalorigin);
		}else{
...
BotGoalName(int number, char *name, int size)
{
	...
	for(li = levelitems; li; li = li->next)
		if(li->number == number){
			strncpy(name, itemconfig->iteminfo[li->iteminfo].name,
				size-1);
			name[size-1] = '\0';
			return;
		}
...
BotRemoveFromAvoidGoals(int goalstate, int number)
{
	...
	for(i = 0; i < MAX_AVOIDGOALS; i++)
		if(gs->avoidgoals[i] == number && gs->avoidgoaltimes[i] >=
		   AAS_Time()){
...
BotSetAvoidGoalTime(int goalstate, int number, float avoidtime)
{
	...
	if(avoidtime < 0){
		...
		for(li = levelitems; li; li = li->next)
			if(li->number == number){
				avoidtime =
					itemconfig->iteminfo[li->iteminfo].
					respawntime;
				if(!avoidtime)
...
BotGetLevelItemGoal(int index, char *name, bot_goal_t *goal)
{
	...
	for(; li; li = li->next){
		...
		}else if(li->flags & IFL_NOTFREE) continue;
		if(li->flags & IFL_NOTBOT) continue;
		/*  */
		if(!Q_stricmp(name, itemconfig->iteminfo[li->iteminfo].name)){
			goal->areanum = li->goalareanum;
			copyv3(li->goalorigin, goal->origin);
			goal->entitynum = li->entitynum;
			copyv3(itemconfig->iteminfo[li->iteminfo].mins,
				goal->mins);
			copyv3(itemconfig->iteminfo[li->iteminfo].maxs,
				goal->maxs);
...
BotGetMapLocationGoal(char *name, bot_goal_t *goal)
{
	...
	for(ml = maplocations; ml; ml = ml->next)
		if(!Q_stricmp(ml->name, name)){
			...
			copyv3(ml->origin, goal->origin);
			goal->entitynum = 0;
			copyv3(mins, goal->mins);
			copyv3(maxs, goal->maxs);
			return qtrue;
		}
	return qfalse;
...
BotFindEntityForLevelItem(levelitem_t *li)
{
	...
	for(ent = AAS_NextEntity(0); ent; ent = AAS_NextEntity(ent)){
		...
		if(ic->iteminfo[li->iteminfo].modelindex == modelindex){
			...
			subv3(li->origin, entinfo.origin, dir);
			if(lenv3(dir) < 30)
	...
	for(ent = AAS_NextEntity(0); ent; ent = AAS_NextEntity(ent)){
		...
		for(li = levelitems; li; li = li->next)
			...
			if(li->entitynum && li->entitynum == ent){
				...
				if(ic->iteminfo[li->iteminfo].modelindex !=
				   modelindex){
...
#ifdef DEBUG
					Log_Write(
						"linked item %s to an entity",
						ic->iteminfo[li->iteminfo].
						classname);
#endif	/* DEBUG */
					break;
				}
		...
		li->iteminfo = i;
		/* origin of the item */
		copyv3(entinfo.origin, li->origin);
		/* get the item goal area and goal origin */
 ...
 * =========================================================================== */
int
BotChooseLTGItem(int goalstate, Vec3 origin, int *inventory, int travelflags)
{
	...
	for(li = levelitems; li; li = li->next){
		...
		weight = FuzzyWeightUndecided(inventory, gs->itemweightconfig,
			weightnum);
#else
	 ...
	 * use weight scale for item_botroam */
		...
		if(weight > 0){
			...
			t =
				AAS_AreaTravelTimeToGoalArea(areanum, origin,
					li->goalareanum,
					travelflags);
			/* if the goal is reachable */
			if(t > 0){
				/* if this item won't respawn before we get there */
				avoidtime = BotAvoidGoalTime(goalstate,
					li->number);
	...
	if(!bestitem)
		 ...
		 *      {
		 *              setv3(goal.mins, -15, -15, -15);
		 *              setv3(goal.maxs, 15, 15, 15);
		 *              goal.entitynum = 0;
		 *              goal.number = 0;
		 *              goal.flags = GFL_ROAM;
		 *              goal.iteminfo = 0;
	...
	copyv3(bestitem->goalorigin, goal.origin);
	copyv3(iteminfo->mins, goal.mins);
	copyv3(iteminfo->maxs, goal.maxs);
 ...
 * =========================================================================== */
int
BotChooseNBGItem(int goalstate, Vec3 origin, int *inventory, int travelflags,
 ...
 * =========================================================================== */
int
BotTouchingGoal(Vec3 origin, bot_goal_t *goal)
{
	int i;
	Vec3	boxmins, boxmaxs;
	Vec3	absmins, absmaxs;
	Vec3	safety_maxs = {0, 0, 0};	/* {4, 4, 10}; */
	Vec3	safety_mins = {0, 0, 0};	/* {-4, -4, 0}; */

	AAS_PresenceTypeBoundingBox(PRESENCE_NORMAL, boxmins, boxmaxs);
	subv3(goal->mins, boxmaxs, absmins);
	subv3(goal->maxs, boxmins, absmaxs);
	addv3(absmins, goal->origin, absmins);
	addv3(absmaxs, goal->origin, absmaxs);
	/* make the box a little smaller for safety */
	subv3(absmaxs, safety_maxs, absmaxs);
	subv3(absmins, safety_mins, absmins);
 ...
 * =========================================================================== */
int
BotItemGoalInVisButNotVisible(int viewer, Vec3 eye, Vec3 viewangles,
			      bot_goal_t *goal)
{
	aas_entityinfo_t entinfo;
	bsp_trace_t trace;
	...
	/*  */
	addv3(goal->mins, goal->mins, middle);
	scalev3(middle, 0.5, middle);
	addv3(goal->origin, middle, middle);
	/*  */
...
BotAllocGoalState(int client)
{
	...
	for(i = 1; i <= MAX_CLIENTS; i++)
		if(!botgoalstates[i]){
			botgoalstates[i] =
				GetClearedMemory(sizeof(bot_goalstate_t));
