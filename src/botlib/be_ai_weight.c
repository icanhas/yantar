...
ReadFuzzySeperators_r(source_t *source)
{
	...
	do {
		...
		if(def || !strcmp(token.string, "case")){
			fs =
				(fuzzyseperator_t*)GetClearedMemory(sizeof(
						fuzzyseperator_t));
			fs->index = index;
			...
			if(def){
				...
				if(!PC_ExpectTokenType(source, TT_NUMBER,
					   TT_INTEGER, &token)){
					FreeFuzzySeperators_r(firstfs);
					return NULL;
				}
	...
	while(PC_ReadToken(source, &token)){
		if(!strcmp(token.string, "weight")){
			...
			config->weights[config->numweights].name =
				(char*)GetClearedMemory(strlen(
						token.string) + 1);
			strcpy(config->weights[config->numweights].name,
				token.string);
			if(!PC_ExpectAnyToken(source, &token)){
				FreeWeightConfig(config);
				FreeSource(source);
			...
			if(!strcmp(token.string, "switch")){
				...
				config->weights[config->numweights].
				firstseperator = fs;
...
WriteFuzzySeperators_r(FILE *fp, fuzzyseperator_t *fs, int indent)
{
	...
	do {
		...
		}else if(fprintf(fp, "default:") < 0) return qfalse;
		if(fs->child){
			if(fprintf(fp, "\n") < 0) return qfalse;
			if(!WriteIndent(fp, indent)) return qfalse;
			if(fprintf(fp, "{\n") < 0) return qfalse;
			if(!WriteFuzzySeperators_r(fp, fs->child, indent +
				   1)) return qfalse;
			if(!WriteIndent(fp, indent)) return qfalse;
			if(fs->next){
				if(fprintf(fp,
					 ...
					 "} //end default\n") <
				 0) return qfalse;
		}else if(!WriteFuzzyWeight(fp, fs)) return qfalse;
		fs = fs->next;
	} while(fs);
	indent--;
	if(!WriteIndent(fp, indent)) return qfalse;
...
WriteWeightConfig(char *filename, weightconfig_t *config)
{
	...
	for(i = 0; i < config->numweights; i++){
		...
		if(ifw->firstseperator->index > 0){
			if(!WriteFuzzySeperators_r(fp, ifw->firstseperator,
				   1)) return qfalse;
		}else{
			if(!WriteIndent(fp, 1)) return qfalse;
			if(!WriteFuzzyWeight(fp,
				   ifw->firstseperator)) return qfalse;
		}
...
FuzzyWeight_r(int *inventory, fuzzyseperator_t *fs)
{
	...
	if(inventory[fs->index] < fs->value){
		...
		if(inventory[fs->index] < fs->next->value){
			...
			/* second weight */
			if(fs->next->child) w2 = FuzzyWeight_r(inventory,
					fs->next->child);
...
FuzzyWeightUndecided_r(int *inventory, fuzzyseperator_t *fs)
{
	...
	if(inventory[fs->index] < fs->value){
		...
		else return fs->minweight + random() *
			       (fs->maxweight - fs->minweight);
	}else if(fs->next){
		if(inventory[fs->index] < fs->next->value){
			/* first weight */
			if(fs->child) w1 = FuzzyWeightUndecided_r(inventory,
					fs->child);
			else w1 = fs->minweight + random() *
				  (fs->maxweight - fs->minweight);
			/* second weight */
			if(fs->next->child) w2 = FuzzyWeight_r(inventory,
					fs->next->child);
			else w2 = fs->next->minweight + random() *
...
FuzzyWeightUndecided(int *inventory, weightconfig_t *wc, int weightnum)
{
	...
	while(1){
		if(inventory[s->index] < s->value){
			if(s->child) s = s->child;
			else return s->minweight + random() *
				       (s->maxweight - s->minweight);
		}else{
			if(s->next) s = s->next;
			else return s->minweight + random() *
				       (s->maxweight - s->minweight);
...
EvolveFuzzySeperator_r(fuzzyseperator_t *fs)
{
	if(fs->child)
		...
		if(random() < 0.01) fs->weight += crandom() *
						  (fs->maxweight - fs->minweight);
		else fs->weight += crandom() *
				   (fs->maxweight - fs->minweight) * 0.5;
...
ScaleWeight(weightconfig_t *config, char *name, float scale)
{
	...
	for(i = 0; i < config->numweights; i++)
		if(!strcmp(name, config->weights[i].name)){
			ScaleFuzzySeperator_r(config->weights[i].firstseperator,
				scale);
			break;
		}
			   ...
			   fuzzyseperator_t *fsout)
{
	if(fs1->child){
		...
		if(!InterbreedFuzzySeperator_r(fs2->child, fs2->child,
			   fsout->child))
			return qfalse;
		...
		if(fsout->weight > fsout->maxweight) fsout->maxweight =
				fsout->weight;
		if(fsout->weight > fsout->minweight) fsout->minweight =
				fsout->weight;
	}	/* end else if */
	if(fs1->next){
