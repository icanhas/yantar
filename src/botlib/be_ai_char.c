...
BotDefaultCharacteristics(bot_character_t *ch, bot_character_t *defaultch)
{
	...
	for(i = 0; i < MAX_CHARACTERISTICS; i++){
		...
		if(defaultch->c[i].type == CT_FLOAT){
			...
			ch->c[i].value.string =
				(char*)GetMemory(strlen(defaultch->c[i].value.
						string)+1);
...
BotFindCachedCharacter(char *charfile, float skill)
{
	...
	for(handle = 1; handle <= MAX_CLIENTS; handle++){
		...
		if(strcmp(botcharacters[handle]->filename, charfile) == 0 &&
		   (skill < 0 || fabs(botcharacters[handle]->skill - skill) <
		    0.01))
			return handle;
	}
	return 0;
...
BotLoadCharacterSkill(char *charfile, float skill)
{
	...
	if(defaultch && ch)
		BotDefaultCharacteristics(botcharacters[ch],
