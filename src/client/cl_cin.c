...
CIN_HandleForVideo(void)
{
	...
	for(i = 0; i < MAX_VIDEO_HANDLES; i++)
		if(cinTable[i].fileName[0] == 0)
			return i;
	Q_Error(ERR_DROP, "CIN_HandleForVideo: none free");
	return -1;
}
...
RoQPrepMcomp(long xoff, long yoff)
{
	...
	for(y=0; y<16; y++){
		...
		for(x=0; x<16; x++){
			...
			cin.mcomp[(x*
				   16)+
				  y] = cinTable[currentHandle].normalBuffer0-
				       (temp2+temp);
		}
	}
}
 ...
 *  */
	...
	if(cinTable[currentHandle].RoQFrameSize>65536||
	   cinTable[currentHandle].roq_id==0x1084){
		Q_DPrintf("roq_size>65536||roq_id==0x1084\n");
		cinTable[currentHandle].status = FMV_EOF;
		if(cinTable[currentHandle].looping)
			RoQReset();
		return;
...
CIN_PlayCinematic(const char *arg, int x, int y, int w, int h, int systemBits)
{
	...
	RoQID = (unsigned short)(cin.file[0]) + (unsigned short)(cin.file[1])*
		256;
	if(RoQID == 0x1084){
		RoQ_init();
