...
cvar_t *s_alMinvec3dist;
cvar_t *s_alMaxvec3dist;
cvar_t *s_alRolloff;
cvar_t *s_alGracevec3dist;
cvar_t *s_alDriver;
cvar_t *s_alDevice;
...
static qbool enumeration_ext = qfalse;
static qbool enumeration_all_ext = qfalse;
#ifdef USE_VOIP
static qbool capture_ext = qfalse;
#endif
...
typedef struct alSfx_s {
	...
	qbool		isDefaultChecked;	/* Sound has been check if it isDefault */
 ...
 */
static void
S_AL_BufferLoad(sfxHandle_t sfx, qbool cache)
{
	ALenum	error;
	ALuint	format;
	...
	if(info.size == 0){
		...
		qalBufferData(curSfx->buffer, AL_FORMAT_MONO16,
			(void*)dummyData, 2,
			22050);
	}else
...
S_AL_BufferInit(void)
{
	...
	default_sfx = S_AL_BufferFind(Psound "/default");
...
static
sfxHandle_t
S_AL_RegisterSound(const char *sample, qbool compressed)
{
...
typedef struct src_s {
	...
	qbool		isStream;	/* Is this source a stream */
	...
	Vec3		loopSpeakerPos;	/* Origin of the loop speaker */
...
static Vec3	lastListenerOrigin = { 0.0f, 0.0f, 0.0f };
...
static void
_S_AL_SanitiseVector(Vec3 v, int line)
{
...
static void
S_AL_ScaleGain(src_t *chksrc, Vec3 origin)
{
	float distance;

	if(!chksrc->local)
		distance = vec3dist(origin, lastListenerOrigin);

	/* If we exceed a certain distance, scale the gain linearly until the sound
	 * vanishes into nothingness. */
	if(!chksrc->local && (distance -= s_alMaxvec3dist->value) > 0){
		float scaleFactor;

		if(distance >= s_alGracevec3dist->value)
			scaleFactor = 0.0f;
		else
			scaleFactor = 1.0f - distance / s_alGracevec3dist->value;
...
static void
S_AL_NewLoopMaster(src_t *rmSource, qbool iskilled)
{
	int index;
			...
			if(!curSfx->loopActiveCnt){
				...
				if(rmSource->isPlaying)
					...
					curSource->lastTimePos =
						rmSource->lastTimePos;
					curSource->lastSampleTime =
						rmSource->lastSampleTime;
				}
			}
		}
	}else
...
S_AL_SrcAlloc(alSrcPriority_t priority, int entnum, int channel)
{
	...
	for(i = 0; i < srcCount; i++){
		...
		if(curSource->isPlaying){
			if(weakest_isplaying && curSource->priority <
			   priority &&
		...
		if((curSource->entity == entnum) &&
		   (curSource->channel == channel) && (channel != 0)){
			S_AL_SrcKill(i);
			return i;
		}
#endif
...
S_AL_SrcFind(int entnum, int channel)
{
	...
	for(i = 0; i < srcCount; i++){
		if(!srcList[i].isActive)
			continue;
		if((srcList[i].entity == entnum) &&
		   (srcList[i].channel == channel))
			return i;
	}
	return -1;
}
...
static
void
S_AL_UpdateEntityPosition(int entityNum, const Vec3 origin)
{
	Vec3 sanOrigin;
 ...
 */
static void
S_AL_StartSound(Vec3 origin, int entnum, int entchannel, sfxHandle_t sfx)
{
	Vec3 sorigin;
	srcHandle_t src;
	src_t *curSource;
	...
	if(origin){
		...
		vec3copy(entityList[entnum].origin, sorigin);
	}

	S_AL_SanitiseVector(sorigin);

	if((srcActiveCnt > 5 * srcCount / 3) &&
	   (vec3distsquared(sorigin, lastListenerOrigin) >=
	    (s_alMaxvec3dist->value +
	     s_alGracevec3dist->value) *
	    (s_alMaxvec3dist->value + s_alGracevec3dist->value)))
...
static
void
S_AL_ClearLoopingSounds(qbool killall)
{
	int i;
	for(i = 0; i < srcCount; i++)
		if((srcList[i].isLooping) && (srcList[i].entity != -1))
			entityList[srcList[i].entity].loopAddedThisFrame =
				qfalse;
}
...
S_AL_SrcLoop(alSrcPriority_t priority, sfxHandle_t sfx,
	     const Vec3 origin, const Vec3 velocity, int entityNum)
{
	int src;
	sentity_t	*sent = &entityList[ entityNum ];
	src_t		*curSource;
	Vec3		sorigin, svelocity;
	...
	if(S_AL_HearingThroughEntity(entityNum)){
		...
		if(origin)
			...
			vec3copy(sent->origin, sorigin);

		S_AL_SanitiseVector(sorigin);

		vec3copy(sorigin, curSource->loopSpeakerPos);

		if(velocity){
			vec3copy(velocity, svelocity);
		...
		qalSourcefv(curSource->alSource, AL_POSITION,
			(ALfloat*)sorigin);
		qalSourcefv(curSource->alSource, AL_VELOCITY,
			(ALfloat*)svelocity);
	}
}
 ...
 */
static void
S_AL_AddLoopingSound(int entityNum, const Vec3 origin, const Vec3 velocity,
		     sfxHandle_t sfx)
{
 ...
 */
static void
S_AL_AddRealLoopingSound(int entityNum, const Vec3 origin,
			 const Vec3 velocity,
			 sfxHandle_t sfx)
{
...
S_AL_SrcUpdate(void)
{
	...
	for(i = 0; i < srcCount; i++){
		...
		if(!curSource->isStream){
			...
			qalGetSourcei(curSource->alSource, AL_SOURCE_STATE,
				&state);
			if(state == AL_STOPPED){
				curSource->isPlaying = qfalse;
				S_AL_SrcKill(i);
...
S_AL_MusicProcess(ALuint b)
{
	...
	if(l == 0){
		...
		l = S_CodecReadStream(curstream, MUSIC_BUFFER_SIZE,
			decode_buffer);
	}
...
static
void
S_AL_Respatialize(int entityNum, const Vec3 origin, Vec3 axis[3],
		  int inwater)
{
	float	orientation[6];
	Vec3	sorigin;
	...
	orientation[0]	= axis[0][0]; orientation[1] = axis[0][1];
	orientation[2]	= axis[0][2];
	orientation[3]	= axis[2][0]; orientation[4] = axis[2][1];
	orientation[5]	= axis[2][2];

	vec3copy(sorigin, lastListenerOrigin);
...
S_AL_Update(void)
{
	...
	s_alMinvec3dist->modified = qfalse;
...
S_AL_AvailableCaptureSamples(void)
{
	...
	if(alCaptureDevice != NULL){
		ALint samples = 0;
		qalcGetIntegerv(alCaptureDevice, ALC_CAPTURE_SAMPLES,
...
S_AL_Init(soundInterface_t *si)
{
	...
	s_alDopplerFactor = Cvar_Get("s_alDopplerFactor", "1.0",
		CVAR_ARCHIVE);
	s_alDopplerSpeed = Cvar_Get("s_alDopplerSpeed", "2200",
		CVAR_ARCHIVE);
	s_alMinvec3dist = Cvar_Get("s_alMinvec3dist", "120", CVAR_CHEAT);
	s_alMaxvec3dist = Cvar_Get("s_alMaxvec3dist", "1024", CVAR_CHEAT);
	s_alRolloff = Cvar_Get("s_alRolloff", "2", CVAR_CHEAT);
	s_alGracevec3dist = Cvar_Get("s_alGracevec3dist", "512", CVAR_CHEAT);

	s_alDriver = Cvar_Get("s_alDriver", ALDRIVER_DEFAULT,
		CVAR_ARCHIVE | CVAR_LATCH);

	s_alInputDevice = Cvar_Get("s_alInputDevice", "",
		CVAR_ARCHIVE | CVAR_LATCH);
	...
	enumeration_all_ext = qalcIsExtensionPresent(NULL,
	...
	if(enumeration_ext || enumeration_all_ext){
		...
		if(enumeration_all_ext){
			devicelist = qalcGetString(NULL,
				ALC_ALL_DEVICES_SPECIFIER);
#ifdef _WIN32
			defaultdevice = qalcGetString(
	...
	qalvec3distModel(AL_INVERSE_DISTANCE_CLAMPED);
	qalDopplerFactor(s_alDopplerFactor->value);
	qalDopplerVelocity(s_alDopplerSpeed->value);
	...
	if(!s_alCapture->integer)
		...
		if(!qalcIsExtensionPresent(NULL, "ALC_EXT_capture"))
			...
			inputdevicelist = qalcGetString(
				NULL, ALC_CAPTURE_DEVICE_SPECIFIER);
			defaultinputdevice = qalcGetString(
				NULL, ALC_CAPTURE_DEFAULT_DEVICE_SPECIFIER);

			/* dump a list of available devices to a cvar for the user to see. */
			while((curlen = strlen(inputdevicelist))){
				Q_strcat(inputdevicenames,
			...
			s_alAvailableInputDevices = Cvar_Get(
				"s_alAvailableInputDevices", inputdevicenames,
				CVAR_ROM |
				CVAR_NORESTART);
