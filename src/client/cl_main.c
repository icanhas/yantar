...
CL_CaptureVoip(void)
{
	...
	if(initialFrame){
		S_MasterGain(Com_Clamp(0.0f, 1.0f,
...
CL_CheckForResend(void)
{
	...
	case CA_CONNECTING:
		...
		Com_sprintf(data, sizeof(data), "getchallenge %d %s",
			clc.challenge,
			com_gamename->string);
...
CL_ServerStatus(char *serverAddress, char *serverStatusString, int maxLen)
{
	...
	if( NET_CompareAdr(to, serverStatus->address)){
		...
		else if( serverStatus->startTime < Com_Milliseconds() -
