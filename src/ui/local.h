...
typedef struct {
	...
	char			foundPlayerServerAddresses[
		MAX_FOUNDPLAYER_SERVERS][MAX_ADDRESSLENGTH];
	char			foundPlayerServerNames[MAX_FOUNDPLAYER_SERVERS][
		MAX_ADDRESSLENGTH];
...
int                             trap_FS_GetFileList(const char *path,
						    ...
						    char *listbuf,
						    int bufsize);
int                             trap_FS_Seek(fileHandle_t f, long offset,
...
void                    trap_LAN_MarkServerVisible(int source, int n,
						   qbool visible);
int                             trap_LAN_ServerIsVisible(int source, int n);
qbool                trap_LAN_UpdateVisiblePings(int source);
int                             trap_LAN_AddServer(int source, const char *name,
						   const char *addr);
...
int                             trap_LAN_ServerStatus(const char *serverAddress,
						      char *serverStatus,
						      int maxLen);
int                             trap_LAN_CompareServers(int source, int sortKey,
							int sortDir, int s1,
							int s2);
...
void                    trap_CIN_SetExtents(int handle, int x, int y, int w,
					    int h);
