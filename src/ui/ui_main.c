...
AssetCache(void)
{
	...
	uiInfo.uiDC.Assets.fxPic[1]	= trap_R_RegisterShaderNoMip(
		ART_FX_YELLOW);
	uiInfo.uiDC.Assets.fxPic[2]	= trap_R_RegisterShaderNoMip(
		ART_FX_GREEN);
	...
	uiInfo.uiDC.Assets.fxPic[6]	= trap_R_RegisterShaderNoMip(
		ART_FX_WHITE);
	uiInfo.uiDC.Assets.scrollBar	= trap_R_RegisterShaderNoMip(
		ASSET_SCROLLBAR);
	...
	uiInfo.newHighScoreSound = trap_S_RegisterSound(
		"sound/feedback/voc_newhighscore.wav", qfalse);
}
...
void
Text_PaintChar(float x, float y, float width, float height, float scale, float s,
	       float t, float s2, float t2,
	       qhandle_t hShader)
{
	float w, h;
	w	= width * scale;
...
Asset_Parse(int handle)
{
	...
	while( 1 ){
		...
		if(Q_stricmp(token.string, "menuEnterSound") == 0){
			if(!PC_String_Parse(handle, &tempStr))
				return qfalse;
			uiInfo.uiDC.Assets.menuEnterSound = trap_S_RegisterSound(
				tempStr, qfalse);
			continue;
		}
		...
		if(Q_stricmp(token.string, "menuExitSound") == 0){
			if(!PC_String_Parse(handle, &tempStr))
				return qfalse;
			uiInfo.uiDC.Assets.menuExitSound = trap_S_RegisterSound(
				tempStr, qfalse);
			continue;
		}
		...
		if(Q_stricmp(token.string, "itemFocusSound") == 0){
			if(!PC_String_Parse(handle, &tempStr))
				return qfalse;
			uiInfo.uiDC.Assets.itemFocusSound = trap_S_RegisterSound(
				tempStr, qfalse);
			continue;
		}
		...
		if(Q_stricmp(token.string, "menuBuzzSound") == 0){
			if(!PC_String_Parse(handle, &tempStr))
				return qfalse;
			uiInfo.uiDC.Assets.menuBuzzSound = trap_S_RegisterSound(
				tempStr, qfalse);
			continue;
		}
		...
		if(Q_stricmp(token.string, "fadeAmount") == 0){
			if(!PC_Float_Parse(handle,
				   &uiInfo.uiDC.Assets.fadeAmount))
				return qfalse;
			continue;
		}
		...
		if(Q_stricmp(token.string, "shadowColor") == 0){
			if(!PC_Color_Parse(handle,
				   &uiInfo.uiDC.Assets.shadowColor))
				return qfalse;
...
{"None","95","90","85","80","75","70","65","60","55","50","45","40","35","30",
 "25","20","15","10","5",NULL};
#ifndef MISSIONPACK
...
UI_DrawClanLogo(rectDef_t *rect, float scale, vec4_t color)
{
	...
	if(i >= 0 && i < uiInfo.teamCount){
		...
		if(uiInfo.teamList[i].teamIcon == -1){
			uiInfo.teamList[i].teamIcon = trap_R_RegisterShaderNoMip(
				uiInfo.teamList[i].imageName);
			uiInfo.teamList[i].teamIcon_Metal =
				trap_R_RegisterShaderNoMip(va("%s_metal",
						uiInfo.teamList[i].imageName));
		...
		if(uiInfo.teamList[i].cinematic >= -2){
			...
			if(uiInfo.teamList[i].cinematic >= 0){
				trap_CIN_RunCinematic(
					uiInfo.teamList[i].cinematic);
				trap_CIN_SetExtents(uiInfo.teamList[i].cinematic,
	...
	if(uiInfo.previewMovie > -2){
		uiInfo.previewMovie =
			trap_CIN_PlayCinematic(va(
					"%s.roq",
					uiInfo.movieList[uiInfo.movieIndex]), 0,
				0, 0, 0,
				(CIN_loop | CIN_silent));
		...
		int textStyle)
{
	int i;
	i =
		UI_TeamIndexFromName(UI_Cvar_VariableString((blue) ?
		  ...
		  int textStyle)
{
	 ...
	 * 2..NumCharacters - Bot */
	int value =
		trap_Cvar_VariableValue(va(blue ? "ui_blueteam%i" :
				"ui_redteam%i", num));
	const char *text;
	if(value <= 0)
		text = "Closed";
...
UI_DrawEffects(rectDef_t *rect, float scale, vec4_t color)
{
	...
	UI_DrawHandlePic(rect->x + uiInfo.effectsColor * 16 + 8, rect->y - 16,
		16, 12,
...
{
	int	i =
		UI_TeamIndexFromName(UI_Cvar_VariableString("ui_opponentName"));
	int	j =
			...
			qboolean net)
{
	...
	if(map >= 0 && map < uiInfo.mapCount)
		Text_Paint(rect->x, rect->y, scale, color,
			uiInfo.mapList[map].mapName, 0, 0,
			textStyle);
}
...
UI_OwnerDrawWidth(int ownerDraw, float scale)
{
	...
	case UI_BLUETEAM5:
		value =
			trap_Cvar_VariableValue(va("ui_blueteam%i", ownerDraw-
					UI_BLUETEAM1 + 1));
		if(value <= 0)
			text = "Closed";
		else if(value == 1)
	...
	case UI_REDTEAM5:
		value =
			trap_Cvar_VariableValue(va("ui_redteam%i", ownerDraw-
					UI_REDTEAM1 + 1));
		if(value <= 0)
			text = "Closed";
		else if(value == 1)
...
UI_DrawSelectedPlayer(rectDef_t *rect, float scale, vec4_t color, int textStyle)
{
	...
	Text_Paint(
		...
		(uiInfo.teamLeader) ? UI_Cvar_VariableString(
			"cg_selectedPlayerName") : UI_Cvar_VariableString(
			"name"), 0, 0, textStyle);
}
			 ...
			 int textStyle)
{
	if(uiInfo.serverStatus.refreshActive){
		...
		LerpColor(color,lowLight,newColor,0.5+0.5*
			sin(uiInfo.uiDC.realTime / PULSE_DIVISOR));
		Text_Paint(rect->x, rect->y, scale, newColor,
			va("Getting info for %d servers (ESC to cancel)",
				trap_LAN_GetServerCount(
					UI_SourceForLAN())), 0, 0, textStyle);
	}else{
		char buff[64];
		Q_strncpyz(buff,
			UI_Cvar_VariableString(va("ui_lastServerRefresh_%i",
...
UI_DrawServerMOTD(rectDef_t *rect, float scale, vec4_t color)
{
	if(uiInfo.serverStatus.motdLen){
		...
		Text_Paint_Limit(
			&maxX, uiInfo.serverStatus.motdPaintX, rect->y +
			rect->h - 3,
			scale, color,
		...
		if(uiInfo.serverStatus.motdOffset && maxX > 0){
			...
			if(uiInfo.serverStatus.motdPaintX2 == -1)
				uiInfo.serverStatus.motdPaintX2 = rect->x +
								  rect->w - 2;
		}else
			uiInfo.serverStatus.motdPaintX2 = -1;
...
UI_DrawGLInfo(rectDef_t *rect, float scale, vec4_t color, int textStyle)
{
	...
	Text_Paint(rect->x + 2, rect->y, scale, color,
		va("VENDOR: %s",
			uiInfo.uiDC.glconfig.vendor_string), 0, 30, textStyle);
	     ...
	     int textStyle)
{
	...
	case UI_BLUETEAM4:
	case UI_BLUETEAM5:
		UI_DrawTeamMember(&rect, scale, color, qtrue, ownerDraw -
			UI_BLUETEAM1 + 1,
			textStyle);
		break;
	case UI_REDTEAM1:
	...
	case UI_REDTEAM4:
	case UI_REDTEAM5:
		UI_DrawTeamMember(&rect, scale, color, qfalse, ownerDraw -
			UI_REDTEAM1 + 1,
			textStyle);
		break;
	case UI_NETSOURCE:
...
UI_OwnerDrawVisible(int flags)
{
	...
	while(flags){
		...
		if(flags & UI_SHOW_NETANYTEAMGAME){
			if(uiInfo.gameTypes[ui_netGameType.integer].gtEnum <=
			   GT_TEAM )
				vis = qfalse;
			...
			int num)
{
	if(key == K_MOUSE1 || key == K_MOUSE2 || key == K_ENTER || key ==
	   K_KP_ENTER){
		 ...
		 * 1 - Human
		 * 2..NumCharacters - Bot */
		char	*cvar = va(blue ? "ui_blueteam%i" : "ui_redteam%i",
			num);
...
UI_Crosshair_HandleKey(int flags, float *special, int key)
{
	if(key == K_MOUSE1 || key == K_MOUSE2 || key == K_ENTER || key ==
	   K_KP_ENTER){
		...
		trap_Cvar_Set("cg_drawCrosshair",
			va("%d", uiInfo.currentCrosshair));
		return qtrue;
	}
	return qfalse;
}
...
UI_LoadMods(void)
{
	...
	for( i = 0; i < numdirs; i++ ){
		...
		uiInfo.modList[uiInfo.modCount].modName		= String_Alloc(
			dirptr);
...
UI_RunMenuScript(char **args)
{
	...
	if(String_Parse(args, &name)){
		if(Q_stricmp(name, "StartServer") == 0){
			...
			UI_SetCapFragLimits(qtrue);
			UI_MapCountByGameType(qtrue);
			ui_mapIndex.integer = UI_GetIndexFromSelection(
			...
			if(trap_VerifyCDKey(buff,
				   UI_Cvar_VariableString("cdkeychecksum"))){
			...
			if(uiInfo.demoAvailable)
				trap_Cmd_ExecuteText(EXEC_APPEND,
					va("demo %s_%i\n",
						uiInfo.mapList[ui_currentMap.
							       integer].
						mapLoadName,
						uiInfo.gameTypes[ui_gameType.
								 integer].gtEnum));
			...
			if(uiInfo.serverStatus.currentServer >= 0 &&
			   ...
			   uiInfo.serverStatus.numDisplayServers){
				trap_LAN_GetServerAddressString(
					UI_SourceForLAN(),
					uiInfo.serverStatus.displayServers[
						uiInfo.
						serverStatus
						.currentServer], buff, 1024);
				trap_Cmd_ExecuteText(EXEC_APPEND,
					va("connect %s\n", buff));
			}
			...
			if(uiInfo.playerIndex >= 0 && uiInfo.playerIndex <
			   uiInfo.playerCount)
				trap_Cmd_ExecuteText(EXEC_APPEND,
					va("callvote kick %s\n",
			...
			if(uiInfo.teamIndex >= 0 && uiInfo.teamIndex <
			   uiInfo.myTeamCount)
				trap_Cmd_ExecuteText(EXEC_APPEND,
					va("callteamvote leader %s\n",
						uiInfo.teamNames[uiInfo.
								 teamIndex]));
		}else if(Q_stricmp(name, "addBot") == 0){
			if(trap_Cvar_VariableValue("g_gametype") >= GT_TEAM)
				trap_Cmd_ExecuteText(EXEC_APPEND,
					va("addbot %s %i %s\n",
						uiInfo.characterList[uiInfo.
								     botIndex].
						name,
						uiInfo.skillIndex+1,
						(uiInfo.redBlue ==
			...
			if(String_Parse(args, &orders)){
				int selectedPlayer = trap_Cvar_VariableValue(
					"cg_selectedPlayer");
				if(selectedPlayer < uiInfo.myTeamCount){
					strcpy(buff, orders);
					trap_Cmd_ExecuteText(EXEC_APPEND,
						va(buff,
							uiInfo.teamClientNums[
								selectedPlayer]));
					trap_Cmd_ExecuteText(EXEC_APPEND, "\n");
				}else{
					int i;
...
UI_MapCountByGameType(qboolean singlePlayer)
{
	...
	c = 0;
	game =
		singlePlayer ? uiInfo.gameTypes[ui_gameType.integer].gtEnum :
		uiInfo.
	...
	if(!init){
		for(i = 0; i < uiInfo.characterCount; i++){
			...
			for(j = 0; j < uiInfo.teamCount; j++)
				if(UI_hasSkinForBase(uiInfo.characterList[i].
					   base, uiInfo.teamList[j].teamName))
		...
		for(j = 0; j < TEAM_MEMBERS; j++){
			if(uiInfo.teamList[tIndex].teamMembers[j] != NULL)
				if(uiInfo.characterList[i].reference&
				   (1<<tIndex)){	/* && Q_stricmp(uiInfo.teamList[tIndex].teamMembers[j], uiInfo.characterList[i].name)==0) { */
	...
	for(j = 0; j < TEAM_MEMBERS; j++)
		for(k = 0; k < uiInfo.aliasCount; k++){
			if(uiInfo.aliasList[k].name != NULL)
				if(Q_stricmp(uiInfo.teamList[tIndex].teamMembers
					   [j], uiInfo.aliasList[k].name)==0){
					for(i = 0; i < uiInfo.characterCount;
						...
						if(uiInfo.characterList[i].
						   ...
						   reference
						   &(1<<tIndex) &&
						   Q_stricmp(uiInfo.aliasList[k]
							   .ai,
							   uiInfo.characterList[
								   i].name)==
						   0){
							if(uiInfo.characterList[
								   i].active ==
							   qfalse){
								uiInfo.
								characterList[i]
...
/*	qboolean startRefresh = qtrue; TTimo: unused */
	...
	if(count == -1 || (ui_netSource.integer == UIAS_LOCAL && count == 0)){
		...
		uiInfo.serverStatus.numDisplayServers	= 0;
		uiInfo.serverStatus.numPlayersOnServers = 0;
		uiInfo.serverStatus.nextDisplayRefresh	= uiInfo.uiDC.realTime +
							  500;
		return;
	}
	...
	for(i = 0; i < count; i++){
		...
		if(ping > 0 || ui_netSource.integer == UIAS_FAVORITES){
			...
			trap_LAN_GetServerInfo(lanSource, i, info,
				MAX_STRING_CHARS);
			...
			if(ui_browserShowEmpty.integer == 0)
				if(clients == 0){
					trap_LAN_MarkServerVisible(lanSource, i,
						qfalse);
					continue;
				}
			...
			if(uiInfo.joinGameTypes[ui_joinGameType.integer].gtEnum
			   != -1){
				game = atoi(Info_ValueForKey(info, "gametype"));
				if(game !=
				   uiInfo.joinGameTypes[ui_joinGameType.integer]
				   .
				   gtEnum){
					trap_LAN_MarkServerVisible(lanSource, i,
						qfalse);
			...
			if(ui_serverFilterType.integer > 0)
				if(Q_stricmp(Info_ValueForKey(info,
						   "game"),
					   serverFilters[ui_serverFilterType.
							 integer].
					   basedir) != 0){
					trap_LAN_MarkServerVisible(lanSource, i,
						qfalse);
	...
	for(i = 0; serverStatusCvars[i].name; i++)
		for(j = 0; j < info->numLines; j++){
			if( !info->lines[j][1] || info->lines[j][1][0] )
				continue;
			if( !Q_stricmp(serverStatusCvars[i].name,
				    info->lines[j][0])){
				/* swap lines */
...
UI_GetServerStatusInfo(const char *serverAddress, serverStatusInfo_t *info)
{
	...
	if( trap_LAN_ServerStatus(serverAddress, info->text,
		    sizeof(info->text))){
...
UI_BuildFindPlayerList(qboolean force)
{
	...
	for(i = 0; i < MAX_SERVERSTATUSREQUESTS; i++){
		...
		if(uiInfo.pendingServerStatus.server[i].valid)
			...
			if(UI_GetServerStatusInfo(uiInfo.pendingServerStatus.
				   server[i].adrstr, &info)){
				/*  */
				numFound++;
			...
			if(uiInfo.pendingServerStatus.num <
			   uiInfo.serverStatus.numDisplayServers){
				...
				trap_LAN_GetServerAddressString(
					...
					adrstr,
					sizeof(uiInfo.pendingServerStatus.server
					       [i].
					       adrstr));
				trap_LAN_GetServerInfo(
					lanSource,
...
UI_FeederCount(float feederID)
{
	if(feederID == FEEDER_HEADS)
		...
		return UI_MapCountByGameType(
			feederID == FEEDER_MAPS ? qtrue : qfalse);
	...
	if(feederID == FEEDER_HEADS){
		...
		if( index >= 0 && index < uiInfo.serverStatusInfo.numLines )
			if( column >= 0 && column < 4 )
				return uiInfo.serverStatusInfo.lines[index][
					       column];
	}else if(feederID == FEEDER_FINDPLAYER){
...
UI_FeederSelection(float feederID, int index)
{
	...
	if(feederID == FEEDER_HEADS){
		...
		map =
			(feederID ==
			 FEEDER_ALLMAPS) ? ui_currentNetMap.integer :
			ui_currentMap.
			integer;
		if(uiInfo.mapList[map].cinematic >= 0){
...
Character_Parse(char **p)
{
	...
	while( 1 ){
		...
		if(token[0] == '{'){
			...
			if(!String_Parse(p,
				   &uiInfo.characterList[uiInfo.characterCount].
				   name) ||
			   !String_Parse(p, &tempStr))
				return qfalse;
	...
	while( 1 ){
		...
		if(token[0] == '{'){
			if(!String_Parse(p,
			   ...
			   ||!Int_Parse(p,
				   &uiInfo.mapList[uiInfo.mapCount].teamMembers))
				return qfalse;

			if(!String_Parse(p,
				   &uiInfo.mapList[uiInfo.mapCount].opponentName))
				return qfalse;
	...
	if(handle >= 0)
		...
		if(handle == UI_MAPCINEMATIC){
			if(uiInfo.mapList[ui_currentMap.integer].cinematic >=
			   0){
				trap_CIN_StopCinematic(uiInfo.mapList[
						ui_currentMap.integer].cinematic);
				uiInfo.mapList[ui_currentMap.integer].cinematic
					= -1;
			}
			   ...
			   !(Q_stricmp(skinname,
				...
				if(!dirty){
					Com_sprintf(
						...
						sizeof(uiInfo.q3HeadNames[uiInfo
									  .
									  q3HeadCount
						       ]), "%s", scratch);
...
_UI_Init(qboolean inGameLoad)
{
	...
	uiInfo.currentCrosshair = (int) trap_Cvar_VariableValue(
		"cg_drawCrosshair");
	trap_Cvar_Set("ui_mousePitch", (trap_Cvar_VariableValue(
						"m_pitch") >= 0) ? "0" : "1");
...
static void
UI_DisplayDownloadInfo(const char *downloadName, float centerPoint, float yStart,
		       float scale)
{
	...
	UI_ReadableSize(dlSizeBuf,             sizeof dlSizeBuf,
		downloadCount);
	...
	if(downloadCount < 4096 || !downloadTime){
		...
		if(downloadSize && xferRate){
			...
			UI_PrintTime (
				...
				(n -
				 (((downloadCount/
				    1024) * n) / (downloadSize/1024))) * 1000);

			Text_PaintCenter(leftWidth, yStart+216, scale,
				colorWhite, dlTimeBuf,
				0);
			Text_PaintCenter(leftWidth, yStart+160, scale,
				colorWhite, va("(%s of %s copied)", dlSizeBuf,
					totalSizeBuf), 0);
		}else{
			Text_PaintCenter(leftWidth, yStart+216, scale,
				colorWhite, "estimating",
				0);
			if(downloadSize)
				Text_PaintCenter(leftWidth, yStart+160, scale,
					colorWhite,
...
UI_DrawConnectScreen(qboolean overlay)
{
	...
	if( cstate.connState < CA_CONNECTED )
		Text_PaintCenter_AutoWrapped(centerPoint, yStart + 176, 630, 20,
			scale, colorWhite, cstate.messageString,
			0);
	...
	case CA_CONNECTED: {
		...
		trap_Cvar_VariableStringBuffer("cl_downloadName", downloadName,
			sizeof(downloadName));
		if(*downloadName){
			UI_DisplayDownloadInfo(downloadName, centerPoint, yStart,
				scale);
			return;
		}
	}
...
static cvarTable_t cvarTable[] = {
	...
	{ &ui_realCaptureLimit, "capturelimit", "8", CVAR_SERVERINFO |
...
UI_StartServerRefresh(qboolean full)
{
	...
	qtime_t q;
	trap_RealTime(&q);
	trap_Cvar_Set(va("ui_lastServerRefresh_%i",
			ui_netSource.integer),
