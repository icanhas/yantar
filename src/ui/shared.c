...
void
Fade(int *flags, float *f, float clamp, int *nextTime, int offsetTime,
     qbool bFlags,
     float fadeAmount)
{
...
Menu_ItemsMatchingGroup(menuDef_t *menu, const char *name)
{
	...
	int	count = 0;
	for(i = 0; i < menu->itemCount; i++)
		if(Q_stricmp(menu->items[i]->window.name,
			   name) == 0 ||
...
Script_SetItemColor(itemDef_t *item, char **args)
{
	...
	if(String_Parse(args, &itemname) && String_Parse(args, &name)){
		...
		for(j = 0; j < count; j++){
			item2 = Menu_GetMatchingItemByNumber(item->parent, j,
				itemname);
			if(item2 != NULL){
				out = NULL;
				if(Q_stricmp(name, "backcolor") == 0)
...
void
Menu_ShowItemByName(menuDef_t *menu, const char *p, qbool bShow)
{
	itemDef_t *item;
	int	i;
...
void
Menu_FadeItemByName(menuDef_t *menu, const char *p, qbool fadeOut)
{
	itemDef_t *item;
	int	i;
	int	count = Menu_ItemsMatchingGroup(menu, p);
...
void
Menu_TransitionItemByName(menuDef_t *menu, const char *p, rectDef_t rectFrom,
	...
	for(i = 0; i < count; i++){
		...
		if(item != NULL){
			...
			memcpy(&item->window.rectEffects, &rectTo,
				sizeof(rectDef_t));
			item->window.rectEffects2.x = abs(
				rectTo.x - rectFrom.x) / amt;
			item->window.rectEffects2.y = abs(
				rectTo.y - rectFrom.y) / amt;
			item->window.rectEffects2.w = abs(
				rectTo.w - rectFrom.w) / amt;
			item->window.rectEffects2.h = abs(
				rectTo.h - rectFrom.h) / amt;
...
void
Menu_OrbitItemByName(menuDef_t *menu, const char *p, float x, float y, float cx,
		     float cy,
		     int time)
{
	itemDef_t *item;
...
Script_Orbit(itemDef_t *item, char **args)
{
	...
	if(String_Parse(args, &name))
		if(Float_Parse(args,
		   ...
		   Float_Parse(args,
			   &cx) &&
		   Float_Parse(args, &cy) && Int_Parse(args, &time))
...
Item_ListBox_ThumbDrawPosition(itemDef_t *item)
{
	...
	if(itemCapture == item){
		if(item->window.flags & WINDOW_HORIZONTAL){
			...
			max	= item->window.rect.x + item->window.rect.w - 2*
				  SCROLLBAR_SIZE - 1;
			...
			max	= item->window.rect.y + item->window.rect.h - 2*
				  SCROLLBAR_SIZE - 1;
...
Item_ListBox_OverLB(itemDef_t *item, float x, float y)
{
	...
	if(item->window.flags & WINDOW_HORIZONTAL){
		...
		r.y	= item->window.rect.y + item->window.rect.h -
			  SCROLLBAR_SIZE;
		...
		r.w	= item->window.rect.x + item->window.rect.w -
			  SCROLLBAR_SIZE;
		if(Rect_ContainsPoint(&r, x, y))
			return WINDOW_LB_PGDN;
	}else{
		r.x = item->window.rect.x + item->window.rect.w -
		      SCROLLBAR_SIZE;
		...
		r.h	= item->window.rect.y + item->window.rect.h -
			  SCROLLBAR_SIZE;
		if(Rect_ContainsPoint(&r, x, y))
			return WINDOW_LB_PGDN;
	}
...
void
Item_SetMouseOver(itemDef_t *item, qbool focus)
{
	if(item){
		if(focus)
			item->window.flags |= WINDOW_MOUSEOVER;
...
qbool
Item_ListBox_HandleKey(itemDef_t *item, int key, qbool down, qbool force)
{
	listBoxDef_t *listPtr = (listBoxDef_t*)item->typeData;
	int	count = DC->feederCount(item->special);
	int	max, viewmax;
			...
			if(item->window.flags & WINDOW_LB_LEFTARROW){
				...
				if(DC->realTime < lastListBoxClickTime &&
				   listPtr->doubleClick)
					Item_RunScript(item,
						listPtr->doubleClick);
				lastListBoxClickTime = DC->realTime +
						       DOUBLE_CLICK_DELAY;
...
Item_Multi_HandleKey(itemDef_t *item, int key)
{
	...
	if(multiPtr){
		if(Rect_ContainsPoint(&item->window.rect, DC->cursorx,
			   DC->cursory) && item->window.flags &
		   WINDOW_HASFOCUS &&
		   item->cvar)
			if(key == K_MOUSE1 || key == K_ENTER || key ==
			   K_MOUSE2 || key == K_MOUSE3){
				int	current = Item_Multi_FindCvarByValue(
					item) + 1;
		...
		if(key & K_CHAR_FLAG){
			...
			if(!DC->getOverstrikeMode()){
				if((len == MAX_EDITFIELD - 1) ||
				   (editPtr->maxChars && len >=
				    editPtr->maxChars))
					return qtrue;
				memmove(&buff[item->cursorPos + 1],
					&buff[item->cursorPos],
					len + 1 - item->cursorPos);
			}else if(editPtr->maxChars && item->cursorPos >=
				 editPtr->maxChars)
				return qtrue;
			...
			if(item->cursorPos < len + 1){
				item->cursorPos++;
				if(editPtr->maxPaintChars && item->cursorPos >
			...
			if(key == K_DEL || key == K_KP_DEL){
				if(item->cursorPos < len){
					memmove(buff + item->cursorPos, buff +
						item->cursorPos + 1,
						len - item->cursorPos);
					DC->setCVar(item->cvar, buff);
				}
...
Scroll_ListBox_ThumbFunc(void *p)
{
	...
	if(si->item->window.flags & WINDOW_HORIZONTAL){
		...
		/*  */
		pos =
			(DC->cursorx - r.x - SCROLLBAR_SIZE/
			 2) * max / (r.w - SCROLLBAR_SIZE);
		if(pos < 0)
			pos = 0;
		else if(pos > max)
			pos = max;
		...
		/*  */
		pos =
			(DC->cursory - r.y - SCROLLBAR_SIZE/
			 2) * max / (r.h - SCROLLBAR_SIZE);
		if(pos < 0)
			pos = 0;
		else if(pos > max)
			pos = max;
...
Item_StartCapture(itemDef_t *item, int key)
{
	...
	case ITEM_TYPE_LISTBOX:
	{
		...
		if(flags & (WINDOW_LB_LEFTARROW | WINDOW_LB_RIGHTARROW)){
			scrollInfo.nextScrollTime = DC->realTime +
						    SCROLL_TIME_START;
			scrollInfo.nextAdjustTime = DC->realTime +
						    SCROLL_TIME_ADJUST;
...
qbool
Item_Slider_HandleKey(itemDef_t *item, int key, qbool down)
{
		...
		if(key == K_MOUSE1 || key == K_ENTER || key == K_MOUSE2 ||
		   key == K_MOUSE3){
			...
			if(editDef){
				...
				/* DC->Print("slider w: %f\n", testRect.w); */
				if(Rect_ContainsPoint(&testRect, DC->cursorx,
					   DC->cursory)){
					work = DC->cursorx - x;
					value	= work / width;
					value	*=
...
qbool
Item_HandleKey(itemDef_t *item, int key, qbool down)
{
	...
	if(menu){
		...
		for(i = 0; i < menu->itemCount; i++){
			...
			if(menu->items[i]->type == ITEM_TYPE_OWNERDRAW)
				DC->stopCinematic(
					0-menu->items[i]->window.ownerDraw);
		}
	}
}
...
void
Menus_HandleOOBClick(menuDef_t *menu, int key, qbool down)
{
	if(menu){
		int i;
		...
		for(i = 0; i < menuCount; i++)
			if(Menu_OverActiveItem(&Menus[i], DC->cursorx,
				   DC->cursory)){
				Menu_RunCloseScript(menu);
				menu->window.flags &=
...
void
Menu_HandleKey(menuDef_t *menu, int key, qbool down)
{
	int i;
	itemDef_t *item = NULL;
	...
	case K_MOUSE2:
		if(item){
			if(item->type == ITEM_TYPE_TEXT){
				if(Rect_ContainsPoint(Item_CorrectedTextRect(
						   item), DC->cursorx,
					   DC->cursory))
					Item_Action(item);
			...
			}else if(Rect_ContainsPoint(&item->window.rect,
					 DC->cursorx, DC->cursory))
				Item_Action(item);
	...
	case K_ENTER:
		if(item){
			if(item->type == ITEM_TYPE_EDITFIELD || item->type ==
			   ITEM_TYPE_NUMERICFIELD){
				item->cursorPos = 0;
				g_editingField	= qtrue;
				g_editItem = item;
...
Item_SetTextExtents(itemDef_t *item, int *width, int *height, const char *text)
{
	...
	*height = item->textRect.h;
	...
	if(*width == 0 ||
	   (item->type == ITEM_TYPE_OWNERDRAW && item->textalignment ==
	    ITEM_ALIGN_CENTER)){
		...
		if(item->type == ITEM_TYPE_OWNERDRAW &&
		   (item->textalignment == ITEM_ALIGN_CENTER ||
			...
			originalWidth += DC->ownerDrawWidth(
				item->window.ownerDraw, item->textscale);
		else if(item->type == ITEM_TYPE_EDITFIELD &&
...
Item_TextField_Paint(itemDef_t *item)
{
	...
	if(item->window.flags & WINDOW_HASFOCUS && g_editingField){
		...
		DC->drawText(item->textRect.x + item->textRect.w + offset,
			item->textRect.y, item->textscale, newColor, buff +
			editPtr->paintOffset, 0, editPtr->maxPaintChars,
			item->textStyle);
...
Item_Multi_Paint(itemDef_t *item)
{
	...
	if(item->text){
		Item_Text_Paint(item);
		DC->drawText(item->textRect.x + item->textRect.w + 8,
			item->textRect.y, item->textscale, newColor, text, 0, 0,
			item->textStyle);
	}else
		DC->drawText(item->textRect.x, item->textRect.y, item->textscale,
			newColor, text, 0, 0,
 ...
 */
void
Controls_SetConfig(qbool restart)
{
	int i;
...
Item_Bind_Paint(itemDef_t *item)
{
	...
	if(item->text){
		...
		DC->drawText(item->textRect.x + item->textRect.w + 8,
			item->textRect.y, item->textscale, newColor, g_nameBind1,
			0,
			maxChars,
			item->textStyle);
	}else
		DC->drawText(item->textRect.x, item->textRect.y, item->textscale,
			newColor, "FIXME", 0, maxChars,
			item->textStyle);
}
...
qbool
Item_Bind_HandleKey(itemDef_t *item, int key, qbool down)
{
	int	id;
	int	i;

	if(Rect_ContainsPoint(&item->window.rect, DC->cursorx,
		   DC->cursory) && !g_waitingForKey){
	...
	if(id != -1){
		...
		else if(g_bindings[id].bind1 != key && g_bindings[id].bind2 ==
			-1)
	...
	if(DC->ownerDrawItem){
		...
		if(item->numColors > 0 && DC->getValue){
			...
			for(i = 0; i < item->numColors; i++)
				if(f >= item->colorRanges[i].low && f <=
				   item->colorRanges[i].high){
					memcpy(&color,
		...
		if(item->window.flags & WINDOW_HASFOCUS){
			...
			lowLight[2]	= 0.8 * parent->focusColor[2];
			lowLight[3]	= 0.8 * parent->focusColor[3];
			LerpColor(parent->focusColor,lowLight,color,0.5+0.5*
				sin(DC->realTime / PULSE_DIVISOR));
		}else if(item->textStyle == ITEM_TEXTSTYLE_BLINK &&
			 !((DC->realTime/BLINK_DIVISOR) & 1)){
			lowLight[0]	= 0.8 * item->window.foreColor[0];
			...
			lowLight[2]	= 0.8 * item->window.foreColor[2];
			lowLight[3]	= 0.8 * item->window.foreColor[3];
			LerpColor(item->window.foreColor,lowLight,color,0.5+0.5*
				sin(
					DC->realTime / PULSE_DIVISOR));
		}
		...
		if(item->text){
			...
			if(item->text[0])
				...
				DC->ownerDrawItem(
					item->textRect.x + item->textRect.w + 8,
				...
				DC->ownerDrawItem(
					item->textRect.x + item->textRect.w,
					item->window.rect.y, item->window.rect.w,
					item->window.rect.h, 0, item->textaligny,
					item->window.ownerDraw,
					item->window.ownerDrawFlags,
	...
	if(item->window.flags & WINDOW_ORBITING)
		if(DC->realTime > item->window.nextTime){
			...
			item->window.rectClient.x =
				(rx * c - ry *
				 s) + item->window.rectEffects.x - w;
			item->window.rectClient.y =
				(rx * s + ry *
				 c) + item->window.rectEffects.y - h;
			Item_UpdatePosition(item);
...
void
Menu_ScrollFeeder(menuDef_t *menu, int feeder, qbool down)
{
	if(menu){
		int i;
		for(i = 0; i < menu->itemCount; i++)
...
Menu_HandleMouseMove(menuDef_t *menu, float x, float y)
{
	...
	for(pass = 0; pass < 2; pass++)
		for(i = 0; i < menu->itemCount; i++){
			...
			/* items can be enabled and disabled based on cvars */
			if(menu->items[i]->cvarFlags &
			   (CVAR_ENABLE | CVAR_DISABLE) &&
			...
			if(Rect_ContainsPoint(&menu->items[i]->window.rect, x,
				   y)){
				if(pass == 1){
...
void
Menu_Paint(menuDef_t *menu, qbool forcePaint)
{
	int i;
...
Item_ValidateTypeData(itemDef_t *item)
{
	...
	}else if(item->type == ITEM_TYPE_EDITFIELD || item->type ==
		 ITEM_TYPE_NUMERICFIELD || item->type == ITEM_TYPE_YESNO ||
		 item->type ==
		 ITEM_TYPE_BIND || item->type == ITEM_TYPE_SLIDER ||
		 item->type ==
		 ITEM_TYPE_TEXT){
	...
	if(PC_Int_Parse(handle, &num)){
		...
		for(i = 0; i < num; i++){
			...
			if(PC_Int_Parse(handle,
				   &pos) &&
			   PC_Int_Parse(handle,
				   &width) && PC_Int_Parse(handle, &maxChars)){
...
ItemParse_cvarStrList(itemDef_t *item, int handle)
{
	...
	while(1){
		...
		if(pass == 0){
			...
			multiPtr->cvarStr[multiPtr->count] = String_Alloc(
				token.string);
			pass = 0;
			multiPtr->count++;
...
void
Display_HandleKey(int key, qbool down, int x, int y)
{
	...
	if(menu && menu->window.flags & (WINDOW_VISIBLE | WINDOW_FORCED))
		if(Rect_ContainsPoint(&menu->window.rect, x, y)){
			...
			for(i = 0; i < menu->itemCount; i++){
				...
				if(Rect_ContainsPoint(&menu->items[i]->window.
					   rect, x, y)){
