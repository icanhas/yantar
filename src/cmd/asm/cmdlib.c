...
_printf(const char *format, ...)
{
	...
	if(!lookedForServer){
		...
		if(hwndOut)
			wm_BroadcastCommand = RegisterWindowMessage(
				"Q3MPS_BroadcastCommand");
	}
	if(hwndOut){
...
char *
ExpandGamePath(const char *path)
{
	static char full[1024];
	if(!qdir[0])
...
void
ExtractFilePath(const char *path, char *dest)
{
	const char *src;
...
void
ExtractFileBase(const char *path, char *dest)
{
	const char *src;
...
void
ExtractFileExtension(const char *path, char *dest)
{
	const char *src;
