...
Pmove(pmove_t *pmove)
{
	...
	t = pmove->cmd.serverTime;
	if(t < pmove->ps->commandTime)
		return;		/* should not happen */
	if(t > pmove->ps->commandTime + 1000)
		pmove->ps->commandTime = t - 1000;
	 ...
	 * dependent behavior
	 */
	while(pmove->ps->commandTime != t){
		int msec;

		msec = t - pmove->ps->commandTime;
