...
G_RankDamage(int self, int attacker, int damage, int means_of_death)
{
	...
	if((attacker != ENTITYNUM_WORLD) && (attacker != self) &&
	   ...
	   (g_entities[attacker].client))
		trap_RankReportInt(attacker, -1, QGR_KEY_SHOT_FIRED_GAUNTLET, 1,
			1);
	...
	if((attacker != ENTITYNUM_WORLD) && (attacker != self)){
		...
		if(g_entities[attacker].client){
			...
			trap_RankReportInt(attacker, -1, QGR_KEY_DAMAGE_GIVEN,
				damage,
				1);
			...
			if(splash != 0){
				trap_RankReportInt(attacker, -1,
					QGR_KEY_SPLASH_GIVEN, splash,
					1);
				trap_RankReportInt(attacker, -1, key_splash,
					splash,
					1);
			}
		}
	...
	if((attacker != self) &&
	   ...
	   (g_entities[attacker].client)){
		/* report teammate hit */
		if(new_hit){
			trap_RankReportInt(self, -1, QGR_KEY_TEAMMATE_HIT_TAKEN,
				1,
				1);
			trap_RankReportInt(attacker, -1,
				QGR_KEY_TEAMMATE_HIT_GIVEN, 1,
				1);
		}

		/* report teammate damage */
		trap_RankReportInt(self, -1, QGR_KEY_TEAMMATE_DAMAGE_TAKEN,
			damage,
			1);
		...
		if(splash != 0){
			trap_RankReportInt(self, -1,
				QGR_KEY_TEAMMATE_SPLASH_TAKEN,
				splash,
				1);
			trap_RankReportInt(attacker, -1,
				QGR_KEY_TEAMMATE_SPLASH_GIVEN,
				splash,
				1);
		}
	}
...
G_RankPlayerDie(int self, int attacker, int means_of_death)
{
	...
	if(attacker == ENTITYNUM_WORLD){
		...
		switch(means_of_death){
		case MOD_GAUNTLET:
			trap_RankReportInt(p1, p2, QGR_KEY_SUICIDE_GAUNTLET, 1,
				1);
			break;
		case MOD_MACHINEGUN:
			trap_RankReportInt(p1, p2, QGR_KEY_SUICIDE_MACHINEGUN, 1,
				1);
			break;
		case MOD_SHOTGUN:
		...
		case MOD_RAILGUN:
			...
			break;
		case MOD_LIGHTNING:
			trap_RankReportInt(p1, p2, QGR_KEY_SUICIDE_LIGHTNING, 1,
				1);
			break;
		case MOD_BFG:
		case MOD_BFG_SPLASH:
