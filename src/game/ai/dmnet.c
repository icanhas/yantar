...
BotGetAirGoal(bot_state_t *bs, bot_goal_t *goal)
{
	...
	if(bsptrace.fraction > 0){
		...
		if(areanum){
			copyv3(bsptrace.endpos, goal->origin);
			goal->origin[2] -= 2;
...
BotGoForAir(bot_state_t *bs, int tfl, bot_goal_t *ltg, float range)
{
	 ...
	 * if we can find an air goal */
		if(BotGetAirGoal(bs, &goal)){
			...
			while(trap_BotChooseNBGItem(bs->gs, bs->origin,
				      bs->inventory, tfl, ltg, range)){
		...
		if(bs->areanum == goal->areanum)
			if(bs->origin[0] > goal->origin[0] + goal->mins[0] &&
			   bs->origin[0] < goal->origin[0] + goal->maxs[0])
				if(bs->origin[1] > goal->origin[1] +
				   goal->mins[1] && bs->origin[1] <
				   goal->origin[1] + goal->maxs[1])
	...
	if(bs->ltg_time < FloatTime()){
		...
		if(trap_BotChooseLTGItem(bs->gs, bs->origin, bs->inventory, tfl))
			...
			BotAI_Print(PRT_MESSAGE,
				"%s: no valid ltg (probably stuck)\n",
				ClientName(bs->client, netname,
					sizeof(netname)));
#endif
...
BotGetLongTermGoal(bot_state_t *bs, int tfl, int retreat, bot_goal_t *goal)
{
	...
	if(bs->ltgtype == LTG_TEAMHELP && !retreat){
		...
		if(entinfo.valid){
			...
			if(areanum && trap_AAS_AreaReachability(areanum)){
				...
				copyv3(entinfo.origin, bs->teamgoal.origin);
				setv3(bs->teamgoal.mins, -8, -8, -8);
				setv3(bs->teamgoal.maxs, 8, 8, 8);
			}
		}
		...
		if(BotEntityVisible(bs->entitynum, bs->eye, bs->viewangles, 360,
			...
			if(lensqrv3(dir) <
			   Square(bs->formation_dist)){
				...
				if(botinfo.origin[2] + botinfo.maxs[2] >
				   entinfo.origin[2] + entinfo.mins[2])
					...
					if(botinfo.origin[0] + botinfo.maxs[0] >
					   entinfo.origin[0] + entinfo.mins[0] -
					   4&&
					   botinfo.origin[0] + botinfo.mins[0] <
					   entinfo.origin[0] + entinfo.maxs[0] +
					   4){
						if(botinfo.origin[1] +
						   botinfo.maxs[1] >
						   entinfo.origin[1] +
						   ...
						   entinfo.maxs[1] + 4)
							if(botinfo.origin[2] +
							   ...
							   entinfo.maxs[2] + 4){
								...
								normv3(
									dir);
								/* subv3(entinfo.origin, entinfo.lastvisorigin, dir); */
								subv3(
									bs->
									origin,
									entinfo.
				...
				if(trap_AAS_Swimming(bs->origin)) bs->
					attackcrouch_time = FloatTime() - 1;
				...
				/* else look strategically around for enemies */
					...
					subv3(target, bs->origin, dir);
					v3toeuler(dir, bs->ideal_viewangles);
					bs->ideal_viewangles[2] *= 0.5;
				}
				/* check if the bot wants to go for air */
				if(BotGoForAir(bs, bs->tfl, &bs->teamgoal,
					   400)){
	...
	if(bs->ltgtype == LTG_DEFENDKEYAREA && !retreat &&
	   bs->defendaway_time < FloatTime()){
		...
		subv3(goal->origin, bs->origin, dir);
		if(lensqrv3(dir) < Square(70)){
			trap_BotResetAvoidReach(bs->ms);
	...
	if((bs->ltgtype == LTG_CAMP ||
	    bs->ltgtype == LTG_CAMPORDER) && !retreat){
		/* check for bot typing status message */
		if(bs->teammessage_time && bs->teammessage_time < FloatTime()){
			if(bs->ltgtype == LTG_CAMPORDER){
				BotAI_BotInitialChat(bs, "camp_start",
					EasyClientName(bs->teammate, netname,
						sizeof(netname)), NULL);
				trap_BotEnterChat(bs->cs, bs->decisionmaker,
					CHAT_TELL);
		...
		if(lensqrv3(dir) < Square(60)){
			...
			if(bs->attackcrouch_time < FloatTime() - 5){
				croucher = trap_Characteristic_BFloat(
					bs->character, CHARACTERISTIC_CROUCHER,
					0, 1);
				if(random() < bs->thinktime * croucher)
			...
			if(bs->attackcrouch_time > FloatTime())
				trap_EA_Crouch(bs->client);
			/* don't crouch when swimming */
			if(trap_AAS_Swimming(bs->origin)) bs->attackcrouch_time
					= FloatTime() - 1;
			/* make sure the bot is not gonna drown */
			if(trap_PointContents(bs->eye,
				   bs->entitynum) &
			   (CONTENTS_WATER|CONTENTS_SLIME|CONTENTS_LAVA)){
				if(bs->ltgtype == LTG_CAMPORDER){
		...
		if(trap_BotTouchingGoal(bs->origin, &bs->curpatrolpoint->goal)){
			if(bs->patrolflags & PATROL_BACK){
				if(bs->curpatrolpoint->prev)
					bs->curpatrolpoint =
						bs->curpatrolpoint->prev;
				else{
					bs->curpatrolpoint =
						bs->curpatrolpoint->next;
	...
	if(gametype == GT_CTF){
		...
		if(bs->ltgtype == LTG_GETFLAG){
			...
			if(bs->teammessage_time && bs->teammessage_time <
			   FloatTime()){
				BotAI_BotInitialChat(bs, "captureflag_start",
					NULL);
				trap_BotEnterChat(bs->cs, 0, CHAT_TEAM);
			...
			/*  */
			switch(BotTeam(bs)){
			case TEAM_RED: memcpy(goal, &ctf_blueflag,
					sizeof(bot_goal_t)); break;
			case TEAM_BLUE: memcpy(goal, &ctf_redflag,
					sizeof(bot_goal_t)); break;
			...
			if(trap_BotTouchingGoal(bs->origin, goal)){
				...
				switch(BotTeam(bs)){
				case TEAM_RED: bs->blueflagstatus = 1;
					break;
				case TEAM_BLUE: bs->redflagstatus = 1;
					break;
				}
				bs->ltgtype = 0;
			}
		...
		if(bs->ltgtype == LTG_RUSHBASE && bs->rushbaseaway_time <
		   FloatTime()){
			switch(BotTeam(bs)){
			case TEAM_RED: memcpy(goal, &ctf_redflag,
					sizeof(bot_goal_t)); break;
			case TEAM_BLUE: memcpy(goal, &ctf_blueflag,
					sizeof(bot_goal_t)); break;
			default: bs->ltgtype = 0; return qfalse;
			}
		...
		if(bs->ltgtype == LTG_RETURNFLAG){
			...
			if(bs->teammessage_time && bs->teammessage_time <
			   FloatTime()){
				BotAI_BotInitialChat(bs, "returnflag_start",
					NULL);
				trap_BotEnterChat(bs->cs, 0, CHAT_TEAM);
			...
			/* if touching the flag */
			if(trap_BotTouchingGoal(bs->origin,
				   goal)) bs->ltgtype = 0;
			/* stop after 3 minutes */
			if(bs->teamgoal_time < FloatTime())
				bs->ltgtype = 0;
...
BotLongTermGoal(bot_state_t *bs, int tfl, int retreat, bot_goal_t *goal)
{
	...
	if(bs->lead_time > 0 && !retreat){
		...
		/*  */
		if(bs->leadmessage_time < 0 && -bs->leadmessage_time <
		   FloatTime()){
		...
		if(entinfo.valid){
			...
			if(areanum && trap_AAS_AreaReachability(areanum)){
				/* update team goal */
				bs->lead_teamgoal.entitynum =
					bs->lead_teammate;
				bs->lead_teamgoal.areanum = areanum;
				copyv3(entinfo.origin,
					bs->lead_teamgoal.origin);
				setv3(bs->lead_teamgoal.mins, -8, -8, -8);
				setv3(bs->lead_teamgoal.maxs, 8, 8, 8);
			}
		}
		/* if the team mate is visible */
		if(BotEntityVisible(bs->entitynum, bs->eye, bs->viewangles, 360,
		...
		subv3(bs->origin, bs->lead_teamgoal.origin, dir);
			...
			if(bs->leadmessage_time < FloatTime() - 20){
				BotAI_BotInitialChat(
					...
					EasyClientName(bs->lead_teammate,
						teammate,
						sizeof(teammate)), NULL);
				trap_BotEnterChat(bs->cs, bs->teammate,
					CHAT_TELL);
				bs->leadmessage_time = FloatTime();
			}
...
AINode_Seek_ActivateEntity(bot_state_t *bs)
{
	...
	if(bs->activatestack->shoot){
		/*  */
		BotAI_Trace(&bsptrace, bs->eye, NULL, NULL,
			bs->activatestack->target, bs->entitynum,
			MASK_SHOT);
	...
	if(targetvisible){
		...
		if(!cmpv3(bs->activatestack->origin, entinfo.origin)){
#ifdef DEBUG
			BotAI_Print(PRT_MESSAGE,
				"hit shootable button or trigger\n");
#endif	/* DEBUG */
	...
	if(moveresult.flags &
	   (MOVERESULT_MOVEMENTVIEWSET|MOVERESULT_MOVEMENTVIEW|
	    MOVERESULT_SWIMVIEW))
		copyv3(moveresult.ideal_viewangles, bs->ideal_viewangles);
	/* if waiting for something */
		...
		if(trap_BotMovementViewTarget(bs->ms, goal, bs->tfl, 300,
			...
			v3toeuler(moveresult.movedir, bs->ideal_viewangles);
		bs->ideal_viewangles[2] *= 0.5;
	}
...
AINode_Seek_NBG(bot_state_t *bs)
{
	...
	/* if waiting for something */
		...
		if(!trap_BotGetSecondGoal(bs->gs, &goal)) trap_BotGetTopGoal(
				bs->gs, &goal);
		if(trap_BotMovementViewTarget(bs->ms, &goal, bs->tfl, 300,
			   target)){
			subv3(target, bs->origin, dir);
		...
		else v3toeuler(moveresult.movedir, bs->ideal_viewangles);
		bs->ideal_viewangles[2] *= 0.5;
	}
	/* if the weapon is used for the bot movement */
	if(moveresult.flags & MOVERESULT_MOVEMENTWEAPON) bs->weaponnum =
			moveresult.weapon;
	/* if there is an enemy */
	if(BotFindEnemy(bs, -1)){
		if(BotWantsToRetreat(bs))
...
AINode_Seek_LTG(bot_state_t *bs)
{
	...
	/* if waiting for something */
		...
		/* FIXME: look at cluster portals? */
		else if(lensqrv3(moveresult.movedir))
			v3toeuler(moveresult.movedir, bs->ideal_viewangles);
		else if(random() < bs->thinktime * 0.8){
			BotRoamGoal(bs, target);
			subv3(target, bs->origin, dir);
...
AINode_Battle_Fight(bot_state_t *bs)
{
	...
	if(areanum && trap_AAS_AreaReachability(areanum)){
		copyv3(target, bs->lastenemyorigin);
		...
		if(bs->enemy == redobelisk.entitynum || bs->enemy ==
		   blueobelisk.entitynum){
			AIEnter_Battle_Chase(
				bs, "battle fight: obelisk out of sight");
			return qfalse;
		}
#endif
	...
	if(moveresult.flags &
	   (MOVERESULT_MOVEMENTVIEWSET|MOVERESULT_MOVEMENTVIEW|
		...
		if(bs->chase_time > FloatTime() - 2)
			...
			if(trap_BotMovementViewTarget(bs->ms, &goal, bs->tfl,
				   300, target)){
				subv3(target, bs->origin, dir);
				v3toeuler(dir, bs->ideal_viewangles);
			}else
				v3toeuler(moveresult.movedir,
					bs->ideal_viewangles);
		}
		bs->ideal_viewangles[2] *= 0.5;
	}
