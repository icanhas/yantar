...
BotClientTravelTimeToGoal(int client, bot_goal_t *goal)
{
	...
	if(!areanum) return 1;
	return trap_AAS_AreaTravelTimeToGoalArea(areanum, ps.origin,
		goal->areanum,
		TFL_DEFAULT);
}
...
BotSayTeamOrderAlways(bot_state_t *bs, int toclient)
{
	...
	if(bs->client == toclient){
		...
		Q_sprintf(teamchat, sizeof(teamchat), EC "(%s"EC ")"EC ": %s",
			name,
			buf);
...
BotVoiceChat(bot_state_t *bs, int toclient, char *voicechat)
{
	...
	if(toclient == -1)
		...
		trap_EA_Command(bs->client,
			va("vtell %d %s", toclient, voicechat));
#endif
}
...
BotVoiceChatOnly(bot_state_t *bs, int toclient, char *voicechat)
{
	...
	if(toclient == -1)
		...
		/* voice only tell single player */
		trap_EA_Command(bs->client,
			va("votell %d %s", toclient, voicechat));
#endif
}
	...
	case 3:
	{
		...
		if(bs->flagcarrier != -1){
			...
			if(bs->flagcarrier == bs->client){
				BotAI_BotInitialChat(bs, "cmd_accompanyme", name,
					NULL);
				BotSayVoiceTeamOrder(bs, other,
					VOICECHAT_FOLLOWME);
			}else{
	...
	default:
	{
		...
		if(bs->flagcarrier != -1){
			...
			for(i = 0; i < defenders; i++){
				...
				if(bs->flagcarrier == bs->client){
					BotAI_BotInitialChat(bs,
						"cmd_accompanyme", name,
						NULL);
					BotSayVoiceTeamOrder(bs, teammates[i],
						VOICECHAT_FOLLOWME);
				}else{
					BotAI_BotInitialChat(bs, "cmd_accompany",
						name, carriername,
						NULL);
					BotSayVoiceTeamOrder(
						bs, teammates[i],
			...
			for(i = 0; i < defenders; i++){
				...
				/*  */
				ClientName(teammates[i], name, sizeof(name));
				BotAI_BotInitialChat(bs, "cmd_getflag", name,
					NULL);
				BotSayVoiceTeamOrder(bs, teammates[i],
					VOICECHAT_GETFLAG);
				BotSayTeamOrder(bs, teammates[i]);
		...
		for(i = 0; i < attackers; i++){
			...
			if(teammates[numteammates - i - 1] == bs->flagcarrier)
				continue;
			/*  */
			ClientName(teammates[numteammates - i - 1], name,
				sizeof(name));
			BotAI_BotInitialChat(bs, "cmd_getflag", name, NULL);
	...
	if(!(bs->ctfstrategy & CTFS_AGRESSIVE)){
		...
		default:
		{
			...
			for(i = 0; i < defenders; i++){
				/*  */
				ClientName(teammates[i], name, sizeof(name));
				BotAI_BotInitialChat(bs, "cmd_defendbase", name,
					NULL);
				BotSayTeamOrder(bs, teammates[i]);
				BotSayVoiceTeamOrder(bs, teammates[i],
					VOICECHAT_DEFEND);
			...
			for(i = 0; i < attackers; i++){
				...
				BotSayVoiceTeamOrder(bs, teammates[0],
					VOICECHAT_GETFLAG);
			}
			/*  */
			break;
...
Bot1FCTFOrders_TeamHasFlag(bot_state_t *bs)
{
	...
	if(!(bs->ctfstrategy & CTFS_AGRESSIVE)){
		...
		case 2:
		{
			...
			ClientName(other, name, sizeof(name));
			BotAI_BotInitialChat(bs, "cmd_attackenemybase", name,
				NULL);
			BotSayTeamOrder(bs, other);
			...
			if(bs->flagcarrier != -1){
				...
				for(i = 0; i < attackers; i++){
					...
					if(teammates[numteammates - i - 1] ==
					   bs->flagcarrier)
						continue;
					/*  */
					ClientName(teammates[numteammates - i -
							     1], name,
						sizeof(name));
					if(bs->flagcarrier == bs->client){
				...
				for(i = 0; i < attackers; i++){
					...
					ClientName(teammates[numteammates - i -
							     1], name,
						sizeof(name));
					BotAI_BotInitialChat(bs, "cmd_getflag",
						name,
						NULL);
					BotSayTeamOrder(bs,
						teammates[numteammates - i - 1]);
	...
	if(!(bs->ctfstrategy & CTFS_AGRESSIVE)){
		...
		default:
		{
			...
			for(i = 0; i < attackers; i++){
				...
				ClientName(teammates[numteammates - i - 1], name,
					sizeof(name));
				BotAI_BotInitialChat(bs, "cmd_returnflag", name,
					NULL);
				BotSayTeamOrder(bs,
					teammates[numteammates - i - 1]);
				BotSayVoiceTeamOrder(
	...
	if(!(bs->ctfstrategy & CTFS_AGRESSIVE)){
		...
		default:
		{
			...
			for(i = 0; i < attackers; i++){
				...
				ClientName(teammates[numteammates - i - 1], name,
					sizeof(name));
				BotAI_BotInitialChat(bs, "cmd_attackenemybase",
					name,
					NULL);
				BotSayTeamOrder(bs,
					teammates[numteammates - i - 1]);
	...
	if(!(bs->ctfstrategy & CTFS_AGRESSIVE)){
		...
		default:
		{
			...
			for(i = 0; i < attackers; i++){
				...
				ClientName(teammates[numteammates - i - 1], name,
					sizeof(name));
				BotAI_BotInitialChat(bs, "cmd_harvest", name,
					NULL);
				BotSayTeamOrder(bs,
					teammates[numteammates - i - 1]);
				BotSayVoiceTeamOrder(
...
BotTeamAI(bot_state_t *bs)
{
	...
	if(!BotValidTeamLeader(bs))
		...
		if(!FindHumanTeamLeader(bs)){
			/*  */
			if(!bs->askteamleader_time &&
			   !bs->becometeamleader_time){
			...
			if(bs->becometeamleader_time &&
			   bs->becometeamleader_time < FloatTime()){
				...
				trap_BotEnterChat(bs->cs, 0, CHAT_TEAM);
				BotSayVoiceTeamOrder(bs, -1,
					VOICECHAT_STARTLEADER);
				ClientName(bs->client, netname, sizeof(netname));
				strncpy(bs->teamleader, netname,
					sizeof(bs->teamleader));
	...
	case GT_TEAM:
	{
		...
		if(bs->teamgiveorders_time && bs->teamgiveorders_time <
		   FloatTime() - 5){
			BotTeamOrders(bs);
