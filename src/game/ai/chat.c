...
BotChat_Death(bot_state_t *bs)
{
	...
	if(TeamPlayIsOn() && BotSameTeam(bs, bs->lastkilledby)){
		...
		if(bs->botdeathtype == MOD_WATER)
			...
			BotAI_BotInitialChat(bs, "death_slime",
				BotRandomOpponentName(
					bs), NULL);
		else if(bs->botdeathtype == MOD_LAVA)
			BotAI_BotInitialChat(bs, "death_lava",
				BotRandomOpponentName(
					bs), NULL);
			...
			BotAI_BotInitialChat(bs, "death_suicide",
				BotRandomOpponentName(
					bs), NULL);
...
BotChat_HitNoDeath(bot_state_t *bs)
{
	...
	rnd =
		trap_Characteristic_BFloat(bs->character,
			CHARACTERISTIC_CHAT_HITNODEATH,
			0,
			1);
	/* don't chat in teamplay */
	if(TeamPlayIsOn()) return qfalse;
