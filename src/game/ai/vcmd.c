...
BotVoiceChat_Defend(bot_state_t *bs, int client, int mode)
{
	...
	if(gametype == GT_OBELISK || gametype == GT_HARVESTER){
		/*  */
		switch(BotTeam(bs)){
		case TEAM_RED: memcpy(&bs->teamgoal, &redobelisk,
				sizeof(bot_goal_t)); break;
		case TEAM_BLUE: memcpy(&bs->teamgoal, &blueobelisk,
				sizeof(bot_goal_t)); break;
		default: return;
		}
	}else
	...
	if(gametype == GT_CTF
	   ...
	   || gametype == GT_1FCTF
		...
		/*  */
		switch(BotTeam(bs)){
		case TEAM_RED: memcpy(&bs->teamgoal, &ctf_redflag,
				sizeof(bot_goal_t)); break;
		case TEAM_BLUE: memcpy(&bs->teamgoal, &ctf_blueflag,
				sizeof(bot_goal_t)); break;
		default: return;
		}
	}else
...
BotVoiceChat_Camp(bot_state_t *bs, int client, int mode)
{
	...
	if(bs->teamgoal.entitynum < 0){
		BotAI_BotInitialChat(bs, "whereareyou",
			EasyClientName(client, netname, sizeof(netname)), NULL);
		trap_BotEnterChat(bs->cs, client, CHAT_TELL);
		return;
	}
	/*  */
