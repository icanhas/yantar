...
BotPrintTeamGoal(bot_state_t *bs)
{
	...
	{
		BotAI_Print(PRT_MESSAGE,
			"%s: I'm gonna harvest for %1.0f secs\n", netname,
			t);
		break;
	}
#endif
	...
	default:
	{
		if(bs->ctfroam_time > FloatTime()){
			...
			BotAI_Print(PRT_MESSAGE,
				"%s: I'm gonna roam for %1.0f secs\n", netname,
				t);
		}else
	...
	if(match->subtype & ST_TIME){
		...
		if(trap_BotFindMatch(timestring, &timematch, MTCONTEXT_TIME)){
			if(timematch.type == MSG_FOREVER)
				...
				trap_BotMatchVariable(&timematch, TIME,
					timestring,
					MAX_MESSAGE_SIZE);
				if(timematch.type == MSG_MINUTES) t = atof(
						timestring) * 60;
				else if(timematch.type == MSG_SECONDS) t = atof(
						timestring);
				else t = 0;
			}
	...
	if(match->subtype & ST_ADDRESSED){
		...
		while(trap_BotFindMatch(addressedto, &addresseematch,
			...
			if(addresseematch.type == MSG_EVERYONE)
				...
				trap_BotMatchVariable(&addresseematch, TEAMMATE,
					name,
					sizeof(name));
				if(strlen(name)){
					if(stristr(botname, name)) return qtrue;
					if(stristr(bs->subteam,
						   name)) return qtrue;
				}
				trap_BotMatchVariable(&addresseematch, MORE,
					addressedto,
					MAX_MESSAGE_SIZE);
			}else{
				trap_BotMatchVariable(&addresseematch, TEAMMATE,
...
BotMatch_Camp(bot_state_t *bs, bot_match_t *match)
{
	...
	if(match->subtype & ST_THERE){
		...
		copyv3(bs->origin, bs->teamgoal.origin);
		setv3(bs->teamgoal.mins, -8, -8, -8);
		setv3(bs->teamgoal.maxs, 8, 8, 8);
...
BotMatch_CheckPoint(bot_state_t *bs, bot_match_t *match)
{
	...
	if(BotAddressedToBot(bs, match)){
		...
		BotAI_BotInitialChat(bs, "checkpoint_confirm", cp->name, buf,
			NULL);
		trap_BotEnterChat(bs->cs, client, CHAT_TELL);
	}
}
...
BotMatch_StartTeamLeaderShip(bot_state_t *bs, bot_match_t *match)
{
	...
	/* chats for someone else */
		...
		client = FindClientByName(teammate);
		if(client >=
		   0) ClientName(client, bs->teamleader, sizeof(bs->teamleader));
	}
}
...
BotNearestVisibleItem(bot_state_t *bs, char *itemname, bot_goal_t *goal)
{
	...
	do {
		...
		if(Q_stricmp(itemname, name) != 0)
			continue;
		subv3(tmpgoal.origin, bs->origin, dir);
		dist = lenv3(dir);
		if(dist < bestdist){
		...
		if(gametype == GT_CTF
		   ...
		   || gametype == GT_1FCTF
			...
			if(redtt < (redtt + bluett) * 0.4)
				BotAI_BotInitialChat(bs, "teamlocation",
					nearbyitems[bestitem], "red",
					NULL);
			else if(bluett < (redtt + bluett) * 0.4)
				BotAI_BotInitialChat(bs, "teamlocation",
					nearbyitems[bestitem], "blue",
					NULL);
			else
				BotAI_BotInitialChat(bs, "location",
					nearbyitems[bestitem],
					NULL);
		}
#ifdef MISSIONPACK
...
BotMatch_LeadTheWay(bot_state_t *bs, bot_match_t *match)
{
	...
	if(entinfo.valid){
		...
		if(areanum){	/* && trap_AAS_AreaReachability(areanum)) { */
			...
			copyv3(entinfo.origin, bs->lead_teamgoal.origin);
		...
		if(match->subtype & ST_GOTFLAG){
			if(!Q_stricmp(flag, "red")){
				...
				if(BotTeam(bs) == TEAM_BLUE){
					trap_BotMatchVariable(match, NETNAME,
						netname,
						sizeof(netname));
